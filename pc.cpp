...
struct fame_list knight_fame_list[MAX_FAME_LIST];
struct fame_list minstrel_fame_list[MAX_FAME_LIST];
...
bool pc_addfame(map_session_data &sd, int count)
{
	...
	if((sd.class_&MAPID_THIRDMASK) == MAPID_MINSTRELWANDERER){
		clif_update_rankingpoint(sd, RANK_MINSTREL, count);
		chrif_updatefamelist(sd, RANK_MINSTREL);
		return true;
	}
	...
	switch(sd.class_&MAPID_UPPERMASK){
		...
		case MAPID_KNIGHT:		ranktype = RANK_KNIGHT; break;
		default:
	...
	switch(job){
		case 333: // Blacksmith
		    for(i = 0; i < MAX_FAME_LIST; i++){
				if(smith_fame_list[i].account_id == char_id)
				    return i + 1;
			}
			break;
		...
		case MAPID_TAEKWON: // Taekwon
			...
			break;
		case MAPID_KNIGHT: // Taekwon
			for(i = 0; i < MAX_FAME_LIST; i++){
				if(knight_fame_list[i].id == char_id)
					return i + 1;
			}
			break;
		case MAPID_MINSTRELWANDERER: // Taekwon
			for(i = 0; i < MAX_FAME_LIST; i++){
				if(minstrel_fame_list[i].id == char_id)
					return i + 1;
			}
			break;
	}
...
void pc_inventory_rentals(struct map_session_data *sd)
{
	...
	for( i = 0; i < MAX_INVENTORY; i++ ) { // Check for Rentals on Inventory
		...
		if( sd->inventory.u.items_inventory[i].bind > 0 && sd->inventory.u.items_inventory[i].bind <= time(NULL) ) {
			sd->inventory.u.items_inventory[i].bind = 0;
			continue;
		}
...
int verifica_marionete(struct item* itm){
	
	int i;
	for(i = 0; i < 4; i++){
		if(itm->card[i] >= 1011260 && itm->card[i] <= 1011440)
			return 1;
		if(itm->card[i] == 2011438 || itm->card[i] == 2011439 || itm->card[i] == 2011440 || itm->card[i] == 2011434 || itm->card[i] == 2011435 || itm->card[i] == 2011436 || itm->card[i] == 2011430 || itm->card[i] == 2011431 || itm->card[i] == 2011432 || itm->card[i] == 2011426 || itm->card[i] == 2011427 || itm->card[i] == 2011428 || itm->card[i] == 2011418 || itm->card[i] == 2011419 || itm->card[i] == 2011420 || itm->card[i] == 2011402 || itm->card[i] == 2011403 || itm->card[i] == 2011404 || itm->card[i] == 2011398 || itm->card[i] == 2011399 || itm->card[i] == 2011400 || itm->card[i] == 2011394 || itm->card[i] == 2011395 || itm->card[i] == 2011396)
			return 1;
	}
	
	return 0;
}
int verifica_bloqueio(struct item* itm){
...
uint8 pc_isequip(struct map_session_data *sd,int n)
{
	...
	if (sd->sc.count) {
		...
		if (item->equip & EQP_ARMS && sd->sc.data[SC__WEAKNESS]){
			int verif = 0;
			if(sd->sc.data[SC_CP_WEAPON]) verif += 1;
			if(sd->sc.data[SC_CP_SHIELD]) verif += 1;
			if(verif < 2)
				return ITEM_EQUIP_ACK_FAIL;
		}
...
void pc_reg_received(struct map_session_data *sd)
{
	...
	for(i=0;i<MAX_REPUTATION;i++)
		sd->reputation[i].points = static_cast<int64>(pc_readglobalreg(sd, reference_uid( add_str( REPUTATION_VAR ), i )));
	
	for(i=0;i<5;i++)
		sd->parceiros[i].account_id = static_cast<int64>(pc_readglobalreg(sd, reference_uid( add_str( "#PARCEIROS" ), i )));
	
	clif_reputation(sd);
	
	for(int i=0;i<=7;i++){
		sd->pontos[i].item_id = 38070 + i;
		sd->pontos[i].qtd = static_cast<int64>(pc_readglobalreg(sd, reference_uid( add_str("#PTSBIOCHEFF"), i )));
		
		sd->addfadiga[i].item_id = 38070 + i;
		sd->addfadiga[i].qtd = 0;
	}
	
	sd->state.hora_prt = static_cast<int>(pc_readglobalreg(sd, add_str("timerprt")));
	sd->state.pkentregue = static_cast<int>(pc_readglobalreg(sd, add_str("#PKENTREGUE")));
	sd->state.hora_assina = static_cast<int>(pc_readglobalreg(sd, add_str("#TIMEASSINAPK")));
	sd->mobs_bio_savage = static_cast<int>(pc_readglobalreg(sd, add_str("#mvpbiosavage")));
	sd->mobs_cheff_savage = static_cast<int>(pc_readglobalreg(sd, add_str("#mvpcheffsavage")));
	sd->state.resetou = static_cast<int>(pc_readglobalreg(sd, add_str("reset_limit_stats")));
	sd->state.arma_visual = static_cast<int>(pc_readglobalreg(sd, add_str("armavisu")));
	sd->state.hidepetall = static_cast<int>(pc_readglobalreg(sd, add_str("#HIDEPET")));
	sd->state.hidepet = static_cast<int>(pc_readglobalreg(sd, add_str("#HIDEPETT")));
	
	sd->history_state.classereborn = static_cast<int>(pc_readglobalreg(sd, add_str("classereborn")));
	sd->history_state.temporadareborn = static_cast<int>(pc_readglobalreg(sd, add_str("temporadareborn")));
	sd->history_state.daystempreborn = static_cast<int>(pc_readglobalreg(sd, add_str("daystempreborn")));
	sd->history_state.burnreborn = static_cast<int>(pc_readglobalreg(sd, add_str("burnreborn")));
	sd->history_state.resetchar = static_cast<int>(pc_readglobalreg(sd, add_str("resetchar")));
	sd->history_state.resetaccount = static_cast<int>(pc_readglobalreg(sd, add_str("#RESETACC")));
	sd->history_state.melhora_duasmaos = static_cast<int>(pc_readglobalreg(sd, add_str("#MELHORAMAOS")));
	sd->history_state.account_casamento = static_cast<int>(pc_readglobalreg(sd, add_str("#PARCEIROID")));
	sd->history_state.doador = static_cast<int>(pc_readglobalreg(sd, add_str("#DOADOR")));
	sd->history_state.doadordays = static_cast<int>(pc_readglobalreg(sd, add_str("#DIASDOADOR")));
	sd->history_state.doadortrava = static_cast<int>(pc_readglobalreg(sd, add_str("#DOADORTRAVA")));
	sd->history_state.bindtype = static_cast<int>(pc_readglobalreg(sd, add_str("#BINDTYPE")));
	
	// parceiros
	sd->history_state.parceiro_one = static_cast<int>(pc_readglobalreg(sd, add_str("#PARCEIROONE")));
	sd->history_state.parceiro_two = static_cast<int>(pc_readglobalreg(sd, add_str("#PARCEIROTWO")));
	sd->history_state.parceiro_three = static_cast<int>(pc_readglobalreg(sd, add_str("#PARCEIROTHREE")));
	sd->history_state.parceiro_four = static_cast<int>(pc_readglobalreg(sd, add_str("#PARCEIROFOUR")));
	
	// Black Friday
	sd->blackfriday_state.totalcash = static_cast<int>(pc_readglobalreg(sd, add_str("#BFTOTALCASH")));
	sd->blackfriday_state.totalkafra = static_cast<int>(pc_readglobalreg(sd, add_str("#BFTOTALKAFRA")));
	sd->blackfriday_state.gastototalcash = static_cast<int>(pc_readglobalreg(sd, add_str("#GASTOTOTALCASH")));
	sd->blackfriday_state.gastototalkafra = static_cast<int>(pc_readglobalreg(sd, add_str("#GASTOTOTALKAFRA")));
	sd->blackfriday_state.horablack = static_cast<int>(pc_readglobalreg(sd, add_str("#TIMEPETBF")));
	
	// Verus
	sd->verus_state.pandemonium = static_cast<int>(pc_readglobalreg(sd, add_str("#VERUSPANDE")));
	sd->verus_state.fenda = static_cast<int>(pc_readglobalreg(sd, add_str("#VERUSFENDA")));
	sd->verus_state.templo = static_cast<int>(pc_readglobalreg(sd, add_str("#VERUSTEMPLO")));
	sd->verus_state.biocheff = static_cast<int>(pc_readglobalreg(sd, add_str("#VERUSBIOCHEF")));
	sd->verus_state.dominio = static_cast<int>(pc_readglobalreg(sd, add_str("#VERUSDOM")));
	
	// Carnaval
	sd->carnaval_state.totalcash = static_cast<int>(pc_readglobalreg(sd, add_str("#CNTOTALCASH")));
	sd->carnaval_state.totalkafra = static_cast<int>(pc_readglobalreg(sd, add_str("#CNTOTALKAFRA")));
	sd->carnaval_state.gastototalcash = static_cast<int>(pc_readglobalreg(sd, add_str("#GCNTOTALCASH")));
	sd->carnaval_state.gastototalkafra = static_cast<int>(pc_readglobalreg(sd, add_str("#GCNTOTALKAFRA")));
	sd->carnaval_state.cacatesouro = static_cast<int>(pc_readglobalreg(sd, add_str("#CACATECOUNT")));
	
	// Aniversário
	sd->aniversario_state.totalcash = static_cast<int>(pc_readglobalreg(sd, add_str("#ANTOTALCASH")));
	sd->aniversario_state.totalkafra = static_cast<int>(pc_readglobalreg(sd, add_str("#ANTOTALKAFRA")));
	sd->aniversario_state.gastototalcash = static_cast<int>(pc_readglobalreg(sd, add_str("#GANTOTALCASH")));
	sd->aniversario_state.gastototalkafra = static_cast<int>(pc_readglobalreg(sd, add_str("#GANTOTALKAFRA")));
	sd->aniversario_state.horablack = static_cast<int>(pc_readglobalreg(sd, add_str("#TIMEROLANV")));
	
	//Pascoa
	sd->cestapascoa = static_cast<int>(pc_readglobalreg(sd, add_str("#CESTAPASCOA")));
	
	sd->killcount = static_cast<int>(pc_readglobalreg(sd, add_str("#KILLCOUNT")));
	sd->state.exceldrop = 1;
	
	sd->history_state.idaura = HAT_EF_effect_yellow_aura;
	
	int auraused = static_cast<int>(pc_readglobalreg(sd, add_str("auraescolhida")));
	if(auraused) sd->history_state.idaura = auraused;
	
	//adriel
	//sd ->state.teleporte_count = static_cast<int>(pc_readglobalreg(sd,add_str("teleporte_count")));
...
void pc_bonus(struct map_session_data *sd,int type,int val)
{
	...
	switch(type){
		...
		case SP_TEMPO_HELL:
			if(sd->state.lr_flag != 2)
				sd->bonus.tempo_hell += val;
			break;
		case SP_DANO_FINAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.dano_final += val;
			break;
		case SP_REDUCT_REFLECT:
			if(sd->state.lr_flag != 2)
				sd->bonus.reduct_reflect += val;
			break;
		case SP_VULNERA_FIRE_SHADOW:
			if(sd->state.lr_flag != 2)
				sd->bonus.vulnera_fire_shadow += val;
			break;
		case SP_VULNERA_WATER_UNDEAD:
			if(sd->state.lr_flag != 2)
				sd->bonus.vulnera_water_undead += val;
			break;
		case SP_VULNERA_WIND_POISON:
			if(sd->state.lr_flag != 2)
				sd->bonus.vulnera_wind_poison += val;
			break;
		case SP_VULNERA_EARTH_NEUTRAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.vulnera_earth_neutral += val;
			break;
		case SP_VULNERA_HOLY_GHOST:
			if(sd->state.lr_flag != 2)
				sd->bonus.vulnera_holy_ghost += val;
			break;
		case SP_VULNERA_ALL:
			if(sd->state.lr_flag != 2)
				sd->bonus.vulnera_all += val;
			break;
		case SP_PERFURACAO:
			if(sd->state.lr_flag != 2)
				sd->bonus.perfuracao += val;
			break;
		case SP_DETERMINACAO:
			if(sd->state.lr_flag != 2)
				sd->bonus.determinacao += val;
			break;
		case SP_PENETRACAO:
			if(sd->state.lr_flag != 2)
				sd->bonus.penetracao += val;
			break;
		case SP_CAP_MILAGRE:
			if(sd->state.lr_flag != 2)
				sd->bonus.cap_milagre += val;
			break;
		case SP_CORRUPCAO_FISICO:
			if(sd->state.lr_flag != 2)
				sd->bonus.corrupcao_fisico += val;
			break;
		case SP_CORRUPCAO_MAGICO:
			if(sd->state.lr_flag != 2)
				sd->bonus.corrupcao_magico += val;
			break;
		case SP_CORRUPCAO_DISTANCIA:
			if(sd->state.lr_flag != 2)
				sd->bonus.corrupcao_distancia += val;
			break;
		case SP_REDUCAO_FINAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.reducao_final += val;
			break;
		case SP_EXP_EXCELENCIA:
			if(sd->state.lr_flag != 2)
				sd->bonus.exp_excelencia += val;
			break;
		case SP_RESIST_CONGELA:
			if(sd->state.lr_flag != 2)
				sd->bonus.resist_congela += val;
			if(sd->bonus.resist_congela > 100) sd->bonus.resist_congela = 100;
			break;
		case SP_RESIST_CAOS:
			if(sd->state.lr_flag != 2)
				sd->bonus.resist_caos += val;
			if(sd->bonus.resist_caos > 100) sd->bonus.resist_caos = 100;
			break;
		case SP_RESIST_ENVENENAR:
			if(sd->state.lr_flag != 2)
				sd->bonus.resist_envenenar += val;
			if(sd->bonus.resist_envenenar > 100) sd->bonus.resist_envenenar = 100;
			break;
		case SP_RESIST_HIPOTERMIA:
			if(sd->state.lr_flag != 2)
				sd->bonus.resist_hipotermia += val;
			if(sd->bonus.resist_hipotermia > 100) sd->bonus.resist_hipotermia = 100;
			break;
		case SP_RESIST_MALDICAO:
			if(sd->state.lr_flag != 2)
				sd->bonus.resist_maldicao += val;
			if(sd->bonus.resist_maldicao > 100) sd->bonus.resist_maldicao = 100;
			break;
		case SP_RESIST_SILENCE:
			if(sd->state.lr_flag != 2)
				sd->bonus.resist_silence += val;
			if(sd->bonus.resist_silence > 100) sd->bonus.resist_silence = 100;
			break;
		case SP_IGNORE_RES:
			if(sd->state.lr_flag != 2)
				sd->bonus.ignore_res += val;
			break;
		case SP_IGNORE_MRES:
			if(sd->state.lr_flag != 2)
				sd->bonus.ignore_mres += val;
			break;
		case SP_DROP_FINAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.drop_final += val;
			break;
		case SP_EXP_FINAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.exp_final += val;
			break;
		case SP_MAGIC_DANO_FINAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.magic_dano_final += val;
			break;
		case SP_LONG_DANO_FINAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.long_dano_final += val;
			break;
		case SP_SHORT_DANO_FINAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.short_dano_final += val;
			break;
		case SP_UNGULA:
			if(sd->state.lr_flag != 2)
				sd->bonus.ungula += val;
		...
		case SP_SPLASH_RANGE:
			...
			break;
		case SP_METALIC_SKILL:
			if(sd->bonus.metalic_skill < val)
				sd->bonus.metalic_skill = val;
			break;
		case SP_RANGE_SKILL:
			if(sd->bonus.range_skill < val)
				sd->bonus.range_skill = val;
			break;
		...
		case SP_DELAYRATE:
			...
			break;
		case SP_AMULETO_NINJA:
			if(sd->state.lr_flag != 2)
				sd->bonus.amuleto_ninja = val;
			break;
		case SP_INSPIRA_DISPELL:
			if(sd->state.lr_flag != 2)
				sd->bonus.inspira_dispell = val;
			break;
		case SP_REDUCAO_NINJA:
			if(sd->state.lr_flag != 2)
				sd->bonus.reducao_ninja += val;
			break;
		case SP_AP_CURA:
			if(sd->state.lr_flag != 2)
				sd->bonus.ap_cura = val;
			break;
		case SP_ILIMITAR:
			if(sd->state.lr_flag != 2)
				sd->bonus.ilimitar = val;
			break;
		case SP_SPLASH_NINJA:
			if(sd->state.lr_flag != 2)
				sd->bonus.splash_ninja = val;
			break;
		case SP_DANO_ELEMENTAL:
			if(sd->state.lr_flag != 2)
				sd->bonus.dano_elemental = val;
			break;
		case SP_STATUS_NO_DEAD:
			if(sd->state.lr_flag != 2)
				sd->bonus.status_no_dead = val;
			break;
		...
		case SP_ABSORB_DMG_MAXHP2:
			...
			break;
		case SP_CAP_DANO:
			sd->bonus.cap_dano += val;
			break;
		case SP_CAP_LIMIT:
			sd->bonus.cap_limit = val;
			break;
		case SP_PICKY_AREA:
			sd->bonus.picky_area = val;
			break;
		case SP_VERIFICA_ONEA:
			sd->bonus.verifica_onea = val;
			break;
		case SP_VERIFICA_ONEB:
			sd->bonus.verifica_oneb = val;
			break;
		case SP_VERIFICA_ONEC:
			sd->bonus.verifica_onec = val;
			break;
		case SP_VERIFICA_ONED:
			sd->bonus.verifica_oned = val;
			break;
		case SP_VERIFICA_ONEE:
			sd->bonus.verifica_onee = val;
			break;
		case SP_VERIFICA_ONEF:
			sd->bonus.verifica_onef = val;
			break;
		case SP_VERIFICA_ONEG:
			sd->bonus.verifica_oneg = val;
			break;
		case SP_VERIFICA_ONEH:
			sd->bonus.verifica_oneh = val;
			break;
		case SP_VERIFICA_ONEI:
			sd->bonus.verifica_onei = val;
			break;
		case SP_VERIFICA_ONEJ:
			sd->bonus.verifica_onej = val;
			break;
		case SP_CAP_REDUCTION:
			sd->bonus.cap_reduction += val;
			break;
...
void pc_bonus2(struct map_session_data *sd,int type,int type2,int val)
{
	...
	case SP_SKILL_ATK: // bonus2 bSkillAtk,sk,n;
		...
		break;
	case SP_SKILL_ATK_DEV: // bonus2 bSkillAtkDev,sk,n;
		if(sd->state.lr_flag == 2)
			break;
		if (sd->skillatkdev.size() == MAX_PC_BONUS) {
			ShowWarning("pc_bonus2: SP_SKILL_ATK_DEV: Reached max (%d) number of skills per character, bonus skill %d (+%d%%) lost.\n", MAX_PC_BONUS, type2, val);
			break;
		}

		pc_bonus_itembonus(sd->skillatkdev, type2, val, false);
		break;
	case SP_SKILL_ATK_FINAL: // bonus2 bSkillAtkFinal,sk,n;
		if(sd->state.lr_flag == 2)
			break;
		if (sd->skillatkfinal.size() == MAX_PC_BONUS) {
			ShowWarning("pc_bonus2: SP_SKILL_ATK_DEV: Reached max (%d) number of skills per character, bonus skill %d (+%d%%) lost.\n", MAX_PC_BONUS, type2, val);
			break;
		}

		pc_bonus_itembonus(sd->skillatkfinal, type2, val, false);
		break;
...
int pc_insert_card(struct map_session_data* sd, int idx_card, int idx_equip)
{
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 44433 || sd->inventory.u.items_inventory[idx_card].nameid == 44435){
		if(item_eq->eggpercent == 90){
			if(sd->inventory.u.items_inventory[idx_card].nameid == 44433) {
				if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 2) return 0;
			}
			if(sd->inventory.u.items_inventory[idx_card].nameid == 44435) {
				if(sd->inventory.u.items_inventory[idx_equip].enchantgrade < 2 || sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 4) return 0;
			}
			if(sd->inventory.u.items_inventory[idx_equip].card[0] != CARD0_PET){
				clif_displaymessage(sd->fd, "Abra esse ovo com uma incubadora antes de adicionar o grade.");
				return 0;
			}
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				clif_insert_card(sd,idx_equip,idx_card,1);
			}
			else
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].enchantgrade = sd->inventory.u.items_inventory[idx_equip].enchantgrade + 1;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
	...
	// Disfarce Pet
	if(sd->inventory.u.items_inventory[idx_card].nameid == 44533){
		if(item_eq->eggpercent == 90){
			if(sd->inventory.u.items_inventory[idx_equip].card[0] != CARD0_PET){
				clif_displaymessage(sd->fd, "Abra esse ovo com uma incubadora antes de adicionar o grade.");
				return 0;
			}
			if(sd->inventory.u.items_inventory[idx_equip].card[3] == 44533){
				clif_displaymessage(sd->fd, "Esse ovo já possui uma fantasia.");
				return 0;
			}
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[3] = 44533;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	int shadowenchant = sd->inventory.u.items_inventory[idx_card].nameid;
	int gradeshadow = 0;
	if(shadowenchant == 44491) gradeshadow = 1;
	if(shadowenchant == 44492) gradeshadow = 1;
	if(shadowenchant == 44493) gradeshadow = 2;
	if(shadowenchant == 44494) gradeshadow = 2;
	if(shadowenchant == 44495) gradeshadow = 3;
	if(shadowenchant == 44496) gradeshadow = 3;
	if(shadowenchant == 44497) gradeshadow = 4;
	if(shadowenchant == 44498) gradeshadow = 4;
	if(shadowenchant == 44499) gradeshadow = 1;
	if(shadowenchant == 44500) gradeshadow = 2;
	if(shadowenchant == 44501) gradeshadow = 3;
	if(shadowenchant == 44502) gradeshadow = 4;

	if(shadowenchant == 44581) gradeshadow = 1;
	if(shadowenchant == 44582) gradeshadow = 1;
	if(shadowenchant == 44583) gradeshadow = 2;
	if(shadowenchant == 44584) gradeshadow = 2;
	if(shadowenchant == 44585) gradeshadow = 3;
	if(shadowenchant == 44586) gradeshadow = 3;
	if(shadowenchant == 44587) gradeshadow = 4;
	if(shadowenchant == 44588) gradeshadow = 4;
	
	if(shadowenchant == 44589) gradeshadow = 1;
	if(shadowenchant == 44590) gradeshadow = 2;
	if(shadowenchant == 44591) gradeshadow = 3;
	if(shadowenchant == 44592) gradeshadow = 4;


	// Shadow Arma
	if((shadowenchant == 44581 || shadowenchant == 44583 || shadowenchant == 44585 || shadowenchant == 44587) && item_eq->equip == EQP_SHADOW_WEAPON && sd->inventory.u.items_inventory[idx_equip].refine >= 9){
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].enchantgrade = gradeshadow;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		return 0;
	...
	// //  Shadow Escudo
	if((shadowenchant == 44582 || shadowenchant == 44584 || shadowenchant == 44586 || shadowenchant == 44588) && item_eq->equip == EQP_SHADOW_SHIELD && sd->inventory.u.items_inventory[idx_equip].refine >= 9){
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	// //  Shadow Bota
	if((shadowenchant == 44589 || shadowenchant == 44590 || shadowenchant == 44591 || shadowenchant == 44592) && item_eq->equip == EQP_SHADOW_SHOES && sd->inventory.u.items_inventory[idx_equip].refine >= 9){
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	if((shadowenchant == 44491 || shadowenchant == 44493 || shadowenchant == 44495 || shadowenchant == 44497) && item_eq->equip == EQP_SHADOW_ACC_R && sd->inventory.u.items_inventory[idx_equip].refine >= 9){
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	// //  Lascas Shadow Colar
	if((shadowenchant == 44492 || shadowenchant == 44494 || shadowenchant == 44496 || shadowenchant == 44498) && item_eq->equip == EQP_SHADOW_ACC_L && sd->inventory.u.items_inventory[idx_equip].refine >= 9){
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	// //  Lascas Shadow Armor
	if((shadowenchant >= 44499 && shadowenchant <= 44502) && item_eq->equip == EQP_SHADOW_ARMOR && sd->inventory.u.items_inventory[idx_equip].refine >= 9){
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	if(shadowenchant >= 44510 && shadowenchant <= 44513 && item_eq->equip == EQP_COSTUME_HEAD_TOP){
			visu = shadowenchant - 44509;
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].enchantgrade = visu;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		return 0;
	...
	// meio
	if(shadowenchant >= 44514 && shadowenchant <= 44517 && item_eq->equip == EQP_COSTUME_HEAD_MID){
			visu = shadowenchant - 44513;
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	// baixo
	if(shadowenchant >= 44518 && shadowenchant <= 44521 && item_eq->equip == EQP_COSTUME_HEAD_LOW){
			visu = shadowenchant - 44517;
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	// Capa
	if(shadowenchant >= 44522 && shadowenchant <= 44525 && item_eq->equip == EQP_COSTUME_GARMENT){
			visu = shadowenchant - 44521;
			if(sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 1){
				clif_displaymessage(sd->fd, "Remova o grade atual do item antes de tentar adicionar outro.");
				return 0;
			}
	...
	// Pedra Mineração
	if(sd->inventory.u.items_inventory[idx_card].nameid == 50304 || sd->inventory.u.items_inventory[idx_card].nameid == 50305 || sd->inventory.u.items_inventory[idx_card].nameid == 50306){
		if(item_eq->nameid == 66004){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[3])
				verifica = 1;
			if(verifica> 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			}
			// --------------------------------------------------------------
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[3] = carta;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Pedra Herbologia
	if(sd->inventory.u.items_inventory[idx_card].nameid == 50307 || sd->inventory.u.items_inventory[idx_card].nameid == 50308 || sd->inventory.u.items_inventory[idx_card].nameid == 50309){
		if(item_eq->nameid == 66003){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
	...
	// Pedra Pesca
	if(sd->inventory.u.items_inventory[idx_card].nameid == 50315 || sd->inventory.u.items_inventory[idx_card].nameid == 50316 || sd->inventory.u.items_inventory[idx_card].nameid == 50317){
		if(item_eq->nameid == 66001){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
	...
	// Aegis da cidade celestial
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312305){
		if(item_eq->nameid == 490337){
			int slot = 1;
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[slot])
				verifica = 1;
			if(verifica > 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[slot] = carta;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312973){
		if(item_eq->nameid == 400541){
			int slot = 3;
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312923){
		if(item_eq->nameid == 550052){
			int slot = 3;
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312396){
		if(item_eq->nameid == 610028 || item_eq->nameid == 510017){
			int slot = 3;
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312924){
		if(item_eq->nameid == 19321 || item_eq->nameid == 19101 || item_eq->nameid == 420110 || item_eq->nameid == 420199){
			int slot = 0;
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 3101970){
		if(item_eq->nameid == 470243){
			int slot = 3;
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
	...
	// Hero's Triumph
	if(sd->inventory.u.items_inventory[idx_card].nameid == 311074){
		if(item_eq->nameid == 480188 || item_eq->nameid == 400213 || item_eq->nameid == 490207 || item_eq->nameid == 490337 || item_eq->nameid == 490545){

			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[2])
				verifica = 1;
			if(verifica> 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[2] = 311074;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
		if(item_eq->nameid == 490381){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[2] && sd->inventory.u.items_inventory[idx_equip].card[2] == sd->inventory.u.items_inventory[idx_card].nameid){
				clif_displaymessage(sd->fd, "Não é possível encantar este item com esse encantamento duas vezes.");
				return 0;
			}
			if(sd->inventory.u.items_inventory[idx_equip].card[2] && sd->inventory.u.items_inventory[idx_equip].card[3])
				verifica = 1;
			if(verifica> 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			...
			{// success
				...
				if(sd->inventory.u.items_inventory[idx_equip].card[2])
					...
					sd->inventory.u.items_inventory[idx_equip].card[2] = carta;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Universalis
	if(sd->inventory.u.items_inventory[idx_card].nameid == 310655){
		if(item_eq->nameid == 420199 || item_eq->nameid == 420236){
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[3] = 310655;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Refinar PET equip
	if(sd->inventory.u.items_inventory[idx_card].nameid == 77770){
		if(item_eq->nameid >= 77730 && item_eq->nameid <= 77732){
			
			int qtddel = 1;
			if(sd->inventory.u.items_inventory[idx_equip].refine > 0) 
				qtddel += sd->inventory.u.items_inventory[idx_equip].refine;
			if(sd->inventory.u.items_inventory[idx_equip].refine >= 15){
				clif_displaymessage(sd->fd, "O nível máximo de refinamento deste equipamento até o momento é +15.");
				return 0;
			}
			if( pc_delitem(sd,idx_card,qtddel,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				char msgfailed[256];
				sprintf(msgfailed, "Você não possui a quantidade necessária de %d Cartões de Refinamento Pet para subir o nível desse item.", qtddel);
				clif_displaymessage(sd->fd, msgfailed);
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			}
			else
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].refine += 1;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);

				clif_refine(sd->fd,0,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
	...
	// Refinar PET equip
	if(sd->inventory.u.items_inventory[idx_card].nameid == 77771){
		if(item_eq->nameid == 77733){
	...
	// Ruler of The End / Granada do Poder / rancor do assassino
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312421 || sd->inventory.u.items_inventory[idx_card].nameid == 311883){
		if(item_eq->nameid == 490381 || item_eq->nameid == 420199 ||
		   item_eq->nameid == 420236 || item_eq->nameid == 420110
		){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
	...
	// rancor do assassino
	if(sd->inventory.u.items_inventory[idx_card].nameid == 29724){
		if(item_eq->nameid == 420199 ||
		   item_eq->nameid == 420236 || item_eq->nameid == 420329
		){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
	...
	// Foreign Conquerer
	if(sd->inventory.u.items_inventory[idx_card].nameid == 311439){
		if(item_eq->nameid == 420110 || item_eq->nameid == 420164){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
	...
	// Abyssal King
	if(sd->inventory.u.items_inventory[idx_card].nameid == 310321){
		if(item_eq->nameid == 420110 || item_eq->nameid == 420164){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
		...
		if(item_eq->nameid == 490381 || item_eq->nameid == 490337){
			int carta = sd->inventory.u.items_inventory[idx_card].nameid;
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 311075){
		if(item_eq->nameid == 480188 || item_eq->nameid == 400213){
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[2] = 311075;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
		if(item_eq->nameid == 410247){
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[3] = 311075;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Dynast
	if(sd->inventory.u.items_inventory[idx_card].nameid == 29436){
		if(
			item_eq->nameid == 400213 || item_eq->nameid == 400203 ||
			item_eq->nameid == 400187 || item_eq->nameid == 400177 ||
			item_eq->nameid == 15892 || item_eq->nameid == 15891 ||
			item_eq->nameid == 450208 || item_eq->nameid == 450209 ||
			item_eq->nameid == 450210 || item_eq->nameid == 450214 ||
			item_eq->nameid == 450216 || item_eq->nameid == 490206 || 
			item_eq->nameid == 480188 || item_eq->nameid == 460023 ||
			item_eq->nameid == 490207 || item_eq->nameid == 490290 || 
			item_eq->nameid == 490381 || item_eq->nameid == 490404 || 
			item_eq->nameid == 490545 || item_eq->nameid == 460078
		){
			...
			int verifica = 0;
			if(item_eq->nameid == 490381){
				if(sd->inventory.u.items_inventory[idx_equip].card[1])
					verifica = 1;
			} else if((item_eq->nameid == 480188 || item_eq->nameid == 460023 || item_eq->nameid == 460023 || item_eq->nameid == 490381 || item_eq->nameid == 490290 || item_eq->nameid == 490545 || item_eq->nameid == 490404) && sd->inventory.u.items_inventory[idx_equip].card[2] && (sd->inventory.u.items_inventory[idx_equip].card[2] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[2] > 4858))
				verifica = 1;
			else if(sd->inventory.u.items_inventory[idx_equip].card[3] && item_eq->nameid != 490290)
				verifica = 1;
			if(verifica > 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				if(item_eq->nameid == 490381)
					sd->inventory.u.items_inventory[idx_equip].card[1] = 29436;
				else if(item_eq->nameid == 480188 || item_eq->nameid == 460023 || item_eq->nameid == 490290 || item_eq->nameid == 490545 || item_eq->nameid == 490404 || item_eq->nameid == 460078)
					sd->inventory.u.items_inventory[idx_equip].card[2] = 29436;
				else
					sd->inventory.u.items_inventory[idx_equip].card[3] = 29436;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	//Sky Emperor
	if(sd->inventory.u.items_inventory[idx_card].nameid == 313062 || sd->inventory.u.items_inventory[idx_card].nameid == 313286){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(
			item_eq->nameid == 400543
		){
			// --------------------------------------------------------------
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
	...
	//Windhawk
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312036 || sd->inventory.u.items_inventory[idx_card].nameid == 312918){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(
			item_eq->nameid == 400539
		){
			// --------------------------------------------------------------
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 313479){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(
			item_eq->nameid == 400541
		){
			// --------------------------------------------------------------
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 313282){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(
			item_eq->nameid == 400546
		){
			// --------------------------------------------------------------
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
	...
	//Archmage
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312031 || sd->inventory.u.items_inventory[idx_card].nameid == 312473){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(
			item_eq->nameid == 400535
		){
			// --------------------------------------------------------------
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
	...
	//Poder Oculto Baixo
	if(sd->inventory_data[idx_card]->nameid == 313490 || sd->inventory_data[idx_card]->nameid == 313321 || sd->inventory_data[idx_card]->nameid == 312972 || sd->inventory_data[idx_card]->nameid == 312974 || sd->inventory_data[idx_card]->nameid == 312646 || sd->inventory_data[idx_card]->nameid == 312928 || sd->inventory_data[idx_card]->nameid == 311895 || sd->inventory_data[idx_card]->nameid == 312918 || sd->inventory_data[idx_card]->nameid == 312964 || 
	   sd->inventory_data[idx_card]->nameid == 312963 || sd->inventory_data[idx_card]->nameid == 312645 || sd->inventory_data[idx_card]->nameid == 312934 || sd->inventory_data[idx_card]->nameid == 312691 || sd->inventory_data[idx_card]->nameid == 312034){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(
			item_eq->nameid == 420236
		){
			// --------------------------------------------------------------
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[0] = carta;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	if(sd->inventory_data[idx_card]->nameid == 312038 || sd->inventory_data[idx_card]->nameid == 312304){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(item_eq->nameid == 400537){
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[3])
	...
	if(sd->inventory_data[idx_card]->nameid == 312933 || sd->inventory_data[idx_card]->nameid == 313279){
		int carta = sd->inventory.u.items_inventory[idx_card].nameid;
		if(item_eq->nameid == 400538){
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[3])
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 311073){
		if(item_eq->nameid == 400213 || item_eq->nameid == 470112){
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[2])
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[2] = 311073;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Epifania
	if(sd->inventory.u.items_inventory[idx_card].nameid == 47777){
		if(
			item_eq->nameid == 400213 || item_eq->nameid == 400203 || item_eq->nameid == 490381 ||
			item_eq->nameid == 400187 || item_eq->nameid == 400177 ||
			item_eq->nameid == 15892 || item_eq->nameid == 15891 ||
			item_eq->nameid == 490206 || item_eq->nameid == 490193 ||
			item_eq->nameid == 490183 || item_eq->nameid == 490207 ||
			item_eq->nameid == 480188 || item_eq->nameid == 450216 ||
			item_eq->nameid == 450214 || item_eq->nameid == 450208 ||
			item_eq->nameid == 450209 || item_eq->nameid == 450210 ||
			item_eq->nameid == 470112 || item_eq->nameid == 490290 ||
			item_eq->nameid == 450242 || item_eq->nameid == 470274 || 
			item_eq->nameid == 490404
		){
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			if(item_eq->nameid == 490381){
				if(sd->inventory.u.items_inventory[idx_equip].card[1])
					verifica = 1;
			} else if(sd->inventory.u.items_inventory[idx_equip].card[3] && item_eq->nameid != 490290 && item_eq->nameid != 490404)
				verifica = 1;
			else if(sd->inventory.u.items_inventory[idx_equip].card[2] && item_eq->nameid == 490290 && (sd->inventory.u.items_inventory[idx_equip].card[2] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[2] > 4858))
				verifica = 1;
			if(verifica > 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				if(item_eq->nameid == 490381)
					sd->inventory.u.items_inventory[idx_equip].card[1] = 4876;
				else if(item_eq->nameid == 490290 || item_eq->nameid == 490404)
					sd->inventory.u.items_inventory[idx_equip].card[2] = 4876;
				else
					sd->inventory.u.items_inventory[idx_equip].card[3] = 4876;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Release
	if(sd->inventory.u.items_inventory[idx_card].nameid == 29371){
		if(
			item_eq->nameid == 400213 || item_eq->nameid == 19315 || item_eq->nameid == 490381 ||
			item_eq->nameid == 490206 || item_eq->nameid == 490193 ||
			item_eq->nameid == 490183 || item_eq->nameid == 490207 ||
			item_eq->nameid == 450220 || item_eq->nameid == 20991 ||
			item_eq->nameid == 470128 || item_eq->nameid == 470037 ||
			item_eq->nameid == 22214 || item_eq->nameid == 470049 ||
			item_eq->nameid == 22224 || item_eq->nameid == 22225 ||
			item_eq->nameid == 470061 || item_eq->nameid == 490290 || item_eq->nameid == 490337 ||
			item_eq->nameid == 470180 || item_eq->nameid == 490404 || item_eq->nameid == 490545 || 
			item_eq->nameid == 4701800
		){
			...
			if(item_eq->nameid == 490381){
				if(sd->inventory.u.items_inventory[idx_equip].card[1])
					verifica = 1;
			} else if(item_eq->nameid == 450220 || item_eq->nameid == 20991 || item_eq->nameid == 470128 || item_eq->nameid == 490290){
				if(sd->inventory.u.items_inventory[idx_equip].card[2] && (sd->inventory.u.items_inventory[idx_equip].card[2] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[2] > 4858))
					verifica = 1;
			} else if (sd->inventory.u.items_inventory[idx_equip].card[3] && (sd->inventory.u.items_inventory[idx_equip].card[3] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[3] > 4858))
					verifica = 1;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				if(item_eq->nameid == 490381)
					sd->inventory.u.items_inventory[idx_equip].card[1] = 29371;
				else if(item_eq->nameid == 450220 || item_eq->nameid == 20991 || item_eq->nameid == 470128 || item_eq->nameid == 490290){
					sd->inventory.u.items_inventory[idx_equip].card[2] = 29371;
				} else {
					sd->inventory.u.items_inventory[idx_equip].card[3] = 29371;
				}
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
	...
	// Arcana
	if(sd->inventory.u.items_inventory[idx_card].nameid == 29585){
		if(item_eq->nameid == 480045 || item_eq->nameid == 400109 || item_eq->nameid == 400110 || item_eq->nameid == 400111 || item_eq->nameid == 50030){
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[3] = 29585;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Pedra Telecinética
	if(sd->inventory.u.items_inventory[idx_card].nameid == 44700){
		if(
			item_eq->nameid == 410028
		){
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[3] = 44700;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 54435){
		if(
			item_eq->nameid == 19446
		){
			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[2] = 54435;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Hero
	if(sd->inventory.u.items_inventory[idx_card].nameid == 29509){
		if(
			item_eq->nameid == 400213 || item_eq->nameid == 490381 ||
			item_eq->nameid == 490206 || item_eq->nameid == 490193 ||
			item_eq->nameid == 490183 || item_eq->nameid == 490207 ||
			item_eq->nameid == 450220 || item_eq->nameid == 470048 ||
			item_eq->nameid == 470128 || item_eq->nameid == 470037 ||
			item_eq->nameid == 22214 || item_eq->nameid == 470049 ||
			item_eq->nameid == 22224 || item_eq->nameid == 22225 ||
			item_eq->nameid == 470061 || item_eq->nameid == 490290 || item_eq->nameid == 490337 ||
			...
			if(item_eq->nameid == 490381){
				if(sd->inventory.u.items_inventory[idx_equip].card[1])
					verifica = 1;
			} else if((item_eq->nameid == 450220 || item_eq->nameid == 470128 || item_eq->nameid == 490290) && sd->inventory.u.items_inventory[idx_equip].card[2] && (sd->inventory.u.items_inventory[idx_equip].card[2] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[2] > 4858))
				verifica = 1;
			else if(sd->inventory.u.items_inventory[idx_equip].card[3] && item_eq->nameid != 490290)
				verifica = 1;
			if(verifica > 0){
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				if(item_eq->nameid == 490381)
					sd->inventory.u.items_inventory[idx_equip].card[1] = 29509;
				else if(item_eq->nameid == 450220 || item_eq->nameid == 470128 || item_eq->nameid == 490290 || item_eq->nameid == 490404)
					sd->inventory.u.items_inventory[idx_equip].card[2] = 29509;
				else
					sd->inventory.u.items_inventory[idx_equip].card[3] = 29509;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Hero
	if(sd->inventory.u.items_inventory[idx_card].nameid == 311365){
		if(
			item_eq->nameid == 490206 || item_eq->nameid == 470180
		){

			// trava adição
			// --------------------------------------------------------------
			int verifica = 0;
			int noblesseQuarto = 0;
			if(sd->inventory.u.items_inventory[idx_equip].card[3] && item_eq->nameid == 490206){
				noblesseQuarto = 1;
				if(sd->inventory.u.items_inventory[idx_equip].card[2] && item_eq->nameid == 490206) verifica = 1;
			}
			if(sd->inventory.u.items_inventory[idx_equip].card[2] && item_eq->nameid == 470180)
				verifica = 1;
			if(verifica > 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				if(item_eq->nameid == 490206 && !noblesseQuarto) sd->inventory.u.items_inventory[idx_equip].card[3] = 311365;
				else if(item_eq->nameid == 490206) sd->inventory.u.items_inventory[idx_equip].card[2] = 311365;	
				else if(item_eq->nameid == 470180) sd->inventory.u.items_inventory[idx_equip].card[2] = 311365;				
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Fury
	if(sd->inventory.u.items_inventory[idx_card].nameid == 312475){
		if(
			item_eq->nameid == 400213 || item_eq->nameid == 490381 ||
			item_eq->nameid == 490206 || item_eq->nameid == 490193 ||
			item_eq->nameid == 490183 || item_eq->nameid == 490207 ||
			...
			item_eq->nameid == 22214 || item_eq->nameid == 470049 ||
			item_eq->nameid == 22224 || item_eq->nameid == 22225 ||
			item_eq->nameid == 470061 || item_eq->nameid == 490290 || 
			item_eq->nameid == 470254 || item_eq->nameid == 490404 
		){
			...
			if(item_eq->nameid == 490381){
				...
				verifica = 1;
			else if(sd->inventory.u.items_inventory[idx_equip].card[3])
				verifica = 1;
			if(verifica > 0){
				clif_displaymessage(sd->fd, "Não é possível encantar um item que já possui encantamento neste slot.");
				return 0;
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				if(item_eq->nameid == 490381)
					sd->inventory.u.items_inventory[idx_equip].card[1] = 312475;
				else if(item_eq->nameid == 450220 || item_eq->nameid == 470128 || item_eq->nameid == 490290)
					sd->inventory.u.items_inventory[idx_equip].card[2] = 312475;
				else
					sd->inventory.u.items_inventory[idx_equip].card[3] = 312475;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	// Raiva
	if(sd->inventory.u.items_inventory[idx_card].nameid == 610198){
		if(
			item_eq->nameid == 470254
		){
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[2] = 610198;
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_inventorylist(sd);
			}
		}
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 48101){
		if(item_eq->nameid == 19379){
			int inicial[17] = { 29012,29248,4900,4801,4818,29135,4811,4832,4815,4817,29136,4810,4833,4814,4946,4944,29372 };
			int segundo[17] = { 29012,29248,4900,4801,4818,29135,4811,4832,4815,4817,29136,4810,4833,4814,4946,4944,29372 };
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[1] = inicial[rnd()%16];
				sd->inventory.u.items_inventory[idx_equip].card[2] = segundo[rnd()%16];
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_insert_card(sd,idx_equip,idx_card,0);
			}
		} else if(item_eq->nameid == 450165){
			int inicial[11] = { 29012,29248,4900,4801,4818,29135,4811,4832,4815,4869,29242 };
			int segundo[15] = { 29012,29248,4900,4801,4818,29135,4811,4832,4815,4869,29242,29305,29302,29303,29304};
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[1] = inicial[rnd()%10];
				sd->inventory.u.items_inventory[idx_equip].card[2] = segundo[rnd()%14];
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_insert_card(sd,idx_equip,idx_card,0);
			}
		} else if(item_eq->nameid == 480114){
			int inicial[13] = { 27398,27397,29214,29215,29216,29217,29218,29219,29220,29221,29222,4825,29243 };
			int segundo[2] = { 29593, 310269 };
			...
			{// success
				log_pick_pc(sd, LOG_TYPE_NPC, -1, &sd->inventory.u.items_inventory[idx_equip]);
				sd->inventory.u.items_inventory[idx_equip].card[1] = inicial[rnd()%12];
				sd->inventory.u.items_inventory[idx_equip].card[2] = segundo[rnd()%1];
				log_pick_pc(sd, LOG_TYPE_NPC,  1, &sd->inventory.u.items_inventory[idx_equip]);
				clif_insert_card(sd,idx_equip,idx_card,0);
			}
		}
	...
	// Pegasus Enchant Box
	if(sd->inventory.u.items_inventory[idx_card].nameid == 48102){
		if(item_eq->nameid == 470128){
			int inicial[9] = { 29214,29215,29216,29217,29218,29219,29220,29221,29222 };
			int segundo[9] = { 29214,29215,29216,29217,29218,29219,29220,29221,29222 };
	...
	// mini-refinadora
	if((sd->inventory.u.items_inventory[idx_card].nameid == 33362 || sd->inventory.u.items_inventory[idx_card].nameid == 33364) && minirefinadora(item_eq->nameid)){
		if(sd->inventory.u.items_inventory[idx_equip].refine == 10 || sd->inventory.u.items_inventory[idx_equip].refine == 11 || sd->inventory.u.items_inventory[idx_equip].refine == 12){
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			} else {
				...
				// devon historico
				if( Sql_Query( mmysql_handle, "INSERT INTO `log_historico_itens`(`item_id`, `amount`, `refinamento`, `card0`, `card1`, `card2`, `card3`, `tipo`, `char_id`, `account_id` ) VALUES( %d, 1, %d, %d, %d, %d, %d, 'REFINADORA_COMPLEXA', %d, %d );",
					sd->inventory.u.items_inventory[idx_equip].nameid, sd->inventory.u.items_inventory[idx_equip].refine, sd->inventory.u.items_inventory[idx_equip].card[0], 
					sd->inventory.u.items_inventory[idx_equip].card[1], sd->inventory.u.items_inventory[idx_equip].card[2], sd->inventory.u.items_inventory[idx_equip].card[3],
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 70153){
		if((item_eq->armor_level == 2 || item_eq->weapon_level == 5) && 
			(sd->inventory.u.items_inventory[idx_equip].refine == 13 || sd->inventory.u.items_inventory[idx_equip].refine == 14)){
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			} else {
				int chance = 8;
				if(sd->inventory.u.items_inventory[idx_equip].refine == 14) chance = 4;
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 33360 || sd->inventory.u.items_inventory[idx_card].nameid == 33361){
		if((item_eq->weapon_level == 5 || item_eq->armor_level == 2) && 
			(sd->inventory.u.items_inventory[idx_equip].refine == 10 || sd->inventory.u.items_inventory[idx_equip].refine == 11)){
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			} else {
	...
	// Refinadora Complexa Avançada
	if(sd->inventory.u.items_inventory[idx_card].nameid == 33363){
		if((item_eq->weapon_level == 5 || item_eq->armor_level == 2) && 
			sd->inventory.u.items_inventory[idx_equip].refine == 12){
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			} else {
	...
	// Despertar
	if(sd->inventory.u.items_inventory[idx_card].nameid == 50318 || sd->inventory.u.items_inventory[idx_card].nameid == 50320){
		int refinedesp = 13;
		if(sd->inventory.u.items_inventory[idx_card].nameid == 50320) refinedesp = 16;
		if(item_eq->weapon_level == 5 && sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 4 && sd->inventory.u.items_inventory[idx_equip].despertar < refinedesp){
			int qtddel = 1;
			if(sd->inventory.u.items_inventory[idx_equip].despertar > 0) 
				qtddel += sd->inventory.u.items_inventory[idx_equip].despertar - 6;
			else 
				qtddel = 3;
			if( pc_delitem(sd,idx_card,qtddel,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				char msgfailed[256];
				sprintf(msgfailed, "Você não possui a quantidade necessária de %d Pedra(s) do Despertar para subir o nível desse item.", qtddel);
				clif_displaymessage(sd->fd, msgfailed);
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			} else {
				if(sd->inventory.u.items_inventory[idx_equip].despertar <= 0)
					sd->inventory.u.items_inventory[idx_equip].despertar = 7;
				else
					sd->inventory.u.items_inventory[idx_equip].despertar += 1;
				clif_refine(sd->fd,0,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
	...
	// Despertar Armadura
	if(sd->inventory.u.items_inventory[idx_card].nameid == 50322){
		if(
		(sd->inventory.u.items_inventory[idx_equip].nameid == 450282 || sd->inventory.u.items_inventory[idx_equip].nameid == 450283 || 
		sd->inventory.u.items_inventory[idx_equip].nameid == 450284 || sd->inventory.u.items_inventory[idx_equip].nameid == 450169 || 
		sd->inventory.u.items_inventory[idx_equip].nameid == 450170 || sd->inventory.u.items_inventory[idx_equip].nameid == 450171 || 
		sd->inventory.u.items_inventory[idx_equip].nameid == 450172 || sd->inventory.u.items_inventory[idx_equip].nameid == 450173) && 
		sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 4 && 
		sd->inventory.u.items_inventory[idx_equip].despertar < 16){
			int qtddel = 1;
			if(sd->inventory.u.items_inventory[idx_equip].despertar > 0) 
				qtddel += sd->inventory.u.items_inventory[idx_equip].despertar - 6;
			else 
	...
	// Mini Despertar
	if(sd->inventory.u.items_inventory[idx_card].nameid == 50319 || sd->inventory.u.items_inventory[idx_card].nameid == 50321){
		int refinedesp = 13;
		if(sd->inventory.u.items_inventory[idx_card].nameid == 50321) refinedesp = 16;
		if(minirefinadora(item_eq->nameid)){
			if(item_eq->weapon_level == 5 && sd->inventory.u.items_inventory[idx_equip].enchantgrade >= 4 && sd->inventory.u.items_inventory[idx_equip].despertar < refinedesp){
				int qtddel = 1;
				if(sd->inventory.u.items_inventory[idx_equip].despertar > 0) 
					qtddel += sd->inventory.u.items_inventory[idx_equip].despertar - 6;
				...
				{// failed
					...
					sd->inventory.u.items_inventory[idx_equip].despertarmenor = 1;
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 33369){
		if((item_eq->weapon_level == 5 || item_eq->armor_level == 2) && 
			(sd->inventory.u.items_inventory[idx_equip].refine == 12 || sd->inventory.u.items_inventory[idx_equip].refine == 13)){
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			} else {
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 33368){
		if((sd->inventory.u.items_inventory[idx_equip].nameid == 470071 || sd->inventory.u.items_inventory[idx_equip].nameid == 470072 || sd->inventory.u.items_inventory[idx_equip].nameid == 470073 || 
				sd->inventory.u.items_inventory[idx_equip].nameid == 470074 || sd->inventory.u.items_inventory[idx_equip].nameid == 470076 || sd->inventory.u.items_inventory[idx_equip].nameid == 470077) && 
			(sd->inventory.u.items_inventory[idx_equip].refine >= 10 && sd->inventory.u.items_inventory[idx_equip].refine <= 12)){
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				clif_refine(sd->fd,1,idx_equip,sd->inventory.u.items_inventory[idx_equip].refine);
			} else {
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 40078){
		if(sd->inventory.u.items_inventory[idx_equip].nameid == 28962 || sd->inventory.u.items_inventory[idx_equip].nameid == 51121){
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				
			} else {
				int rand = 4853 + rnd()%6, rand2 = 4853 + rnd()%6;
				sd->inventory.u.items_inventory[idx_equip].card[2] = rand;
				sd->inventory.u.items_inventory[idx_equip].card[3] = rand2;
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 40079){
		if(sd->inventory.u.items_inventory[idx_equip].nameid == 490290){
			if(
				(sd->inventory.u.items_inventory[idx_equip].card[1] > 0 && (sd->inventory.u.items_inventory[idx_equip].card[1] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[1] > 4858)) ||
				(sd->inventory.u.items_inventory[idx_equip].card[2] > 0 && (sd->inventory.u.items_inventory[idx_equip].card[2] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[2] > 4858)) ||
				(sd->inventory.u.items_inventory[idx_equip].card[3] > 0 && (sd->inventory.u.items_inventory[idx_equip].card[3] < 4853 || sd->inventory.u.items_inventory[idx_equip].card[3] > 4858))
			){
				clif_displaymessage(sd->fd, "Remova o encantamento especial antes de adicionar outro.");
				return 0;
			}
			if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
			{// failed
				...
				int rand = 4853 + rnd()%6, rand2 = 4853 + rnd()%6, rand3 = 4853 + rnd()%6;

				sd->inventory.u.items_inventory[idx_equip].card[1] = rand;
				sd->inventory.u.items_inventory[idx_equip].card[2] = rand2;
				sd->inventory.u.items_inventory[idx_equip].card[3] = rand3;
	...
	if(sd->inventory_data[idx_equip]->equip==EQP_COSTUME_HEAD_LOW &&
	(sd->inventory.u.items_inventory[idx_card].nameid >= 2011369 && sd->inventory.u.items_inventory[idx_card].nameid <= 2011404)) {
		if(sd->inventory.u.items_inventory[idx_equip].card[3] > 0) {
			clif_displaymessage(sd->fd, "Remova a carta antes de adicionar outra.");
			return 0;
		} else {
			int cardid = sd->inventory.u.items_inventory[idx_card].nameid;
			if(pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1) {
				clif_insert_card(sd,idx_equip,idx_card,1);
			} else {
				sd->inventory.u.items_inventory[idx_equip].card[3] = cardid;
				clif_displaymessage(sd->fd, "Carta marionete adicionada com sucesso ao visual.");
				clif_insert_card(sd,idx_equip,idx_card,0); 
				clif_inventorylist(sd);
			}
		}
	...
	if(sd->inventory_data[idx_equip]->equip==EQP_COSTUME_GARMENT &&
	(sd->inventory.u.items_inventory[idx_card].nameid >= 2011405 && sd->inventory.u.items_inventory[idx_card].nameid <= 2011440)) {
		if(sd->inventory.u.items_inventory[idx_equip].card[1] > 0) {
			clif_displaymessage(sd->fd, "Remova a carta antes de adicionar outra.");
			return 0;
		} else {
			int cardid = sd->inventory.u.items_inventory[idx_card].nameid;
			if(pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1) {
				...
				sd->inventory.u.items_inventory[idx_equip].card[1] = cardid;
				clif_displaymessage(sd->fd, "Carta marionete adicionada com sucesso ao visual.");
				clif_insert_card(sd,idx_equip,idx_card,0); 
				clif_inventorylist(sd);
			}
	...
	// carta marionete topo slot 2
	if(sd->inventory_data[idx_equip]->equip==EQP_COSTUME_HEAD_TOP &&
	(sd->inventory.u.items_inventory[idx_card].nameid >= 1011260 && sd->inventory.u.items_inventory[idx_card].nameid <= 1011296)) {
		if(sd->inventory.u.items_inventory[idx_equip].card[1] > 0) {
			clif_displaymessage(sd->fd, "Remova a carta antes de adicionar outra.");
			return 0;
		} else {
	...
	// carta marionete topo slot 4
	if(sd->inventory_data[idx_equip]->equip==EQP_COSTUME_HEAD_TOP &&
	(sd->inventory.u.items_inventory[idx_card].nameid >= 1011297 && sd->inventory.u.items_inventory[idx_card].nameid <= 1011332)) {
		if(sd->inventory.u.items_inventory[idx_equip].card[3] > 0) {
			clif_displaymessage(sd->fd, "Remova a carta antes de adicionar outra.");
			return 0;
		} else {
	...
	// carta marionete meio 
	if(sd->inventory_data[idx_equip]->equip==EQP_COSTUME_HEAD_MID &&
	(sd->inventory.u.items_inventory[idx_card].nameid >= 1011333 && sd->inventory.u.items_inventory[idx_card].nameid <= 1011368)) {
		if(sd->inventory.u.items_inventory[idx_equip].card[3] > 0) {
			clif_displaymessage(sd->fd, "Remova a carta antes de adicionar outra.");
			return 0;
		} else {
	...
	// carta marionete baixo
	if(sd->inventory_data[idx_equip]->equip==EQP_COSTUME_HEAD_LOW &&
	(sd->inventory.u.items_inventory[idx_card].nameid >= 1011369 && sd->inventory.u.items_inventory[idx_card].nameid <= 1011404)) {
		if(sd->inventory.u.items_inventory[idx_equip].card[1] > 0) {
			clif_displaymessage(sd->fd, "Remova a carta antes de adicionar outra.");
			return 0;
		} else {
	...
	// carta marionete capa 
	if(sd->inventory_data[idx_equip]->equip==EQP_COSTUME_GARMENT &&
	(sd->inventory.u.items_inventory[idx_card].nameid >= 1011405 && sd->inventory.u.items_inventory[idx_card].nameid <= 1011440)) {
		if(sd->inventory.u.items_inventory[idx_equip].card[3] > 0) {
			clif_displaymessage(sd->fd, "Remova a carta antes de adicionar outra.");
			return 0;
		} else {
	...
	// devon
	if(sd->inventory_data[idx_equip]->equip==EQP_SHIELD && sd->inventory.u.items_inventory[idx_equip].card[0] && (
		(sd->inventory.u.items_inventory[idx_card].nameid >= 27050 && sd->inventory.u.items_inventory[idx_card].nameid <= 27054) || (sd->inventory.u.items_inventory[idx_card].nameid >= 27129 && sd->inventory.u.items_inventory[idx_card].nameid<= 27140) || 
		(sd->inventory.u.items_inventory[idx_card].nameid>= 27364 && sd->inventory.u.items_inventory[idx_card].nameid <= 27367) || (sd->inventory.u.items_inventory[idx_card].nameid >= 300131 && sd->inventory.u.items_inventory[idx_card].nameid <= 300136) || 
		(sd->inventory.u.items_inventory[idx_card].nameid>= 300163 && sd->inventory.u.items_inventory[idx_card].nameid <= 300168) || (sd->inventory.u.items_inventory[idx_card].nameid >= 300282 && sd->inventory.u.items_inventory[idx_card].nameid <= 300289))){
			int cardid = sd->inventory.u.items_inventory[idx_card].nameid;
		if( pc_delitem(sd,idx_card,1,1,0,LOG_TYPE_NPC) == 1 )
		{// failed
			clif_insert_card(sd,idx_equip,idx_card,1);
	...
	// devon
	if(item_card->equip == EQP_COSTUME_GARMENT){
		if(item_card->dualcard >= 1){
			if(sd->inventory.u.items_inventory[idx_equip].card[0] == 0) return 0;
			item_eq->slots = 2;
		} else 
			item_eq->slots = 1;
	}

	if(item_card->equip == EQP_COSTUME_HEAD_MID || item_card->equip == EQP_COSTUME_HEAD_TOP || item_card->equip == EQP_COSTUME_HEAD_LOW){
		int slots = 1;
		if(item_eq->equip == (EQP_COSTUME_HEAD_MID|EQP_COSTUME_HEAD_TOP)) slots = 2;
		if(item_eq->equip == (EQP_COSTUME_HEAD_MID|EQP_COSTUME_HEAD_LOW)) slots = 2;
		if(item_eq->equip == (EQP_COSTUME_HEAD_TOP|EQP_COSTUME_HEAD_LOW)) slots = 2;
		if(item_eq->equip == (EQP_COSTUME_HEAD_MID|EQP_COSTUME_HEAD_TOP|EQP_COSTUME_HEAD_LOW)) slots = 3;
		if(item_card->dualcard >= 1){
			if(sd->inventory.u.items_inventory[idx_equip].card[0] == 0) return 0;
			slots += 1;
		}
	...
	if(sd->inventory.u.items_inventory[idx_card].nameid == 29300 && sd->inventory.u.items_inventory[idx_equip].nameid == 32242){
		item_eq->slots = 1;
	}
	...
	int tradecard0 = sd->inventory.u.items_inventory[idx_card].tradecard0;
	...
	{// success
		...
		if(tradecard0 && i == 0){
			sd->inventory.u.items_inventory[idx_equip].tradecard0 = 1;
			clif_inventorylist(sd);
		}
		if(tradecard0 && i == 1){
			sd->inventory.u.items_inventory[idx_equip].tradecard1 = 1;
			clif_inventorylist(sd);
		}
		if(tradecard0 && i == 2){
			sd->inventory.u.items_inventory[idx_equip].tradecard2 = 1;
			clif_inventorylist(sd);
		}
		if(tradecard0 && i == 3){
			sd->inventory.u.items_inventory[idx_equip].tradecard3 = 1;
			clif_inventorylist(sd);
		}
		log_pick_pc(sd, LOG_TYPE_OTHER,  1, &sd->inventory.u.items_inventory[idx_equip]);
		clif_insert_card(sd,idx_equip,idx_card,0);
...
enum e_additem_result pc_additem(struct map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type) {
	...
	// DEVON
	if(item->nameid >= 55609 && item->nameid <= 55653)
		item->refine = 6;
	...
	if (i >= MAX_INVENTORY) {
		...
		if( sd->bindsave.nameid && sd->bindsave.nameid == item->nameid ){
			int now = (int)time(NULL);
			if(sd->bindsave.lasttime > now){
				sd->inventory.u.items_inventory[i].bind = sd->bindsave.bind;
				sd->bindsave.nameid = 0;
				sd->bindsave.bind = 0;
				sd->bindsave.lasttime = 0;
			}
		}
		if( sd->tradecardsave.card0 && sd->tradecardsave.card0 == item->nameid ){
			sd->inventory.u.items_inventory[i].tradecard0 = 1;
			sd->tradecardsave.card0 = 0;
		}
		if( sd->tradecardsave.card1 && sd->tradecardsave.card1 == item->nameid ){
			sd->inventory.u.items_inventory[i].tradecard1 = 1;
			sd->tradecardsave.card1 = 0;
		}
		if( sd->tradecardsave.card2 && sd->tradecardsave.card2 == item->nameid ){
			sd->inventory.u.items_inventory[i].tradecard2 = 1;
			sd->tradecardsave.card2 = 0;
		}
		if( sd->tradecardsave.card3 && sd->tradecardsave.card3 == item->nameid ){
			sd->inventory.u.items_inventory[i].tradecard3 = 1;
			sd->tradecardsave.card3 = 0;
		}
...
// Restock System
bool pc_restock_check(int m) {
	struct map_data *map = map_getmapdata(m);

	if (map == NULL)
		return false;
	if (map->flag[MF_RESTOCK_ENABLED])
		return true;
	if (map->flag[MF_RESTOCK_DISABLED])
		return false;
	if (!map->flag[MF_PVP] && !map->flag[MF_GVG] && !map->flag[MF_BATTLEGROUND] && !map->flag[MF_GVG_CASTLE])
		return true;
	if ((battle_config.maps_released_for_use&0x1) && map->flag[MF_PVP])
		return true;
	if ((battle_config.maps_released_for_use&0x10) && map->flag[MF_GVG])
		return true;
	if ((battle_config.maps_released_for_use&0x100) && map->flag[MF_BATTLEGROUND])
		return true;
	if ((battle_config.maps_released_for_use&0x200) && map->flag[MF_GVG_CASTLE] && !agit_flag && !agit2_flag && !agit3_flag)
		return true;
	if ((battle_config.maps_released_for_use&0x400) && map->flag[MF_GVG_CASTLE] && (agit_flag || agit2_flag || agit3_flag))
		return true;
...
char pc_delitem(struct map_session_data *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
	...
	// Restock System
	if (battle_config.restock_system_enable)
		if (sd->inventory.u.items_inventory[n].amount <= 0 && pc_restock_check(sd->bl.m))
			restock_get_item(sd, sd->inventory.u.items_inventory[n].nameid);
	...
	if( sd->inventory.u.items_inventory[n].amount <= 0 ){
		if(sd->inventory.u.items_inventory[n].equip){
			if(sd->inventory.u.items_inventory[n].bind){
				int now = (int)time(NULL);
				if(sd->inventory.u.items_inventory[n].bind > now){
					sd->bindsave.nameid = sd->inventory.u.items_inventory[n].nameid;
					sd->bindsave.bind = sd->inventory.u.items_inventory[n].bind;
					sd->bindsave.lasttime = now + 5;
				}
				if(sd->inventory.u.items_inventory[n].tradecard0){
					sd->tradecardsave.card0 = sd->inventory.u.items_inventory[n].card[0];
				}
				if(sd->inventory.u.items_inventory[n].tradecard1){
					sd->tradecardsave.card1 = sd->inventory.u.items_inventory[n].card[1];
				}
				if(sd->inventory.u.items_inventory[n].tradecard2){
					sd->tradecardsave.card2 = sd->inventory.u.items_inventory[n].card[2];
				}
				if(sd->inventory.u.items_inventory[n].tradecard3){
					sd->tradecardsave.card3 = sd->inventory.u.items_inventory[n].card[3];
				}
			}
...
bool pc_dropitem(struct map_session_data *sd,int n,int amount)
{
	...
	if( sd->inventory.u.items_inventory[n].refine >= 10 )
	{
		clif_displaymessage (sd->fd, "[Trava] Nenhum item com refinamento +10 ou maior pode ser derrubado no chão.");
		return false;
	}

	if(
		(sd->inventory_data[n]->equip == EQP_COSTUME_HEAD_TOP || sd->inventory_data[n]->equip == EQP_COSTUME_HEAD_MID || sd->inventory_data[n]->equip == EQP_COSTUME_HEAD_LOW || sd->inventory_data[n]->equip == EQP_COSTUME_GARMENT) && 
		sd->inventory.u.items_inventory[n].enchantgrade > 0)
	{
		clif_displaymessage(sd->fd, "Visuais com Grade não podem ser transferidos.");
		return false;
	}
	
	if(
		(sd->inventory_data[n]->equip == EQP_COSTUME_HEAD_TOP || sd->inventory_data[n]->equip == EQP_COSTUME_HEAD_MID || sd->inventory_data[n]->equip == EQP_COSTUME_HEAD_LOW || sd->inventory_data[n]->equip == EQP_COSTUME_GARMENT) && 
		sd->inventory.u.items_inventory[n].card[1] >= 3130650)
	{
		clif_displaymessage(sd->fd, "Visuais com encantamento Footprint não podem ser transferidos.");
		return false;
	}
	
	if(verifica_bloqueio(&sd->inventory.u.items_inventory[n]))
	{
		clif_displaymessage(sd->fd, "Este item não pode ser negociado, remova as cartas/encantamentos.");
		return false;
	}
	if(verifica_marionete(&sd->inventory.u.items_inventory[n]))
	{
		clif_displaymessage(sd->fd, "Visuais com carta marionete não podem ser transferidos.");
		return false;
	}
	if(sd->inventory.u.items_inventory[n].bind)
	{
		clif_displaymessage(sd->fd, "Itens com bind não podem ser transferidos.");
		return false;
	}
	...
	{
		clif_displaymessage(sd->fd, "Carta NN ou item com Carta NN não podem ser transferidos");
		return false;
	}
	...
	switch( nameid ) {
		...
		case ITEMID_N_FLY_WING:
			if (sd->sc.data[SC_EFEITO_RH_7] && battle_config.prevent_logout && sd->canlog_pk_tick != 0 && DIFF_TICK(gettick(), sd->canlog_pk_tick) < battle_config.prevent_logout){
				clif_displaymessage(sd->fd, "[Modo PK] Não é posssível utilizar teleportes durante uma batalha. Tente novamente em 10 segundos.");
				return false;
			}
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	if(map_getmapflag(sd->bl.m, MF_RESTIGUALITARIA)){
		if(id->flag.no_equip_igualitario&32) {
			clif_displaymessage(sd->fd, "[Pandemonium] Esse consumível não pode ser utilizado nesse conteúdo, confira na página do Pandemonium na wiki quais são permitidos.");
			return 0;
		}
	}
	...
	{
		// EDIÃ‡ÃƒO BY DEVON -------- :D
		if( (nameid == ITEMID_FORMULARIO_INFORMATIVO || nameid == ITEMID_TICKET_UNION || nameid == ITEMID_REFINADORA_PORTATIL ) || 
			(pc_isvip(sd) && (nameid == 601 || nameid == 645 || nameid == 656 || nameid == 657 || nameid == 643 || 
			nameid == 12114 || nameid == 12115 || nameid == 12116 || nameid == 12117))
		) 
		{
			clif_useitemack(sd, n, amount, true);
		}
		else if( item.expire_time == 0 && nameid != ITEMID_REINS_OF_MOUNT )
		{
...
enum e_additem_result pc_cart_additem(struct map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type)
{
	...
	if(
		(data->equip == EQP_COSTUME_HEAD_TOP || data->equip == EQP_COSTUME_HEAD_MID || data->equip == EQP_COSTUME_HEAD_LOW || data->equip == EQP_COSTUME_GARMENT) && 
		item->enchantgrade > 0)
	{
		clif_displaymessage(sd->fd, "Visuais com Grade não podem ser transferidos.");
		return ADDITEM_INVALID;
	...
	if(verifica_bloqueio(item))
	{
		clif_displaymessage(sd->fd, "Este item não pode ser negociado, remova as cartas/encantamentos.");
		return ADDITEM_INVALID;
	}
	if(verifica_marionete(item))
	{
		clif_displaymessage(sd->fd, "Visuais com carta marionete não podem ser transferidos.");
		return ADDITEM_INVALID;
	}
	...
	if(item->bind)
	{
		clif_displaymessage(sd->fd, "Itens com bind não podem ser tranferidos.");
		return ADDITEM_INVALID;
	}
	
	if(item->tradecard0 || item->tradecard1 || item->tradecard2 || item->tradecard3)
	{
		clif_displaymessage(sd->fd, "Carta NN ou item com Carta NN não podem ser transferidos");
		return ADDITEM_INVALID;
	}
...
void pc_putitemtocart(struct map_session_data *sd,int idx,int amount)
{
	...
	if(
		(sd->inventory_data[idx]->equip == EQP_COSTUME_HEAD_TOP || sd->inventory_data[idx]->equip == EQP_COSTUME_HEAD_MID || sd->inventory_data[idx]->equip == EQP_COSTUME_HEAD_LOW || sd->inventory_data[idx]->equip == EQP_COSTUME_GARMENT) && 
		sd->inventory.u.items_inventory[idx].enchantgrade > 0)
	{
		clif_displaymessage(sd->fd, "Visuais com Grade não podem ser transferidos.");
		return;
	}
	...
	if(
		(sd->inventory_data[idx]->equip == EQP_COSTUME_HEAD_TOP || sd->inventory_data[idx]->equip == EQP_COSTUME_HEAD_MID || sd->inventory_data[idx]->equip == EQP_COSTUME_HEAD_LOW || sd->inventory_data[idx]->equip == EQP_COSTUME_GARMENT) && 
		sd->inventory.u.items_inventory[idx].card[1] >= 3130650)
	{
		clif_displaymessage(sd->fd, "Visuais com encantamento Footprint não podem ser transferidos.");
		return;
	}
	...
	if(item_data->tradecard0 || item_data->tradecard1 || item_data->tradecard2 || item_data->tradecard3)
	{
		clif_displaymessage(sd->fd, "Carta NN ou item com Carta NN não podem ser transferidos");
		return;
	}
		
	if(verifica_bloqueio(item_data))
	{
		clif_displaymessage(sd->fd, "Este item não pode ser negociado, remova as cartas/encantamentos.");
		return;
	}
	if(verifica_marionete(item_data))
	{
		clif_displaymessage(sd->fd, "Visuais com carta marionete não podem ser transferidos.");
		return;
	}
...
enum e_setpos pc_setpos(struct map_session_data* sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	/*
	if(battle_config.multiserver > 1 && mapindex_name2id("prontera") == mapindex){
		clif_displaymessage(sd->fd, "[Servidor Secundário] Nesse canal não é possível acessar a cidade de prontera."); // Warped.
		mapindex = mapindex_name2id("eden_n");
		x = 55;
		y = 69;
	}
	if(battle_config.multiserver == 1 && mapindex_name2id("eden_n") == mapindex){
		clif_displaymessage(sd->fd, "[Servidor Principal] Nesse canal não é possível acessar esse mapa."); // Warped.
		mapindex = mapindex_name2id("prontera");
		x = 155;
		y = 188;
	}
	*/
...
static void pc_calcexp(struct map_session_data *sd, t_exp *base_exp, t_exp *job_exp, struct block_list *src)
{
	...
	if(mapdata->flag[MF_VARMUNDT]){
		if(*base_exp) *base_exp = (70374980 / 100) * battle_config.exp_rate_adicional;
		if(*job_exp) *job_exp = (58969170 / 100) * battle_config.exp_rate_adicional;
 	} else if(mapdata->flag[MF_MAPAS_275]){
		if(*base_exp) *base_exp = (140374980 / 100) * battle_config.exp_rate_adicional;
		if(*job_exp) *job_exp = (108969170 / 100) * battle_config.exp_rate_adicional;
 	} else if(battle_config.exp_rate_adicional > 100) {
		if(*base_exp) *base_exp = (*base_exp / 100) * battle_config.exp_rate_adicional;
		if(*job_exp) *job_exp = (*base_exp / 100) * battle_config.exp_rate_adicional;
	}

	if(sd->status.base_level >= 260){
		if(!mapdata->flag[MF_MAPAS_275]){
			if(sd->state.avisoexp) {
				clif_displaymessage(sd->fd, "[Nível 260+] Você não pode receber EXP nesse mapa, utilize '~ Mapas [Nv. 260+]' do teleportador para continuar a receber EXP.");
				sd->state.avisoexp = 0;
			}
			if(*base_exp) *base_exp = 0;
			if(*job_exp) *job_exp = 0;
			

			return;
		}
	}
	else if(sd->status.base_level >= 250 && !mapdata->flag[MF_VARMUNDT]){ 
		if(sd->state.avisoexp) {
			clif_displaymessage(sd->fd, "[Nível 250+] Você não pode receber EXP nesse mapa, utilize '~ Mapas [Nv. 250+]' do teleportador para continuar a receber EXP.");
			sd->state.avisoexp = 0;
		}
		if(*base_exp) *base_exp = 0;
		if(*job_exp) *job_exp = 0;
	...
	int reborns = 0, divisao = 9;
	if(sd->history_state.resetchar > 0) reborns += sd->history_state.resetchar;
	if(sd->history_state.resetaccount > 0) reborns += sd->history_state.resetaccount;
	if(sd->history_state.burnreborn > 0) reborns += sd->history_state.burnreborn;
	
	if(reborns > 15) reborns = 15;
	divisao += reborns;

	if(reborns > 0 || sd->history_state.classereborn || sd->history_state.temporadareborn){
		if(!mapdata->flag[MF_EXPREBORN]){ 
			if(sd->state.avisoexp) {
				clif_displaymessage(sd->fd, "[Reborn] Você não pode receber EXP nesse mapa, utilize o 'Guia de UP Reborn' do teleportador para continuar a receber EXP.");
				sd->state.avisoexp = 0;
			}
			if(*base_exp) *base_exp = 0;
			if(*job_exp) *job_exp = 0;
		...
		if(mapdata->flag[MF_TRAVANV_70]){ 
			if(sd->status.base_level < 61 || sd->status.base_level > 106){
				if(sd->state.avisoexp) {
					clif_displaymessage(sd->fd, "[Trava de UP] Você não pode receber EXP nesse mapa, tente do nível 61 ao 106.");
					sd->state.avisoexp = 0;
				}
				if(*base_exp) *base_exp = 0;
				if(*job_exp) *job_exp = 0;
		...
		if(mapdata->flag[MF_TRAVANV_106]){ 
			if(sd->status.base_level < 106 || sd->status.base_level > 180){
				if(sd->state.avisoexp) {
					clif_displaymessage(sd->fd, "[Trava de UP] Você não pode receber EXP nesse mapa, tente do nível 106 ao 180.");
					sd->state.avisoexp = 0;
				}
				if(*base_exp) *base_exp = 0;
				if(*job_exp) *job_exp = 0;
		...
		if(mapdata->flag[MF_TRAVANV_180]){ 
			if(sd->status.base_level < 180 || sd->status.base_level > 200){
				if(sd->state.avisoexp) {
					clif_displaymessage(sd->fd, "[Trava de UP] Você não pode receber EXP nesse mapa, tente do nível 180 ao 200.");
					sd->state.avisoexp = 0;
				}
				if(*base_exp) *base_exp = 0;
				if(*job_exp) *job_exp = 0;
		...
		if(mapdata->flag[MF_TRAVANV_200]){ 
			if(sd->status.base_level < 200 || sd->status.base_level > 230){
				if(sd->state.avisoexp) {
					clif_displaymessage(sd->fd, "[Trava de UP] Você não pode receber EXP nesse mapa, tente do nível 200 ao 230.");
					sd->state.avisoexp = 0;
				}
				if(*base_exp) *base_exp = 0;
				if(*job_exp) *job_exp = 0;
		...
		if(mapdata->flag[MF_TRAVANV_230]){ 
			if(sd->status.base_level < 230 || sd->status.base_level > 240){
				if(sd->state.avisoexp) {
					clif_displaymessage(sd->fd, "[Trava de UP] Você não pode receber EXP nesse mapa, tente do nível 230 ao 240.");
					sd->state.avisoexp = 0;
				}
				if(*base_exp) *base_exp = 0;
				if(*job_exp) *job_exp = 0;
		...
		if(mapdata->flag[MF_TRAVANV_240]){ 
			if(sd->status.base_level < 240 || sd->status.base_level > 250){
				if(sd->state.avisoexp) {
					clif_displaymessage(sd->fd, "[Trava de UP] Você não pode receber EXP nesse mapa, tente do nível 240 ao 250.");
					sd->state.avisoexp = 0;
				}
				if(*base_exp) *base_exp = 0;
				if(*job_exp) *job_exp = 0;
	...
	if(sd->status.base_level < 200 && mapdata->flag[MF_LABIRINTOVALK]){ 
		if(*base_exp) *base_exp = 0;
		if(*job_exp) *job_exp = 0;
	...
	// devon
	if(sd->state.rate_servidor > 1){
		if(*base_exp) *base_exp = *base_exp * sd->state.rate_servidor;
		if(*job_exp) *job_exp = *job_exp * sd->state.rate_servidor;
	}

	if(!reborns && sd->state.temporada == 1 && sd->status.base_level >= 200){ 
		int redutor = 10;
		if(sd->status.base_level >= 250) redutor = 5;
		if(*base_exp) *base_exp = *base_exp / redutor;
		if(*job_exp) *job_exp = *job_exp / redutor;
	}

	//if(	battle_config.base_exp_rate > 100 &&
	///	((sd->class_&MAPID_BASEMASK) == MAPID_THIEF ||
	//	(sd->class_&MAPID_UPPERMASK) == MAPID_KNIGHT ||
	//	(sd->class_&MAPID_UPPERMASK) == MAPID_MONK ||
	//	(sd->class_&MAPID_BASEMASK) == MAPID_TAEKWON ||
	//	(sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE)
	//){
	//	//ShowWarning("entrei");
	//	if(*base_exp) *base_exp += *base_exp / 2;
	//	if(*job_exp) *job_exp += *job_exp / 2;
	//}

	if(sd->status.base_level >= 250){
		if(mapdata->flag[MF_VARMUNDTEXP]){
			if(*base_exp) *base_exp += *base_exp / 2;
			if(*job_exp) *job_exp += *job_exp / 2;
		}
	}

	if(mapdata->flag[MF_ROTATIONEXP]){
		if(*base_exp) *base_exp += *base_exp / 2;
		if(*job_exp) *job_exp += *job_exp / 2;
	}
	if(mapdata->flag[MF_REDUCAOEXP]){
		if(*base_exp) *base_exp = *base_exp / 2;
		if(*job_exp) *job_exp = *job_exp / 2;
	}
	
	if(sd->history_state.classereborn || sd->history_state.temporadareborn){
		if(*base_exp) {
			*base_exp = *base_exp / 24;
			*base_exp = *base_exp / 5;
			if(*base_exp < 20) *base_exp = 20;
		}
		if(*job_exp) {
			*job_exp = *job_exp / 24;
			*job_exp = *job_exp / 5;
			if(*job_exp < 20) *job_exp = 20;
		}
	} else if(reborns > 0){
		if(*base_exp) {
			*base_exp = *base_exp / divisao;
			if(*base_exp < 20) *base_exp = 20;
		}
		if(*job_exp) {
			*job_exp = *job_exp / divisao;
			if(*job_exp < 20) *job_exp = 20;
		}
	}
	...
	if (src) {
		...
		if (src && src->type == BL_MOB && pc_isvip(sd)) { // EXP bonus for VIP player
			if(sd->state.dias_assinatura){
				vip_bonus_base = battle_config.vip_base_exp_increase * 2;
				vip_bonus_job = battle_config.vip_job_exp_increase * 2;
			} else {
	...
	if(!mapdata->flag[MF_BIOEXTREME] && !mapdata->flag[MF_CHEFFEXTREME] && sd->sc.data[SC_EFEITO_RH_7]){
		bonus += 25;
	}

	// devon
	// reputation
	if(sd->reputation[0].points > 1000){
		bonus += ((sd->reputation[0].points / 1000) / 1) * 10;
	}
	// reputation temporada
	if(sd->reputation[6].points > 1000){
		bonus += ((sd->reputation[6].points / 1000) / 1) * 50;
	}

	if (sd->sc.data[SC_TOP_FIS_GVG] || sd->sc.data[SC_TOP_MAG_GVG] || sd->sc.data[SC_TOP_DIS_GVG]) {
		bonus += 20;
	}

	if (sd->sc.data[SC_TOP_GUILD_GVG1] || sd->sc.data[SC_TOP_GUILD_GVG2] || sd->sc.data[SC_TOP_GUILD_GVG3]) {
		bonus += 50;
	}
	...
	if (sd->sc.data[SC_HISTORY_GC] && sd->status.base_level <= 200) {
		bonus += 50;
	}
	...
	// devon
	if (sd->sc.data[SC_EFEITO_RH_1]) {
		bonus += sd->sc.data[SC_EFEITO_RH_1]->val1;
	}

	// devon
	if (sd->sc.data[SC_EFEITO_RH_25]) {
		bonus += sd->sc.data[SC_EFEITO_RH_25]->val1;
	}

	// devon
	if (sd->sc.data[SC_EFEITO_RH_5]) {
		bonus += 10;
	}
	...
	if (sd->sc.data[SC_EFEITO_RH_23]) {
		bonus += 60;
	} else if (sd->sc.data[SC_EFEITO_RH_22]) {
		bonus += 40;
	} else if (sd->sc.data[SC_EFEITO_RH_21]) {
		bonus += 20;
	}
	
	if(battle_config.exp_rate_relogio > 0)
		bonus += 25;

	if (*base_exp) {
		t_exp exp = (t_exp)(*base_exp + ((double)*base_exp * ((bonus + vip_bonus_base) / 100.)));
		if(sd->state.dias_assinatura){ exp += (int)exp / 2; }
		if(sd->sc.data[SC_BLACKFRIDAY]){ exp += (int)((exp / 100) * 6); }
		
		int aventureiro = pc_checkskill(sd,NT_AVENTUREIRO);
		if(aventureiro == 1)
			exp += (int)((exp / 100) * 1);
		else if(aventureiro == 2)
			exp += (int)((exp / 100) * 3);
		else if(aventureiro == 3)
			exp += (int)((exp / 100) * 5);
		else if(aventureiro == 4)
			exp += (int)((exp / 100) * 8);
		
		if (sd->reputation[8].points >= 6000)
			exp += (int)((exp / 100) * 1);
		if (sd->reputation[9].points >= 6000)
			exp += (int)((exp / 100) * 1);
		if (sd->reputation[10].points >= 6000)
			exp += (int)((exp / 100) * 1);
		
		int expfinal = 0;
		
		int espectro = pc_checkskill(sd,HL_MAESTRIA);
		if(espectro == 1)
			expfinal += 6;
		else if(espectro == 2)
			expfinal += 12;
		...
		if(sd->sc.data[SC_ESCOLHIDOS])
			expfinal += sd->sc.data[SC_ESCOLHIDOS]->val1;
		if(sd->sc.data[SC_JOIA_ALMAS])
			expfinal += sd->sc.data[SC_JOIA_ALMAS]->val1;
		if(sd->sc.data[SC_ENERGETICO_MONSTRO])
			expfinal += 3;
		if(sd->sc.data[SC_BENCAO_HEROI])
			expfinal += 5;
		if(sd->bonus.exp_final)
			expfinal += sd->bonus.exp_final;
		
		if(expfinal > 0)
			exp += (int)((exp / 100) * expfinal);
...
int pc_skillatk_bonus_devon(struct map_session_data *sd, uint16 skill_id)
{
	int bonus = 0;
...
int pc_skillatk_bonus_final(struct map_session_data *sd, uint16 skill_id)
{
	int bonus = 0;
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
		...
		&& !mapdata->flag[MF_NOEXPPENALTY] && !mapdata_flag_gvg2(mapdata))
	{
		...
		if(!mapdata->flag[MF_BIOEXTREME] && !mapdata->flag[MF_CHEFFEXTREME] && sd->sc.data[SC_EFEITO_RH_7] && src && src->type == BL_PC){
			base_penalty = 1500;
			job_penalty = 1500;
		}
...
bool pc_revive_item(struct map_session_data *sd) {
	...
	int16 ziegraid = pc_search_inventory( sd, 25310 );

	if(mapdata->flag[MF_ILLUSIONGUILD] && ziegraid > -1){
		int16 item_position = ziegraid;
		uint8 hp = 100, sp = 100;
	...
	} else if(mapdata->flag[MF_MASMORRAINVERNAL] || mapdata->flag[MF_SAMURAIMESTRE]){
		
		int16 item_position = pc_search_inventory( sd, 6316 );
	...
	if (mapdata->flag[MF_TORREDECRISTAL]){
		 if (mapdata->flag[MF_NOAMULETOSIEG] && sd->status.class_ != JOB_SPIRIT_HANDLER && sd->status.class_ != JOB_CARDINAL && sd->status.class_ != JOB_SOUL_ASCETIC){ // devon
			clif_displaymessage( sd->fd, msg_txt(sd,1519) );
			return false;
		}
	}
	else if (mapdata->flag[MF_NOAMULETOSIEG] || 
		(mapdata->flag[MF_ILLUSIONGUILD] && sd->status.class_ != JOB_CARDINAL) || 
		(mapdata->flag[MF_GUERRAPVM] && sd->status.class_ != JOB_SPIRIT_HANDLER && sd->status.class_ != JOB_CARDINAL && sd->status.class_ != JOB_SOUL_ASCETIC)){ // devon
		clif_displaymessage( sd->fd, msg_txt(sd,1519) );
		return false;
	}
...
int64 pc_readparam(struct map_session_data* sd,int64 type)
{
	...
	switch(type) {
		...
		case SP_KILLEDMOBX:       val = sd->killedmobx; break;
		case SP_KILLEDMOBY:       val = sd->killedmoby; break;
		...
		case SP_METALIC_SKILL:val = sd->bonus.metalic_skill; break;
		...
		case SP_AMULETO_NINJA:   val = sd->bonus.amuleto_ninja; break;
		case SP_INSPIRA_DISPELL:   val = sd->bonus.inspira_dispell; break;
		case SP_REDUCAO_NINJA:   val = sd->bonus.reducao_ninja; break;
		case SP_AP_CURA:   val = sd->bonus.ap_cura; break;
		case SP_ILIMITAR:   val = sd->bonus.ilimitar; break;
		case SP_SPLASH_NINJA:   val = sd->bonus.splash_ninja; break;
		case SP_DANO_ELEMENTAL:   val = sd->bonus.dano_elemental; break;
		case SP_STATUS_NO_DEAD:   val = sd->bonus.status_no_dead; break;
...
int pc_percentheal(struct map_session_data *sd,int hp,int sp)
{
	...
	// Devon - Modificaï¿½ï¿½o + 50% rank Alchemist
	if (potion_flag > 1)
		hp = (hp * 1.5) / 1;
...
bool pc_equipitem(struct map_session_data *sd,short n,int req_pos,bool equipswitch)
{
	...
	if( sd->sc.count && (sd->sc.cant.equip || ((sd->sc.data[SC_PYROCLASTIC] || sd->sc.data[SC_HNNOWEAPON]) && sd->inventory_data[n]->type == IT_WEAPON)) ) {
		if( equipswitch ){
			clif_equipswitch_add( sd, n, req_pos, ITEM_EQUIP_ACK_FAIL );
		}else{
			clif_equipitemack(sd,n,0,ITEM_EQUIP_ACK_FAIL); //Fail
...
bool pc_unequipitem(struct map_session_data *sd, int n, int flag) {
	...
	if (!(flag&2) && sd->sc.count &&( sd->sc.cant.unequip ||
		((sd->sc.data[SC_PYROCLASTIC] || sd->sc.data[SC_HNNOWEAPON]) &&	sd->inventory_data[n]->type == IT_WEAPON)))	// can't switch weapon
	{
		clif_unequipitemack(sd,n,0,0);
		return false;
	}
...
{
	int hp = 0, sp = 0, max_hp = 0, hpes = 0, spes = 0, ratees = 0;
	...
	if(
		(mapdata->flag[MF_DOMINIOHARD] && !sd->sc.data[SC_DOM_POT_HARD]) || 
		(mapdata->flag[MF_DOMINIOUNREAL] && !sd->sc.data[SC_DOM_POT_UNREAL] ) || 
		(mapdata->flag[MF_DOMINIOULTIMATE] && mapdata->flag[MF_DOMINIOGERAL] && !sd->sc.data[SC_DOM_POT_ULTIMATE_2] && !sd->sc.data[SC_DOM_POT_ULTIMATE] && !sd->state.exceldrop) || 
		mapdata->flag[MF_TEMPLOULTIMATE] || 
		mapdata->flag[MF_ESCURIDAO] || 
		mapdata->flag[MF_ESCURIDAOPVP] || 
		mapdata->flag[MF_HPDROP] ||
		(mapdata->flag[MF_GUERRAPVM] && mapdata->flag[MF_GUERRADANO]) ||
		sd->sc.data[SC_VENEPROFUN_1] ||
		sd->sc.data[SC_VENEPROFUN_2] ||
		sd->sc.data[SC_VENEPROFUN_3] ||
		sd->sc.data[SC_VENEPROFUN_4]
	){
		int inivalor = 7;
		int inirates = 4000;
		if(mapdata->flag[MF_DOMINIOHARD]) { inivalor = 50; inirates = 1000;}
		if(mapdata->flag[MF_DOMINIOUNREAL]) { inivalor = 65; inirates = 1000;}
		if(mapdata->flag[MF_DOMINIOULTIMATE]) { inivalor = 25; inirates = 1000;}
		if(mapdata->flag[MF_GUERRAPVM]) { inivalor = 10; inirates = 1000;}
		
		if(sd->sc.data[SC_VENEPROFUN_1]) { inivalor = 4; inirates = 1000; }
		if(sd->sc.data[SC_VENEPROFUN_2]) { inivalor = 8; inirates = 1000; }
		if(sd->sc.data[SC_VENEPROFUN_3]) { inivalor = 12; inirates = 1000; }
		if(sd->sc.data[SC_VENEPROFUN_4]) { inivalor = 16; inirates = 1000; }

		ratees = inirates;
		hpes = (sd->status.max_hp / 100) * inivalor;


		sd->hp_loss.tick += diff_tick;
		while (sd->hp_loss.tick >= ratees) {
			hp += hpes;
			sd->hp_loss.tick -= ratees;
		}
...
void pc_addspiritcharm(struct map_session_data *sd, int interval, int max, int type)
{
	...
	if( sd && (
		(sd->bonus.amuleto_ninja == 1 && type == 1) || 
		(sd->bonus.amuleto_ninja == 2 && (type == 4 || type == 1)) ||
		(sd->bonus.amuleto_ninja == 3 && (type == 4 || type == 1 || type == 3)))
	){
		for(f = 0; f < MAX_SPIRITCHARM; f++){
...
void pc_set_costume_view(struct map_session_data *sd) {
	...
	// Costumes check
	if (!map_getmapflag(sd->bl.m, MF_NOCOSTUME) && sd->status.show_costume) {
...
void pc_jail(map_session_data &sd, int32 duration) {
	...
	if (duration > 0)
		...
		status_change_end(&sd.bl, SC_JAILED, INVALID_TIMER);
}
...
static void pc_macro_punishment(map_session_data &sd, e_macro_detect_status stype) {
	...
	if (battle_config.macro_detection_punishment == 0) { // Ban
		...
		npc_event(&sd,"macrochecker::OnPunicao",0);
...
void pc_macro_detector_process_answer(map_session_data &sd, char captcha_answer[CAPTCHA_ANSWER_SIZE]) {
	...
	if (strcmp(captcha_answer, cd->captcha_answer) == 0) {
		...
		pc_setreg(&sd, add_str("checkmap"), 0);
 ...
 * @return count of successfully parsed rows
 */
uint64 CaptchaDatabase::parseBodyNode(const YAML::Node& node) {
	uint16 index;
