...
uint64 EnchantgradeDatabase::parseBodyNode( const YAML::Node& node ){
	...
	for( const YAML::Node& levelNode : node["Levels"] ){
		...
		for( const YAML::Node& gradeNode : levelNode["Grades"] ){
			uint16 gradeLevel;
			...
			if( this->nodeExists( gradeNode, "Catalysator") ){
				const YAML::Node& catalysatorNode = gradeNode["Catalysator"];
			...
			if( this->nodeExists( gradeNode, "Options" ) ){
				for( const YAML::Node& optionNode : gradeNode["Options"] ){
					uint16 optionIndex;
					...
					if( this->nodeExists( optionNode, "Breakarmor" ) ){
						uint32 breakarmor;

						if( !this->asUInt32( optionNode, "Breakarmor", breakarmor ) ){
							return 0;
						}

						option->breakarmor = breakarmor;
					}else{
						if( !optionExists ){
							option->breakarmor = 0;
						}
...
std::shared_ptr<s_enchantgradelevel> EnchantgradeDatabase::findCurrentLevelInfo( const struct item_data& data, struct item& item ){
	...
	int levelwhere = data.weapon_level;

	if (data.type == IT_ARMOR)
		levelwhere = data.armor_level;
...
int status_damage(struct block_list *src,struct block_list *target,int64 dhp, int64 dsp, int64 dap, t_tick walkdelay, int flag, uint16 skill_id)
{
	...
	if(target->type == BL_ELEM){
		...
		if(esd && esd->bonus.dano_elemental) hp = 0;
	}
	if(target->type == BL_MOB && sp > 0){
		sp = 0;
	}
	
	struct map_data *mapdata = map_getmapdata(target->m);
	if(mapdata->flag[MF_DOMINIOENT] && hp > 2){
		return 0;
	}
	...
	// devon
	if(target->type == BL_MOB && mapdata->flag[MF_GUERRAPVM] && status->max_shield == 9999999 && hp > 0){
		struct map_session_data *sdg = BL_CAST(BL_PC, src);
		if(sdg){
			sdg->state.gpvmsobra += hp;
		
			if(sdg->state.gpvmsobra >= 1000000){
				int inisdfeito = (sdg->state.gpvmsobra / 1000000);
				sdg->state.gpvmdano += inisdfeito;
				sdg->state.gpvmsobra -= (inisdfeito * 1000000);
			}
		}
		
		if((hp / 1000000) >= 1) {
			int inishield = status->danorecebido;
			int inifeito = (hp / 1000000);
			
			if(inifeito < 2000){
				int conta = hp - (inifeito * 1000000);
				status->feito += conta;
			
				if(status->feito >= 1000000){
					inifeito = (status->feito / 1000000);
					status->feito -= (inifeito * 1000000);
					hp += (inifeito * 1000000);
				}
			}

			int shield = inishield + (hp / 1000000);
			status->danorecebido = shield;
		} else {
			status->feito += hp;
		
			if(status->feito >= 1000000){
				int inifeito = (status->feito / 1000000);
				status->danorecebido += inifeito;
				status->feito -= (inifeito * 1000000);
			}
		}
	} else if(status->shield > 0 && target->type == BL_MOB && hp > 0){
		if((hp / 1000000) >= 1) {
			int inishield = status->shield;
			int inifeito = (hp / 1000000);
			...
			if(shield <= 0) { 
				...
				if(src){
					...
					if(mapdata->flag[MF_BIOEXTREME] || mapdata->flag[MF_CHEFFEXTREME] || mapdata->flag[MF_BIOHARD]){
						int hpreduct = hp - (inishield * 1000000);
						if(hpreduct > 0){
							if ((unsigned int)hpreduct >= status->hp) {
								if (flag&2) return 0;
								hpreduct = status->hp;
							}
							status->hp-= hpreduct;
	...
	**/
	...
	if( sc && sc->data[SC_KAIZEL] && !sc->data[SC_EFEITO_RH_7] && !map_flag_gvg2(target->m) ) { // flag&8 = disable Kaizel
...
int status_heal(struct block_list *bl,int64 hhp,int64 hsp, int64 hap, int flag)
{
	...
	if(map_getmapflag(bl->m, MF_TUMBAVAZIO))
		return 0;
	...
	if(hp > 1 && bl->type == BL_PC && map_getmapflag(bl->m, MF_GUERRAPVM)){
		hp = hp / 2;
	}
...
int status_percent_change(struct block_list *src, struct block_list *target, int8 hp_rate, int8 sp_rate, int8 ap_rate, uint8 flag)
{
	...
	if(target->type == BL_MOB && sp_rate > 0) sp_rate = 0; 
	...
	if(flag){
		if(status->shield > 0) status->shield = 0;
	}
	...
	switch( target->type ) {
		case BL_PC: {
				...
				if (pc_isinvisible(tsd))
					return false;
				if (tsc) {
					if (tsc->data[SC_HIDING] && skill_id == WZ_FROSTNOVA){
						return true;
						///ShowWarning("2"); devon
						//struct map_session_data *ssd = (TBL_PC*)src;
						//uint16 skill_lv;
						//skill_lv = pc_checkskill(ssd, WZ_FROSTNOVA);
						//sc_start(src,target,SC_FREEZE,skill_lv*5+33,skill_lv,skill_get_time2(skill_id,skill_lv));
					}
...
static int status_get_hpbonus(struct block_list *bl, enum e_status_bonus type) {
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (sc) {
			...
			if(sc->data[SC_OVERCOMING_CRISIS])
				bonus += sc->data[SC_OVERCOMING_CRISIS]->val3;
#ifdef RENEWAL
...
static int status_get_spbonus(struct block_list *bl, enum e_status_bonus type) {
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (sc) {
			//Increasing
			// Bolinho Brilhante
			if(sc->data[SC_BOLINHO_BRILHANTE]){
				bonus += 15;
			}
...
static unsigned int status_calc_maxhpsp_pc(struct map_session_data* sd, unsigned int stat, bool isHP) {
	...
	// devon
	// reputation
	int bonusrepu = 0, bonushprepu = 0;
	struct map_data *mapdata = map_getmapdata(sd->bl.m);
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		if(sd->reputation[1].points >= 1000){ // Instancia
			bonusrepu += 5;
		}
		if(sd->reputation[8].points >= 2000){ // Pescaria
			bonusrepu += 1;
		}
		if(sd->reputation[9].points >= 2000){ // Minera??o
			bonusrepu += 1;
		}
		if(sd->reputation[10].points >= 2000){ // Herbologia
			bonusrepu += 1;
		}
		...
		if(battle_config.desativarexcelencia){
			if(mapdata->flag[MF_DOMINIOGERAL] || mapdata->flag[MF_FLORESTAPES] || mapdata->flag[MF_TEMPLOULTIMATE]){
				int repudom = sd->reputation[12].points;
				if(repudom >= 1000) bonusrepu += 1;
				if(repudom >= 2000) bonusrepu += 1;
				if(repudom >= 3000) bonusrepu += 1;
				if(repudom >= 4000) bonusrepu += 2;
				if(repudom >= 5000) bonusrepu += 2;
				if(repudom >= 6000) bonusrepu += 2;
				if(repudom >= 7000) bonusrepu += 3;
				if(repudom >= 8000) bonusrepu += 3;
			}
		}
	}
	
	
	// Trava na WoE pra n funfar
	if(mapdata->flag[MF_EQUIPSGVG]) bonusrepu = 0;
	...
	if(guerreiro == 1)
		...
		bonusrepu += 10;
	if(sd->sc.data[SC_BOLINHO_BRILHANTE])
		bonushprepu += 20;
	...
	if (isHP) { //Calculates MaxHP
		...
		// -----------------------------------------------------
		// devon
		// triplo de HP super novice
		int basehp;
		if(level >= 261){
			basehp = job->base_hp[260-1] + (job->base_hp[260-1] - job->base_hp[260-2]) * (level - 260);
			//ShowWarning("base hp = %d \n", basehp);
		} else {
			basehp = job->base_hp[level-1];
		}
		if(level >= 260) basehp += 100;
		if(level >= 275) basehp += 300;
		dmax = ((sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE ? basehp * 5 : basehp) * (1 + (umax(stat,1) * 0.01)) * ((sd->class_&JOBL_UPPER)?1.25:(pc_is_taekwon_ranker(sd))?3:1);
		// -----------------------------------------------------
		...
		dmax += (dmax * (bonusrepu + bonushprepu) / 100); // reputacao
		...
		double equip_bonus = 0, item_bonus = 0;
		// devon
		// triplo de SP super novice
		int basesp;
		if(level >= 256){
			basesp = job->base_sp[255-1] + (job->base_sp[255-1] - job->base_sp[255-2]) * (level - 255);
			//ShowWarning("base sp = %d \n", basesp);
		} else {
			basesp = job->base_sp[level-1];
		}
		if(level >= 260) basesp += 100;
		if(level >= 275) basesp += 100;
		dmax = ((sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE ? basesp * 4 : basesp) * (1 + (umax(stat,1) * 0.01)) * ((sd->class_&JOBL_UPPER)?1.25:(pc_is_taekwon_ranker(sd))?3:1);
		// -----------------------------------------------------
		...
		dmax += (dmax * bonusrepu / 100); // reputacao
	...
	if (flag&CALCWT_MAXBONUS) {
		...
		for (j=0; j<EQI_MAX; j++) {
			...
			if(!sd->inventory_data[index])
				continue;
			if (
				!mapdata->flag[MF_EQUIPSGVG] && 
				(
				sd->inventory.u.items_inventory[index].card[1] == 38894 || 
				sd->inventory.u.items_inventory[index].card[1] == 38895 || 
				sd->inventory.u.items_inventory[index].card[1] == 3108500 || 
				sd->inventory.u.items_inventory[index].card[1] == 310850 || 
				sd->inventory.u.items_inventory[index].card[1] == 292240 || 
				sd->inventory.u.items_inventory[index].card[1] == 29224 || 
				sd->inventory.u.items_inventory[index].card[1] == 30017 || 
				sd->inventory.u.items_inventory[index].card[1] == 30007 || 
				sd->inventory.u.items_inventory[index].card[1] == 25136 || 
				sd->inventory.u.items_inventory[index].card[1] == 29161 ||
				sd->inventory.u.items_inventory[index].card[1] == 313065 ||
				sd->inventory.u.items_inventory[index].card[1] == 313067 ||
				sd->inventory.u.items_inventory[index].card[1] == 313068 ||
				sd->inventory.u.items_inventory[index].card[1] == 313069 ||
				sd->inventory.u.items_inventory[index].card[1] == 313559 ||
				sd->inventory.u.items_inventory[index].card[1] == 3130650 ||
				sd->inventory.u.items_inventory[index].card[1] == 3130670 ||
				sd->inventory.u.items_inventory[index].card[1] == 3130680 ||
				sd->inventory.u.items_inventory[index].card[1] == 3130690 ||
				sd->inventory.u.items_inventory[index].card[1] == 3135590
				)
			)
			mini += 20000;
		}
		if (
			!mapdata->flag[MF_EQUIPSGVG] && 
			(sd->pd && sd->pd->pet.egg_id == 92980 ||
			sd->pd && sd->pd->pet.egg_id == 93300 ||
			sd->pd && sd->pd->pet.egg_id == 93100 ||
			sd->pd && sd->pd->pet.egg_id == 93120 ||
			sd->pd && sd->pd->pet.egg_id == 93102 ||
			sd->pd && sd->pd->pet.egg_id == 93310 ||
			sd->state.pet_incubado == 92980 ||
			sd->state.pet_incubado == 93300 ||
			sd->state.pet_incubado == 93100 ||
			sd->state.pet_incubado == 93120 ||
			sd->state.pet_incubado == 93102 ||
			sd->state.pet_incubado == 93310)
		){
			mini += 50000;
		}
		if (mini)
...
int status_calc_pc_sub(struct map_session_data* sd, uint8 opt)
{
	...
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		//dano add dominio
		if (sd->reputation[8].points >= 4000) { // Pescaria
			
			struct item_data *item = itemdb_exists(70245);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if (sd->reputation[9].points >= 4000) { // Minera?
			
			struct item_data *item = itemdb_exists(70245);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if (sd->reputation[10].points >= 4000) { // Herbologia
		...
		if (sd->reputation[18].points >= 1000) { // ReputaÃ§Ã£o Casamento
			struct item_data *item = itemdb_exists(70246);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if (sd->reputation[17].points >= 1000) { // ReputaÃ§Ã£o Reborn Charge
			struct item_data *item = itemdb_exists(70247);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	}
	...
	// temporada 260
	if (sd->state.temporada == 1 && sd->status.base_level >= 260) {
		struct item_data *item = itemdb_exists(70003);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}

	if(sd->state.dias_assinatura){
		struct item_data *item = itemdb_exists(70010);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(sd->sc.data[SC_DEEPCORRUPTION_EFFECT]){
		struct item_data *item = itemdb_exists(10013121);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		if(sd->sc.data[SC_BLACKFRIDAY]){
			struct item_data *item = itemdb_exists(70059);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_REBORN_CHARGE]) {
			struct item_data *item = itemdb_exists(70048);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		
		if(sd->sc.data[SC_LATAO_SKUL]) {
			struct item_data *item = itemdb_exists(51168);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		} 
		if(sd->sc.data[SC_ALL_IN_ONE]) {
			struct item_data *item = itemdb_exists(51169);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_FORCE_BOOSTER]) {
			struct item_data *item = itemdb_exists(290450);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_ATIVADOR]) {
			struct item_data *item = itemdb_exists(290451);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		
		if(sd->sc.data[SC_OVO_SORTE_PERF] || sd->sc.data[SC_OVO_SORTE_PEN] || sd->sc.data[SC_OVO_SORTE_COR] || sd->sc.data[SC_OVO_SORTE_DROP] ||
		   sd->sc.data[SC_OVO_SORTE_EXP] || sd->sc.data[SC_OVO_SORTE_TRAIT]) {
			struct item_data *item = itemdb_exists(48502);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		
		if(sd->sc.data[SC_MINI_OVO_SORTE_PERF] || sd->sc.data[SC_MINI_OVO_SORTE_PEN] || sd->sc.data[SC_MINI_OVO_SORTE_COR] || sd->sc.data[SC_MINI_OVO_SORTE_DROP] ||
		   sd->sc.data[SC_MINI_OVO_SORTE_EXP] || sd->sc.data[SC_MINI_OVO_SORTE_TRAIT]) {
			struct item_data *item = itemdb_exists(48504);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	}
	
	struct item_data *item_repu_charge = itemdb_exists(70068);
	if(item_repu_charge)
		run_script(item_repu_charge->script,0,sd->bl.id,0);
	/*
	struct item_data *item_repu_casamento = itemdb_exists(70069);
	if(item_repu_casamento)
		run_script(item_repu_casamento->script,0,sd->bl.id,0);
	*/
		
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		struct item_data *item_eco = itemdb_exists(70024);
		if(item_eco)
			run_script(item_eco->script,0,sd->bl.id,0);
	}

	sd->state.grade_shadow_colar = 0;
	sd->state.grade_shadow_brinco = 0;

	int qtdshadow = 0;	
	int temarma = 0;
	int despertartrava = 0;
	...
	sd->history_state.bindconta = 0;
	int now = (int)time(NULL);
	
	
	if(sd->history_state.melhora_duasmaos){
		for (i = 0; i < EQI_MAX; i++) {
			...
			if (i == EQI_AMMO)
				continue;
			if(sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON){
				if(!idleft){
					idleft = sd->inventory.u.items_inventory[index].nameid;
				} else {
					idright = sd->inventory.u.items_inventory[index].nameid;
				}
			}
		}
	}
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (i == EQI_AMMO)
			continue;
		if(sd->inventory_data[index]){
			if(sd->inventory.u.items_inventory[index].bind > now) sd->history_state.bindconta++;
		}
		if (pc_is_same_equip_index((enum equip_index)i, sd->equip_index, index))
			continue;
		if (!sd->inventory_data[index])
		...
		//trava ghostfire
		if(sd->inventory.u.items_inventory[index].nameid == 420199 && sd->inventory.u.items_inventory[index].card[0]) {
			struct item tmp_item;
			memset(&tmp_item,0,sizeof(tmp_item));
			tmp_item.nameid = sd->inventory.u.items_inventory[index].card[0];
			tmp_item.amount = 1;
			tmp_item.identify = itemdb_isidentified(itemid);
			sd->inventory.u.items_inventory[index].card[0] = 0;
			pc_additem(sd, &tmp_item, 1, LOG_TYPE_SCRIPT);
			clif_displaymessage(sd->fd, "[AVISO] Black Mythril Guard teve sua carta removida e adicionada em seu inventÃ¡rio.");
		}
		
		
		// remoÃ¯Â¿Â½Ã¯Â¿Â½o dos cards do escudo que foram adicionados
		
		bool allowedCard = false;
		if((sd->inventory.u.items_inventory[index].card[1] >= 27050 && sd->inventory.u.items_inventory[index].card[1] <= 27054) || (sd->inventory.u.items_inventory[index].card[1] >= 27129 && sd->inventory.u.items_inventory[index].card[1] <= 27140) || 
		(sd->inventory.u.items_inventory[index].card[1] >= 27364 && sd->inventory.u.items_inventory[index].card[1] <= 27367) || (sd->inventory.u.items_inventory[index].card[1] >= 300131 && sd->inventory.u.items_inventory[index].card[1] <= 300136) || 
		(sd->inventory.u.items_inventory[index].card[1] >= 300163 && sd->inventory.u.items_inventory[index].card[1] <= 300168) || (sd->inventory.u.items_inventory[index].card[1] >= 300282 && sd->inventory.u.items_inventory[index].card[1] <= 300289)){
			allowedCard = true;
		}

		if(sd->inventory.u.items_inventory[index].equip == EQP_SHIELD && sd->inventory_data[index]->type != IT_WEAPON && sd->inventory.u.items_inventory[index].nameid != 28941
			&& !allowedCard && sd->inventory.u.items_inventory[index].card[1] ) {
			...
			title = "DevoluÃ§Ã£o de Carta";
			safestrncpy(msg.title, title, MAIL_TITLE_LENGTH);
			body = "Aqui estÃ¡ sua carta retirada do escudo!!";
			safestrncpy(msg.body, body, MAIL_BODY_LENGTH);
			msg.item[0].nameid = sd->inventory.u.items_inventory[index].card[1];
			msg.item[0].amount = 1;
			...
			sd->inventory.u.items_inventory[index].card[1] = 0; 
			clif_displaymessage(sd->fd, "[AVISO] A carta que estava no slot 2 do seu escudo foi removida e adicionada em seu rodex.");
			clif_inventorylist(sd);
		}

		if(!mapdata->flag[MF_RESTIGUALITARIA]){
			if(sd->inventory.u.items_inventory[index].card[2] && (sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_TOP || sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_MID || 
				sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_LOW || sd->inventory_data[index]->equip == EQP_COSTUME_GARMENT)){
				qtdshadow++;
				if(qtdshadow == 3){
					struct item_data *item = itemdb_exists(70008);
					if(item)
						run_script(item->script,0,sd->bl.id,0);
				}
				if(qtdshadow == 4){
					struct item_data *item = itemdb_exists(70009);
					if(item)
						run_script(item->script,0,sd->bl.id,0);
				}
			}
			...
			// Devon
			if (
				(sd->inventory_data[index]->type == IT_ARMOR && sd->inventory.u.items_inventory[index].refine >= 15 && sd->inventory_data[index]->armor_level == 1) || 
				(sd->inventory.u.items_inventory[index].refine >= 15 && (sd->inventory_data[index]->equip == EQP_HAND_R || sd->inventory_data[index]->equip == EQP_ARMS) && sd->inventory_data[index]->weapon_level == 4)
				) {
				struct item_data *item = itemdb_exists(70000);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			
			// Devon
			// armadura 2 + 15 : bDanoFinal,5; bMaxHPrate,5; bDelayrate,-2;
			if (
				sd->inventory_data[index]->type == IT_ARMOR &&	
				(
				sd->inventory_data[index]->equip == EQP_SHOES || sd->inventory_data[index]->equip == EQP_GARMENT || 
				sd->inventory_data[index]->equip == EQP_HEAD_TOP || sd->inventory_data[index]->equip == EQP_HAND_L || sd->inventory_data[index]->equip == EQP_ARMOR
				) && 
				sd->inventory.u.items_inventory[index].refine >= 15 && sd->inventory_data[index]->armor_level == 2) {
				struct item_data *item = itemdb_exists(70040);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			// armadura 2 + 14 : bDanoFinal,2; bMaxHPrate,2; bDelayrate,-1;
			if (
				sd->inventory_data[index]->type == IT_ARMOR &&	
				(
				sd->inventory_data[index]->equip == EQP_SHOES || sd->inventory_data[index]->equip == EQP_GARMENT || 
				...
				sd->inventory.u.items_inventory[index].refine >= 14 && sd->inventory_data[index]->armor_level == 2) {
				struct item_data *item = itemdb_exists(70044);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			
			// arma 5 + 14 : bDanoFinal,2;
			if (sd->inventory.u.items_inventory[index].refine >= 14 &&
				((sd->inventory_data[index]->equip == EQP_HAND_R || sd->inventory_data[index]->equip == EQP_ARMS) && sd->inventory_data[index]->weapon_level == 5)
			) {
				struct item_data *item = itemdb_exists(71001);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			// arma 5 + 15 : bDanoFinal,5;
			if (sd->inventory.u.items_inventory[index].refine >= 15 &&
				((sd->inventory_data[index]->equip == EQP_HAND_R || sd->inventory_data[index]->equip == EQP_ARMS) && sd->inventory_data[index]->weapon_level == 5)
			) {
				struct item_data *item = itemdb_exists(71002);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
		}
		...
		// Devon
		// arma 5 + 14 : bDanoFinal,2; bMaxHPrate,2; bDelayrate,-1;
		/*
		if (sd->inventory.u.items_inventory[index].refine >= 14 &&
			((sd->inventory_data[index]->equip == EQP_HAND_R || sd->inventory_data[index]->equip == EQP_ARMS) && sd->inventory_data[index]->weapon_level == 5)
		) {
		...
		*/
		// arma 5 e armadura 2 + 13 : bDanoFinal,5; bMaxHPrate,5; bDelayrate,-2;
		if (sd->inventory.u.items_inventory[index].refine >= 13 &&
			(sd->inventory_data[index]->armor_level == 2 ||
			((sd->inventory_data[index]->equip == EQP_HAND_R || sd->inventory_data[index]->equip == EQP_ARMS) && sd->inventory_data[index]->weapon_level == 5))
		) {
			struct item_data *item = itemdb_exists(70012);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		
		if(!mapdata->flag[MF_RESTIGUALITARIA]){
			if(sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_TOP){
				struct item_data *item = itemdb_exists(70014);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if(sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_MID){
				struct item_data *item = itemdb_exists(70015);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if(sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_LOW){
				struct item_data *item = itemdb_exists(70016);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if(sd->inventory_data[index]->equip == EQP_COSTUME_GARMENT){
				struct item_data *item = itemdb_exists(70017);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
		}

		if(!mapdata->flag[MF_EQUIPSGVG] && !mapdata->flag[MF_RESTIGUALITARIA]){

			if(sd->inventory_data[index]->equip == EQP_SHADOW_ACC_R){
				sd->state.grade_shadow_colar = sd->inventory.u.items_inventory[index].enchantgrade;

				if(sd->state.grade_shadow_colar >= 1){
					struct item_data *item = itemdb_exists(70005);
					if(item)
						run_script(item->script,0,sd->bl.id,0);
				}
			}

			if(sd->inventory_data[index]->equip == EQP_SHADOW_ACC_L){
				sd->state.grade_shadow_brinco = sd->inventory.u.items_inventory[index].enchantgrade;

				if(sd->state.grade_shadow_brinco >= 1){
					struct item_data *item = itemdb_exists(70006);
					if(item)
						run_script(item->script,0,sd->bl.id,0);
				}
			}

			if(sd->inventory_data[index]->equip == EQP_SHADOW_ARMOR){
				struct item_data *item = itemdb_exists(70007);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if(sd->inventory_data[index]->equip == EQP_SHADOW_WEAPON){
				struct item_data *item = itemdb_exists(70041);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if(sd->inventory_data[index]->equip == EQP_SHADOW_SHIELD){
				struct item_data *item = itemdb_exists(70042);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if(sd->inventory_data[index]->equip == EQP_SHADOW_SHOES){
				struct item_data *item = itemdb_exists(70043);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
		}
		...
		if (sd->inventory_data[index]->type == IT_WEAPON) {
			...
			if( enchantgrade_info != nullptr ){
				bonusgrade = enchantgrade_info->bonus;
			}
			...
			if(i == EQI_HAND_L && sd->inventory.u.items_inventory[index].equip == EQP_HAND_L) {
				...
				wa = &base_status->lhw;
				sd->state.idarma = sd->inventory.u.items_inventory[index].nameid;
				sd->state.idenchant = sd->inventory.u.items_inventory[index].card[3];
				if(!mapdata->flag[MF_RESTIGUALITARIA]){
					sd->state.despertartwo = sd->inventory.u.items_inventory[index].despertar;
					sd->state.despertarmenortwo = sd->inventory.u.items_inventory[index].despertarmenor;
				}
			} else {
				wd = &sd->right_weapon;
				wa = &base_status->rhw;
				...
				sd->state.idenchant = sd->inventory.u.items_inventory[index].card[3];
				if(!mapdata->flag[MF_RESTIGUALITARIA]){
					sd->state.despertar = sd->inventory.u.items_inventory[index].despertar;
					sd->state.despertarmenor = sd->inventory.u.items_inventory[index].despertarmenor;
				}
			}
			wa->atk += sd->inventory_data[index]->atk;
			if( info != nullptr ){
				wa->atk2 += info->bonus / 100;
				if(idleft == idright) wa->atk2 += info->bonus / 100;

#ifdef RENEWAL
				if( enchantgrade_info != nullptr ){
					wa->atk2 += ( ( ( info->bonus / 100 ) * bonusgrade ) / 100 );
					if(idleft == idright) wa->atk2 += ( ( ( info->bonus / 100 ) * bonusgrade ) / 100 );
				}
#endif
			...
			wa->wlv = wlv;
			if(!mapdata->flag[MF_RESTIGUALITARIA]){
				int des = sd->inventory.u.items_inventory[index].despertar;
				if(des >= 7){
					
					int des = sd->inventory.u.items_inventory[index].despertar;
					//if(wa->matk > 0) wa->matk += wa->matk * 0.25;
					//if(wa->atk > 0) wa->atk += wa->atk * 0.25;
					if(!sc->data[SC_ELE_TIMER] && !despertartrava){
						despertartrava = 1;
						sc_start(&sd->bl,&sd->bl,SC_ELE_TIMER,100,1,3000);
					}
					int v = 5;
					if(des == 8) { 
						v = 10;
					...
					if(sd->inventory.u.items_inventory[index].despertarmenor) v = v / 4;
					wa->matk += v; 
					wa->atk += v; 
					struct item_data *itemd = itemdb_exists(44625);
					if(itemd)
						run_script(itemd->script,0,sd->bl.id,0);
				}
			}
			...
			if( info != nullptr && sd->weapontype1 != W_BOW ){
				...
				if( enchantgrade_info != nullptr ){
					wa->matk += ( ( ( info->bonus / 100 ) * bonusgrade ) / 100 );
				}
			...
			if(sd->inventory.u.items_inventory[index].despertar){
				struct item_data *item = itemdb_exists(50323);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if(sd->inventory_data[index]->script && (pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(sd->inventory_data[index],sd->bl.m))) {
	...
	if(temarma == 0){
		sd->state.idarma = 0;
		sd->state.idenchant = 0;
		sd->state.despertar = 0;
		sd->state.despertarmenor = 0;
	}
	if(!sd->state.despertar){
		sd->state.eledespertar = 1 + rnd()%9;
		if(sc->data[SC_ELE_TIMER]) { 
			status_change_end(&sd->bl,SC_ELE_TIMER,INVALID_TIMER); 
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_NEUTRAL]) { 
			status_change_end(&sd->bl,SC_ELE_NEUTRAL,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_WATER]) { 
			status_change_end(&sd->bl,SC_ELE_WATER,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_EARTH]) { 
			status_change_end(&sd->bl,SC_ELE_EARTH,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_FIRE]) { 
			status_change_end(&sd->bl,SC_ELE_FIRE,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_WIND]) { 
			status_change_end(&sd->bl,SC_ELE_WIND,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_POISON]) { 
			status_change_end(&sd->bl,SC_ELE_POISON,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_HOLY]) { 
			status_change_end(&sd->bl,SC_ELE_HOLY,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_DARK]) { 
			status_change_end(&sd->bl,SC_ELE_DARK,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_GHOST]) { 
			status_change_end(&sd->bl,SC_ELE_GHOST,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
		if(sc->data[SC_ELE_UNDEAD]) { 
			status_change_end(&sd->bl,SC_ELE_UNDEAD,INVALID_TIMER);
			sc_start(&sd->bl, &sd->bl, SC_DELAYDESPERTAR, 100, 0, 10000);
		}
	}
	...
	if (!sd->combos.empty()) {
		for (const auto &combo : sd->combos) {
			...
			while (j < item_combo->nameid.size()) {
				...
				if(mapdata->flag[MF_RESTIGUALITARIA]){
					if(id->flag.no_equip_igualitario&16)
						no_run = true;
					if(c == 29509 || c == 29371 || c == 310321 || c == 29585 || c == 29436 || c == 311074 || c == 4876 || 
					c == 311073 || c == 311075 || c == 310655 || c == 312475 || c == 311439 || c == 311365 || c == 27109)
						no_run = true;
						
					if(id->equip == EQP_COSTUME_HEAD_TOP || id->equip == EQP_COSTUME_HEAD_MID || id->equip == EQP_COSTUME_HEAD_LOW || id->equip == EQP_COSTUME_GARMENT)
						no_run = true;
				}
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if(mapdata->flag[MF_ANIVERSARIO]) continue;
		...
		if (sd->inventory_data[index]) {
			...
			// bloqueio terceiro enchant woe
			if((sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_TOP || sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_MID || sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_LOW) &&
				sd->inventory.u.items_inventory[index].card[2] &&
				mapdata->flag[MF_EQUIPSGVG]){
				continue;
			}
			if(mapdata->flag[MF_RESTIGUALITARIA]){
				if(sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_TOP || sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_MID || sd->inventory_data[index]->equip == EQP_COSTUME_HEAD_LOW || sd->inventory_data[index]->equip == EQP_COSTUME_GARMENT)
					continue;
			}
			...
			for (j = 0; j < MAX_SLOTS; j++) { // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
				...
				if(!pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) && itemdb_isNoEquip(sd->inventory_data[index],sd->bl.m) && (mapdata->flag[MF_EQUIPSGVG] || mapdata->flag[MF_CHEFFTRANS] || mapdata->flag[MF_RESTIGUALITARIA])) // Card restriction checks.
					continue;
				if(!pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) && itemdb_isNoEquipCard(data,sd->bl.m)) // Card restriction checks.
					continue;
	...
	if(contadorDrake >= 3) {
		clif_displaymessage(sd->fd, "[Carta Drake Selada] Remova uma das cartas Drake Selada que você possui pois apenas duas terÃ£o seus efeitos contabilizados.");
	}

	pc_setaccountreg(sd, add_str("perfuracao"), 0);
	...
	if(sc->data[SC_PET_BONUS]) {
		status_change_end(&sd->bl, SC_PET_BONUS, INVALID_TIMER);
	}
	if(sc->data[SC_PET_COLECAO]) {
		status_change_end(&sd->bl, SC_PET_COLECAO, INVALID_TIMER);
	}
	
	if(sd->state.pet_incubado){
		if(!sc->data[SC_PET_INCUBADO]) {
			sc_start(&sd->bl, &sd->bl, SC_PET_INCUBADO, 100, 0, -1);
		}
		struct item_data *item = itemdb_exists(sd->state.pet_incubado);
		if(item)
			run_script(item->script,0,sd->bl.id,0);

		struct item_data *itemm = itemdb_exists(70013);
		if(itemm)
			run_script(itemm->script,0,sd->bl.id,0);
	} else {
		if(sc->data[SC_PET_INCUBADO]) {
			status_change_end(&sd->bl, SC_PET_INCUBADO, INVALID_TIMER);
		}
	}

	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		if(sd->state.visu_colecao > 0){
			struct item_data *item = itemdb_exists(sd->state.visu_colecao);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		int verus = 0;
		if(sd->verus_state.pandemonium > 0){
			verus = 1;
			struct item_data *item = itemdb_exists(sd->verus_state.pandemonium);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->verus_state.fenda > 0){
			verus = 1;
			struct item_data *item = itemdb_exists(sd->verus_state.fenda);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->verus_state.templo > 0){
			verus = 1;
			struct item_data *item = itemdb_exists(sd->verus_state.templo);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->verus_state.biocheff > 0){
			verus = 1;
			struct item_data *item = itemdb_exists(sd->verus_state.biocheff);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->verus_state.dominio > 0){
			verus = 1;
			struct item_data *item = itemdb_exists(sd->verus_state.dominio);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(verus){
			if(!sc->data[SC_VERUS]) {
				sc_start(&sd->bl, &sd->bl, SC_VERUS, 100, 0, -1);
			}
		} else {
			if(sc->data[SC_VERUS]) {
				status_change_end(&sd->bl, SC_VERUS, INVALID_TIMER);
			}
		}
		// combo de artefatos
		if(sd->verus_state.dominio == 123956 && sd->verus_state.fenda == 123957 && sd->verus_state.pandemonium == 123958 && sd->verus_state.templo == 123959 && sd->verus_state.biocheff == 123960){
			struct item_data *item = itemdb_exists(123979);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	}
	
	if(sc->data[SC_ANTIGA_RUNA_CORROMP]){
		if(sc->data[SC_ANTIGA_RUNA_CORROMP]->val1 == 1){
			struct item_data *item = itemdb_exists(50060);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		else if(sc->data[SC_ANTIGA_RUNA_CORROMP]->val1 == 2){
			struct item_data *item = itemdb_exists(50061);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	}
	if(sc->data[SC_ANTIGA_RUNA_CORROMP_SUP]){
		if(sc->data[SC_ANTIGA_RUNA_CORROMP_SUP]->val1 == 1){
			struct item_data *item = itemdb_exists(50062);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		else if(sc->data[SC_ANTIGA_RUNA_CORROMP_SUP]->val1 == 2){
			struct item_data *item = itemdb_exists(50063);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	}
	if(sc->data[SC_RUNA_CORROMP]){
		struct item_data *item = itemdb_exists(50064);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(sc->data[SC_RUNA_CORROMP_SUP]){
		struct item_data *item = itemdb_exists(50065);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(sc->data[SC_PARFAIT_AVENTUREIRO]){
		struct item_data *item = itemdb_exists(290447);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(sc->data[SC_ELIXIR_DETERMINACAO]){
		struct item_data *item = itemdb_exists(290448);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(sc->data[SC_SUPREMO_INFINITO]){
		struct item_data *item = itemdb_exists(44642);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	} else {
		if(sc->data[SC_MTF_MLEATKED]){
			struct item_data *item = itemdb_exists(44638);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sc->data[SC_MTF_CRIDAMAGE2]){
			struct item_data *item = itemdb_exists(44638);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	...
	if(sc->data[SC_RESETE]){
		struct item_data *item = itemdb_exists(44639);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(sc->data[SC_JOIA_ALMAS]){
		struct item_data *item = itemdb_exists(44640);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	if(sc->data[SC_BOLINHO_BRILHANTE]){
		struct item_data *item = itemdb_exists(51150);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	
	if(sc->data[SC_ENERGETICO_MONSTRO]){
		struct item_data *item = itemdb_exists(51159);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	
	if(sd->history_state.bindconta >= 10){
		if(!sd->sc.data[SC_ITEM_BIND]) 
			sc_start(&sd->bl, &sd->bl, SC_ITEM_BIND, 100, 0, -1);
		sd->history_state.bindconta = 10;
	} else {
		if(sd->sc.data[SC_ITEM_BIND]) 
			status_change_end(&sd->bl, SC_ITEM_BIND, INVALID_TIMER);
	}
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		// halloween
		if(pc_checkskill(sd, HL_MAESTRIA) > 0){
			struct item_data *item = itemdb_exists(44626);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// aniversario
		if(pc_checkskill(sd, RB_MAESTRIA_ANIVERSARIO) > 0){
			struct item_data *item = itemdb_exists(44730);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// carnaval
		if(pc_checkskill(sd, HR_CACA_TESOURO) > 0){
			struct item_data *item = itemdb_exists(44726);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// aniversario
		if(pc_checkskill(sd, AN_FISICO) > 0){
			struct item_data *item = itemdb_exists(44621);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(pc_checkskill(sd, AN_MAGICO) > 0){
			struct item_data *item = itemdb_exists(44622);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(pc_checkskill(sd, AN_DISTANCIA) > 0){
			struct item_data *item = itemdb_exists(44623);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(mapdata->flag[MF_GUERRAPVM]){
			struct item_data *item = itemdb_exists(44624);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	
		if(sd->history_state.resetchar > 0){
			struct item_data *item = itemdb_exists(44630);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->history_state.resetaccount > 0){
			struct item_data *item = itemdb_exists(44631);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->history_state.burnreborn > 0){
			struct item_data *item = itemdb_exists(44636);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->history_state.burnreborn > 0 || sd->history_state.resetaccount > 0 || sd->history_state.resetchar > 0){
			struct item_data *item = itemdb_exists(44641);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_TOP_FIS_GVG] || sd->sc.data[SC_TOP_MAG_GVG] || sd->sc.data[SC_TOP_DIS_GVG]){
			struct item_data *item = itemdb_exists(44632);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_TOP_GUILD_GVG1]){
			struct item_data *item = itemdb_exists(44633);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		} else if(sd->sc.data[SC_TOP_GUILD_GVG2]){
			struct item_data *item = itemdb_exists(44634);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		} else if(sd->sc.data[SC_TOP_GUILD_GVG3]){
			struct item_data *item = itemdb_exists(44635);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_ROCHAPURPURA]){
			struct item_data *item = itemdb_exists(44637);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_ROCHAPURPURATWO]){
			struct item_data *item = itemdb_exists(44684);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	}
	
	if(sd->history_state.account_casamento){
		struct map_session_data* csd = map_id2sd(sd->history_state.account_casamento);
		if(csd && csd->bl.m == sd->bl.m && !pc_isdead(csd)) {
			struct item_data *itemd = itemdb_exists(70035);
			if(itemd){
				run_script(itemd->script,0,sd->bl.id,0);
				sc_start(&sd->bl, &sd->bl, SC_CASAMENTO, 100, 0, -1);
			}
		} else if(sd->sc.data[SC_CASAMENTO])
			status_change_end(&sd->bl, SC_CASAMENTO, INVALID_TIMER);
	} else if(sd->sc.data[SC_CASAMENTO])
			status_change_end(&sd->bl, SC_CASAMENTO, INVALID_TIMER);
	
	// history help
	if((sd->status.guild_id == 2082) && sd->status.base_level <= 200){
		struct item_data *itemd = itemdb_exists(70030);
		if(itemd)
			run_script(itemd->script,0,sd->bl.id,0);
	}
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		// reborn charge
		if(sd->history_state.daystempreborn){
			struct item_data *item = itemdb_exists(70031);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// Runa Sombria
		if(sd->sc.data[SC_RUNA_SOMBRIA]){
			struct item_data *item = itemdb_exists(70025);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// Runa DracÃ¯Â¿Â½nica
		if(sd->sc.data[SC_RUNA_DRACONICA]){
			struct item_data *item = itemdb_exists(70026);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// Runa Celestial
		if(sd->sc.data[SC_RUNA_CELESTIAL]){
			struct item_data *item = itemdb_exists(70027);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// Runa TÃ¯Â¿Â½xica
		if(sd->sc.data[SC_RUNA_TOXICA]){
			struct item_data *item = itemdb_exists(70028);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		// Runa Atlantes
		if(sd->sc.data[SC_RUNA_ATLANTES]){
			struct item_data *item = itemdb_exists(70029);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(pc_checkskill(sd,NT_GUERREIRO) >= 4)
			sd->bonus.penetracao += 1;
		
		if(sd->sc.data[SC_ESCUDO_DOURADO]){
			struct item_data *item = itemdb_exists(70047);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_ESCUDO_PRATA]){
			struct item_data *item = itemdb_exists(413150);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_ENVELOPE_VERMELHO]){
			struct item_data *item = itemdb_exists(413151);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_CAIXA_DOCES_KAFRA]){
			struct item_data *item = itemdb_exists(413152);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_DOCE_PEGASO]){
			struct item_data *item = itemdb_exists(413153);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		
		if(sd->sc.data[SC_ELIXIR_CARNAVALESCO]){
			sd->bonus.penetracao += 1;
			sd->bonus.drop_final += 2;
			sd->bonus.dano_final += 3;
			sd->bonus.exp_excelencia += 20;
		}
		if(sd->state.arma_visual){
			sd->bonus.penetracao += 1;
		}
		if(sd->sc.data[SC_BENCAO_HEROI]){
			struct item_data *item = itemdb_exists(70037);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->sc.data[SC_PERGA_HEROI] || sd->state.assinatura_pk == 2){
			struct item_data *item = itemdb_exists(70038);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
		if(sd->status.class_ == JOB_DRAGON_KNIGHT){
			struct item_data *item = itemdb_exists(70039);
			if(item)
				run_script(item->script,0,sd->bl.id,0);
		}
	...
	if(mapdata->flag[MF_SKILLPARRY]){
		struct item_data *item = itemdb_exists(70023);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
	
	if(sd->sc.data[SC_DROP_BIOCHEFF] && (mapdata->flag[MF_BIOHARD] || mapdata->flag[MF_BIOSAVAGE] || mapdata->flag[MF_BIOEXTREME] || mapdata->flag[MF_BIOCHEFFNORMAL] || mapdata->flag[MF_CHEFFEXTREME])){
		sd->bonus.drop_final += 5;
	}
	if(sd->sc.data[SC_RESIST_BIOCHEFF] && (mapdata->flag[MF_BIOHARD] || mapdata->flag[MF_BIOSAVAGE] || mapdata->flag[MF_BIOEXTREME] || mapdata->flag[MF_BIOCHEFFNORMAL] || mapdata->flag[MF_CHEFFEXTREME])){
		sd->bonus.reducao_ninja += 10;
	}
	...
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		if( sd->pd ) { // Pet Bonus
			...
			if(pet_db_ptr != nullptr && !sd->state.pet_incubado){
				struct item_data *itemm = itemdb_exists(70013);
				if(itemm)
					run_script(itemm->script,0,sd->bl.id,0);
			}
			if (pet_db_ptr != nullptr && pd->pet.equip >= 77730 && pd->pet.equip <= 77740){
				struct item_data *item = itemdb_exists(pd->pet.equip);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			if (pet_db_ptr != nullptr && pd->pet.equiptwo == 77733){
				struct item_data *item = itemdb_exists(pd->pet.equiptwo);
				if(item)
					run_script(item->script,0,sd->bl.id,0);
			}
			// DEVON PET ENCHANT
			if(pd->pet.egg_id >= 90000){
				int indexpet = pet_egg_search( sd, pd->pet.pet_id );
				if (sd->inventory_data[indexpet]) {
					current_equip_opt_index = -1;
					current_equip_item_index = indexpet;
					
					if (sd->inventory_data[index]) {
						for (uint8 j = 0; j < MAX_ITEM_RDM_OPT; j++) {
							short opt_id = sd->inventory.u.items_inventory[indexpet].option[j].id;
					...
					current_equip_opt_index = -1;
					// fantasia de pet
					if(sd->inventory.u.items_inventory[indexpet].card[3]){
						struct item_data *item = itemdb_exists(sd->inventory.u.items_inventory[indexpet].card[3]);
						if(item)
							run_script(item->script,0,sd->bl.id,0);
					}
				}
		...
		// cole??o
		struct item_data *item = itemdb_exists(70018);
		if(item)
			run_script(item->script,0,sd->bl.id,0);
	}
...
// ----- STATS CALCULATION -----
	...
	// devon Reputation
	// -------------------------------------------------
	int allstatsreputcheff = 0, allstatsreputbio = 0;
	if(!mapdata->flag[MF_RESTIGUALITARIA]){
		if(sd && sd->reputation[3].points >= 1000) // Cheff Savage
			allstatsreputcheff += ((sd->reputation[3].points / 1000) / 1);
		if(sd && sd->reputation[5].points >= 1000) // Bio Savage
			allstatsreputbio += ((sd->reputation[5].points / 1000) / 1);
		
		if(battle_config.desativarexcelencia){
			if(sd && sd->reputation[12].points >= 1000) // excelencia
				allstatsreputcheff += 2;
			if(sd && sd->reputation[12].points >= 3000) // excelencia
				allstatsreputcheff += 5;
		}
	}
	
	// Trava na WoE pra n funfar
	if(mapdata->flag[MF_EQUIPSGVG]) { allstatsreputcheff = 0; allstatsreputbio = 0; }


	if(sd->sc.data[SC_EFEITO_RH_9]) allstatsreputbio += sd->sc.data[SC_EFEITO_RH_9]->val1; // Bolinho de Cerejeira

	int travafenda;
	if(sd->sc.data[SC_EFEITO_RH_23] && !travafenda) { allstatsreputbio += 12; travafenda = 1; }// buff fenda D Ultimate
	if(sd->sc.data[SC_EFEITO_RH_22] && !travafenda) { allstatsreputbio += 6; travafenda = 1; } // buff fenda D unreal
	if(sd->sc.data[SC_FENDAD_INFERNAL] && !travafenda) { allstatsreputbio += 3; travafenda = 1; } // buff fenda D Infernal
	if(sd->sc.data[SC_EFEITO_RH_21] && !travafenda) { allstatsreputbio += 3; travafenda = 1; } // buff fenda savage
	int addpow = 0, addsta = 0, addwis = 0, addspl = 0, addcon = 0, addctr = 0;
	int addfor = 0, addagi = 0, addvit = 0, addint = 0, adddex = 0, addluk = 0;
	// Godlys
	if(sd->sc.data[SC_BRYNHILDR]){
		addvit += 40; 
		addsta += 20; 
		allstatsreputcheff += 10; 
	} else if(sd->sc.data[SC_ASFRIKA]){
		addagi += 40; 
		addwis += 20; 
		allstatsreputcheff += 20; 
	} else if(sd->sc.data[SC_BRISINGAMEN]){
		addluk += 40; 
		addctr += 20; 
		allstatsreputcheff += 10; 
	} else if(sd->sc.data[SC_MJOLLNIR]){
		adddex += 40; 
		addcon += 20; 
		allstatsreputcheff += 10; 
	} else if(sd->sc.data[SC_SLEIPNIR]){
		addint += 40; 
		addspl += 20; 
		allstatsreputcheff += 10; 
	} else if(sd->sc.data[SC_MEGINGJARD]){
		addfor += 40; 
		addpow += 20; 
		allstatsreputcheff += 10; 
	}
	...
	// rank trial
	if(sd->sc.data[SC_RANK_TRIAL]){
		allstatsreputbio += 5; 
	}
	// Mega Palito
	if(sd->sc.data[SC_MEGA_PALITO]){
		allstatsreputcheff += 15; 
		allstatsreputbio += 5;
	}
	// Bolinho Brilhante
	if(sd->sc.data[SC_BOLINHO_BRILHANTE]){
	...
	i = base_status->str + sd->status.str + sd->indexed_bonus.param_bonus[PARAM_STR] + sd->indexed_bonus.param_equip[PARAM_STR] + allstatsreputcheff + addfor;
	base_status->str = cap_value(i,0,USHRT_MAX);
	i = base_status->agi + sd->status.agi + sd->indexed_bonus.param_bonus[PARAM_AGI] + sd->indexed_bonus.param_equip[PARAM_AGI] + allstatsreputcheff + addagi;
	base_status->agi = cap_value(i,0,USHRT_MAX);
	i = base_status->vit + sd->status.vit + sd->indexed_bonus.param_bonus[PARAM_VIT] + sd->indexed_bonus.param_equip[PARAM_VIT] + allstatsreputcheff + addvit;
	base_status->vit = cap_value(i,0,USHRT_MAX);
	i = base_status->int_+ sd->status.int_+ sd->indexed_bonus.param_bonus[PARAM_INT] + sd->indexed_bonus.param_equip[PARAM_INT] + allstatsreputcheff + addint;
	base_status->int_ = cap_value(i,0,USHRT_MAX);
	i = base_status->dex + sd->status.dex + sd->indexed_bonus.param_bonus[PARAM_DEX] + sd->indexed_bonus.param_equip[PARAM_DEX] + allstatsreputcheff + adddex;
	base_status->dex = cap_value(i,0,USHRT_MAX);
	i = base_status->luk + sd->status.luk + sd->indexed_bonus.param_bonus[PARAM_LUK] + sd->indexed_bonus.param_equip[PARAM_LUK] + allstatsreputcheff + addluk;
	base_status->luk = cap_value(i,0,USHRT_MAX);
	i = base_status->pow + sd->status.pow + sd->indexed_bonus.param_bonus[PARAM_POW] + sd->indexed_bonus.param_equip[PARAM_POW] + allstatsreputbio + addpow;
	base_status->pow = cap_value(i, 0, USHRT_MAX);
	i = base_status->sta + sd->status.sta + sd->indexed_bonus.param_bonus[PARAM_STA] + sd->indexed_bonus.param_equip[PARAM_STA] + allstatsreputbio + addsta;
	base_status->sta = cap_value(i, 0, USHRT_MAX);
	i = base_status->wis + sd->status.wis + sd->indexed_bonus.param_bonus[PARAM_WIS] + sd->indexed_bonus.param_equip[PARAM_WIS] + allstatsreputbio + addwis;
	base_status->wis = cap_value(i, 0, USHRT_MAX);
	i = base_status->spl + sd->status.spl + sd->indexed_bonus.param_bonus[PARAM_SPL] + sd->indexed_bonus.param_equip[PARAM_SPL] + allstatsreputbio + addspl;
	base_status->spl = cap_value(i, 0, USHRT_MAX);
	i = base_status->con + sd->status.con + sd->indexed_bonus.param_bonus[PARAM_CON] + sd->indexed_bonus.param_equip[PARAM_CON] + allstatsreputbio + addcon;
	base_status->con = cap_value(i, 0, USHRT_MAX);
	i = base_status->crt + sd->status.crt + sd->indexed_bonus.param_bonus[PARAM_CRT] + sd->indexed_bonus.param_equip[PARAM_CRT] + allstatsreputbio + addctr;
...
// ----- MISC CALCULATIONS -----
	...
	//f(sc->data[SC_EFEITO_RH_16] && sd && mapdata->flag[MF_FENDA])
	//	sd->dsprate -= 99;
	...
	if(sc->count) {
		...
		if (sc->data[SC_MTF_CRIDAMAGE2])
			sd->bonus.crit_atk_rate += sc->data[SC_MTF_CRIDAMAGE2]->val1;
...
void status_calc_bl_main(struct block_list *bl, std::bitset<SCB_MAX> flag)
{
	...
	**/
	...
	if(flag[SCB_BATK] && b_status->batk) {
		...
		int skill_lv;
		if ((skill_lv = pc_checkskill(sd, NV_BREAKTHROUGH)) > 0)
			status->batk += 15 * skill_lv + (skill_lv > 4 ? 25 : 0);
	...
	if(flag[SCB_SPEED]) {
		...
		//if( sd && map_getmapflag(sd->bl.m, MF_FENDADIMENSIONAL)) status->speed = status->speed * 1.3;
	...
	**/
	...
	if(flag[SCB_ASPD]) {
		...
		if ( bl->type&BL_HOM ) {
			...
			amotion = status_calc_fix_aspd(bl, sc, amotion);
			struct map_data *mapdata = map_getmapdata(sd->bl.m);
			if(mapdata->flag[MF_DOMINIOENT]) {
				amotion = 530;
				int repu;
				if(sd->state.idarma == 66007 || sd->state.idarma == 66004){
					repu += ((sd->reputation[9].points / 1000) / 1); // Minera??o
					if(repu >= 1) amotion += -20;
					if(repu >= 3) amotion += -40;
					if(repu >= 5) amotion += -100;
				}
				if(sd->state.idarma == 66002 || sd->state.idarma == 66003){
					repu += ((sd->reputation[10].points / 1000) / 1); // Herbologia
					if(repu >= 1) amotion += -20;
					if(repu >= 3) amotion += -40;
					if(repu >= 5) amotion += -100;
				}
			}
			if(mapdata->name == "f_interior") {
				amotion = 800;
				if(sc && sc->data[SC_SIMON]) amotion += 200;
			}
...
static unsigned short status_calc_str(struct block_list *bl, struct status_change *sc, int str)
{
	...
	if(sc->data[SC_MEGA_BUZZY])
		str += sc->data[SC_MEGA_BUZZY]->val1;
	...
	if(sc->data[SC_2011RWC_SCROLL] && !sc->data[SC_STRFOOD] && !sc->data[SC_FOOD_STR_CASH])
...
static unsigned short status_calc_agi(struct block_list *bl, struct status_change *sc, int agi)
{
	...
	if(sc->data[SC_MEGA_BUZZY])
		agi += sc->data[SC_MEGA_BUZZY]->val1;
	...
	if(sc->data[SC_2011RWC_SCROLL] && !sc->data[SC_AGIFOOD] && !sc->data[SC_FOOD_AGI_CASH])
...
static unsigned short status_calc_vit(struct block_list *bl, struct status_change *sc, int vit)
{
	...
	if(sc->data[SC_2011RWC_SCROLL] && !sc->data[SC_VITFOOD] && !sc->data[SC_FOOD_VIT_CASH])
...
static unsigned short status_calc_int(struct block_list *bl, struct status_change *sc, int int_)
{
	...
	if(sc->data[SC_MEGA_BUZZY])
		int_ += sc->data[SC_MEGA_BUZZY]->val1;
	...
	if(sc->data[SC_2011RWC_SCROLL] && !sc->data[SC_INTFOOD] && !sc->data[SC_FOOD_INT_CASH])
...
static unsigned short status_calc_dex(struct block_list *bl, struct status_change *sc, int dex)
{
	...
	if(sc->data[SC_MEGA_BUZZY])
		dex += sc->data[SC_MEGA_BUZZY]->val1;
	...
	if(sc->data[SC_2011RWC_SCROLL] && !sc->data[SC_DEXFOOD] && !sc->data[SC_FOOD_DEX_CASH])
...
static unsigned short status_calc_luk(struct block_list *bl, struct status_change *sc, int luk)
{
	...
	if(sc->data[SC_CURSE] || sc->data[SC_MALDICAO_V2])
		return 0;
	if(sc->data[SC_INCALLSTATUS])
	...
	if(sc->data[SC_MEGA_BUZZY])
		luk += sc->data[SC_MEGA_BUZZY]->val1;
	...
	if(sc->data[SC_2011RWC_SCROLL] && !sc->data[SC_LUKFOOD] && !sc->data[SC_FOOD_LUK_CASH])
...
static unsigned short status_calc_pow(struct block_list *bl, struct status_change *sc, int pow)
{
	...
	if (sc->data[SC_MARINE_FESTIVAL_OF_KI_SUL])
		pow += sc->data[SC_MARINE_FESTIVAL_OF_KI_SUL]->val1*2;
...
static unsigned short status_calc_sta(struct block_list *bl, struct status_change *sc, int sta)
{
	...
	if (sc->data[SC_SANDY_FESTIVAL])
		sta += sc->data[SC_SANDY_FESTIVAL]->val1 * 2;
...
static unsigned short status_calc_wis(struct block_list *bl, struct status_change *sc, int wis)
{
	...
	if (sc->data[SC_SANDY_FESTIVAL])
		wis += sc->data[SC_SANDY_FESTIVAL]->val1 * 2;
...
static unsigned short status_calc_spl(struct block_list *bl, struct status_change *sc, int spl)
{
	...
	if (sc->data[SC_SANDY_FESTIVAL])
		spl += sc->data[SC_SANDY_FESTIVAL]->val1 * 2;
...
static unsigned short status_calc_con(struct block_list *bl, struct status_change *sc, int con)
{
	...
	if (sc->data[SC_MARINE_FESTIVAL_OF_KI_SUL])
		con += sc->data[SC_MARINE_FESTIVAL_OF_KI_SUL]->val1 * 2;
...
static unsigned short status_calc_crt(struct block_list *bl, struct status_change *sc, int crt)
{
	...
	if (sc->data[SC_MARINE_FESTIVAL_OF_KI_SUL])
		crt += sc->data[SC_MARINE_FESTIVAL_OF_KI_SUL]->val1 * 2;
...
static unsigned short status_calc_batk(struct block_list *bl, struct status_change *sc, int batk)
{
	...
	if(sc->data[SC_JOINTBEAT] && sc->data[SC_JOINTBEAT]->val2&BREAK_WAIST)
		batk -= batk * 25/100;
	if(sc->data[SC_CURSE] || sc->data[SC_SOMBRIO_02] || sc->data[SC_MALDICAO_V2])
		batk -= batk * 25/100;
	...
	if (sc->data[SC_SHRIMP])
		batk += batk * sc->data[SC_SHRIMP]->val1 / 100;
#ifdef RENEWAL
	if (sc->data[SC_LOUD])
		batk += 30;
...
static defType status_calc_def(struct block_list *bl, struct status_change *sc, int def)
{
	...
	if(sc->data[SC_STONE])
		def >>=1;
	if(sc->data[SC_FREEZE] || sc->data[SC_CONGELAR_V2])
		def >>=1;
	...
	if( sc->data[SC_FREEZING] || sc->data[SC_HANDICAPSTATE_FROSTBITE] || sc->data[SC_HANDICAPSTATE_DEADLYPOISON] || sc->data[SC_HIPOTERMIA_V2] )
...
#endif
	if(sc->data[SC_POISON] || sc->data[SC_ENVENENAR_V2])
		def2 -= def2 * 25/100;
...
static unsigned short status_calc_speed(struct block_list *bl, struct status_change *sc, int speed)
{
	...
	if(sd){
		struct map_data *mapdata = map_getmapdata(bl->m);
		if(sd->state.goblinferido == 1 && sd->md) {
			if(mapdata->flag[MF_DOMINIOGERAL])
				return (unsigned short)cap_value(speed, MIN_WALK_SPEED, MAX_WALK_SPEED);
			else {
				mercenary_delete(sd->md, 0);
			}
		}

		if (pc_ismadogear(sd) && !mapdata->flag[bl->m, MF_MONSTROMVPSORTE]) { // Mado speed is not affected by other statuses
			int val = 0;
			...
			if (pc_checkskill(sd, NC_MADOLICENCE) < 5)
				...
				val -= 25;
			if (sc->data[SC_ACCELERATION] && !sc->data[SC_SPEEDUP1])
				val -= 25;
			if( sc->data[SC_SPEEDUP1] ) // !FIXME: used both by NPC_AGIUP and Speed Potion script
				val -= sc->data[SC_SPEEDUP1]->val1;
			//if( sc->data[SC_ALL_IN_ONE] ) // !FIXME: used both by NPC_AGIUP and Speed Potion script
			//	val -= 30;
			if( sc->data[SC_SPEED_MAESTRIA] )
				val -= sc->data[SC_SPEED_MAESTRIA]->val1;
			if( sc->data[SC_VELO_ESPECIAL] )
				val -= sc->data[SC_VELO_ESPECIAL]->val1;
	...
	if( sd && sd->ud.skilltimer != INVALID_TIMER && (pc_checkskill(sd,SA_FREECAST) > 0 || sd->ud.skill_id == LG_EXEEDBREAK) ) {
		...
		if( sd && sc->data[SC_HIDING] && pc_checkskill(sd,RG_TUNNELDRIVE) > 0 )
			...
			if( sc->data[SC_VENEPROFUN_1] )
				val = max( val, 10 );
			if( sc->data[SC_VENEPROFUN_2] )
				val = max( val, 15 );
			if( sc->data[SC_VENEPROFUN_3] )
				val = max( val, 20 );
			if( sc->data[SC_VENEPROFUN_4] )
				val = max( val, 25 );
			if( sc->data[SC_DECREASEAGI] )
				val = max( val, 25 );
			if( sc->data[SC_AGULHA_VENENOSA] )
				val = max( val, 100 );
			if( sc->data[SC_QUAGMIRE] || sc->data[SC_HALLUCINATIONWALK_POSTDELAY] || (sc->data[SC_GLOOMYDAY] && sc->data[SC_GLOOMYDAY]->val4) || 
				(sc->data[SC_HANDICAPSTATE_DEADLYPOISON] && map_getmapflag(bl->m, MF_FENDA)) )
				val = max( val, 50 );
			...
			if( sc->data[SC_SKA] )
				val = max( val, 25 );
			if( sc->data[SC_FREEZING] || sc->data[SC_HIPOTERMIA_V2] )
				val = max( val, 30 );
			if( sc->data[SC_MARSHOFABYSS] )
		...
		// GetMoveHasteValue1()
		if( sd && sc->data[SC_EFEITO_RH_13] && map_getmapflag(sd->bl.m, MF_FENDA)) 
			val = max( val, sc->data[SC_EFEITO_RH_13]->val1 );
		if( sc->data[SC_SPEEDUP1] ) // !FIXME: used both by NPC_AGIUP and Speed Potion script
			val = max( val, sc->data[SC_SPEEDUP1]->val1 );
		if( sc->data[SC_SPEED_MAESTRIA] )
			val = max( val, sc->data[SC_SPEED_MAESTRIA]->val1 );
		if( sc->data[SC_VELO_ESPECIAL] )
			val = max( val, sc->data[SC_VELO_ESPECIAL]->val1 );
		if( sc->data[SC_INCREASEAGI] )
			val = max( val, 25 );
		if( sc->data[SC_WINDWALK] )
		...
		if( sc->data[SC_HOVERING] )
			val = max( val, 10 );
		if( sc->data[SC_GN_CARTBOOST] && !sc->data[SC_MARSHOFABYSS] && !sc->data[SC_FREEZING] )
			val = max( val, sc->data[SC_GN_CARTBOOST]->val2 );
		if(map_getmapflag(bl->m, MF_MONSTROMVPSORTE) && !sc->data[SC_GN_CARTBOOST])
			val = max( val, 100 );
		if(sc->data[SC_DOCE_AGI])
			val = max( val, 20 );
...
static unsigned short status_calc_dmotion(struct block_list *bl, struct status_change *sc, int dmotion)
{
	...
	if (bl->type == BL_PC) {
		...
		if (((TBL_PC *)bl)->special_state.no_walk_delay || ((TBL_PC *)bl)->sc.data[SC_STEELBODY])
			return 0;
	}
...
static signed short status_calc_patk(struct block_list *bl, struct status_change *sc, int patk)
{
	...
	if (sc->data[SC_TEMPORARY_COMMUNION])
		patk += sc->data[SC_TEMPORARY_COMMUNION]->val1 * 3;
	if (sc->data[SC_BLESSING_OF_M_CREATURES])
		patk += sc->data[SC_BLESSING_OF_M_CREATURES]->val1 * 10;
	if (sc->data[SC_TEMPERING])
		patk += sc->data[SC_TEMPERING]->val2;
	if (sc->data[SC_OVERCOMING_CRISIS])
		patk += sc->data[SC_OVERCOMING_CRISIS]->val2;

	struct map_data *mapdata = map_getmapdata(bl->m);
	int bonusmonsterap = 300;
	if( bl->type == BL_MOB && mapdata->flag[MF_APSTATSMONSTERS] ){
		patk += 300;
	}
...
static signed short status_calc_smatk(struct block_list *bl, struct status_change *sc, int smatk)
{
	...
	if (sc->data[SC_TEMPORARY_COMMUNION])
		smatk += sc->data[SC_TEMPORARY_COMMUNION]->val1 * 3;
	if (sc->data[SC_BLESSING_OF_M_CREATURES])
		smatk += sc->data[SC_BLESSING_OF_M_CREATURES]->val1 * 10;
	if (sc->data[SC_ATTACK_STANCE])
		smatk += sc->data[SC_ATTACK_STANCE]->val3;
	if (sc->data[SC_OVERCOMING_CRISIS])
		smatk += sc->data[SC_OVERCOMING_CRISIS]->val2;
...
static unsigned char status_calc_element(struct block_list *bl, struct status_change *sc, int element)
{
	...
	if(sc->data[SC_FREEZE] || sc->data[SC_CRYSTAL_ARMOR_OPTION] || sc->data[SC_HANDICAPSTATE_FROSTBITE] || sc->data[SC_CONGELAR_V2])
		return ELE_WATER;
...
int status_change_start(struct block_list* src, struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,t_tick duration,unsigned char flag, int32 delay) {
	...
	if( bl->type != BL_NPC && (status_isdead(bl) && type != SC_SOMBRIO_01) && ( type != SC_NOCHAT && type != SC_JAILED ) ) // SC_NOCHAT and SC_JAILED should work even on dead characters
		return 0;
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	if (type == SC_HELLPOWER && sd && sd->bonus.tempo_hell) {
		duration = 180000 - sd->bonus.tempo_hell;
	}
	...
	if(bl->type == BL_PC){
		switch (type) {
			case SC_GRAVITYCONTROL:
				if(sd->status.class_ == JOB_SPIRIT_HANDLER || sd->status.class_ == JOB_CARDINAL || sd->status.class_ == JOB_SOUL_ASCETIC)
					return 0;
			break;
			case SC_CLOAKING:
			case SC_CAMOUFLAGE:
	...
	switch (type) {
		...
		case SC_HANDICAPSTATE_LIGHTNINGSTRIKE:
			if(sd && status->mres >= 262 && !map_getmapflag(bl->m, MF_ANIVERSARIO))
				return 0;
			break;
	}
	switch (type) {
	...
	switch(type)
	{
		...
		case SC_ENSEMBLEFATIGUE:
			...
			break;
		case SC_RICHMANKIM:
			val2 = 25 + 11 * val1; // Exp increase bonus
			break;
		case SC_DRUMBATTLE:
		...
		case SC_COMA: { // Coma. Sends a char to 1HP. If val2, do not zap sp
			...
			// devon
			if(tsd && tsd->sc.data[SC_STEELBODY] && map_getmapflag(bl->m, MF_BIOSAVAGE)) break;
			if(map_getmapflag(bl->m, MF_TRIALVALE)) return 1;
		...
		case SC_GUARDIAN_S:
			...
			val2 = status->max_hp * (25 * val1) / 100;// Barrier HP
			break;
		case SC_REBOUND_S:
		...
		case SC_STAR_BURST:
			...
			val4 = tick - tick_time; // Remaining time
			break;
		case SC_MARINE_FESTIVAL_OF_KI_SUL:
...
int status_change_clear(struct block_list* bl, int type)
{
	...
	for (const auto &it : status_db) {
		...
		if (type == 0) { // Type 0: PC killed
			...
			if(status == SC_HIDDEN_CARD && map_getmapflag(bl->m, MF_BIOSAVAGE))
				continue;
			if(sc->data[SC_POCAO_DE_BUFF] && (status == SC_BLESSING || status == SC_INCREASEAGI || status == SC_GLORIA || status == SC_IMPOSITIO))
				continue;
			if(status == SC_HEAVEN_AND_EARTH){
				struct map_session_data *sd = BL_CAST(BL_PC,bl);
				if(sd && checar_carta(EQI_ARMOR,sd, 4, 311177, 0) > 0) continue;
			}
			if(status == SC_MASSIVE_F_BLASTER){
				struct map_session_data *sd = BL_CAST(BL_PC,bl);
				if(sd && checar_carta(EQI_ARMOR,sd, 4, 311166, 0) > 0) continue;
			}
			if(status == SC_UNLIMITEDHUMMINGVOICE){
				struct map_session_data *sd = BL_CAST(BL_PC,bl);
				if(sd && checar_carta(EQI_HAND_R,sd, 2, 311242, 0) > 0) continue;
			}
			if(status == SC_COMPETENTIA){
				struct map_session_data *sd = BL_CAST(BL_PC,bl);
				if(sd && checar_equip(EQI_HAND_R,sd, 0, 550011, 0) > 0) continue;
			}
			if(status == SC_RUSH_QUAKE2){
				struct map_session_data *sd = BL_CAST(BL_PC,bl);
				if(sd && checar_equip(EQI_HAND_R,sd, 0, 620003, 13) > 0) continue;
			}
			if(status == SC_INSPIRATION){
				struct map_session_data *sd = BL_CAST(BL_PC,bl);
				if(sd && checar_equip(EQI_HAND_R,sd, 4, 530034, 13) > 0) continue;
			}
		}
...
int status_change_end_(struct block_list* bl, enum sc_type type, int tid, const char* file, int line)
{
	...
	if (tid == INVALID_TIMER) {
		if (type == SC_PCCAFE_PLAY_TIME && !sd->state.presencaparado && !sd->state.autotrade)
			return 0;
	...
	switch(type) {
		case SC_DANO_DPS:
			if(sd) {
				...
				if(sd->danomilhao > 0 || sd->danorestante > 0){
					char msgnew[256];
					sprintf(msgnew, "[@dps] Dano nos últimos 5 segundos: %dM e %d", sd->danomilhao, sd->danorestante);
					clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msgnew, false, SELF);
					sd->danomilhao = 0;
					sd->danorestante = 0;
				}
				...
				if(now > sd->timerdps){
					sd->forceinidps = 0;
					clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[@dps] o tempo de 1 minutos acabou, ative novamente caso precise de mais tempo.", false, SELF);
				}
			}
		break;
		case SC_PCCAFE_PLAY_TIME:
			...
			if(pc_isvip(sd) || sd->group_id >= 10)
				sd->goldPCPoints = cap_value((sd->goldPCPoints + 2), 0, 300);
			else 
		...
		case SC_ELE_TIMER:
			...
			if(sd->state.despertar >= 7){
				if(sd->state.eledespertar == 10) sd->state.eledespertar = 0;
				sd->state.eledespertar += 1;
				int timedesp = 3000;
				if(sd->state.eledespertar == sd->state.desp_ele_selected)
					timedesp = 6000;
				
				sc_start(&sd->bl,&sd->bl,SC_ELE_TIMER,100,1,timedesp);
				if(!sd->sc.data[SC_DELAYDESPERTAR]){
					if(sd->state.eledespertar == 1) sc_start(&sd->bl,&sd->bl,SC_ELE_NEUTRAL,100,1,timedesp);
					if(sd->state.eledespertar == 2) sc_start(&sd->bl,&sd->bl,SC_ELE_WATER,100,1,timedesp);
					if(sd->state.eledespertar == 3) sc_start(&sd->bl,&sd->bl,SC_ELE_EARTH,100,1,timedesp);
					if(sd->state.eledespertar == 4) sc_start(&sd->bl,&sd->bl,SC_ELE_FIRE,100,1,timedesp);
					if(sd->state.eledespertar == 5) sc_start(&sd->bl,&sd->bl,SC_ELE_WIND,100,1,timedesp);
					if(sd->state.eledespertar == 6) sc_start(&sd->bl,&sd->bl,SC_ELE_POISON,100,1,timedesp);
					if(sd->state.eledespertar == 7) sc_start(&sd->bl,&sd->bl,SC_ELE_HOLY,100,1,timedesp);
					if(sd->state.eledespertar == 8) sc_start(&sd->bl,&sd->bl,SC_ELE_DARK,100,1,timedesp);
					if(sd->state.eledespertar == 9) sc_start(&sd->bl,&sd->bl,SC_ELE_GHOST,100,1,timedesp);
					if(sd->state.eledespertar == 10){ sc_start(&sd->bl,&sd->bl,SC_ELE_UNDEAD,100,1,timedesp);}
				}
			}
		break;
		case SC_PRESENCA_PRT: {
			if( sd->state.autotrade) {
				break;
			}
			struct map_data *mapdata = map_getmapdata(sd->bl.m);
			uint32 date = date_get(DT_DAYOFMONTH);
			if(sd->state.dia_hoje != date){

				sd->state.hora_assina = 0;
				sd->state.pkentregue = 0;
				sd->carnaval_state.cacatesouro = 0;
				
				sd->state.dia_hoje = date;
				npc_event_do_id("func_he_man::OnRegistro", sd->status.account_id);
			}
			if(battle_config.blackfriday == 1 && date == 29 && sd->blackfriday_state.horablack < 180){
				sd->blackfriday_state.horablack += 1;
				
				if(sd->blackfriday_state.horablack == 60) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Black Friday] Faltam 2 horas de presença para receber a Caixa de Acessório Pet pelo rodex.", false, SELF);
				if(sd->blackfriday_state.horablack == 120) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Black Friday] Faltam 1 hora de presença para receber a Caixa de Acessório Pet pelo rodex.", false, SELF);
				if(sd->blackfriday_state.horablack == 150) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Black Friday] Faltam 30 minutos de presença para receber a Caixa de Acessório Pet pelo rodex.", false, SELF);
				if(sd->blackfriday_state.horablack >= 180){
					npc_event_do_id("Loja Black Friday::OnAccBlackFriday", sd->status.account_id);
				}
			}
			if(battle_config.aniversario == 1 && date == 28 && sd->aniversario_state.horablack < 180){
				sd->aniversario_state.horablack += 1;
				
				if(sd->aniversario_state.horablack == 60) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Black Friday] Faltam 2 horas de presença para receber a Caixa de Acessório Pet pelo rodex.", false, SELF);
				if(sd->aniversario_state.horablack == 120) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Black Friday] Faltam 1 hora de presença para receber a Caixa de Acessório Pet pelo rodex.", false, SELF);
				if(sd->aniversario_state.horablack == 150) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Black Friday] Faltam 30 minutos de presença para receber a Caixa de Acessório Pet pelo rodex.", false, SELF);
				if(sd->aniversario_state.horablack >= 180){
					npc_event_do_id("Loja Aniversário::OnAccRola", sd->status.account_id);
				}
			}
			
			//if(sd->status.guild_id == 2082 || sd->status.guild_id == 2302 || sd->status.guild_id == 2303 || sd->status.guild_id == 2304 || sd->status.guild_id == 37){
			//	if (Sql_Query(mmysql_handle,
			//		"SELECT `char_id` FROM `char` WHERE `online` = 1 and `guild_id` in (2082, 2302, 2303, 2304, 37);") == SQL_SUCCESS )
			//	{
			//		sd->state.serverteste = (uint16)Sql_NumRows(mmysql_handle);
			//	}
			//}
			
			// Fadiga: Bio Extreme
			// --------------------------------------------------
			if(sd->addfadiga[2].qtd >= 1){
				int qtd;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `fadiga` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d LIMIT 1;", sd->status.account_id, 38072) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
							Sql_GetData(mmysql_handle, 0, &data, NULL); qtd = (uint16)atoi(data);
						}
						Sql_FreeResult(mmysql_handle);
					}
				}
				
				qtd += sd->addfadiga[2].qtd; // 10 + 5 = 15

				int pt = pc_readglobalreg(sd, add_str("#FADIGABIO")) + sd->addfadiga[2].qtd; // 3 + 5 = 8
				if(qtd > pt) {
					pc_setaccountreg(sd, add_str("#FADIGABIO"), qtd);
					pt = qtd;
				} else {
					pc_setaccountreg(sd, add_str("#FADIGABIO"), pt);
				}
				
				sd->addfadiga[2].qtd = 0;
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `fadiga` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d;", pt, sd->status.account_id, 38072) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt < 2500 && sd->sc.data[SC_BIO_EXT_1]) status_change_end(&sd->bl, SC_BIO_EXT_1, INVALID_TIMER);
				if(pt < 5000 && sd->sc.data[SC_BIO_EXT_2]) status_change_end(&sd->bl, SC_BIO_EXT_2, INVALID_TIMER);

				if(pt >= 2500 && pt < 5000) { 
					if(!sd->sc.data[SC_BIO_EXT_1])
						sc_start(&sd->bl, &sd->bl, SC_BIO_EXT_1, 100, 1, -1);
				}
				if(pt >= 5000) { 
					if(sd->sc.data[SC_BIO_EXT_1]) 
						status_change_end(&sd->bl, SC_BIO_EXT_1, INVALID_TIMER); 
					if(!sd->sc.data[SC_BIO_EXT_2]) 
						sc_start(&sd->bl, &sd->bl, SC_BIO_EXT_2, 100, 1, -1);
				}
			}			
			// Fadiga: Cheff Extreme
			// --------------------------------------------------				
			if(sd->addfadiga[6].qtd >= 1){
				int qtd;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `fadiga` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d LIMIT 1;", sd->status.account_id, 38076) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				qtd += sd->addfadiga[6].qtd; // 10 + 5 = 15

				int pt = pc_readglobalreg(sd, add_str("#FADIGACHEFF")) + sd->addfadiga[6].qtd; // 3 + 5 = 8
				if(qtd > pt) {
					pc_setaccountreg(sd, add_str("#FADIGACHEFF"), qtd);
					pt = qtd;
				} else {
					pc_setaccountreg(sd, add_str("#FADIGACHEFF"), pt);
				}
				
				sd->addfadiga[6].qtd = 0;
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `fadiga` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d;", pt, sd->status.account_id, 38076) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt < 2500 && sd->sc.data[SC_EFEITO_RH_27]) status_change_end(&sd->bl, SC_EFEITO_RH_27, INVALID_TIMER);
				if(pt < 5000 && sd->sc.data[SC_EFEITO_RH_28]) status_change_end(&sd->bl, SC_EFEITO_RH_28, INVALID_TIMER);

				if(pt >= 2500 && pt < 5000) { 
					if(!sd->sc.data[SC_EFEITO_RH_27])
						sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_27, 100, 1, -1);
				}
				if(pt >= 5000) { 
					if(sd->sc.data[SC_EFEITO_RH_27]) 
						status_change_end(&sd->bl, SC_EFEITO_RH_27, INVALID_TIMER); 
					if(!sd->sc.data[SC_EFEITO_RH_28]) 
						sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_28, 100, 1, -1);
				}
			}	
			...
			// Fadiga: Bio Hard
			// --------------------------------------------------
			if(sd->addfadiga[1].qtd >= 1){
				int qtd;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `fadiga` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d LIMIT 1;", sd->status.account_id, 38071) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				qtd += sd->addfadiga[1].qtd; // 10 + 5 = 15

				int pt = pc_readglobalreg(sd, add_str("#FADIGAHBIO")) + sd->addfadiga[1].qtd; // 3 + 5 = 8
				if(qtd > pt) {
					pc_setaccountreg(sd, add_str("#FADIGAHBIO"), qtd);
					pt = qtd;
				} else {
					pc_setaccountreg(sd, add_str("#FADIGAHBIO"), pt);
				}
				
				sd->addfadiga[1].qtd = 0;
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `fadiga` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d;", pt, sd->status.account_id, 38071) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt < 6000 && sd->sc.data[SC_BIO_HARD_1]) status_change_end(&sd->bl, SC_BIO_HARD_1, INVALID_TIMER);
				if(pt < 12000 && sd->sc.data[SC_BIO_HARD_2]) status_change_end(&sd->bl, SC_BIO_HARD_2, INVALID_TIMER);

				if(pt >= 6000 && pt < 12000) { 
					if(!sd->sc.data[SC_BIO_HARD_1])
						sc_start(&sd->bl, &sd->bl, SC_BIO_HARD_1, 100, 1, -1);
				}
				if(pt >= 12000) { 
					if(sd->sc.data[SC_BIO_HARD_1]) 
						status_change_end(&sd->bl, SC_BIO_HARD_1, INVALID_TIMER); 
					if(!sd->sc.data[SC_BIO_HARD_2]) 
						sc_start(&sd->bl, &sd->bl, SC_BIO_HARD_2, 100, 1, -1);
				}
			}			
			// Fadiga: Cheff Hard
			// --------------------------------------------------				
			if(sd->addfadiga[5].qtd >= 1){
				int qtd;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `fadiga` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d LIMIT 1;", sd->status.account_id, 38075) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				qtd += sd->addfadiga[5].qtd; // 10 + 5 = 15

				int pt = pc_readglobalreg(sd, add_str("#FADIGAHCHEFF")) + sd->addfadiga[5].qtd; // 3 + 5 = 8
				if(qtd > pt) {
					pc_setaccountreg(sd, add_str("#FADIGAHCHEFF"), qtd);
					pt = qtd;
				} else {
					pc_setaccountreg(sd, add_str("#FADIGAHCHEFF"), pt);
				}
				
				sd->addfadiga[5].qtd = 0;
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `fadiga` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d;", pt, sd->status.account_id, 38075) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt < 6000 && sd->sc.data[SC_EFEITO_RH_29]) status_change_end(&sd->bl, SC_EFEITO_RH_29, INVALID_TIMER);
				if(pt < 12000 && sd->sc.data[SC_EFEITO_RH_30]) status_change_end(&sd->bl, SC_EFEITO_RH_30, INVALID_TIMER);

				if(pt >= 6000 && pt < 12000) { 
					if(!sd->sc.data[SC_EFEITO_RH_29])
						sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_29, 100, 1, -1);
				}
				if(pt >= 12000) { 
					if(sd->sc.data[SC_EFEITO_RH_29]) 
						status_change_end(&sd->bl, SC_EFEITO_RH_29, INVALID_TIMER); 
					if(!sd->sc.data[SC_EFEITO_RH_30]) 
						sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_30, 100, 1, -1);
				}
			}	
			...
			// Fadiga: Bio Savage
			// --------------------------------------------------
			if(sd->addfadiga[3].qtd >= 1){
				int qtd;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `fadiga` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d LIMIT 1;", sd->status.account_id, 38073) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				qtd += sd->addfadiga[3].qtd; // 10 + 5 = 15

				int pt = pc_readglobalreg(sd, add_str("#FADIGASBIO")) + sd->addfadiga[3].qtd; // 3 + 5 = 8
				if(qtd > pt) {
					pc_setaccountreg(sd, add_str("#FADIGASBIO"), qtd);
					pt = qtd;
				} else {
					pc_setaccountreg(sd, add_str("#FADIGASBIO"), pt);
				}
				
				sd->addfadiga[3].qtd = 0;
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `fadiga` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d;", pt, sd->status.account_id, 38073) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt < 1500 && sd->sc.data[SC_BIO_SAV_1]) status_change_end(&sd->bl, SC_BIO_SAV_1, INVALID_TIMER);
				if(pt < 3000 && sd->sc.data[SC_BIO_SAV_2]) status_change_end(&sd->bl, SC_BIO_SAV_2, INVALID_TIMER);

				if(pt >= 1500 && pt < 3000) { 
					if(!sd->sc.data[SC_BIO_SAV_1])
						sc_start(&sd->bl, &sd->bl, SC_BIO_SAV_1, 100, 1, -1);
				}
				if(pt >= 3000) { 
					if(sd->sc.data[SC_BIO_SAV_1]) 
						status_change_end(&sd->bl, SC_BIO_SAV_1, INVALID_TIMER); 
					if(!sd->sc.data[SC_BIO_SAV_2]) 
						sc_start(&sd->bl, &sd->bl, SC_BIO_SAV_2, 100, 1, -1);
				}
			}			
			// Fadiga: Cheff Savage
			// --------------------------------------------------				
			if(sd->addfadiga[7].qtd >= 1){
				int qtd;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `fadiga` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d LIMIT 1;", sd->status.account_id, 38077) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				qtd += sd->addfadiga[7].qtd; // 10 + 5 = 15

				int pt = pc_readglobalreg(sd, add_str("#FADIGASCHEFF")) + sd->addfadiga[7].qtd; // 3 + 5 = 8
				if(qtd > pt) {
					pc_setaccountreg(sd, add_str("#FADIGASCHEFF"), qtd);
					pt = qtd;
				} else {
					pc_setaccountreg(sd, add_str("#FADIGASCHEFF"), pt);
				}
				
				sd->addfadiga[7].qtd = 0;
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `fadiga` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND item_id = %d;", pt, sd->status.account_id, 38077) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt < 1500 && sd->sc.data[SC_EFEITO_RH_31]) status_change_end(&sd->bl, SC_EFEITO_RH_31, INVALID_TIMER);
				if(pt < 3000 && sd->sc.data[SC_EFEITO_RH_32]) status_change_end(&sd->bl, SC_EFEITO_RH_32, INVALID_TIMER);

				if(pt >= 1500 && pt < 3000) { 
					if(!sd->sc.data[SC_EFEITO_RH_31])
						sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_31, 100, 1, -1);
				}
				if(pt >= 3000) { 
					if(sd->sc.data[SC_EFEITO_RH_31]) 
						status_change_end(&sd->bl, SC_EFEITO_RH_31, INVALID_TIMER); 
					if(!sd->sc.data[SC_EFEITO_RH_32]) 
						sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_32, 100, 1, -1);
				}
			}
			// -----------------------------------------------------------
			// -----------------------------------------------------------
			...
			if(sd->state.dias_assinatura || sd->state.assinatura_pk){
				if(!sd->state.pkentregue){
					sd->state.hora_assina += 1;

					if(sd->state.hora_assina == 30) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Assinatura(s) Benção] Faltam 1 hora de presença para receber os Pontos Kafra.", false, SELF);
					if(sd->state.hora_assina == 60) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Assinatura(s) Benção] Faltam 30 minutos de presença para receber os Pontos Kafra.", false, SELF);

					if(sd->state.hora_assina >= 90){
						sd->state.pkentregue = 1;
						npc_event_do_id("Maquina de Tickets::OnPontosKafra", sd->status.account_id);
					}
				}
			}
			if(mapdata->flag[MF_PRONTERA]){
				int prtmins = sd->state.hora_prt;
				if(pc_checkskill(sd,NT_AVENTUREIRO) >= 4){
					if(prtmins == 10) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Pergaminho Temporal de Prontera] Faltam 20 minutos de presença em prontera para receber 1 amuleto.", false, SELF);
					if(prtmins == 20) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Pergaminho Temporal de Prontera] Faltam 10 minutos de presença em prontera para receber 1 amuleto.", false, SELF);

					if(prtmins < 30) 
						sd->state.hora_prt += 1;
					else {
						sd->state.hora_prt = 0;
						npc_event_do_id("Maquina de Tickets::OnPrtTwo", sd->status.account_id);
					}
				} else {
					if(prtmins == 10) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Pergaminho Temporal de Prontera] Faltam 50 minutos de presença em prontera para receber 1 amuleto.", false, SELF);
					if(prtmins == 20) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Pergaminho Temporal de Prontera] Faltam 40 minutos de presença em prontera para receber 1 amuleto.", false, SELF);
					if(prtmins == 30) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Pergaminho Temporal de Prontera] Faltam 30 minutos de presença em prontera para receber 1 amuleto.", false, SELF);
					if(prtmins == 40) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Pergaminho Temporal de Prontera] Faltam 20 minutos de presença em prontera para receber 1 amuleto.", false, SELF);
					if(prtmins == 50) clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Pergaminho Temporal de Prontera] Faltam 10 minutos de presença em prontera para receber 1 amuleto.", false, SELF);

					if(prtmins < 60) 
						sd->state.hora_prt += 1;
					else {
						sd->state.hora_prt = 0;
						npc_event_do_id("Maquina de Tickets::OnPrt", sd->status.account_id);
					}
				}
			}
			
			if(mapdata->flag[MF_DOMINIOENT]){
				if(DIFF_TICK(last_tick, sd->idletime) >= 900){
					npc_event_do_id("dominiofunc::OnPrt", sd->status.account_id);
				}
			}
			if(mapdata->flag[MF_DOMINIOGERAL] && !mapdata->flag[MF_DOMINIOULTIMATE]){
				if(DIFF_TICK(last_tick, sd->idletime) >= 30 && pc_isdead(sd)){
					npc_event_do_id("dominiofunc::OnPrt", sd->status.account_id);
				}
			}
			if(mapdata->flag[MF_DOMINIOGERAL] && mapdata->flag[MF_DOMINIONORMAL]){
				if(DIFF_TICK(last_tick, sd->idletime) >= 120){
					npc_event_do_id("dominiofunc::OnPrt", sd->status.account_id);
				}
			}
			/*
			if(mapdata->flag[MF_FLORESTAPES] && !mapdata->flag[MF_VULCAOPESADELO]){
				int tempo = pc_readglobalreg(sd, add_str("#TIMERFLORES"));
				if(tempo < 240){
					if (Sql_Query(mmysql_handle,
						"UPDATE `fadiga_time` SET `minutos`= `minutos`+1 WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 1;", sd->status.account_id) == SQL_SUCCESS )
					{
						if(Sql_NumRows(mmysql_handle) > 0){
							while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
								char *data;
					...
					pc_setaccountreg(sd, add_str("#TIMERFLORES"), tempo + 1);
					tempo = 240 - tempo; 
					if(!sd->sc.data[SC_FADIGA_FLORESTA])
						sc_start(&sd->bl, &sd->bl, SC_FADIGA_FLORESTA, 100, 3, ((tempo*60) * 1000));
				}
			}
			*/
			if(mapdata->flag[MF_DOMINIOGERAL] && mapdata->flag[MF_DOMINIONORMAL]){
			...
			if(mapdata->flag[MF_VULCAOPESADELO]){
				int tempo = pc_readglobalreg(sd, add_str("#TIMERVULCAO"));
				if(tempo < 240){
					if (Sql_Query(mmysql_handle,
						"UPDATE `fadiga_time` SET `minutos`= `minutos`+1 WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 2;", sd->status.account_id) == SQL_SUCCESS )
					{
						if(Sql_NumRows(mmysql_handle) > 0){
							while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
								char *data;
					...
					pc_setaccountreg(sd, add_str("#TIMERVULCAO"), tempo + 1);
					tempo = 240 - tempo; 
					if(!sd->sc.data[SC_FADIGA_VULCAO])
						sc_start(&sd->bl, &sd->bl, SC_FADIGA_VULCAO, 100, 3, ((tempo*60) * 1000));
				}
			}
			if(mapdata->flag[MF_TEMPLOULTIMATE]){
				int tempo = pc_readglobalreg(sd, add_str("#TIMERTEMPPES"));
				if(tempo < 240){
					
					if (Sql_Query(mmysql_handle,
						"UPDATE `fadiga_time` SET `minutos`= `minutos`+1 WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 3;", sd->status.account_id) == SQL_SUCCESS )
					{
						if(Sql_NumRows(mmysql_handle) > 0){
							while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
								char *data;
					...
					pc_setaccountreg(sd, add_str("#TIMERTEMPPES"), tempo + 1);
					tempo = 240 - tempo; 
					if(!sd->sc.data[SC_FADIGA_TEM_PESA])
						sc_start(&sd->bl, &sd->bl, SC_FADIGA_TEM_PESA, 100, 3, ((tempo*60) * 1000));
				}
			}
			if((mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP]) && !mapdata->flag[MF_TEMPLOULTIMATE]){
				int tempo = pc_readglobalreg(sd, add_str("#TIMERESCURIDAO"));
				if(tempo < 240){
					
					if (Sql_Query(mmysql_handle,
						"UPDATE `fadiga_time` SET `minutos`= `minutos`+1 WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 4;", sd->status.account_id) == SQL_SUCCESS )
					{
						if(Sql_NumRows(mmysql_handle) > 0){
							while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
								char *data;
					...
					pc_setaccountreg(sd, add_str("#TIMERESCURIDAO"), tempo + 1);
					tempo = 240 - tempo; 
					if(!sd->sc.data[SC_FADIGA_ESCU])
						sc_start(&sd->bl, &sd->bl, SC_FADIGA_ESCU, 100, 3, ((tempo*60) * 1000));
				}
			}
		...
		case SC_UNIVERSESTANCE:
			...
			break;
		case SC_GRAVITYCONTROL:
			if(!map_getmapflag(bl->m, MF_MASMORRAINVERNAL)){
		...
		case SC_SOUNDBLEND: {
				...
				struct map_data *mapdata = map_getmapdata(bl->m);
				if(sd && mapdata->flag[MF_TEMPLOULTIMATE] && tid != INVALID_TIMER && (!src || src->type == BL_MOB)){
					status_kill(bl);
					break;
				}
				if (src && tid != INVALID_TIMER){
					if(src->type == BL_MOB && mapdata->flag[MF_FENDA])
						map_foreachinarea(skill_area_sub, bl->m, bl->x - 5, bl->y - 5, bl->x + 5, bl->y + 5, BL_CHAR,
							src, NPC_COMET_BOSS, 1, gettick(), BCT_ENEMY | SD_SPLASH | 1, skill_castend_damage_id);
					else {
						//struct map_session_data *ssd = BL_CAST(BL_PC, src);
						//if(ssd && (ssd->bonus.range_skill == 5 || ssd->bonus.range_skill == 11)){
						//	int area = 2;
						//	map_foreachinarea(skill_area_sub, bl->m, bl->x - area, bl->y - area, bl->x + area, bl->y + area, BL_CHAR,
						//		src, TR_SOUNDBLEND, sce->val1, gettick(), BCT_ENEMY | SD_SPLASH | 1, skill_castend_damage_id);
						//} else {
	...
	if(type == SC_DANO_DPS && sd && sd->forceinidps == 1) {
		sc_start(&sd->bl, &sd->bl, SC_DANO_DPS, 100, 0, 5000);
	}
	return 1;
}
...
TIMER_FUNC(status_change_timer){
	...
	case SC_SOMBRIO_03:
		if (sce->val4 >= 0 && status_get_class_(bl) != CLASS_BOSS && !map_getmapflag(bl->m, MF_FENDA)) {
	...
	case SC_INSPIRATION:
		if(--(sce->val4) >= 0) {
			if(status_isdead(bl) && sd && checar_equip(EQI_HAND_R,sd, 4, 530034, 13)){
				sc_timer_next(5000+tick);
				return 0;
			}
...
int status_change_timer_sub(struct block_list* bl, va_list ap)
{
	...
	case SC_CONCENTRATE:
		...
		status_change_end(bl, SC_NEWMOON, INVALID_TIMER);
		if(tsc && tsc->data[SC__SHADOWFORM] && map_getmapflag(bl->m, MF_EQUIPSGVG)) status_change_end(bl, SC__SHADOWFORM, INVALID_TIMER);
		if (tsc && tsc->data[SC__SHADOWFORM] && (sce && sce->val4 > 0 && sce->val4%2000 == 0) && // For every 2 seconds do the checking
			rnd()%100 < 100 - tsc->data[SC__SHADOWFORM]->val1 * 3) // [100 - (Skill Level x 10)] %
...
// devon
int checar_carta(int part, struct map_session_data *sd, int grade, int itemid, int refine){
	if(!battle_config.desativarchecar){ return 0; }

	int i=-1,slot = 3,t;

	nullpo_retr(0, sd);

	if (equip_index_check(part))
		i=pc_checkequip(sd,equip_bitmask[part]);

	if(i >= 0 && sd->inventory.u.items_inventory[i].refine < refine) return 0;
	for(t = 0; t <= slot; t++){
		if(grade && i >= 0 && itemid == sd->inventory.u.items_inventory[i].card[t]){
			if(sd->inventory.u.items_inventory[i].enchantgrade >= grade) return sd->inventory.u.items_inventory[i].card[t];
		}
		if(i >= 0 && itemid == sd->inventory.u.items_inventory[i].card[t]) return sd->inventory.u.items_inventory[i].card[t];
	}
	return 0;
}


int checar_equip(int part, struct map_session_data *sd, int grade, int itemid, int refine){
	if(!battle_config.desativarchecar){ return 0; }
	int i=-1;
	...
	if (equip_index_check(part))
		i=pc_checkequip(sd,equip_bitmask[part]);
	if(itemid && i >= 0){
		if(sd->inventory.u.items_inventory[i].refine < refine) return 0;
		if(grade && i >= 0 && itemid == sd->inventory.u.items_inventory[i].nameid){
			if(sd->inventory.u.items_inventory[i].enchantgrade >= grade) return sd->inventory.u.items_inventory[i].nameid;
		}
		if(i >= 0 && itemid == sd->inventory.u.items_inventory[i].nameid) return sd->inventory.u.items_inventory[i].nameid;
	} else if(grade && i >= 0){
		if(sd->inventory.u.items_inventory[i].refine < refine) return 0;
		if(sd->inventory.u.items_inventory[i].enchantgrade >= grade) return sd->inventory.u.items_inventory[i].nameid;
	}
	if(refine >= sd->inventory.u.items_inventory[i].refine) return refine;
	return 0;
}
...
static int status_natural_heal(struct block_list* bl, va_list args)
{
	...
	if (sd) {
		if (sd->hp_loss.value || sd->sp_loss.value || 
			mapdata->flag[MF_DOMINIOHARD] || mapdata->flag[MF_DOMINIOUNREAL] || 
			mapdata->flag[MF_TEMPLOULTIMATE] || mapdata->flag[MF_ESCURIDAO] || 
			mapdata->flag[MF_ESCURIDAOPVP] || mapdata->flag[MF_HPDROP] || mapdata->flag[MF_MECANICAS] || mapdata->flag[MF_GUERRAPVM] || (mapdata->flag[MF_DOMINIOULTIMATE] && mapdata->flag[MF_DOMINIOGERAL]))
...
uint64 StatusDatabase::parseBodyNode(const YAML::Node &node) {
	...
	if (this->nodeExists(node, "EndReturn")) {
		const YAML::Node &endNode = node["EndReturn"];
