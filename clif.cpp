...
static inline unsigned char clif_bl_type(struct block_list *bl, bool walking) {
	...
	case BL_MOB:
		if( pcdb_checkid( status_get_viewdata( bl )->class_ ) ){
			...
			if(map_getmapflag( bl->m, MF_GUERRAPVM )){
				mob_data *md = BL_CAST(BL_MOB, bl);
				if(md && (md->db->id < 4000 || md->db->id == 31841)) return 0xe;
			}
			if(map_getmapflag( bl->m, MF_FENDADIMENSIONAL ) || map_getmapflag( bl->m, MF_PANDEMONIUM )){
				mob_data *md = BL_CAST(BL_MOB, bl);
				if(md && md->db->id == 31052) return 0x3;
			}
...
static int clif_send_sub(struct block_list *bl, va_list ap)
{
	...
	if(sd->state.hidepetall && src_bl->type == BL_PET){
		return 0;
	}
	else if(sd->state.hidepet && src_bl->type == BL_PET){
		TBL_PET* pdd;
		pdd = map_id2pd(src_bl->id);
		if(!sd->pd || sd->pd != pdd)
			return 0;
	}
		...
		if (g) {
			...
			if (type == GUILD_ALLIANCE) {
				for (const auto &al : g->alliance) {
					...
					if (!ag)
						continue;
					for (i = 0; i < ag->max_member; i++) {
						if ((sd = ag->member[i].sd) != nullptr) {
							if (!session_isActive(fd = sd->fd))
								continue;
							WFIFOHEAD(fd, len);
							memcpy(WFIFOP(fd, 0), buf, len);
...
void clif_get_weapon_view(struct map_session_data* sd, t_itemid *rhand, t_itemid *lhand)
{
	...
	*lhand = sd->status.shield;
	...
	// Devon Armas
	if(sd->state.arma_visual > 0 && *rhand){
		*rhand = sd->state.arma_visual;
	}
}
	...
	case BL_PC:
		{
			...
			int contareborn = sd->history_state.burnreborn + sd->history_state.resetchar + sd->history_state.resetaccount;
			
			if(contareborn >= 15){
				auto it = util::vector_get( sd->hatEffects, sd->history_state.idaura );
				
				if( it == sd->hatEffects.end() ){
					sd->hatEffects.push_back( sd->history_state.idaura );
				}
			}
			if(sd->sc.data[SC_PERGA_HEROI]){
				auto it = util::vector_get( sd->hatEffects, HAT_EF_rebound_shield_B );
				
				if( it == sd->hatEffects.end() ){
					sd->hatEffects.push_back( HAT_EF_rebound_shield_B );
				}
			}
			if(sd->history_state.doador >= 4 && sd->history_state.doadordays > 0 && !sd->history_state.doadortrava){
				auto it = util::vector_get( sd->hatEffects, HAT_EF_new_chainlightning );
				
				if( it == sd->hatEffects.end() ){
					sd->hatEffects.push_back( HAT_EF_new_chainlightning );
				}
			}
...
/// - if set to clear on next mes, clear contents
/// - append this text
void clif_scriptmes2(map_session_data *sd, int npcid, const char *mes, int type)
{
	nullpo_retv(sd);
	nullpo_retv(mes);

	const int fd = sd->fd;
	const int slen= strlen(mes) + 1;
	const int len = sizeof(struct PACKET_ZC_SAY_DIALOG2) + slen;
	if(slen <= INT16_MAX) return;

	//pc_update_idle_time(sd, BCIDLE_SCRIPT);
	//sd->state.dialog = 1;

	WFIFOHEAD(fd, len);

	struct PACKET_ZC_SAY_DIALOG2 *p = (struct PACKET_ZC_SAY_DIALOG2*)RFIFOP( fd, 0 );
...
/// - set to clear on next mes
/// - remove 'next' button
void clif_scriptnext2(map_session_data *sd, int npcid, int type)
{
    nullpo_retv(sd);
    ...
    struct PACKET_ZC_WAIT_DIALOG2 *p = (struct PACKET_ZC_WAIT_DIALOG2*)WFIFOP( fd, 0 );
...
static void clif_addcards( struct EQUIPSLOTINFO* buf, struct item* item ){
	...
	if( item->card[0] == CARD0_PET ){
		...
		buf->card[2] = 0;
		if( item->card[3] > 0 )
			buf->card[3] = client_nameid( item->card[3] );
		else
			buf->card[3] = 0;
		return;
	}
...
void clif_additem( struct map_session_data *sd, int n, int amount, unsigned char fail ){
	...
	if( fail ){
		...
		if(sd->inventory.u.items_inventory[n].bind){
			p.HireExpireDate = sd->inventory.u.items_inventory[n].bind;
			p.bindOnEquipType = 2;
		}
		if(sd->inventory.u.items_inventory[n].tradecard0 || sd->inventory.u.items_inventory[n].tradecard1 || sd->inventory.u.items_inventory[n].tradecard2 || sd->inventory.u.items_inventory[n].tradecard3){
			p.bindOnEquipType = 2;
		}
		...
		p.grade = sd->inventory.u.items_inventory[n].enchantgrade + sd->inventory.u.items_inventory[n].despertar;
#endif
#endif
#endif
#endif
...
static void clif_item_equip( short idx, struct EQUIPITEM_INFO *p, struct item *it, struct item_data *id, int eqp_pos ){
	...
	if(it->bind){
		p->HireExpireDate = it->bind;
		p->bindOnEquipType = 2;
	}
	
	if(it->tradecard0 || it->tradecard1 || it->tradecard2 || it->tradecard3){
		p->bindOnEquipType = 2;
	}
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
	p->grade = it->enchantgrade + it->despertar;
#endif
}
...
void clif_inventorylist( struct map_session_data *sd ){
	...
	for( int i = 0; i < MAX_INVENTORY; i++ ){
		...
		if( !itemdb_isstackable2( sd->inventory_data[i] ) || sd->inventory.u.items_inventory[i].tradecard0 || sd->inventory.u.items_inventory[i].tradecard1 || sd->inventory.u.items_inventory[i].tradecard2 || sd->inventory.u.items_inventory[i].tradecard3 ){
...
void clif_storagelist(struct map_session_data* sd, struct item* items, int items_length, const char *storename){
	...
	for( int i = 0; i < items_length; i++ ){
		...
		// Non-stackable (Equippable)
		if( !itemdb_isstackable2( id ) || sd->inventory.u.items_inventory[i].tradecard0 || sd->inventory.u.items_inventory[i].tradecard1 || sd->inventory.u.items_inventory[i].tradecard2 || sd->inventory.u.items_inventory[i].tradecard3 ){
...
void clif_sprite_change( struct block_list *bl, int id, int type, int val, int val2, enum send_target target ){
	...
	if(bl->type == BL_PC){
		map_session_data* sd;
		sd = BL_CAST(BL_PC, bl);
		if(sd && map_getmapflag( bl->m, MF_SAMBODROMO )){
			if(sd->carnaval_state.top && type == LOOK_HEAD_TOP) val = sd->carnaval_state.top;
			else if(sd->carnaval_state.mid && type == LOOK_HEAD_MID) val = sd->carnaval_state.mid;
			else if(sd->carnaval_state.low && type == LOOK_HEAD_BOTTOM) val = sd->carnaval_state.low;
			else if(sd->carnaval_state.robe && type == LOOK_ROBE) val = sd->carnaval_state.robe;
		}
	}
...
void clif_tradeadditem( map_session_data* sd, map_session_data* tsd, int index, int amount ){
	...
	if( index ){
		...
		p.grade = sd->inventory.u.items_inventory[index].enchantgrade + sd->inventory.u.items_inventory[index].despertar;
#endif
#endif
	}else{
		p = {};
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
	p.enchantgrade = i->enchantgrade + i->despertar;
#endif
#endif
...
void clif_getareachar_unit( struct map_session_data* sd,struct block_list *bl ){
	...
	if(sd && sd->state.hidepetall && bl->type == BL_PET){
		return;
	}
	else if(sd && sd->state.hidepet && bl->type == BL_PET){
		TBL_PET* pdd;
		pdd = map_id2pd(bl->id);
		if(!sd->pd || sd->pd != pdd)
			return;
	...
	case BL_PC:
		{
			...
			// devon
			if(tsd->sc.data[SC_EFEITO_RH_7]){
				if(sd->sc.data[SC_EFEITO_RH_7]) sd->pk_id = 1;

				clif_sendbgemblem_pkmode(sd->fd,tsd);
			}
			if ( tsd->status.robe )
...
int clif_damage(struct block_list* src, struct block_list* dst, t_tick tick, int sdelay, int ddelay, int64 sdamage, int div, enum e_damage_type type, int64 sdamage2, bool spdamage)
{
	...
	//ShowWarning("sdelay = %d\n", sdelay);
	//ShowWarning("ddelay = %d\n", ddelay);
	//ShowWarning("type = %d\n", type);
...
void clif_skillinfoblock(struct map_session_data *sd)
{
	...
	for ( i = 0, len = 4; i < MAX_SKILL; i++)
	{
		if( (id = sd->status.skill[i].id) != 0 )
		{
			...
			if(id == DM_FORJA_DESPERTAR){
				int temporada = sd->state.temporada;
				if(!temporada){
					temporada = static_cast<int>(pc_readglobalreg(sd, add_str("CHARTEMPORADA")));
				}
				if (!temporada || sd->state.resetou) {
					continue;
				}
			}
			WFIFOW(fd,len)   = id;
...
/// 0441 <skill id>.W
void clif_deleteskill(struct map_session_data *sd, int skill_id, bool skip_infoblock)
{
...
void clif_skillcasting(struct block_list* bl, int src_id, int dst_id, int dst_x, int dst_y, uint16 skill_id, uint16 skill_lv, int property, int casttime)
{
	...
	if (bl->type == BL_MOB && (mapdata->flag[MF_FENDA] || mapdata->flag[MF_FLORESTAPES] || mapdata->flag[MF_MASMORRAINVERNAL] || mapdata->flag[MF_MECANICAS] || mapdata->flag[MF_DOMINIOENT] || mapdata->flag[MF_GUERRAPVM] || mapdata->flag[MF_RESTIGUALITARIA] ||  skill_id == NPC_ONLY_SIT )) {
		clif_showscript(bl, skill_get_desc(skill_id), AREA);
		if(skill_id == NPC_JOKENPO_PEDRA) clif_emotion(bl, ET_ROCK);
		if(skill_id == NPC_JOKENPO_PAPEL) clif_emotion(bl, ET_WRAP);
		if(skill_id == NPC_JOKENPO_TESOURA) clif_emotion(bl, ET_SCISSOR);
	}
	
	if(skill_id == SH_HYUN_ROKS_BREEZE) skill_id = SH_HOGOGONG_STRIKE;
	if(skill_id == NPC_COMET_BOSS) skill_id = WL_COMET;
	if(skill_id == NPC_PARRY) skill_id = WL_COMET;
	if(skill_id == NT_STORMGUST) skill_id = WL_COMET;
	if(skill_id == NPC_EARTHSTRAIN) skill_id = WL_EARTHSTRAIN;

	if(skill_id == NPC_ONLY_SIT) skill_id = NPC_COMET;
	if(skill_id == NPC_JOKENPO_PEDRA) skill_id = CD_COMPETENTIA;
	if(skill_id == NPC_JOKENPO_PAPEL) skill_id = CD_COMPETENTIA;
	if(skill_id == NPC_JOKENPO_TESOURA) skill_id = CD_COMPETENTIA;
...
int clif_skill_damage(struct block_list *src,struct block_list *dst,t_tick tick,int sdelay,int ddelay,int64 sdamage,int div,uint16 skill_id,uint16 skill_lv,enum e_damage_type type)
{
	...
	// devon
	//if(skill_id == SN_SHARPSHOOTING) return 1;
	//if(skill_id == SH_HYUN_ROKS_BREEZE) skill_id = SH_HYUN_ROKS_BREEZE_ANIMA;
	if(skill_id == SH_HYUN_ROK_CANNON) skill_id = SH_HYUN_ROK_CANNON_ANIMA;
	if(skill_id == SU_SV_STEMSPEAR) skill_id = SU_SV_STEMSPEAR_ANIMA;
	if(skill_id == SU_PICKYPECK) skill_id = SU_PICKYPECK_ANIMA;
	if(skill_id == NPC_COMET_BOSS) skill_id = WL_COMET;
	if(skill_id == SH_HYUN_ROKS_BREEZE) skill_id = SH_HOGOGONG_STRIKE;
	if(skill_id == NPC_PARRY) skill_id = WL_COMET;
	if(skill_id == NT_STORMGUST) {
		if(map_getmapflag(src->m, MF_MASMORRAINVERNAL))
			skill_id = WZ_STORMGUST;
		else
			skill_id = WZ_VERMILION;
	}
	if(skill_id == NPC_EARTHSTRAIN) skill_id = WL_EARTHSTRAIN;
	if((skill_id == NJ_HYOUSENSOU || skill_id == NJ_KOUENKA || skill_id == NJ_HUUJIN) && div > 3) div = 3;


	if(skill_id == WM_SEVERE_RAINSTORM) { skill_id = WM_SEVERE_RAINSTORM_MELEE; type = DMG_MULTI_HIT; }

	if(skill_id == LG_CANNONSPEAR){
		clif_damage(src,dst,tick,sdelay,ddelay,sdamage,div,type, 0, 0);
		return 0;
	}
	//if(skill_id == SN_SHARPSHOOTING || skill_id == NEW_SHARPSHOOT){
	//	clif_damage(src,dst,tick,sdelay,ddelay,sdamage,div,DMG_NORMAL, 0, 0);
	//	return 0;
	//}

	//if(skill_id == HN_JUPITEL_THUNDER_STORM){
	//	clif_damage(src,dst,tick,sdelay,ddelay,sdamage,div,type, 0, 0);
	//	return 0;
	///}
	...
	WBUFL(buf,4)=(skill_id == 5498?0:src->id);
	WBUFL(buf,8)=dst->id;
	...
	//ShowWarning("type = %d \n",type);
	// devon
	if(skill_id == SHC_DANCING_KNIFE || skill_id == WM_SEVERE_RAINSTORM)
		type = DMG_SPLASH;

	if(skill_id == NC_ARMSCANNON && type == DMG_ENDURE)
		type = DMG_SPLASH;
	if(skill_id == NC_ARMSCANNON && type == DMG_MULTI_HIT_ENDURE)
		type = DMG_MULTI_HIT;
...
{
	if(skill_id == DM_JOKENPO_PEDRA) clif_emotion(src, ET_ROCK);
	if(skill_id == DM_JOKENPO_PAPEL) clif_emotion(src, ET_WRAP);
	if(skill_id == DM_JOKENPO_TESOURA) clif_emotion(src, ET_SCISSOR);
	//if(skill_id == SH_HYUN_ROKS_BREEZE) skill_id = SH_HYUN_ROKS_BREEZE_ANIMA;
	if(skill_id == SH_HYUN_ROK_CANNON) skill_id = SH_HYUN_ROK_CANNON_ANIMA;
	if(skill_id == SU_SV_STEMSPEAR) skill_id = SU_SV_STEMSPEAR_ANIMA;
	if(skill_id == SU_PICKYPECK) skill_id = SU_PICKYPECK_ANIMA;
...
void clif_channel_msg(struct Channel *channel, const char *msg, unsigned long color) {
	...
	if(battle_config.multiserver > 1)
		return;
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((mapdata->flag[MF_PVP]?1:0 || (bl->type == BL_PC && ((TBL_PC *)bl)->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)
		((!mapdata->flag[MF_TOWN] && !mapdata->flag[MF_HISTORYSTAGE] && bl->type == BL_PC && ((TBL_PC *)bl)->sc.data[SC_EFEITO_RH_7])<<4)| // PARTY - Show attack cursor on non-party members (PvP)
...
bool minirefinadora(int itemid)
{
	if( itemid== 810007 || itemid == 540044 ||
		itemid == 550062 || itemid == 510057 ||
		itemid == 550060 || itemid == 510056 ||
		itemid == 550061 || itemid == 700025 ||
		itemid == 500046 || itemid == 830010 ||
		itemid == 640031 || itemid == 600014 ||
		itemid == 540041 || itemid == 630009 ||
		itemid == 530023 || itemid == 460013 ||
		itemid == 550054 || itemid == 620006 ||
		itemid == 510050 || itemid == 590016 ||
		itemid == 500044 || itemid == 590017 ||
		itemid == 610035 || itemid == 500020 ||
		itemid == 590036 || itemid == 640014 ||
		itemid == 510051 || itemid == 640015 ||
		itemid == 500045 || itemid == 550019 ||
		itemid == 620016 || itemid == 540014 ||
		itemid == 540042 || itemid == 540015 ||
		itemid == 550056 || itemid == 550020 ||
		itemid == 510052 || itemid == 560012 ||
		itemid == 570028 || itemid == 560013 ||
		itemid == 580028 || itemid == 610016 ||
		itemid == 560030 || itemid == 510027 ||
		itemid == 700050 || itemid == 700022 ||
		itemid == 800009 || itemid == 510028 ||
		itemid == 810005 || itemid == 700023 ||
		itemid == 820004 || itemid == 700024 ||
		itemid == 830008 || itemid == 570013 ||
		itemid == 840004 || itemid == 580013 ||
		itemid == 650019 || itemid == 700025 ||
		itemid == 400245 || itemid == 840006 ||
		itemid == 400265 || itemid == 800011 ||
		itemid == 400246 || itemid == 400154 ||
		itemid == 400261 || itemid == 400262 ||
		itemid == 400263 || itemid == 400264 ||
		itemid == 400266 || itemid == 400267 ||
		itemid == 400156 || itemid == 400155 ||
		itemid == 450206 || itemid == 450207 ||
		itemid == 480159 || itemid == 480160 ||
		itemid == 470115 || itemid == 470116 ||
		itemid == 400152 || itemid == 460020 ||
		itemid == 470122 || itemid == 450217 ||
		itemid == 530010 || itemid == 470094 ||
		itemid == 500019 || itemid == 400109 ||
		itemid == 400400 || itemid == 470182 ||
		itemid == 50030 || itemid == 470205 || 
		itemid == 470199 || itemid == 590063 || 
		itemid == 630028 || itemid == 550117 || 
		itemid == 590062 || itemid == 610054 || 
		itemid == 640046 || itemid == 700076 || 
		itemid == 540075 || itemid == 510104 || 
		itemid == 550119 || itemid == 540074 || 
		itemid == 560048 || itemid == 840028 || 
		itemid == 510103 || itemid == 820028 || 
		itemid == 500074 || itemid == 570045 || 
		itemid == 550118 || itemid == 580046 || 
		itemid == 810034 || itemid == 520031 || 
		itemid == 830033 || itemid == 510102 ||
		itemid == 480174 || itemid == 400235 ||
		itemid == 400237 || itemid == 400239 ||
		itemid == 400241 || itemid == 400243 ||
		itemid == 400233 || itemid == 400230 ||
		itemid == 400228 || itemid == 400226 ||
		itemid == 400218 || itemid == 400216 ||
		itemid == 400200 || itemid == 400198 ||
		itemid == 400191 || itemid == 400189 ||
		itemid == 400178 || itemid == 400236 ||
		itemid == 400238 || itemid == 400240 ||
		itemid == 400242 || itemid == 400244 ||
		itemid == 400234 || itemid == 400231 ||
		itemid == 400229 || itemid == 400227 ||
		itemid == 400219 || itemid == 400217 ||
		itemid == 400199 || itemid == 400190 ||
		itemid == 400181 || itemid == 400179 ||
		itemid == 400192 || itemid == 400201 ||
		itemid == 400180 || itemid == 480210 ||
		itemid == 450272)
		return true;
...
void clif_use_card(struct map_session_data *sd,int idx)
{
	...
	for(i=c=0;i<MAX_INVENTORY;i++){
		...
		// Visual Sayagin
		if(sd->inventory_data[i]->nameid == 48090) continue;

		//  Lascas Shadow Brinco
		if(sd->inventory_data[idx]->nameid == 44491 || sd->inventory_data[idx]->nameid == 44493 || sd->inventory_data[idx]->nameid == 44495 || sd->inventory_data[idx]->nameid == 44497){
			if(sd->inventory_data[i]->type == IT_SHADOWGEAR && sd->inventory_data[i]->equip == EQP_SHADOW_ACC_R && sd->inventory.u.items_inventory[i].refine >= 9){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Lascas Shadow Colar
		if(sd->inventory_data[idx]->nameid == 44492 || sd->inventory_data[idx]->nameid == 44494 || sd->inventory_data[idx]->nameid == 44496 || sd->inventory_data[idx]->nameid == 44498){
			if(sd->inventory_data[i]->type == IT_SHADOWGEAR && sd->inventory_data[i]->equip == EQP_SHADOW_ACC_L && sd->inventory.u.items_inventory[i].refine >= 9){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Shadow Arma
		if(sd->inventory_data[idx]->nameid == 44581 || sd->inventory_data[idx]->nameid == 44583 || sd->inventory_data[idx]->nameid == 44585 || sd->inventory_data[idx]->nameid == 44587){
			if(sd->inventory_data[i]->type == IT_SHADOWGEAR && sd->inventory_data[i]->equip == EQP_SHADOW_WEAPON && sd->inventory.u.items_inventory[i].refine >= 9){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Shadow Escudo
		if(sd->inventory_data[idx]->nameid == 44582 || sd->inventory_data[idx]->nameid == 44584 || sd->inventory_data[idx]->nameid == 44586 || sd->inventory_data[idx]->nameid == 44588){
			if(sd->inventory_data[i]->type == IT_SHADOWGEAR && sd->inventory_data[i]->equip == EQP_SHADOW_SHIELD && sd->inventory.u.items_inventory[i].refine >= 9){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Shadow Bota
		if(sd->inventory_data[idx]->nameid == 44589 || sd->inventory_data[idx]->nameid == 44590 || sd->inventory_data[idx]->nameid == 44591 || sd->inventory_data[idx]->nameid == 44592){
			if(sd->inventory_data[i]->type == IT_SHADOWGEAR && sd->inventory_data[i]->equip == EQP_SHADOW_SHOES && sd->inventory.u.items_inventory[i].refine >= 9){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Lascas Shadow Armor
		if(sd->inventory_data[idx]->nameid >= 44499 && sd->inventory_data[idx]->nameid <= 44502){
			if(sd->inventory_data[i]->type == IT_SHADOWGEAR && sd->inventory_data[i]->equip == EQP_SHADOW_ARMOR && sd->inventory.u.items_inventory[i].refine >= 9){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Grade Visu Topo
		if(sd->inventory_data[idx]->nameid >= 44510 && sd->inventory_data[idx]->nameid <= 44513){
			if(sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_TOP && sd->inventory_data[i]->type == IT_ARMOR){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Grade Visu Meio
		if(sd->inventory_data[idx]->nameid >= 44514 && sd->inventory_data[idx]->nameid <= 44517){
			if(sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_MID && sd->inventory_data[i]->type == IT_ARMOR){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Grade Visu Baixo
		if(sd->inventory_data[idx]->nameid >= 44518 && sd->inventory_data[idx]->nameid <= 44521){
			if(sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_LOW && sd->inventory_data[i]->type == IT_ARMOR){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		// Grade Visu Capa
		if(sd->inventory_data[idx]->nameid >= 44522 && sd->inventory_data[idx]->nameid <= 44525){
			if(sd->inventory_data[i]->equip == EQP_COSTUME_GARMENT && sd->inventory_data[i]->type == IT_ARMOR){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}
			continue;
		...
		//  Estrela de Grade Pet
		if(sd->inventory_data[idx]->nameid == 44433 || sd->inventory_data[idx]->nameid == 44435 || sd->inventory_data[idx]->nameid == 44533){
			if(sd->inventory_data[i]->type != IT_PETEGG || sd->inventory.u.items_inventory[i].attribute)
				continue;
			if(sd->inventory_data[i]->eggpercent == 90){
				WFIFOW(fd,4+c*2)=i+2;
				c++;
			}	
			continue;
		...
		// Refinadora Complexa Avanï¿½ada
		if(sd->inventory_data[idx]->nameid == 33363){
			if((sd->inventory_data[i]->weapon_level == 5 || sd->inventory_data[i]->armor_level == 2) && 
			(sd->inventory.u.items_inventory[i].refine == 12)){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// Despertar
		if(sd->inventory_data[idx]->nameid == 50318 || sd->inventory_data[idx]->nameid == 50320){
			int refinedesp = 13;
			if(sd->inventory_data[idx]->nameid == 50320) refinedesp = 16;
			if(sd->inventory_data[i]->weapon_level == 5 && sd->inventory.u.items_inventory[i].enchantgrade >= 4 && sd->inventory.u.items_inventory[i].despertar < refinedesp){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// Despertar Armadura
		if(sd->inventory_data[idx]->nameid == 50322){
			if(
			(sd->inventory_data[i]->nameid == 450282 || sd->inventory_data[i]->nameid == 450283 || sd->inventory_data[i]->nameid == 450284 || sd->inventory_data[i]->nameid == 450169 || sd->inventory_data[i]->nameid == 450170 || sd->inventory_data[i]->nameid == 450171 || sd->inventory_data[i]->nameid == 450172 || sd->inventory_data[i]->nameid == 450173) && 
			sd->inventory.u.items_inventory[i].enchantgrade >= 4 && 
			sd->inventory.u.items_inventory[i].despertar < 16){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// despertar mini
		if(sd->inventory_data[idx]->nameid == 50319 || sd->inventory_data[idx]->nameid == 50321){
			int refinedesp = 13;
			if(sd->inventory_data[idx]->nameid == 50321) refinedesp = 16;
			if(sd->inventory_data[i]->weapon_level == 5 && sd->inventory.u.items_inventory[i].enchantgrade >= 4 && sd->inventory.u.items_inventory[i].despertar < refinedesp){
				if(minirefinadora(sd->inventory_data[i]->nameid)){
					WFIFOW(fd,4+c*2)=i+2;
					c++;	
				}
			}
		...
		if(sd->inventory_data[idx]->nameid == 33360 || sd->inventory_data[idx]->nameid == 33361){
			if((sd->inventory_data[i]->weapon_level == 5 || sd->inventory_data[i]->armor_level == 2) && 
			(sd->inventory.u.items_inventory[i].refine == 10 || sd->inventory.u.items_inventory[i].refine == 11)){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		}
		if(sd->inventory_data[idx]->nameid == 70153){
			if(
				(sd->inventory_data[i]->armor_level == 2 || sd->inventory_data[i]->weapon_level == 5) && 
				(sd->inventory.u.items_inventory[i].refine == 13 || sd->inventory.u.items_inventory[i].refine == 14)
				)
			{
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
		...
		if(sd->inventory_data[idx]->nameid == 33369){
			if((sd->inventory_data[i]->weapon_level == 5 || sd->inventory_data[i]->armor_level == 2) && 
			(sd->inventory.u.items_inventory[i].refine == 12 || sd->inventory.u.items_inventory[i].refine == 13)){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// mini-refinadora
		if(sd->inventory_data[idx]->nameid == 33362 || sd->inventory_data[idx]->nameid == 33364){
			if(minirefinadora(sd->inventory_data[i]->nameid)){
				if(sd->inventory_data[idx]->nameid == 33364){
						if(sd->inventory.u.items_inventory[i].refine == 12){
							WFIFOW(fd,4+c*2)=i+2;
							c++;	
						}
					} else {
						if(sd->inventory.u.items_inventory[i].refine == 10 || sd->inventory.u.items_inventory[i].refine == 11){
							WFIFOW(fd,4+c*2)=i+2;
							c++;	
						}
					}
		...
		// Refinadora Complexa Unknown
		if(sd->inventory_data[idx]->nameid == 33368){
			if(
				(sd->inventory_data[i]->nameid == 470071 || sd->inventory_data[i]->nameid == 470072 || sd->inventory_data[i]->nameid == 470073 || 
				sd->inventory_data[i]->nameid == 470074 || sd->inventory_data[i]->nameid == 470076 || sd->inventory_data[i]->nameid == 470077) &&
				(sd->inventory.u.items_inventory[i].refine >= 10 && sd->inventory.u.items_inventory[i].refine <= 12)
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Poder oculto Shinkiro
		if(sd->inventory_data[idx]->nameid == 312973){
			if(
				sd->inventory_data[i]->nameid == 400541
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Poder oculto Soul Ascetic I
		if(sd->inventory_data[idx]->nameid == 312923){
			if(
				sd->inventory_data[i]->nameid == 550052
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Poder oculto Shadow Cross III 
		if(sd->inventory_data[idx]->nameid == 312396){
			if(
				sd->inventory_data[i]->nameid == 610028 || sd->inventory_data[i]->nameid == 510017
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Poder oculto Night Watch I
		if(sd->inventory_data[idx]->nameid == 312924){
			if(
				sd->inventory_data[i]->nameid == 19321 || sd->inventory_data[i]->nameid == 19101 || sd->inventory_data[i]->nameid == 420110 || sd->inventory_data[i]->nameid == 420199
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Hero's Triumph
		if(sd->inventory_data[idx]->nameid == 311074){
			if(
				sd->inventory_data[i]->nameid == 480188 || sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 490381 || sd->inventory_data[i]->nameid == 490207 || sd->inventory_data[i]->nameid == 490337 || sd->inventory_data[i]->nameid == 490545
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Universalis
		if(sd->inventory_data[idx]->nameid == 310655){
			if(
				sd->inventory_data[i]->nameid == 420199 || sd->inventory_data[i]->nameid == 420236
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Refinar PET equip
		if(sd->inventory_data[idx]->nameid == 77770){
			if(
				sd->inventory_data[i]->nameid >= 77730 && sd->inventory_data[i]->nameid <= 77732
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Refinar PET equip Pingente
		if(sd->inventory_data[idx]->nameid == 77771){
			if(
				sd->inventory_data[i]->nameid == 77733
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Pedra Mineração
		if(sd->inventory_data[idx]->nameid == 50304 || sd->inventory_data[idx]->nameid == 50305 || sd->inventory_data[idx]->nameid == 50306){
			if(
				sd->inventory_data[i]->nameid == 66004
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Pedra Herbologia
		if(sd->inventory_data[idx]->nameid == 50307 || sd->inventory_data[idx]->nameid == 50308 || sd->inventory_data[idx]->nameid == 50309){
			if(
				sd->inventory_data[i]->nameid == 66003
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Pedra Pesca
		if(sd->inventory_data[idx]->nameid == 50315 || sd->inventory_data[idx]->nameid == 50316 || sd->inventory_data[idx]->nameid == 50317){
			if(
				sd->inventory_data[i]->nameid == 66001
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Pedra Pesca
		if(sd->inventory_data[idx]->nameid == 312305){
			if(
				sd->inventory_data[i]->nameid == 490337
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Pedra Telecinética
		if(sd->inventory_data[idx]->nameid == 44700){
			if(
				sd->inventory_data[i]->nameid == 410028
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Encantamento Dreaming Red Sheep
		if(sd->inventory_data[idx]->nameid == 54435){
			if(
				sd->inventory_data[i]->nameid == 19446
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Foreign Conquerer
		if(sd->inventory_data[idx]->nameid == 311439){
			if(
				sd->inventory_data[i]->nameid == 420110 || sd->inventory_data[i]->nameid == 420164 || sd->inventory_data[i]->nameid == 490381
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Demonlord's Calamity
		if(sd->inventory_data[idx]->nameid == 311075){
			if(
				sd->inventory_data[i]->nameid == 480188 || sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 490381 || sd->inventory_data[i]->nameid == 410247
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Dynast
		if(sd->inventory_data[idx]->nameid == 29436){
			if(
				sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 400203 ||
				sd->inventory_data[i]->nameid == 400187 || sd->inventory_data[i]->nameid == 400177 ||
				sd->inventory_data[i]->nameid == 15892 || sd->inventory_data[i]->nameid == 15891 ||
				sd->inventory_data[i]->nameid == 450208 || sd->inventory_data[i]->nameid == 450209 ||
				sd->inventory_data[i]->nameid == 450210 || sd->inventory_data[i]->nameid == 450214 ||
				sd->inventory_data[i]->nameid == 450216 || sd->inventory_data[i]->nameid == 490206 || 
				sd->inventory_data[i]->nameid == 480188 || sd->inventory_data[i]->nameid == 460023 || 
				sd->inventory_data[i]->nameid == 490207 || sd->inventory_data[i]->nameid == 490290 || 
				sd->inventory_data[i]->nameid == 490381 || sd->inventory_data[i]->nameid == 490404 || 
				sd->inventory_data[i]->nameid == 490545 || sd->inventory_data[i]->nameid == 460078
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Fallen of Heaven and Earth
		if(sd->inventory_data[idx]->nameid == 3101970){
			if(
				sd->inventory_data[i]->nameid == 470243
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		//Sky Emperor
		if((sd->inventory_data[idx]->nameid == 313062 || sd->inventory_data[idx]->nameid == 313286) && !sd->inventory.u.items_inventory[i].card[3] && sd->inventory_data[i]->nameid == 400543){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		//Windhawk
		if((sd->inventory_data[idx]->nameid == 312036 || sd->inventory_data[idx]->nameid == 312918) && !sd->inventory.u.items_inventory[i].card[3] && sd->inventory_data[i]->nameid == 400539){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		//Archmage
		if((sd->inventory_data[idx]->nameid == 312031 || sd->inventory_data[idx]->nameid == 312473) && !sd->inventory.u.items_inventory[i].card[3] && sd->inventory_data[i]->nameid == 400535){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		//Poder Oculto Baixo
		if((sd->inventory_data[idx]->nameid == 313490 || sd->inventory_data[idx]->nameid == 312974 || sd->inventory_data[idx]->nameid == 312645 || sd->inventory_data[idx]->nameid == 312934 || sd->inventory_data[idx]->nameid == 312034 || sd->inventory_data[idx]->nameid == 312691 || sd->inventory_data[idx]->nameid == 312972 || sd->inventory_data[idx]->nameid == 313321 || sd->inventory_data[idx]->nameid == 312928 || sd->inventory_data[idx]->nameid == 311895 || sd->inventory_data[idx]->nameid == 312646 || sd->inventory_data[idx]->nameid == 312918 || sd->inventory_data[idx]->nameid == 312964 || sd->inventory_data[idx]->nameid == 312963) && !sd->inventory.u.items_inventory[i].card[0] && (sd->inventory_data[i]->nameid == 420236)){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		if((sd->inventory_data[idx]->nameid == 312038 || sd->inventory_data[idx]->nameid == 312304 ) && !sd->inventory.u.items_inventory[i].card[3] && (sd->inventory_data[i]->nameid == 400537)){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		if((sd->inventory_data[idx]->nameid == 312933 || sd->inventory_data[idx]->nameid == 313279 ) && !sd->inventory.u.items_inventory[i].card[3] && (sd->inventory_data[i]->nameid == 400538)){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		
		if((sd->inventory_data[idx]->nameid == 313479) && !sd->inventory.u.items_inventory[i].card[3] && sd->inventory_data[i]->nameid == 400541){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		if((sd->inventory_data[idx]->nameid == 313282) && !sd->inventory.u.items_inventory[i].card[3] && sd->inventory_data[i]->nameid == 400546){
			WFIFOW(fd,4+c*2)=i+2;
			c++;	
		}
		...
		if(sd->inventory_data[idx]->nameid == 311073){
			if(sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 470112 || sd->inventory_data[i]->nameid == 490381){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// Epifania
		if(sd->inventory_data[idx]->nameid == 47777){
			if(
				sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 400203 || sd->inventory_data[i]->nameid == 490381 ||
				sd->inventory_data[i]->nameid == 400187 || sd->inventory_data[i]->nameid == 400177 ||
				sd->inventory_data[i]->nameid == 15892 || sd->inventory_data[i]->nameid == 15891 ||
				sd->inventory_data[i]->nameid == 490206 || sd->inventory_data[i]->nameid == 490193 ||
				sd->inventory_data[i]->nameid == 490183 || sd->inventory_data[i]->nameid == 490207 ||
				sd->inventory_data[i]->nameid == 480188 || sd->inventory_data[i]->nameid == 450216 ||
				sd->inventory_data[i]->nameid == 450214 || sd->inventory_data[i]->nameid == 450208 ||
				sd->inventory_data[i]->nameid == 450209 || sd->inventory_data[i]->nameid == 450210 ||
				sd->inventory_data[i]->nameid == 470112 || sd->inventory_data[i]->nameid == 490290 ||
				sd->inventory_data[i]->nameid == 450242 || sd->inventory_data[i]->nameid == 470274 ||
				sd->inventory_data[i]->nameid == 490404
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Hero
		if(sd->inventory_data[idx]->nameid == 29509){
			if(
				sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 490381 ||
				sd->inventory_data[i]->nameid == 490206 || sd->inventory_data[i]->nameid == 490193 ||
				sd->inventory_data[i]->nameid == 490183 || sd->inventory_data[i]->nameid == 490207 ||
				sd->inventory_data[i]->nameid == 450220 || sd->inventory_data[i]->nameid == 470048 ||
				sd->inventory_data[i]->nameid == 470128 || sd->inventory_data[i]->nameid == 470037 ||
				sd->inventory_data[i]->nameid == 22214 || sd->inventory_data[i]->nameid == 470049 ||
				sd->inventory_data[i]->nameid == 22224 || sd->inventory_data[i]->nameid == 22225 ||
				sd->inventory_data[i]->nameid == 470061 || sd->inventory_data[i]->nameid == 490290 || sd->inventory_data[i]->nameid == 490337 ||
				sd->inventory_data[i]->nameid == 470180 || sd->inventory_data[i]->nameid == 490404 || sd->inventory_data[i]->nameid == 490545 || 
				sd->inventory_data[i]->nameid == 4701800
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Gloria Real
		if(sd->inventory_data[idx]->nameid == 311365){
			if(
				sd->inventory_data[i]->nameid == 490206 || sd->inventory_data[i]->nameid == 470180
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Fury
		if(sd->inventory_data[idx]->nameid == 312475){
			if(
				sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 490381 ||
				sd->inventory_data[i]->nameid == 490206 || sd->inventory_data[i]->nameid == 490193 ||
				sd->inventory_data[i]->nameid == 490183 || sd->inventory_data[i]->nameid == 490207 ||
				...
				sd->inventory_data[i]->nameid == 22214 || sd->inventory_data[i]->nameid == 470049 ||
				sd->inventory_data[i]->nameid == 22224 || sd->inventory_data[i]->nameid == 22225 ||
				sd->inventory_data[i]->nameid == 470061 || sd->inventory_data[i]->nameid == 490290 || 
				sd->inventory_data[i]->nameid == 470254 || sd->inventory_data[i]->nameid == 490404 
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Raiva
		if(sd->inventory_data[idx]->nameid == 610198){
			if(
				sd->inventory_data[i]->nameid == 470254
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Arcana
		if(sd->inventory_data[idx]->nameid == 29585){
			if(sd->inventory_data[i]->nameid == 480045 || sd->inventory_data[i]->nameid == 400110 || sd->inventory_data[i]->nameid == 400109 || sd->inventory_data[i]->nameid == 400111 || sd->inventory_data[i]->nameid == 50030){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// Abyssal King
		if(sd->inventory_data[idx]->nameid == 310321){
			if(
				sd->inventory_data[i]->nameid == 420110 || sd->inventory_data[i]->nameid == 420164 || sd->inventory_data[i]->nameid == 490381 || sd->inventory_data[i]->nameid == 490337
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Ruler of The End / Granada do Poder
		if(sd->inventory_data[idx]->nameid == 312421 || sd->inventory_data[idx]->nameid == 311883){
			if(
				sd->inventory_data[i]->nameid == 490381 || sd->inventory_data[i]->nameid == 420199 ||
				sd->inventory_data[i]->nameid == 420236 || sd->inventory_data[i]->nameid == 420110
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// rancor do assassino
		if(sd->inventory_data[idx]->nameid == 29724){
			if(
				sd->inventory_data[i]->nameid == 420199 ||
				sd->inventory_data[i]->nameid == 420236 || sd->inventory_data[i]->nameid == 420329
			){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
		...
		// Release
		if(sd->inventory_data[idx]->nameid == 29371){
			if(
				sd->inventory_data[i]->nameid == 400213 || sd->inventory_data[i]->nameid == 19315 || sd->inventory_data[i]->nameid == 490381 ||
				sd->inventory_data[i]->nameid == 490206 || sd->inventory_data[i]->nameid == 490193 ||
				sd->inventory_data[i]->nameid == 490183 || sd->inventory_data[i]->nameid == 490207 ||
				sd->inventory_data[i]->nameid == 450220 || sd->inventory_data[i]->nameid == 20991 ||
				sd->inventory_data[i]->nameid == 470128 || sd->inventory_data[i]->nameid == 470037 ||
				sd->inventory_data[i]->nameid == 22214 || sd->inventory_data[i]->nameid == 470049 ||
				sd->inventory_data[i]->nameid == 22224 || sd->inventory_data[i]->nameid == 22225 ||
				sd->inventory_data[i]->nameid == 470061 || sd->inventory_data[i]->nameid == 490290 || sd->inventory_data[i]->nameid == 490337 ||
		...
		// Striking Enchant Box
		if(sd->inventory_data[idx]->nameid == 48101){
			if(sd->inventory_data[i]->nameid == 19379 || sd->inventory_data[i]->nameid == 450165 || sd->inventory_data[i]->nameid == 480114){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// Pegasus Enchant Box
		if(sd->inventory_data[idx]->nameid == 48102){
			if(sd->inventory_data[i]->nameid == 470128){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		//
		if(sd->inventory_data[idx]->nameid == 40079){
			if(sd->inventory.u.items_inventory[i].nameid == 490290){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// dual card shield
		if(sd->inventory_data[i]->equip == EQP_SHIELD && sd->inventory.u.items_inventory[i].card[0] && !sd->inventory.u.items_inventory[i].card[1]){
			if((sd->inventory_data[idx]->nameid >= 27050 && sd->inventory_data[idx]->nameid <= 27054) || (sd->inventory_data[idx]->nameid >= 27129 && sd->inventory_data[idx]->nameid <= 27140) || 
			(sd->inventory_data[idx]->nameid >= 27364 && sd->inventory_data[idx]->nameid <= 27367) || (sd->inventory_data[idx]->nameid >= 300131 && sd->inventory_data[idx]->nameid <= 300136) || 
			(sd->inventory_data[idx]->nameid >= 300163 && sd->inventory_data[idx]->nameid <= 300168) || (sd->inventory_data[idx]->nameid >= 300282 && sd->inventory_data[idx]->nameid <= 300289)){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
				continue;
			}
		...
		// ssf
		// cartas marionete baixo slot 4
		if(
		sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_LOW &&
		(!sd->inventory.u.items_inventory[i].card[3])
		) {
			if(
			 sd->inventory_data[idx]->nameid >= 2011369 && sd->inventory_data[idx]->nameid <= 2011404
			)
			{
		...
		// ssf
		// cartas marionete capa slot 2
		if(
		sd->inventory_data[i]->equip == EQP_COSTUME_GARMENT &&
		(!sd->inventory.u.items_inventory[i].card[1])
		) {
			if(
			 sd->inventory_data[idx]->nameid >= 2011405 && sd->inventory_data[idx]->nameid <= 2011440
			)
			{
		...
		// ssf
		// cartas marionete topo slot 2
		if(
		sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_TOP &&
		(!sd->inventory.u.items_inventory[i].card[1])
		) {
			if(
			 sd->inventory_data[idx]->nameid >= 1011260 && sd->inventory_data[idx]->nameid <= 1011296
			)
			{
		...
		// cartas marionete topo slot 4
		if(
		(sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_TOP) &&
		!(sd ->inventory.u.items_inventory[i].card[3])
		&& (sd->inventory_data[idx]->nameid >= 1011297 && sd->inventory_data[idx]->nameid <= 1011332)
		) {
				WFIFOW(fd,4+c*2)=i+2;
				c++;
				continue;
		}
		// cartas marionete meio 
		if(
		(sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_MID) &&
		!(sd ->inventory.u.items_inventory[i].card[3])
		&& (sd->inventory_data[idx]->nameid >= 1011333 && sd->inventory_data[idx]->nameid <= 1011368)
		) {
				WFIFOW(fd,4+c*2)=i+2;
				c++;
				continue;
		}
		// cartas marionete baixo 
		if(
		(sd->inventory_data[i]->equip == EQP_COSTUME_HEAD_LOW) &&
		!(sd ->inventory.u.items_inventory[i].card[1])
		&& (sd->inventory_data[idx]->nameid >= 1011369 && sd->inventory_data[idx]->nameid <= 1011404)
		) {
				WFIFOW(fd,4+c*2)=i+2;
				c++;
				continue;
		}
		// cartas marionete capa 
		if(
		(sd->inventory_data[i]->equip == EQP_COSTUME_GARMENT) &&
		!(sd ->inventory.u.items_inventory[i].card[3]) && sd->inventory_data[i]->type != IT_CARD
		&& (sd->inventory_data[idx]->nameid >= 1011405 && sd->inventory_data[idx]->nameid <= 1011440)
		) {
				WFIFOW(fd,4+c*2)=i+2;
				c++;
				continue;
		...
		if(sd->inventory_data[idx]->nameid == 29300){
			if(sd->inventory.u.items_inventory[i].nameid == 32242){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// escudo divino
		if(sd->inventory_data[idx]->nameid == 40078){
			if(sd->inventory.u.items_inventory[i].nameid == 28962 || sd->inventory.u.items_inventory[i].nameid == 51121){
				WFIFOW(fd,4+c*2)=i+2;
				c++;	
			}
			continue;
		...
		// devon
		// costume e dual enchant
		if(ep == EQP_COSTUME_GARMENT){ 
			if(sd->inventory_data[idx]->dualcard >= 1){
				if(sd->inventory.u.items_inventory[i].card[0] == 0 ) continue;
				sd->inventory_data[i]->slots = 2;
			} else
				sd->inventory_data[i]->slots = 1;
		}

		if(ep == EQP_COSTUME_HEAD_MID || ep == EQP_COSTUME_HEAD_TOP || ep == EQP_COSTUME_HEAD_LOW){ 
			int slots = 1;
			if(sd->inventory_data[i]->equip == (EQP_COSTUME_HEAD_MID|EQP_COSTUME_HEAD_TOP)) slots = 2;
			if(sd->inventory_data[i]->equip == (EQP_COSTUME_HEAD_MID|EQP_COSTUME_HEAD_LOW)) slots = 2;
			if(sd->inventory_data[i]->equip == (EQP_COSTUME_HEAD_TOP|EQP_COSTUME_HEAD_LOW)) slots = 2;
			if(sd->inventory_data[i]->equip == (EQP_COSTUME_HEAD_MID|EQP_COSTUME_HEAD_TOP|EQP_COSTUME_HEAD_LOW)) slots = 3;
			...
			sd->inventory_data[i]->slots = slots;

			if(sd->inventory_data[idx]->nameid == sd->inventory.u.items_inventory[i].card[0] || sd->inventory_data[idx]->nameid == sd->inventory.u.items_inventory[i].card[1] || sd->inventory_data[idx]->nameid == sd->inventory.u.items_inventory[i].card[3]) continue;
		}
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
	p.grade = sd->cart.u.items_cart[n].enchantgrade + sd->cart.u.items_cart[n].despertar;
#endif
#endif
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
		p->items[i].grade = vsd->cart.u.items_cart[index].enchantgrade + vsd->cart.u.items_cart[index].despertar;
#endif
#endif
#endif
	}
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
		p->items[i].grade = sd->cart.u.items_cart[index].enchantgrade + sd->cart.u.items_cart[index].despertar;
#endif
#endif
	}
...
/// DEVON.
void clif_sendegggrade(struct map_session_data *sd)
{
	int i,n=0,fd;
	...
	for(i=0,n=0;i<MAX_INVENTORY;i++){
		if(sd->inventory.u.items_inventory[i].nameid == 0 || sd->inventory_data[i] == NULL ||
		   ...
		   sd->inventory.u.items_inventory[i].amount <= 0)
			continue;
		if(sd->inventory.u.items_inventory[i].nameid != 9275 && sd->inventory.u.items_inventory[i].nameid != 9271 && sd->inventory.u.items_inventory[i].nameid != 9268 
			&& sd->inventory.u.items_inventory[i].nameid != 9263 && sd->inventory.u.items_inventory[i].nameid != 9260 && sd->inventory.u.items_inventory[i].nameid != 9259 
			&& sd->inventory.u.items_inventory[i].nameid != 9258 && sd->inventory.u.items_inventory[i].nameid != 9257 && sd->inventory.u.items_inventory[i].nameid != 9256 
			&& sd->inventory.u.items_inventory[i].nameid != 9253 && sd->inventory.u.items_inventory[i].nameid != 9250 && sd->inventory.u.items_inventory[i].nameid != 9246)
			continue;
		WFIFOW(fd,n*2+4)=client_index(i);
		n++;
	}
...
void clif_send_petdata(struct map_session_data* sd, struct pet_data* pd, int type, int param)
{
	...
	if(pd->pet.equip >= 77730 && pd->pet.equip <= 77740) {
		if(pd->pet.refine >= 5){
			...
			p->status = 1;
			if(pd->pet.refine >= 15){
				p->packetLength = (int16)( sizeof( struct PACKET_ZC_HAT_EFFECT ) + sizeof( int16 ) * 5 );
				p->effects[0] = HAT_EF_dancing_knife_A;
				p->effects[1] = HAT_EF_dancing_knife_B;
				p->effects[2] = HAT_EF_serpens_B;
				p->effects[3] = HAT_EF_judge_C;
				p->effects[4] = HAT_EF_judge_B;
			} 
			else if(pd->pet.refine >= 10){
				p->packetLength = (int16)( sizeof( struct PACKET_ZC_HAT_EFFECT ) + sizeof( int16 ) * 3 );
				p->effects[0] = HAT_EF_serpens_B;
				p->effects[1] = HAT_EF_judge_C;
				p->effects[2] = HAT_EF_judge_B;
			} 
			else {
				p->packetLength = (int16)( sizeof( struct PACKET_ZC_HAT_EFFECT ) + sizeof( int16 ) * 2 );
				p->effects[0] = HAT_EF_venom_swamp_A;
				p->effects[1] = HAT_EF_gef_nocturn_B;
			}
			
			clif_send( p, p->packetLength, &pd->bl, AREA );
		}
		if(type == 3){
			param = 0;
		}
...
void clif_guild_belonginfo(struct map_session_data& sd, struct guild *g)
{
	...
	if(sd.status.grandcomp > 0 && battle_config.desativarhgc)
		position = sd.status.grandcomp;
	else
		position = guild_getposition( &guild, &sd );
...
void clif_guild_alliance_message(struct guild *g, const char *mes, int len)
{
	map_session_data *sd;
	uint8 buf[256];
...
void clif_parse_GuildAllianceMessage(int fd, map_session_data* sd){
	...
	if (!g)
		return;
	clif_guild_alliance_message(g, output, strlen(output));
}
...
void clif_guild_memberlist( struct map_session_data& sd ){
	...
	if(sd.status.grandcomp > 0 && battle_config.desativarhgc){
		...
		member_info.AID = sd.status.account_id;
		member_info.GID = sd.status.char_id;
		member_info.head = sd.status.hair;
		member_info.headPalette = sd.status.hair_color;
		member_info.sex = sd.status.sex;
		member_info.job = sd.status.class_;
		member_info.level = sd.status.base_level;
		member_info.contributionExp = 0;
		member_info.currentState = 1;
		member_info.positionID = sd.status.grandcomp;
#if PACKETVER >= 20200902
		member_info.lastLoginTime = (uint32)time(NULL);
		safestrncpy( member_info.char_name, sd.status.name, sizeof( member_info.char_name ) );
	...
	for( int i = 0; i < guild.max_member; i++ ){
...
void clif_guild_emblem_hgc(struct block_list* bl, int guild_id, int emblem_id)
{
	// TODO this packet doesn't force the update of ui components that have the emblem visible
	//      (emblem in the flag npcs and emblem over the head in agit maps) [FlavioJS]
	PACKET_ZC_CHANGE_GUILD p{};
...
void clif_guild_notice_help(struct map_session_data* sd)
{
	struct guild* g = guild_search(2082);
	...
	if (Sql_Query(mmysql_handle,
		"SELECT `id` FROM `membros_hgc`;",
		g->guild_id ) == SQL_SUCCESS )
	{
		at = (uint16)Sql_NumRows(mmysql_handle);
	}
	*/
	
	if(sd->state.aviso_hgc >= 1) return;
	
	sd->state.aviso_hgc += 1;
...
void clif_name( struct block_list* src, struct block_list *bl, send_target target ){
	...
	switch( bl->type ){
		case BL_PC: {
			...
			if( p && ( sd->guild || sd->bg_id || battle_config.display_party_name || (sd->status.grandcomp > 0 && battle_config.desativarhgc) ) ){
				safestrncpy( packet.party_name, p->party.name, NAME_LENGTH );
			}
			
			//if(sd->status.grandcomp && sd->guild) { clif_guild_emblem_hgc(&sd->bl, sd->guild->guild_id, sd->guild->emblem_id); }
		...
		case BL_MOB: {
			...
			else if (md->bg_id && (bg = (util::umap_find(bg_team_db, md->bg_id))) && (g = bg->g) != NULL)
			{
			...
			else if( battle_config.show_mob_info && md->db->id == 31839){
				PACKET_ZC_ACK_REQNAMEALL packet = { 0 };
				...
				if( battle_config.show_mob_info&1 ){
					str_p += sprintf( str_p, "Dano: %uM | ", md->status.danorecebido );
				}
			...
			else if( battle_config.show_mob_info && md->status.max_shield > 0){
				PACKET_ZC_ACK_REQNAMEALL packet = { 0 };
				...
				if( battle_config.show_mob_info&1 ){
					str_p += sprintf( str_p, "Escudo: %uM/%uM | ", md->status.shield, md->status.max_shield );
				}
...
void clif_parse_LoadEndAck(int fd,struct map_session_data *sd)
{
	...
	if(battle_config.desativarhgc){
		if(sd->status.grandcomp > 0){
			struct guild* g = guild_search(2082);
			if(g){
				sd->guild = g;
				sd->guild_emblem_id = g->emblem_id;
				clif_guild_belonginfo( *sd ,g);
				if(!sd->sc.data[SC_HISTORY_GC]) 
					sc_start(&sd->bl, &sd->bl, SC_HISTORY_GC, 100, 1, -1);
				if(sd->status.grandcomp < 19 && !sd->sc.data[SC_MEM_HISTORY_GC]) 
					sc_start(&sd->bl, &sd->bl, SC_MEM_HISTORY_GC, 100, 1, -1);
				clif_guild_emblem_hgc(&sd->bl, g->guild_id, g->emblem_id);
				
				if(sd->status.grandcomp == 19 && sd->sc.data[SC_MEM_HISTORY_GC])
					status_change_end(&sd->bl, SC_MEM_HISTORY_GC, INVALID_TIMER);
			}
		} else {
			if(sd->sc.data[SC_HISTORY_GC]) 
				status_change_end(&sd->bl, SC_HISTORY_GC, INVALID_TIMER);
			if(sd->sc.data[SC_MEM_HISTORY_GC]) 
				status_change_end(&sd->bl, SC_MEM_HISTORY_GC, INVALID_TIMER);
		}
	}
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if(sd->state.connect_new) {
		...
		uint32 date = date_get(DT_DAYOFMONTH);
		sd->state.dia_hoje = date;

		// Restock System
		int i, max;

		max = pc_isvip(sd) ? battle_config.max_items_for_vip : battle_config.max_items_for_non_vip;

		if (!sd->state.autotrade) {
			for (i = 0; i < MAX_RESTOCK; i++) {
				struct item_data* nameid;
				if (sd->status.restock[i].nameid == 0)
					continue;
				else if (sd->status.restock[i].amount == 0 || (nameid = itemdb_exists(sd->status.restock[i].nameid)) == NULL) {
					sd->status.restock[i].nameid = 0;
					sd->status.restock[i].amount = 0;
				}
			}
			if(battle_config.multiserver == 3 && sd->bl.m != mapindex_name2id("eden_n")){
				pc_setpos(sd,mapindex_name2id("eden_n"),55,69,CLR_OUTSIGHT);
			}
		}
	} else {
	...
	if (!sd->state.autotrade && !sd->state.connect_new) {
		if(!mapdata->flag[MF_MASMORRAINVERNAL]){
			if(sd->sc.data[SC_MASMORRA_DANO]){
					status_change_end(&sd->bl, SC_MASMORRA_DANO, INVALID_TIMER);
			}
			if(sd->sc.data[SC_MASMORRA_DROP]){
					status_change_end(&sd->bl, SC_MASMORRA_DROP, INVALID_TIMER);
			}
		}
		if(mapdata->flag[MF_PRONTERA]){
			if(sd->sc.data[SC_EFEITO_RH_21]){
				const struct TimerData *td = (sd && sd->sc.data[SC_EFEITO_RH_21] ? get_timer(sd->sc.data[SC_EFEITO_RH_21]->timer) : NULL);
				if (td) {
					pc_setglobalreg(sd, add_str("LASTTIMEEXTREME"), td->tick - gettick());
					status_change_end(&sd->bl, SC_EFEITO_RH_21, INVALID_TIMER);
					clif_displaymessage(sd->fd, "[Buff Diário Fenda] O tempo do seu buff da quest Fenda Savage Diária foi parado durante seu período em prontera.");
				}
			}
			if(sd->sc.data[SC_FENDAD_INFERNAL]){
				const struct TimerData *td = (sd && sd->sc.data[SC_FENDAD_INFERNAL] ? get_timer(sd->sc.data[SC_FENDAD_INFERNAL]->timer) : NULL);
				if (td) {
					pc_setglobalreg(sd, add_str("LASTTIMEINFERNAL"), td->tick - gettick());
					status_change_end(&sd->bl, SC_FENDAD_INFERNAL, INVALID_TIMER);
					clif_displaymessage(sd->fd, "[Buff Diário Fenda] O tempo do seu buff da quest Fenda Dimensional Infernal Diária foi parado durante seu período em prontera.");
				}
			}
			if(sd->sc.data[SC_EFEITO_RH_22]){
				const struct TimerData *td = (sd && sd->sc.data[SC_EFEITO_RH_22] ? get_timer(sd->sc.data[SC_EFEITO_RH_22]->timer) : NULL);
				if (td) {
					pc_setglobalreg(sd, add_str("LASTTIMEUNREAL"), td->tick - gettick());
					status_change_end(&sd->bl, SC_EFEITO_RH_22, INVALID_TIMER);
					clif_displaymessage(sd->fd, "[Buff Diário Fenda] O tempo do seu buff da quest Fenda Dimensional Pesadelo Diária foi parado durante seu período em prontera.");
				}
			}
			if(sd->sc.data[SC_EFEITO_RH_23]){
				const struct TimerData *td = (sd && sd->sc.data[SC_EFEITO_RH_23] ? get_timer(sd->sc.data[SC_EFEITO_RH_23]->timer) : NULL);
				if (td) {
					pc_setglobalreg(sd, add_str("LASTTIMESAVAGE"), td->tick - gettick());
					status_change_end(&sd->bl, SC_EFEITO_RH_23, INVALID_TIMER);
					clif_displaymessage(sd->fd, "[Buff Diário Fenda] O tempo do seu buff da quest Fenda Dimensional Ultimate Diária foi parado durante seu período em prontera.");
				}
			}
		} else {
			if(!sd->sc.data[SC_EFEITO_RH_21]){
				int valueext = pc_readglobalreg(sd, add_str("LASTTIMEEXTREME"));
				if(valueext) {
					pc_setglobalreg(sd, add_str("LASTTIMEEXTREME"),0);
					sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_21, 100, 3, valueext);
				}
			}
			if(!sd->sc.data[SC_FENDAD_INFERNAL]){
				int valueext = pc_readglobalreg(sd, add_str("LASTTIMEINFERNAL"));
				if(valueext) {
					pc_setglobalreg(sd, add_str("LASTTIMEINFERNAL"),0);
					sc_start(&sd->bl, &sd->bl, SC_FENDAD_INFERNAL, 100, 4, valueext);
				}
			}
			if(!sd->sc.data[SC_EFEITO_RH_22]){
				int valueunreal = pc_readglobalreg(sd, add_str("LASTTIMEUNREAL"));
				if(valueunreal) {
					pc_setglobalreg(sd, add_str("LASTTIMEUNREAL"),0);
					sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_22, 100, 8, valueunreal);
				}
			}
			if(!sd->sc.data[SC_EFEITO_RH_23]){
				int valuesavage = pc_readglobalreg(sd, add_str("LASTTIMESAVAGE"));
				if(valuesavage) {
					pc_setglobalreg(sd, add_str("LASTTIMESAVAGE"),0);
					sc_start(&sd->bl, &sd->bl, SC_EFEITO_RH_23, 100, 12, valuesavage);
				}
			}
		}
	}
	...
	if( sd->state.changemap ) {// restore information that gets lost on map-change
		...
		if (sd->status.guild_id && sd->guild && battle_config.guild_notice_changemap == 1){
	...
	if( sd->status.guild_id && sd->guild && ( battle_config.guild_notice_changemap == 2 || guild_notice ) ){
		clif_guild_notice(sd); // Displays at end
	}

	if (sd->status.grandcomp > 0 && battle_config.desativarhgc){
		clif_guild_notice_help(sd); // Displays after VIP
	}
	...
	if(!sd->state.autotrade){
		if(mapdata->flag[MF_DOMINIOGERAL] && mapdata->flag[MF_DOMINIONORMAL]){
			sd->state.aviso = 0;
		} else if(sd->sc.data[SC_FADIGA_FLORESTA]){
			status_change_end(&sd->bl, SC_FADIGA_FLORESTA, INVALID_TIMER);
		}
		/*
		if(mapdata->flag[MF_FLORESTAPES] && !mapdata->flag[MF_VULCAOPESADELO]){
			sd->state.aviso = 0;
		} else if(sd->sc.data[SC_FADIGA_FLORESTA]){
			status_change_end(&sd->bl, SC_FADIGA_FLORESTA, INVALID_TIMER);
		}
		*/
		if(mapdata->flag[MF_VULCAOPESADELO]){
			sd->state.aviso = 0;
		} else if(sd->sc.data[SC_FADIGA_VULCAO]){
			status_change_end(&sd->bl, SC_FADIGA_VULCAO, INVALID_TIMER);
		}
		if(mapdata->flag[MF_TEMPLOULTIMATE]){
			sd->state.aviso = 0;
		} else if(sd->sc.data[SC_FADIGA_TEM_PESA]){
			status_change_end(&sd->bl, SC_FADIGA_TEM_PESA, INVALID_TIMER);
		}
		if((mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP]) && !mapdata->flag[MF_TEMPLOULTIMATE]){
			sd->state.aviso = 0;
		} else if(sd->sc.data[SC_FADIGA_ESCU]){
			status_change_end(&sd->bl, SC_FADIGA_ESCU, INVALID_TIMER);
		}
	}

	sd->state.serverteste = 0;
	sd->state.last_aumento = 0;
	sd->state.last_minuto = 0;

	if (!sd->state.autotrade) {
		// devon
		if (mapdata->flag[MF_BATTLEROYALE] && sd->status.party_id){
			party_leave(sd);
		}
		if ((mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP]) && sd->status.party_id){
			struct party_data *pp;
			int ii, conta = 0;
			pp = party_search(sd->status.party_id);
			if( pp != NULL ){
				for (ii = 0; ii < MAX_PARTY; ii++) {
					if (pp->party.member[ii].online > 0)
						conta++;
				}
		...
		if (mapdata->flag[MF_TEMPLOSAGRADO] && sd->status.party_id){
			struct party_data *pp;
			int ii, conta = 0;
			pp = party_search(sd->status.party_id);
			if( pp != NULL ){
...
void clif_parse_WalkToXY(int fd, struct map_session_data *sd)
{
	...
	if(sd->sc.data[SC_RUN] || sd->sc.data[SC_WUGDASH] || sd->sc.data[SC_ENRAIZAR])
		return;
...
{
	int fd = sd->fd;
	if(battle_config.multiserver > 1) return;
	
	// Devon
	int at = 0, newcount = 0, somador = 0, canaltwo = 0;
	if (Sql_Query(mmysql_handle,
			"SELECT count(`id`) FROM `%s` WHERE `autotrade` = 1;", vendings_table) == SQL_SUCCESS )
	{
		if(Sql_NumRows(mmysql_handle) > 0){
			while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
				char *data;
				Sql_GetData(mmysql_handle, 0, &data, NULL); at = (uint16)atoi(data);
			}
	...
	if (Sql_Query(mmysql_handle,
			"SELECT `somador_players` FROM `variaveis_globais` WHERE `id` = 4;") == SQL_SUCCESS )
	{
		if(Sql_NumRows(mmysql_handle) > 0){
			while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
				char *data;
				Sql_GetData(mmysql_handle, 0, &data, NULL); somador = (uint16)atoi(data);
			}
			Sql_FreeResult(mmysql_handle);
		}
	}
	...
	if (Sql_Query(mmysql_handle,
			"SELECT sum(`valor`) FROM `estatisticas` WHERE `id` in (4, 5);") == SQL_SUCCESS )
	{
		if(Sql_NumRows(mmysql_handle) > 0){
			while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
				char *data;
				Sql_GetData(mmysql_handle, 0, &data, NULL); canaltwo = (uint16)atoi(data);
			}
			Sql_FreeResult(mmysql_handle);
		}
	}
	...
	newcount = count - at;
	newcount += canaltwo;
	newcount = (newcount * 3.6) / 1;
	newcount = newcount + at;
	//ShowWarning("newcount = %d \n",newcount);
	// -------------------------------------
...
void clif_parse_WisMessage(int fd, struct map_session_data* sd)
{
	...
	struct map_data *mapdata = map_getmapdata(dstsd->bl.m);

	// if player ignores everyone
	if (dstsd->state.ignoreAll && pc_get_group_level(sd) <= pc_get_group_level(dstsd) 
		&& (mapdata->flag[MF_TEMPLOSAGRADO] || mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP]
		|| mapdata->flag[MF_BIOSAVAGE] || mapdata->flag[MF_BIOHARD] || mapdata->flag[MF_TOWN] || mapdata->flag[MF_BIOEXTREME] 
		|| mapdata->flag[MF_CHEFFTRANS] || mapdata->flag[MF_CHEFFEXTREME] || mapdata->flag[MF_FENDA] || mapdata->flag[MF_TRIAL]
		|| mapdata->flag[MF_VILAGRANDORC] || mapdata->flag[MF_NOTELEPORT])
		) {
...
void clif_parse_EquipItem(int fd,struct map_session_data *sd)
{
	...
	int nameid = sd->inventory.u.items_inventory[index].nameid;

	if(sd->inventory_data[index]->type == IT_PETARMOR || (nameid >= 77730 && nameid <= 77740)){
...
void clif_parse_PutItemToCart( int fd, struct map_session_data *sd ){
	if (pc_istrading(sd) || !pc_iscarton(sd))
		return;
...
void clif_parse_PartyInvite(int fd, struct map_session_data *sd)
{
	...
	if(mapdata->flag[MF_PARTYLOCK] || mapdata->flag[MF_CHEFFSAVAGE] || mapdata->flag[MF_FENDA]) {// Party locked.
		clif_displaymessage(fd, msg_txt(sd,227));
		return;
	}
...
void clif_parse_OpenVending(int fd, struct map_session_data* sd){
	...
	int trava;
	// --------------------------------------------------------------
	// [DEVON] - sem dual venda na mesma celula
	// --------------------------------------------------------------	
	if (Sql_Query(mmysql_handle,
		"SELECT `id`, `account_id`, `char_id`, `map`, `x`, `y` FROM `%s` where `map` = '%s' AND `x` = %d AND `y` = %d;",
		vendings_table, map_getmapdata(sd->bl.m)->name, sd->bl.x, sd->bl.y) != SQL_SUCCESS )
	{
...
void clif_parse_GuildRequestInfo(int fd, struct map_session_data *sd)
{
	...
	if( !sd->status.guild_id && !sd->bg_id && (sd->status.grandcomp <= 0 && battle_config.desativarhgc) )
		return;
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
		p.grade = item->enchantgrade + item->despertar;
#endif
	}
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
				mailitem->grade = item->enchantgrade + item->despertar;
#endif
...
void clif_mail_removeitem( struct map_session_data* sd, bool success, int index, int amount ){
	...
	for( int i = 0; i < MAIL_MAX_ITEM; i++ ){
		...
		if (sd->inventory_data[sd->mail.item[i].index] == nullptr) {
			break;
		}
...
void clif_bossmapdevon(struct map_session_data *sd, int x, int y)
{
	if(!sd) return;
	...
	safestrncpy(WFIFOCP(fd,19), "Fate Boss", NAME_LENGTH);
...
int clif_visual_guild_id(struct block_list *bl)
{
	...
	if(sd && sd->status.grandcomp > 0 && sd->guild)
		return sd->guild->guild_id;		
	else if( (bg_id = bg_team_get_id(bl)) > 0 && (bg = util::umap_find(bg_team_db, bg_team_get_id(bl))) && bg->g )
...
// devon
void clif_sendbgemblem_pkmode(int fd, struct map_session_data *sd)
{
	nullpo_retv(sd);
...
void clif_parse_bg_queue_apply_request(int fd, struct map_session_data *sd)
{
	...
	if (type == 1) // Solo
		gr_queue_join_solo_gr(name, sd);
	else if (type == 2) // Party
		gr_queue_join_party(name, sd);
	else if (type == 4) // Guild
		gr_queue_join_guild(name, sd);
	else {
...
void clif_instance_create(int instance_id, int num)
{
	...
	WBUFW(buf,0) = 0x2cb;
	if(db->id >= 200 && db->id <= 206)
		safestrncpy(WBUFCP(buf,2), "Fenda", INSTANCE_NAME_LENGTH);
	else
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
	p.grade = item_data->enchantgrade + item_data->despertar;
#endif
...
static void clif_parse_ReqOpenBuyingStore( int fd, struct map_session_data* sd ){
	...
	char out_msg[1024];
	memset(out_msg, '\0', sizeof(out_msg));
	strcat(strcat(strcat(strcat(out_msg, "["), "Zeny"), "] "), storename);


	buyingstore_create( sd, p->zenyLimit, p->result, out_msg, p->items, packet_len / sizeof( struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub ), NULL );
}
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
		p->items[i].grade = ssitem->enchantgrade + ssitem->despertar;
#endif
#endif
	}
...
void clif_sub_ranklist(unsigned char *buf,int idx,struct map_session_data* sd, enum e_rank rankingtype){
	...
	switch(rankingtype) {
		...
		case RANK_KNIGHT:		list = knight_fame_list; break;
		case RANK_MINSTREL:		list = minstrel_fame_list; break;
...
void clif_parse_roulette_open( int fd, struct map_session_data* sd ){
	...
	npc_event(sd,"entradapk::OnChamada",0);
...
static uint8 clif_roulette_getitem(struct map_session_data *sd) {
	...
	if ((res = pc_additem(sd, &it, rd.qty[sd->roulette.prizeStage][sd->roulette.prizeIdx], LOG_TYPE_ROULETTE)) == 0) {
		...
		int at = 0;
		if (Sql_Query(mmysql_handle,
			"SELECT `id` FROM `log_roleta` WHERE `item_id` = %d;",
			it.nameid ) == SQL_SUCCESS )
		{
			at = (uint16)Sql_NumRows(mmysql_handle);
		}
		if(at > 0){
			Sql_Query(mmysql_handle,
			"UPDATE `log_roleta` SET `qtd`=`qtd`+1 WHERE item_id = %d;",
			it.nameid );
		} else {
			Sql_Query(mmysql_handle,
			"INSERT INTO `log_roleta`(`item_id`) VALUES (%d);",
			it.nameid );
		}
		
		Sql_Query(mmysql_handle,
			"INSERT INTO `log_roleta_user`(`char_id`, `account_id`, `item_id`, `amount`) VALUES (%d, %d, %d, %d);",
			sd->status.char_id, sd->status.account_id, it.nameid, rd.qty[sd->roulette.prizeStage][sd->roulette.prizeIdx] );
	} else {
			...
			msg.item[0].nameid = it.nameid;

			msg.item[0].amount = rd.qty[sd->roulette.prizeStage][sd->roulette.prizeIdx];
...
void clif_parse_roulette_generate( int fd, struct map_session_data* sd ){
	...
	if( !sd->roulette.stage && sd->roulette_point.bronze <= 0 && sd->roulette_point.silver < 10 && sd->roulette_point.gold < 10 ){
		...
		int rand = rnd()%10000;
		int i, d, stagio = rd.items[sd->roulette.stage] - 1;

		//ShowWarning("rand = %d \n", rand);

		for(i = 0; i < rd.items[sd->roulette.stage]; i++){
				//ShowWarning("Item = %d \n", rd.nameid[sd->roulette.stage][i]);
				//ShowWarning("Chance = %d \n", rd.chance[sd->roulette.stage][i]);
			
			if(rand > d && rand <= (d + rd.chance[sd->roulette.stage][i])){
				stagio = i;
				//ShowWarning("Item sorteado = %d \n", rd.nameid[sd->roulette.stage][i]);
				break;
			} else
				d = d + rd.chance[sd->roulette.stage][i];
		}
		
		if(stagio < 0){
			clif_displaymessage(sd->fd, "[Roleta] fale com a administração.");
			return;
		}
	
		sd->roulette.prizeStage = sd->roulette.stage;
		sd->roulette.prizeIdx = stagio;
		sd->roulette.claimPrize = true;  
		sd->roulette.tick = gettick();

		if( rd.flag[sd->roulette.stage][sd->roulette.prizeIdx]&1 ){
			result = GENERATE_ROULETTE_LOSING;
			sd->roulette.stage = 0;
			sd->roulette.failed = true;
		}else{
...
void clif_parse_private_airship_request( int fd, struct map_session_data* sd ){
	...
	// devon
	if(item_id == 6909){
			if (sd->sc.data[SC_EFEITO_RH_7] && battle_config.prevent_logout && sd->canlog_pk_tick != 0 && DIFF_TICK(gettick(), sd->canlog_pk_tick) < battle_config.prevent_logout){
				clif_displaymessage(sd->fd, "[Modo PK] Não é possível utilizar teleportes durante uma batalha. Tente novamente em 10 segundos.");
				return;
			}

		pc_setpos( sd, mapindex, 0, 0, CLR_TELEPORT );
	} else {
		npc_event_do_id("naviInfo::OnInfo", sd->status.account_id);
	}
...
void clif_refineui_info( struct map_session_data* sd, uint16 index ){
	...
	if( 
			...
			item->nameid == 24426 ||
			item->nameid == 24770){
			if(p->blacksmithBlessing == 4)
				p->blacksmithBlessing = 44;
			else
	...
	if(sd->state.bsb && p->blacksmithBlessing > 0){
		clif_displaymessage(sd->fd, "[@bsb] Comando reconhecido, lembre-se de conferir se suas BSBs estão em seu inventário antes de refinar.");
	}
...
void clif_parse_refineui_refine( int fd, struct map_session_data* sd ){
	...
	int16 blacksmith_index = -1, blacksmith_index_n = pc_search_inventory( sd, 44444 );
	uint16 blacksmith_amount = 0;
	int comeco = 0;

	if(sd->state.bsb && info->blessing_amount > 0 && material != 6439 && material != 6438 && material != 70002 && material != 70001){
		p->blacksmithBlessing = (uint8)info->blessing_amount;
	}

	if(info->blessing_amount > 0 && blacksmith_index_n > -1 && material != 6439 && material != 6438 && material != 70002 && material != 70001 ){
		p->blacksmithBlessing = (uint8)info->blessing_amount;
		comeco = 1;
	}
	...
	if( p->blacksmithBlessing != 0 ){
		...
		if( 
			...
			item->nameid == 24426 ||
			item->nameid == 24770){
			if((uint8)info->blessing_amount == 4)
				blacksmith_amount = 44;
			else
		...
		if(comeco){
			...
			if( (( blacksmith_index = pc_search_inventory( sd, ITEMID_BLACKSMITH_BLESSING ) ) < 0) && ( blacksmith_index_n < 0) ){
				return;
			}

			int totalbless = sd->inventory.u.items_inventory[blacksmith_index].amount + sd->inventory.u.items_inventory[blacksmith_index_n].amount;
			// Check if the player has enough blacksmith blessings
			if( totalbless < blacksmith_amount ){
				blacksmith_amount = 0;
			}
		} else {
			// Check if the player has blacksmith blessings
	...
	int inv_bsb = sd->inventory.u.items_inventory[blacksmith_index].amount, inv_bsb_nn = 0;
	
	if(sd->state.bsb && sd->inventory.u.items_inventory[index].refine >= 7 && 
		sd->inventory.u.items_inventory[index].refine <= 14 && ((sd->inventory_data[index]->weapon_level >= 1 && sd->inventory_data[index]->weapon_level <= 4) || sd->inventory_data[index]->armor_level == 1)) {
		int totaltrava = 0;
		if(blacksmith_amount > 0){
			if(blacksmith_index_n && sd->inventory.u.items_inventory[blacksmith_index_n].amount > 0) totaltrava += sd->inventory.u.items_inventory[blacksmith_index_n].amount;
			if(blacksmith_index && sd->inventory.u.items_inventory[blacksmith_index].amount > 0) totaltrava += sd->inventory.u.items_inventory[blacksmith_index].amount;
			if(totaltrava < blacksmith_amount) {
				clif_displaymessage(sd->fd, "[@bsb] Você não possui a quantidade de Bênção(s) do Ferreiro para refinar com segurança, desative o camando ou volte com a quantidade necessária.");
				return;
			}
		} else {
				clif_displaymessage(sd->fd, "[@bsb] Você não possui a quantidade de Bênção(s) do Ferreiro para refinar com segurança, desative o camando ou volte com a quantidade necessária.");
	...
	if(comeco){
		inv_bsb_nn = sd->inventory.u.items_inventory[blacksmith_index_n].amount;
		if( blacksmith_amount > 0) {
			if(sd->inventory.u.items_inventory[blacksmith_index_n].amount >= blacksmith_amount){
				if(pc_delitem( sd, blacksmith_index_n, blacksmith_amount, 0, 0, LOG_TYPE_CONSUME )) return;
			} else {
				int restante = blacksmith_amount - sd->inventory.u.items_inventory[blacksmith_index_n].amount;
				if(pc_delitem( sd, blacksmith_index_n, sd->inventory.u.items_inventory[blacksmith_index_n].amount, 0, 0, LOG_TYPE_CONSUME )) return;
				if(pc_delitem( sd, blacksmith_index, restante, 0, 0, LOG_TYPE_CONSUME )) return;
			}
		}
	} else {
	...
	if( cost->chance >= ( rnd() % 10000 ) ){
		...
		// devon historico MVP card
		if( Sql_Query( mmysql_handle, "INSERT INTO `log_refinamento` (`char_id`, `item_id`, `refinamento`, `sucess`, `qtd_bsb`, `inv_bsb`, `inv_bsb_nn`, `card0`, `card1`, `card2`, `card3`) VALUES( %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d );",
			sd->status.char_id, item->nameid, item->refine, 0, blacksmith_amount, inv_bsb, inv_bsb_nn, item->card[0], item->card[1], item->card[2], item->card[3] ) != SQL_SUCCESS ){
			Sql_ShowDebug(mmysql_handle);
		}
		...
		if( blacksmith_amount > 0 ){
			...
			blacksmith_index_n = pc_search_inventory( sd, 44444 );
			if(comeco && blacksmith_index_n == -1){
				clif_displaymessage(sd->fd, "AVISO: Suas BSBs [NN] Acabaram, portanto não esqueça de selecionar a BSB normal antes de tentar refinar novamente.");
			}
...
void clif_reputation(struct map_session_data *sd)
{
#if PACKETVER_MAIN_NUM >= 20201104 || PACKETVER_RE_NUM >= 20211103 || PACKETVER_ZERO_NUM >= 20201118
	nullpo_retv( sd );
	...
	int len = sizeof( struct PACKET_ZC_REPUTATION ) + MAX_REPUTATION * sizeof( struct PACKET_ZC_REPUTATION_sub );

	WFIFOHEAD( fd, len );

	struct PACKET_ZC_REPUTATION *p = (struct PACKET_ZC_REPUTATION *)WFIFOP( fd, 0 );

	p->PacketType = HEADER_PACKET_ZC_REPUTATION;
	p->PacketLength = len;
	p->refresh = 1;

	for( int i = 0; i < MAX_REPUTATION; i++ ) {
		p->list[i].id = i+1;
		p->list[i].useless1 = 0;
		p->list[i].useless2 = 0;
		p->list[i].useless3 = 0;
		p->list[i].points = sd->reputation[i].points;
	}
...
void clif_reputation_open(struct map_session_data *sd, uint8 groupID, uint8 reputID)
{
#if PACKETVER_MAIN_NUM >= 20201104 || PACKETVER_RE_NUM >= 20211103 || PACKETVER_ZERO_NUM >= 20201118
	nullpo_retv( sd );
	...
	int len = sizeof( struct PACKET_ZC_REPUTATION_OPEN );

	WFIFOHEAD( fd, len );

	struct PACKET_ZC_REPUTATION_OPEN *p = (struct PACKET_ZC_REPUTATION_OPEN *)WFIFOP( fd, 0 );

	p->PacketType = HEADER_PACKET_ZC_REPUTATION_OPEN;
	p->groupID = groupID;
	p->useless1 = 0;
	p->useless2 = 0;
	p->useless3 = 0;
...
void clif_stylist_response( struct map_session_data* sd, bool failed ){
	...
	//clif_send( &p, sizeof( p ), &sd->bl, SELF );
	if(!failed){
		clif_displaymessage(sd->fd, "[Estilista] A alteração de estilo falhou.");
	} else {
		clif_ui_open( sd, OUT_UI_STYLIST, 0 );
		clif_displaymessage(sd->fd, "[Estilista] A alteração de estilo foi um sucesso!");
	}
...
bool clif_parse_stylist_buy_sub( struct map_session_data* sd, _look look, int16 index ){
	...
	switch( look ){
		case LOOK_HAIR: {
			if(sd->group_id == 0 && entry->value > 29){
				clif_displaymessage(sd->fd, "[Estilista] Apenas jogadores VIP podem utilizar esse cabelo, compre dias VIP na loja de ROPs ou de votos.");
				return false;
			}
			int cabeloconta = pc_readglobalreg(sd, add_str("#PACOTECABELO"));
			int cabeloperso = pc_readglobalreg(sd, add_str("pacotecabelos"));
			if(!cabeloconta && !cabeloperso && entry->value > 42){
				clif_displaymessage(sd->fd, "[Estilista] Para utilizar esse cabelo você deve comprar um Pacote de Cabelos na Loja de ROPs.");
				return false;
			}
		}
		break;
		case LOOK_BODY2: {
			//ShowWarning("classe: %d \n ",entry->classe);
			if(entry->classe != sd->status.class_){
				clif_displaymessage(sd->fd, "[Estilista] Você não pode se tornar essa classe.");
				return false;
			}
			int third = pc_readglobalreg(sd, add_str("#SPRITETHIRD"));
			int trans = pc_readglobalreg(sd, add_str("#SPRITETRANS"));
			int expand = pc_readglobalreg(sd, add_str("#SPRITEEXPAND"));
			int segunda = pc_readglobalreg(sd, add_str("#SPRITESECOND"));
			if(((entry->value >= 4332 && entry->value <= 4344) || (entry->value >= 4060 && entry->value <= 4079)) && !third){
				clif_displaymessage(sd->fd, "[Estilista] Essa é uma roupa exclusiva do 'Ticket de Sprites', compre um e utilize a versão 3RD.");
				return false;
			}
			if(entry->value >= 4008 && entry->value <= 4021 && !trans){
				clif_displaymessage(sd->fd, "[Estilista] Essa é uma roupa exclusiva do 'Ticket de Sprites', compre um e utilize a versão Transclasse.");
				return false;
			}
			if(entry->value >= 7 && entry->value <= 20 && !segunda){
				clif_displaymessage(sd->fd, "[Estilista] Essa é uma roupa exclusiva do 'Ticket de Sprites 2ND', conquiste um na Roleta da Sorte e utilize.");
				return false;
			}
			if(entry->value == JOB_STAR_GLADIATOR && !expand){
				clif_displaymessage(sd->fd, "[Estilista] Essa é uma roupa exclusiva do 'Ticket de Sprites Expandidas', compre um na Loja de ROPs e utilize.");
				return false;
			}
			if(entry->value == JOB_SOUL_LINKER && !expand){
				clif_displaymessage(sd->fd, "[Estilista] Essa é uma roupa exclusiva do 'Ticket de Sprites Expandidas', compre um na Loja de ROPs e utilize.");
				return false;
			}
			if(entry->value == 0 && !expand){
				clif_displaymessage(sd->fd, "[Estilista] Essa é uma roupa exclusiva do 'Ticket de Sprites Expandidas', compre um na Loja de ROPs e utilize.");
				return false;
			}
			if(((entry->value >= 23 && entry->value <= 25) || (entry->value >= 4211 && entry->value <= 4240)) && !expand){
				clif_displaymessage(sd->fd, "[Estilista] Essa é uma roupa exclusiva do 'Ticket de Sprites Expandidas', compre um e utilize.");
				return false;
			}
		}
		break;
...
void clif_parse_stylist_buy2( int fd, map_session_data* sd ){

#if PACKETVER >= 20240101
	static std::map<uint32, _look> lookMap = {
		{0, LOOK_HAIR_COLOR},
		{1, LOOK_HAIR},
		{2, LOOK_CLOTHES_COLOR},
		{3, LOOK_HEAD_TOP},
		{4, LOOK_HEAD_MID},
		{5, LOOK_HEAD_BOTTOM},
		{6, LOOK_BODY2},
		{8, LOOK_CLOTHES_COLOR},
		{9, LOOK_BODY2},
	};

	struct PACKET_CZ_REQ_STYLE_CHANGE3* p = (struct PACKET_CZ_REQ_STYLE_CHANGE3*)RFIFOP( fd, 0 );

	int count = ( p->packetLength - sizeof( struct PACKET_CZ_REQ_STYLE_CHANGE3 ) ) / sizeof( struct PACKET_STYLE_CHANGE3_sub );
	...
	for(int i=0; i<count; i++){
		//ShowError("look %d - Index = %d \n", p->list[i].look, p->list[i].index);
		auto it = lookMap.find(p->list[i].look);
		if (it == lookMap.end()) {
			ShowError("clif_parse_stylist_buy2: no mapping found in lookMap for value %d\n", p->list[i].look);
			clif_stylist_response( sd, false );
			return;
		}
/*
		if(it->second == LOOK_BODY2 && ( sd->class_ & JOBL_THIRD ) == 0 && ( sd->class_ & JOBL_FOURTH ) != 0){
			clif_stylist_response( sd, false );
			return;
		}
	*/	
		if(!clif_parse_stylist_buy_sub( sd, it->second, p->list[i].index )){
			clif_stylist_response( sd, false );
			return;
		}
	}
...
void clif_goldpc_points(struct map_session_data* sd) {
	...
	if( sd->state.autotrade ||  sd->state.presencaparado ) {
		if(sd->sc.data[SC_PCCAFE_PLAY_TIME])
			status_change_end(&sd->bl, SC_PCCAFE_PLAY_TIME, INVALID_TIMER);
		return;
	}

	int value = pc_readglobalreg(sd, add_str("LASTTIMEPRESENCA"));
	if(value) pc_setglobalreg(sd, add_str("LASTTIMEPRESENCA"),0);

       // If system disabled then remove status
       if (!battle_config.feature_goldpc_timer)
               status_change_end(&sd->bl, SC_PCCAFE_PLAY_TIME, INVALID_TIMER);
       else if(!sd->sc.data[SC_PCCAFE_PLAY_TIME])
               sc_start(&sd->bl, &sd->bl, SC_PCCAFE_PLAY_TIME, 100, 0, value ? value : 3600000);

	if(!sd->sc.data[SC_PRESENCA_PRT])
               sc_start(&sd->bl, &sd->bl, SC_PRESENCA_PRT, 100, 0, 60000);


       p.packetType = HEADER_ZC_GOLDPCCAFE_POINT;
       p.show = battle_config.feature_goldpc_timer ? 1 : 0;

	if(pc_isvip(sd) || sd->group_id >= 10)
		p.type = 2;
	else
	...
	p.time = 0;
	const struct TimerData *td = (sd && sd->sc.data[SC_PCCAFE_PLAY_TIME] ? get_timer(sd->sc.data[SC_PCCAFE_PLAY_TIME]->timer) : NULL);
	if (td) p.time = sd->sc.data[SC_PCCAFE_PLAY_TIME] ? (3600 - static_cast<int32>(DIFF_TICK(get_timer(sd->sc.data[SC_PCCAFE_PLAY_TIME]->timer)->tick, gettick()) / 1000)) : 0;


       clif_send(&p, sizeof(p), &sd->bl, SELF);
}

void clif_prontera(struct map_session_data* sd) {
    nullpo_retv(sd);

	if( sd->state.autotrade) {
		if(sd->sc.data[SC_PRESENCA_PRT])
			status_change_end(&sd->bl, SC_PRESENCA_PRT, INVALID_TIMER);
		return;
	}
...
void clif_goldpc_npc_request(int fd, struct map_session_data* sd) {
       ...
       npc_event_do_id("Maquina de Tickets::OnClick", sd->status.account_id);
}
...
void clif_parse_laphine_synthesis( int fd, struct map_session_data* sd ){
	...
	for( size_t i = 0; i < count; i++ ){
		int16 indexx = server_index( p->items[i].index );

		if( indexx >= MAX_INVENTORY ){
			return;
		}
		...
		struct item* item = &sd->inventory.u.items_inventory[indexx];

		if( synthesis->cardsAllowed ){
			for( int l = 0; l < MAX_SLOTS; l++ ){
				if( item->card[l] != 0 ){
					struct item tmp_item;
					memset(&tmp_item,0,sizeof(tmp_item));
					tmp_item.nameid = item->card[l];
					tmp_item.amount = 1;
					tmp_item.identify = 1;
					pc_additem(sd, &tmp_item, 1, LOG_TYPE_NONE);
				}
			}
		}
	}
...
void clif_parse_laphine_upgrade( int fd, struct map_session_data* sd ){
	...
	if(p->itemId == 11153 && sd->pd){
		clif_displaymessage(sd->fd, "[Encantamento de Mascotes] Volte seu pet para o ovo antes de encantar.");
		return;
	}
	...
	//ShowWarning("Slot init: %d",upgrade->randomOptionGroup->slot_init);
	//ShowWarning("Item id: %d", p->itemId);
	//ShowWarning("Itens option id: %d %d %d",item->option[0].id,item->option[1].id,item->option[2].id);
	
	if(upgrade->randomOptionGroup != nullptr && (p->itemId == 47041 || p->itemId == 47042 || p->itemId == 47043 || p->itemId == 47083 || p->itemId == 47085 || p->itemId == 47086 ) && (upgrade->randomOptionGroup->slot_init == 3 && (!item->option[0].id || !item->option[1].id))){
		
		clif_displaymessage(sd->fd, "[Aviso] Não é possível adicionar o encantamento nesse slot sem ter adicionado nos anteriores.");
		return;
	}
	
	if(item->option[2].id && (p->itemId == 47041 || p->itemId == 47042 || p->itemId == 47043 || p->itemId == 47083 || p->itemId == 47085 || p->itemId == 47086)){
			clif_displaymessage(sd->fd, "[Aviso] Você já possui encantamento na 3a Linha.");
			return;
	}
	if(upgrade->randomOptionGroup != nullptr && !item->option[1].id && (p->itemId == 46121 || p->itemId == 46120)){
		clif_displaymessage(sd->fd, "[Aviso] Não é possível adicionar o encantamento nesse slot sem ter adicionado nos anteriores.");
		return;
	}
	if(upgrade->randomOptionGroup != nullptr && upgrade->randomOptionGroup->slot_init > 0){
		int inislot = 0;
		if(upgrade->randomOptionGroup->slot_init == 3){
			inislot = 1;
		}
		if(upgrade->randomOptionGroup->slot_init == 4){
			inislot = 2;
		}
		if(!item->option[inislot].id){
			clif_displaymessage(sd->fd, "[Aviso] Não é possível adicionar o encantamento nesse slot sem ter adicionado nos anteriores.");
			return;
		}
	}
	...
	// devon
	if(p->itemId == 46070 || p->itemId == 47029 || p->itemId == 47030 || p->itemId == 47082 || p->itemId == 47036){
		while(item->option[0].id == item->option[1].id){
			upgrade->randomOptionGroup->apply( *item );
		}
	}
	int contador = 0;
	if(p->itemId == 47083){
		while(item->option[0].id == item->option[1].id){
			...
			upgrade->randomOptionGroup->apply( *item );
			if(contador >= 6){
				clif_displaymessage(sd->fd, "[Aviso] Não é possivel encantar esse item nessa linha pois ele possui encantamentos repetidos em linhas anteriores, altere antes de tentar novamente.");
				return;
			}
			while(item->option[0].id == item->option[2].id){
				contador++;
				upgrade->randomOptionGroup->apply( *item );
				if(contador >= 6){
					clif_displaymessage(sd->fd, "[Aviso] Não é possivel encantar esse item nessa linha pois ele possui encantamentos repetidos em linhas anteriores, altere antes de tentar novamente.");
	...
	if(p->itemId == 47084){
		while(item->option[1].id == item->option[0].id){
			contador++;
			upgrade->randomOptionGroup->apply( *item );
			if(contador >= 6){
				clif_displaymessage(sd->fd, "[Aviso] Não é possivel encantar esse item nessa linha pois ele possui encantamentos repetidos em linhas anteriores, altere antes de tentar novamente.");
			...
			while(item->option[1].id == item->option[2].id){
				contador++;
				upgrade->randomOptionGroup->apply( *item );
				if(contador >= 6){
					clif_displaymessage(sd->fd, "[Aviso] Não é possivel encantar esse item nessa linha pois ele possui encantamentos repetidos em linhas anteriores, altere antes de tentar novamente.");
	...
	if(p->itemId == 47085){
		while(item->option[2].id == item->option[1].id){
			contador++;
			upgrade->randomOptionGroup->apply( *item );
			if(contador >= 6){
				clif_displaymessage(sd->fd, "[Aviso] Não é possivel encantar esse item nessa linha pois ele possui encantamentos repetidos em linhas anteriores, altere antes de tentar novamente.");
			...
			while(item->option[2].id == item->option[0].id){
				contador++;
				upgrade->randomOptionGroup->apply( *item );
				if(contador >= 6){
					clif_displaymessage(sd->fd, "[Aviso] Não é possivel encantar esse item nessa linha pois ele possui encantamentos repetidos em linhas anteriores, altere antes de tentar novamente.");
	...
	if(upgrade->consomeItem){
		int16 indexconsome = pc_search_inventory( sd, upgrade->consomeItem );
		
		if( indexconsome < 0 ){
			clif_displaymessage(sd->fd, "[Aviso] Você não possui os itens requisitos para refinar esse item, confira a descrição do item de refinamento.");
			return;
		}
		
		if( pc_delitem( sd, indexconsome, upgrade->consomeAmount, 0, 0, LOG_TYPE_LAPHINE ) != 0 ){
			clif_displaymessage(sd->fd, "[Aviso] Você não possui os itens requisitos para refinar esse item, confira a descrição do item de refinamento.");
			return;
		}
	}
	...
	if( upgrade->adicionalRefine > 0 ){
		// Absolute refine level change
		item->refine = item->refine + upgrade->adicionalRefine;
	}
	else if( upgrade->resultRefine > 0 ){
		// Absolute refine level change
		...
		if( upgrade->resultRefineMinimum ){
			...
			//ShowWarning("%d\n",p->itemId);
			// DEVON -> Logica para martelo Sombrio
			if(p->itemId == 23436){
				int novoresult = rnd_value(1,100);
				if(novoresult == 50) item->refine = 10;
				else if(novoresult >= 61 && novoresult <= 63) item->refine = 9;
				else item->refine = rnd_value( upgrade->resultRefineMinimum, 8 );
			} else {
...
// Notifiy the client to open the ReformUI with a given item id.
// 0b8f <itemid>.L (ZC_ITEM_REFORM_OPEN)
void clif_item_reform_open(struct map_session_data* sd, const uint32 InReformItemID)
{
	nullpo_retv(sd);

	sd->state.ReformItemID = InReformItemID;

	PACKET_ZC_ITEM_REFORM_OPEN Packet = {};

	Packet.ID = HEADER_ZC_ITEM_REFORM_OPEN;
	Packet.ReformItemID = InReformItemID;

	clif_send(&Packet, sizeof(PACKET_ZC_ITEM_REFORM_OPEN), &sd->bl, SELF);
}

// Notify the server that the client had closed the ReformUI.
// 0b90 (CZ_ITEM_REFORM_CLOSE)
void clif_parse_item_reform_close(int fd, struct map_session_data* sd)
{
	sd->state.ReformItemID = 0;
	sd->state.bReformItemScope = false;
}

// Sends the parsed result from ReformUI information.
// 0b92 <ItemInventoryID>.W <Failure>.B (ZC_ITEM_REFORM_RESULT)
void clif_item_reform_result(struct map_session_data* sd, const uint16 InItemInventoryID, const bool bInFailure)
{
	nullpo_retv(sd);

	PACKET_ZC_ITEM_REFORM_RESULT Packet = {};
	Packet.ID = HEADER_ZC_ITEM_REFORM_RESULT;
	Packet.ItemInventoryID = InItemInventoryID;
	Packet.Failure = bInFailure;

	clif_send(&Packet, sizeof(PACKET_ZC_ITEM_REFORM_RESULT), &sd->bl, SELF);
}

// Parses the ReformUI request information.
void clif_item_reform(struct map_session_data* sd, const uint32 InReformItemID, const uint16 InItemInventoryID)
{
	nullpo_retv(sd);

	const uint16 ServerItemInventoryID = server_index(InItemInventoryID);

	std::shared_ptr<RReformList> ReformList = ItemReformDB.find(InReformItemID);
	if (!ReformList)
	{
		ShowError("clif_item_reform: %u is not a valid Reform ID value.\n", InReformItemID);
		return;
	}

	item* ReformItem = &sd->inventory.u.items_inventory[ServerItemInventoryID];

	if (!ReformItem->identify)
	{
		clif_item_reform_result(sd, InItemInventoryID, true);
		return;
	}
	...
	std::shared_ptr<RReformInfo> ReformInfo = util::umap_find(ReformList->ReformInfo, ReformItem->nameid);
	if (!ReformInfo)
	{
		ShowError("clif_item_reform: %u is not a valid base item from %u item reform.\n", ReformItem->nameid, InReformItemID);
		return;
	}

	if (ReformItem->refine < ReformInfo->RefineMin)
	{
		clif_item_reform_result(sd, InItemInventoryID, true);
		return;
	}

	if (ReformItem->refine > ReformInfo->RefineMax)
	{
		clif_item_reform_result(sd, InItemInventoryID, true);
		return;
	}

	if (!ReformInfo->Flags.bCardsAllowed)
	{
		for (size_t i = 0; i < MAX_SLOTS; i++)
		{
			if (ReformItem->card[i] != 0)
			{
				clif_item_reform_result(sd, InItemInventoryID, true);
				return;
			}
	...
	if (ReformInfo->RequiredRandomOptions > 0)
	{
		int16 RandomOptionsCount = 0;
		for (RandomOptionsCount = MAX_ITEM_RDM_OPT - 1; RandomOptionsCount >= 0; --RandomOptionsCount)
		{
			if (ReformItem->option[RandomOptionsCount].id != 0)
			{
				break;
			}
		}

		if (ReformInfo->RequiredRandomOptions > (RandomOptionsCount + 1))
		{
			clif_item_reform_result(sd, InItemInventoryID, true);
			return;
		}
	...
	struct InventoryMaterial
	{
		int16 InventoryIndex;
		uint16 Amount;
	};

	std::vector<InventoryMaterial> ExistentMaterials;
	for (const RReformMaterial& Material : ReformInfo->Material)
	{
		const int16 InventoryIndex = pc_search_inventory(sd, Material.ItemID);
		...
		if (sd->inventory.u.items_inventory[InventoryIndex].amount < Material.Amount)
		{
			clif_item_reform_result(sd, InItemInventoryID, true);
			return;
		}

		ExistentMaterials.push_back({ InventoryIndex, Material.Amount });
	}

	// Script was called inside an item scope.
	if (!sd->state.bReformItemScope)
	{
		const int16 ServerItemReformIndex = pc_search_inventory(sd, InReformItemID);

		// Reform Item ID was not found.
		if (ServerItemReformIndex < 0)
		{
			clif_item_reform_result(sd, InItemInventoryID, true);
			return;
		}

		// Consumable must be delay consume.
		if ((sd->inventory_data[ServerItemReformIndex]->flag.delay_consume & DELAYCONSUME_NOCONSUME) == 0)
		{
			if (pc_delitem(sd, ServerItemReformIndex, 1, 0, 0, LOG_TYPE_CONSUME) != 0)
			{
				return;
			}
		}
	...
	// Deletes all materials from the inventory.
	for (const InventoryMaterial& Material : ExistentMaterials)
	{
		if (pc_delitem(sd, Material.InventoryIndex, Material.Amount, 0, 0, LOG_TYPE_REFORM) != 0)
		{
			return;
		}
	}

	// Updates the Base Item ID to the new Result Item ID.
	ReformItem->nameid = ReformInfo->ResultItemID;
	// devon
	sd->inventory_data[ServerItemInventoryID] = itemdb_search( ReformInfo->ResultItemID );

	int8 Refine = 0;
	if (ReformItem->refine + ReformInfo->ChangeRefineValue > MAX_REFINE)
	{
		Refine = MAX_REFINE;
	}
	else if (ReformItem->refine + ReformInfo->ChangeRefineValue < 0)
	{
		Refine = 0;
	}
	else
	{
		Refine = ReformItem->refine + ReformInfo->ChangeRefineValue;
	}

	// Updates the item refine to the new refine after refine delta.
	ReformItem->refine = Refine;

	// Preserves the Item Enchant Grade if the PreserveGrade flag is set to true.
	if (!ReformInfo->Flags.bPreserveGrade)
	{
		ReformItem->enchantgrade = 0;
	}

	// Preserves all Item Sockets Items if the PreserveSockets flag is set to true.
	if (!ReformInfo->Flags.bPreserveSockets)
	{
		std::memset(ReformItem->card, 0, sizeof(ReformItem->card));
	}

	// Log removal of item
	log_pick_pc(sd, LOG_TYPE_REFORM, -1, ReformItem);

	// Visually remove it from the client
	clif_delitem(sd, ServerItemInventoryID, 1, 0);

	// devon
	pc_setglobalreg(sd, add_str("relogarrefino"), 1);

	// Log retrieving the item again -> with the new options
	log_pick_pc(sd, LOG_TYPE_REFORM, 1, ReformItem);

	// Make it visible for the client again
	clif_additem(sd, ServerItemInventoryID, 1, 0);

	clif_item_reform_result(sd, client_index(ServerItemInventoryID), false);
	
	clif_inventorylist(sd);
}

// Sends the parsed result from ReformUI information.
// 0b91 <ReformItemID>.L <ItemInventoryID>.W (CZ_ITEM_REFORM_REQUEST)
void clif_parse_item_reform_request(int fd, struct map_session_data* sd)
{
	nullpo_retv(sd);

	PACKET_CZ_ITEM_REFORM_REQUEST Packet = *reinterpret_cast<PACKET_CZ_ITEM_REFORM_REQUEST*>(RFIFOP(fd, 0));
	Packet.ItemInventoryID = server_index(Packet.ItemInventoryID);

	if (!Packet.ReformItemID)
	{
		ShowDebug("clif_parse_item_reform_request: Null Reform ID value sent by client! (AID: %d, CID: %d).\n", sd->status.account_id, sd->status.char_id);
		return;
	}

	if (sd->state.ReformItemID != Packet.ReformItemID)
	{
		ShowDebug("clif_parse_item_reform_request: Reform ID value (%u) is different from 'Reform ID' value (%u) sent by client! (AID: %d, CID: %d).\n", sd->state.ReformItemID, Packet.ReformItemID, sd->status.account_id, sd->status.char_id);
		return;
	}

	if (Packet.ItemInventoryID >= MAX_INVENTORY)
	{
		ShowDebug("clif_parse_item_reform_request: out-of-range item inventory index (%hu) sent by client! (AID: %d, CID: %d).\n", Packet.ItemInventoryID, sd->status.account_id, sd->status.char_id);
		return;
	}

	if (!sd->inventory_data[Packet.ItemInventoryID])
	{
		ShowDebug("clif_parse_item_reform_request: non existent item inventory index (%hu) sent by client! (AID: %d, CID: %d).\n", Packet.ItemInventoryID, sd->status.account_id, sd->status.char_id);
		return;
	}

	clif_item_reform(sd, Packet.ReformItemID, client_index(Packet.ItemInventoryID));
}
...
void clif_enchantgrade_add( struct map_session_data* sd, uint16 index = UINT16_MAX, std::shared_ptr<s_enchantgradelevel> gradeLevel = nullptr ){
	...
	if( index < UINT16_MAX ){
		...
		for( const auto& pair : gradeLevel->options ){
			...
			p->options[i].canBreak = option->breakarmor > 0;
			i++;
		}
...
void clif_parse_enchantgrade_add( int fd, struct map_session_data* sd ){
	...
	int levelwhere = sd->inventory_data[index]->weapon_level;

	if (sd->inventory_data[index]->type == IT_ARMOR)
		levelwhere = sd->inventory_data[index]->armor_level;


	const auto& enchantgradelevels = enchantgrade->levels.find( levelwhere );
...
// DEVON
TIMER_FUNC(clif_openenchant){
	map_session_data* sd = map_id2sd( id );

	if( sd != nullptr ){
		clif_enchantwindow_open( *sd, static_cast<int>(data) );
	}
...
void clif_enchantwindow_result( struct map_session_data& sd, bool success, t_itemid enchant = 0 ){
	...
	if( success ){
		...
		add_timer(gettick()+1100,clif_openenchant,sd.bl.id,sd.state.item_enchant_index);
	}else{
...
void clif_parse_enchantwindow_random_upgrade(int fd, map_session_data* sd) {
	...
	std::shared_ptr<s_item_enchant_random_upgrade> random_upgrade = util::umap_find(enchant_slot->random_upgrade.enchants, selected_item.card[slot]);

	if (random_upgrade == nullptr) {
		return;
	}

	if (sd->status.zeny < random_upgrade->zeny) {
		return;
	}

	std::unordered_map<uint16, uint16> materials;

	for (const auto& entry : random_upgrade->materials) {
		int16 idx = pc_search_inventory(sd, entry.first);
	...
	if (pc_payzeny(sd, random_upgrade->zeny, LOG_TYPE_ENCHANT, NULL) != 0) {
		return;
	}
	...
	//selected_item.card[slot] = random_upgrade->upgrade_item_id;
	size_t maximum = 3 * random_upgrade->enchants.size();
	bool enchanted = false;

	for (int i = 0; i < maximum; i++) {
		std::shared_ptr<s_item_enchant_normal_sub> random_enchant = util::umap_random(random_upgrade->enchants);

		if (rnd_value(0, 10000) < random_enchant->chance) {
			selected_item.card[slot] = random_enchant->item_id;
			enchanted = true;
			break;
		}
	}
...
void clif_updateSpecialPopup(map_session_data* sd){
	...
	if (map_getmapflag(m, MF_SPECIALPOPUP)) {
		clif_special_popup(sd, map_getmapflag(m, MF_SPECIALPOPUP));
	}
}

void clif_special_popup(map_session_data* sd, int popupId){
	...
	struct PACKET_ZC_SPECIALPOPUP *p = (struct PACKET_ZC_SPECIALPOPUP *)WFIFOP( fd, 0 );
