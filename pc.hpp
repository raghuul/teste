...
#define GOLDPC_POINTS_VAR "#PONTOSDEPRESENCA"
...
#define REPUTATION_VAR "#REPUTATION"
...
struct map_session_data {
	...
	struct s_state {
		...
		uint32 ReformItemID;
		bool bReformItemScope;
		// devon
		unsigned int rate_servidor;
		unsigned int temporada;
		unsigned int skillgx;
		unsigned int hidepet;
		unsigned int hidepetall;
		unsigned int nomado;
		unsigned int nopeco;
		unsigned int danofenda;
		unsigned int danoboss;
		unsigned int capfenda;
		unsigned int reducaofenda;
		unsigned int bsb;
		unsigned int protection_acc : 1; // Proteção de Conta - [Orce brAthena]
		unsigned int dias_assinatura;
		unsigned int assinatura_pk;
		unsigned int grade_shadow_colar;
		unsigned int grade_shadow_brinco;
		unsigned int no_exp;
		unsigned int pet_incubado;
		unsigned int visu_colecao;
		unsigned int dias_blackfriday;
		unsigned int percent_dano;
		unsigned int countavent;
		unsigned int drop_extra;
		unsigned int contafadiga;
		unsigned int contafadiga_pts;
		unsigned int contafadiga_delay;
		unsigned int biohq;
		unsigned int cheffhq;
		unsigned int aviso;
		unsigned int aviso_hgc;
		unsigned int goblinferido;
		unsigned int exceldrop;
		unsigned int nivelconteudo;
		
		//adriel
		//unsigned int teleporte_count;
		
		// fadiga
		unsigned int bio5_ext_drop;
		unsigned int cheff_ext_drop;
		unsigned int bio5_hard_drop;
		unsigned int cheff_hard_drop;
		unsigned int bio5_sav_drop;
		unsigned int cheff_sav_drop;
		unsigned int idarma;
		unsigned int idenchant;
		unsigned int mineracao;
		unsigned int herbologia;
		unsigned int pescaria;
		unsigned int eledespertar;
		unsigned int despertar;
		unsigned int despertarmenor;
		unsigned int despertartwo;
		unsigned int despertarmenortwo;
		unsigned int desp_ele_selected;
		unsigned int hora_prt;
		unsigned int hora_assina;
		unsigned int pkentregue;
		unsigned int timeflores;
		unsigned int timevulcao;
		unsigned int timeultimate;
		unsigned int add_timeflores;
		unsigned int add_timevulcao;
		unsigned int add_timeultimate;
		unsigned int dia_hoje;
		unsigned int serverteste;
		unsigned int arma_visual;
		unsigned int last_aumento;
		unsigned int last_minuto;
		unsigned int mobs_cheff_ext;
		unsigned int mobs_bio_ext;
		unsigned int count_mobs_ext;
		unsigned int stop_drop_ext;
		unsigned int resetou;
		unsigned int gpvmdano;
		unsigned int gpvmsobra;
		unsigned int avisoexp;
	...
	unsigned int contagrupo;
	unsigned int mobs_cheff_savage;
	unsigned int mobs_bio_savage;
	unsigned int danomilhao;
	unsigned int danorestante;
	unsigned int forceinidps;
	unsigned int timerdps;
	unsigned int killcount_timer;
	unsigned int killcount;
	time_t timermilagre;
	unsigned int cestapascoa;
	unsigned int engarrafar; 
	struct {
		unsigned int resetchar;
		unsigned int resetaccount;
		unsigned int bindconta;
		unsigned int burnreborn;
		unsigned int melhora_duasmaos;
		unsigned int idaura;
		unsigned int account_casamento;
		unsigned int parceiro_one;
		unsigned int parceiro_two;
		unsigned int parceiro_three;
		unsigned int parceiro_four;
		unsigned int classereborn;
		unsigned int temporadareborn;
		unsigned int daystempreborn;
		unsigned int doador;
		unsigned int doadordays;
		unsigned int doadortrava;
		unsigned int tipodanorank;
	...
	struct {
		...
		unsigned int totalkafra;
		unsigned int gastototalcash;
		unsigned int gastototalkafra;
		unsigned int horablack;
	} blackfriday_state;
	
	struct {
		unsigned int pandemonium;
		unsigned int fenda;
		unsigned int templo;
		unsigned int biocheff;
	...
	std::vector<s_item_bonus> skillatk, skillusesprate, skillusesp, skillheal, skillheal2, skillblown, skillcastrate, skillfixcastrate, subskill, skillcooldown, skillfixcast,
		skillvarcast, skilldelay, itemhealrate, add_def, add_mdef, add_mdmg, reseff, itemgrouphealrate, itemsphealrate, itemgroupsphealrate, skillatkdev, skillatkfinal;
	...
	struct s_bonus {
		...
		int speed_rate, speed_add_rate, aspd_add, dano_final, magic_dano_final, ungula, long_dano_final, short_dano_final, drop_final, tempo_hell, perfuracao, penetracao, exp_excelencia, reducao_final;
		int vulnera_fire_shadow, vulnera_water_undead, vulnera_wind_poison, vulnera_earth_neutral, vulnera_holy_ghost, vulnera_all, reduct_reflect, exp_final, corrupcao_fisico, corrupcao_magico, corrupcao_distancia, cap_milagre, determinacao;
		// Resistencia a status 2.0
		int resist_congela;
		int resist_caos;
		int resist_envenenar;
		int resist_hipotermia;
		int resist_maldicao;
		int resist_silence;
		// ignora res e mres
		int ignore_res, ignore_mres;
		...
		int add_heal_rate, add_heal2_rate, range_skill, metalic_skill;
		...
		int verifica_onea, verifica_oneb, verifica_onec, verifica_oned, verifica_onee, verifica_onef, verifica_oneg, verifica_oneh, verifica_onei, verifica_onej, picky_area, cap_limit;
	} bonus;
	...
	int killerrid, killedrid, killedgid, killedmobx, killedmoby;
...
static bool pc_cant_act2( struct map_session_data* sd ){
	return sd->state.vending || sd->state.buyingstore || (sd->sc.opt1 && sd->sc.opt1 != OPT1_BURNING)
		...
		|| sd->state.ReformItemID || (sd)->state.enchantgrade_open || sd->state.item_enchant_index;
}
...
#define pc_ismadogear(sd) ( (sd)->state.nomado ? 1 : (sd)->sc.option&OPTION_MADOGEAR )
// Rune Knight Dragon
#define pc_isridingdragon(sd) ( (sd)->state.nopeco ? 1 : (sd)->sc.option&OPTION_DRAGON )
...
extern struct fame_list knight_fame_list[MAX_FAME_LIST];
extern struct fame_list minstrel_fame_list[MAX_FAME_LIST];
