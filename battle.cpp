...
int64 battle_attr_fix(struct block_list *src, struct block_list *target, int64 damage,int atk_elem,int def_type, int def_lv)
{
	...
	if (sc && sc->count) { //increase dmg by src status
		if( sc->data[SC_BURNT_ZILANT] ) {
			uint16 skill_id = battle_getcurrentskill(src);
			if(skill_id == RK_DRAGONBREATH){
				ratio += 400;
			}
		}
		switch(atk_elem){
...
#ifdef RENEWAL
			if(target && map_getmapflag(target->m, MF_TEMPLOULTIMATE))
				ratio += 10;
			else
				ratio += 50;
#else
		...
		case BF_MAGIC:
			...
			if( tsd && !nk[NK_IGNOREDEFCARD] ) {
				...
				if( !nk[NK_IGNOREELEMENT] ) { // Affected by Element modifier bonuses
					...
					if(src->type == BL_MOB){
						if(ele_fix > 90) ele_fix = 90; // devon
					}
					cardfix = cardfix * (100 - ele_fix) / 100;
				}

				size_fix = tsd->indexed_bonus.subsize[sstatus->size] + tsd->indexed_bonus.subsize[SZ_ALL];
				if(size_fix > 95) size_fix = 95; // devon

				cardfix = cardfix * (100 - size_fix) / 100;
				...
				if(race_fix > 95) race_fix = 95; // devon

				cardfix = cardfix * (100 - race_fix) / 100;
				// devon
				if(sstatus->class_ == CLASS_BOSS){
					int classe = tsd->indexed_bonus.subclass[sstatus->class_] - tsd->indexed_bonus.subclass[CLASS_ALL];
					if(classe > 95) classe = 95;
					cardfix = cardfix * (100 - classe) / 100;
				} else {
		...
		case BF_WEAPON:
			...
			// Affected by target DEF bonuses
				...
				if( !nk[NK_IGNOREELEMENT] ) { // Affected by Element modifier bonuses
					...
					if(src->type == BL_MOB){
						if(ele_fix_lh > 90) ele_fix_lh = 90; // devon
					}
...
* @param src -> pessoa
**/
static void battle_absorb_damage(struct block_list *bl, struct Damage *d, uint16 skill_id, struct block_list *src,uint16 skill_lv) {
		...
		case BL_PC:
			{
				...
				if (!sd || map_getmapflag(sd->bl.m, MF_NOCAPDANO))
					return;
				...
				//} else {
					...
					// devon
					if(skill_id != MO_EXTREMITYFIST && !ssc->data[SC_EXEEDBREAK] && !ssc->data[SC_CRUSHSTRIKE] && skill_id != NJ_ISSEN && skill_id != SJ_NOVAEXPLOSING){
						int reduction = 40;
						if(sd->bonus.absorb_dmg_maxhp2){
							if(sd->bonus.absorb_dmg_maxhp2 >= 40) 
								reduction = reduction - (sd->bonus.absorb_dmg_maxhp2 - 20);
							else
								reduction = reduction - sd->bonus.absorb_dmg_maxhp2;
						}

						if(src->type == BL_PC){
							struct map_session_data *sd_src = BL_CAST(BL_PC, src);
							if(sd_src && sd_src->bonus.cap_reduction)
								reduction += sd_src->bonus.cap_reduction;
						}
						if (!sd->status.shield) reduction += 20;
						if(sc && sc->data[SC_AETERNA]) reduction += 15;
						if(sc && sc->data[SC_DARKCROW]) reduction += 15;

						if(sc && sc->data[SC_EFEITO_RH_6])
							reduction = 0;
						if (reduction) {
							int hp = reduction * status_get_max_hp(bl) / 100;
							if (dmg_ori > hp) {
								dmg_new = hp;
							}
							//if (ssc && ssc->data[SC_BIONIC_WOODENWARRIOR] && skill_id == GN_CARTCANNON)
							//	dmg_new = dmg_new /2;

							//if (skill_id == SKE_STAR_CANNON)
							//	dmg_new = dmg_new / ((skill_lv+1) / 2);

						}
					} else {
						sc_start(src,src,SC_EFEITO_RH_6,100,0,5000);
					}
				//}

				if (ssc && ssc->data[SC_CRUSHSTRIKE]) 
	...
	//if(sc && sc->data[SC_AETERNA] && skill_id != PF_SOULBURN) {
	//	if( skill_id != ASC_BREAKER || !(d->flag&BF_WEAPON) )
...
bool battle_status_block_damage(struct block_list *src, struct block_list *target, struct status_change *sc, struct Damage *d, int64 &damage, uint16 skill_id, uint16 skill_lv) {
	...
	if (!sc->data[SC_MAXPAIN] && skill_id != NPC_MAXPAIN_ATK && skill_id != NPC_MAXPAIN){
		
		// devon
		if ((mapdata->flag[MF_BIOHARD] || mapdata->flag[MF_VARMEAL]) && src->type == BL_MOB){
			if(mapdata->flag[MF_DROPBABY])
				damage += (damage / 100) * 70;
			else
				damage += damage;
		}
		if (mapdata->flag[MF_BIOEXTREME] && src->type == BL_MOB && damage > 0){
			damage += (damage / 100) * battle_config.dano_extremebio;
		}
		if (mapdata->flag[MF_CHEFFEXTREME] && src->type == BL_MOB && damage > 0){
			damage += (damage / 100) * battle_config.dano_extremecheff;
		}
		if (mapdata->flag[MF_BIOSAVAGE] && src->type == BL_MOB){
			damage += damage * 3;
			mob_data *md = BL_CAST(BL_MOB, src);
			if(md->db->id != 28026 && md->db->id != 28027 && md->db->id != 28028){
				damage += damage * 3;
			}
		}

		if (mapdata->flag[MF_ILLUSIONGUILD] && src->type == BL_MOB){
			damage += damage * 6;
		}
		if (mapdata->flag[MF_CONSTUNREAL] && src->type == BL_MOB){
			damage += damage * 3;
		}

		if ((mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP] || mapdata->flag[MF_TEMPLOSAGRADO]) && src->type == BL_MOB && damage > 0){
			if(skill_id == ASC_BREAKER){
				damage += damage * 3;
			}
			if(skill_id == WL_EARTHSTRAIN || skill_id == WZ_STORMGUST || skill_id == WZ_METEOR || skill_id == WZ_VERMILION){
				damage = 2000000000;
			}
		}
	}
	...
	int64 dano = damage;
	if ((mapdata->flag[MF_EQUIPSGVG] || mapdata_flag_gvg2(mapdata)) && target->type != BL_MOB && src->type != BL_MOB)
		dano = battle_calc_gvg_damage(src,target,dano,skill_id,d->flag);
	else if( mapdata->flag[MF_BATTLEGROUND] && target->type != BL_MOB && src->type != BL_MOB )
		dano = battle_calc_bg_damage(src,target,dano,skill_id,d->flag);
	else if ((mapdata->flag[MF_PVP] || ((TBL_PC *)target)->sc.data[SC_EFEITO_RH_7]) && target->type != BL_MOB && src->type != BL_MOB)
		dano = battle_calc_pvp_damage(src,target,dano,skill_id,d->flag);
	...
	if ((sce = sc->data[SC_KYRIE]) && damage > 0 && ((flag & BF_WEAPON) || skill_id == TF_THROWSTONE)) {
		sce->val2 -= static_cast<int>(cap_value(dano, INT_MIN, INT_MAX));
		if (sce->val2 >= 0)
			damage = 0;
		else
			damage = -sce->val2;
...
/*
	if ((sce = sc->data[SC_EFEITO_RH_14]) && damage > 0 && mapdata->flag[MF_FENDA]) {
		sce->val2 -= static_cast<int>(cap_value(dano, INT_MIN, INT_MAX));
		if (sce->val2 >= 0)
			damage = 0;
		else
			damage = -sce->val2;
		if (sce->val2 <= 0)
			status_change_end(target, SC_EFEITO_RH_14, INVALID_TIMER);
	}
*/
...
#ifdef RENEWAL
				if (group->val3 - dano > 0)
					group->val3 -= static_cast<int>(cap_value(dano, INT_MIN, INT_MAX));
				else
					skill_delunitgroup(group);
#endif
			}
	...
	if ((sce = sc->data[SC_MILLENNIUMSHIELD]) && sce->val2 > 0 && damage > 0) {
		sce->val3 -= static_cast<int>(cap_value(dano, INT_MIN, INT_MAX)); // absorb damage
		d->dmg_lv = ATK_BLOCK;
	...
	if ((sce = sc->data[SC_LIGHTNINGWALK]) && !(flag&BF_MAGIC) && flag&BF_LONG && rnd() % 100 < sce->val1) {
		...
		if(!mapdata->flag[MF_BATTLEGROUND] && !mapdata->flag[MF_GVG]){
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if( sc && sc->count ) {
		...
		if (sc->data[SC_MISTYFROST] && !status_bl_has_mode(bl, MD_STATUSIMMUNE))
		...
		if(skill_id == SR_RAMPAGEBLASTER && sc->data[SC_HOLY_OIL] && tsd && tsd->bonus.ungula) //aqui
			damage += 0;// Need official adjustment. [Rytech]
		else if (sc->data[SC_HOLY_OIL] && (flag&(BF_LONG|BF_WEAPON)) == (BF_LONG|BF_WEAPON))
		...
		if( sc->data[SC_STEELBODY] ){
			if(map_getmapflag(bl->m, MF_BIOSAVAGE)){
				damage = damage > 100 ? ((damage / 100) * 4) : 1;
			} else {
	...
	if (sc && sc->count) {
		if (sc->data[SC_HEAVEN_AND_EARTH] && (flag&BF_WEAPONMASK) != BF_MISC)
			damage += damage * (5 + 2 * sc->data[SC_HEAVEN_AND_EARTH]->val1) / 100;
	...
	if (bl->type == BL_MOB){
		...
		if (!mapdata->flag[MF_OGHCHALLENGE]) { // Reduces damage received for Green Aura MVP
		
			mob_data *md = BL_CAST(BL_MOB, bl);
			
			int damagetaken = md->db->damagetaken; 
			if(md->damagetaken) damagetaken = md->damagetaken;

			if(mapdata->flag[MF_TEMPLOULTIMATE])
				damagetaken = 1;

			// devon
			if(mapdata->flag[MF_CHEFFCELESTIAL] || mapdata->flag[MF_CHEFFSOMBRIA] || mapdata->flag[MF_CHEFFTRANS] || mapdata->flag[MF_BIOHARD] || mapdata->flag[MF_BIOEXTREME] || mapdata->flag[MF_CHEFFEXTREME] || mapdata->flag[MF_BIOSAVAGE])
				damagetaken = 10;

			if (md && damagetaken != 100){
				damage = i64max(damage * damagetaken / 100, 1);
			}
		}
	}
...
int64 battle_calc_pvp_damage(struct block_list *src,struct block_list *bl,int64 damage,uint16 skill_id,int flag)
{
	if (!damage) //No reductions to make.
		return 0;
...
{
	struct map_data *mapdata = map_getmapdata(src->m);
	if(mapdata->flag[MF_DOMINIOENT]) return false;
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	if(sc && skill_id != PA_SACRIFICE) {
		if((skill_id == RK_DRAGONBREATH || skill_id == RK_DRAGONBREATH_WATER) && sc->data[SC_MAXOVERTHRUST] && sd){
			if(checar_carta(EQI_ACC_L,sd, 0, 27345, 0) > 0 || checar_carta(EQI_ACC_R,sd, 0, 27345, 0) > 0){
				status_change_end(src,SC_MAXOVERTHRUST,INVALID_TIMER);
				sc = status_get_sc(src);
			}
		}
	...
	switch(skill_id) {
		...
		case SS_HITOUAKUMU:
			...
			break;
		case SS_KAGENOMAI:
			skillratio += -100 + 600 + (900 + pc_checkskill(sd, SS_KAGEGARI) * 70) * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			if (wd->miscflag & SKILL_ALTDMG_FLAG)
				skillratio = skillratio * 9 / 10;
		...
		case SS_FUUMASHOUAKU:
			...
			break;
		case SS_FUUMAKOUCHIKU:
			skillratio += -100 + 900 + (1750 + pc_checkskill(sd, SS_FUUMASHOUAKU) * 100) * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case SS_FUUMAKOUCHIKU_BLASTING:
			skillratio += -100 + 900 + (1750 + pc_checkskill(sd, SS_FUUMASHOUAKU) * 100) * skill_lv + 5 * sstatus->pow;
		...
		case SS_KUNAIKUSSETSU:
			...
			break;
		case SS_KUNAIKAITEN:
			skillratio += -100 + 100 + (1050 + pc_checkskill(sd, SS_KUNAIWAIKYOKU) * 100) * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case SS_KAGEGISSEN:
			skillratio += -100 + 1500 + (950 + pc_checkskill(sd, SS_KAGENOMAI) * 150) * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			if (wd->miscflag & SKILL_ALTDMG_FLAG)
				skillratio = skillratio * 9 / 10;
		...
		case SH_CHUL_HO_BATTERING:
			skillratio += -100 + 480 + 260 * skill_lv;
		...
		case SKE_MIDNIGHT_KICK:
			...
			if( sc != nullptr && ( sc->data[SC_MIDNIGHT_MOON] != nullptr || sc->data[SC_SKY_ENCHANT] != nullptr ) ){
		...
		case SKE_DAWN_BREAK:
			...
			if( sc != nullptr && ( sc->data[SC_DAWN_MOON] != nullptr || sc->data[SC_SKY_ENCHANT] != nullptr ) ){
		...
		case NW_WILD_SHOT:
			...
			if (sd && sc && sc->data[SC_HIDDEN_CARD] != nullptr) {
		...
		case NW_MIDNIGHT_FALLEN:
			...
			break;
		case SH_CHUL_HO_SONIC_CLAW:
			skillratio += -100 + 400 + 2300 * skill_lv + 50 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY) + 5 * sstatus->pow;
			if ((sd && pc_checkskill(sd, SH_COMMUNE_WITH_CHUL_HO)) || (sc && sc->data[SC_TEMPORARY_COMMUNION]))
				skillratio += 100 + 400 * skill_lv + 50 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
			RE_LVL_DMOD(100);
			break;
		...
		case MO_EXTREMITYFIST:
		{
			...
			struct map_data *mapdata = map_getmapdata(src->m);
			if(mapdata->flag[MF_GVG] || mapdata->flag[MF_PVP] || mapdata->flag[MF_BATTLEGROUND] || (sc && sc->data[SC_EFEITO_RH_7] && tsc && tsc->data[SC_EFEITO_RH_7]))
				skillratio = skillratio * 5;
		}
			break;
		...
		case NC_ARMSCANNON:
			...
			skillratio += (skillratio / 100) * 30; // devon
			RE_LVL_DMOD(100);
			break;
		...
		case WM_SEVERE_RAINSTORM:
			...
			skillratio += -100 + 100 * skill_lv + ((status_get_dex(src) + status_get_agi(src)) / 2);
		...
		case DK_SERVANTWEAPON_ATK:
			skillratio += -100 + 500 + 870 * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		...
		case DK_HACKANDSLASHER:
		case DK_HACKANDSLASHER_ATK:
			skillratio += -100 + 300 + 825 * skill_lv;
		...
		case DK_MADNESS_CRUSHER:
			skillratio += -100 + 400 + 3920 * skill_lv + 10 * sstatus->pow;
			if (sd) {
				short index = sd->equip_index[EQI_HAND_R];
		...
		case IQ_EXPOSION_BLASTER:
			skillratio += -100 + 2690 * skill_lv + 15 * sstatus->pow;
		...
		case IQ_THIRD_PUNISH:
			skillratio += -100 + 350 + 1820 * skill_lv + 10 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		...
		case IQ_THIRD_CONSECRATION:
			skillratio += -100 + 700 * skill_lv + 15 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case IQ_BLAZING_FLAME_BLAST:
		...
		case IG_SHIELD_SHOOTING:
			skillratio += -100 + 1400 + 3420 * skill_lv + 10 * sstatus->pow + skill_lv * 15 * pc_checkskill(sd, IG_SHIELD_MASTERY);
			if (sd) {
				short index = sd->equip_index[EQI_HAND_L];
				if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_ARMOR)
					skillratio += sd->inventory_data[index]->weight / 3 + sd->inventory.u.items_inventory[index].refine * 100;
			}
			RE_LVL_DMOD(100);
			break;
		case IG_OVERSLASH:
		...
		case IG_RADIANT_SPEAR:
			...
			skillratio += pc_checkskill(sd, IG_SPEAR_SWORD_M) * 250;
			skillratio += 7 * sstatus->pow;	// !TODO: check POW ratio
		...
		case IG_IMPERIAL_CROSS:
			...
			break;
		case CD_EFFLIGO:
			skillratio += -100 + 2500 * skill_lv + 12 * sstatus->pow;
			...
			break;
		case CD_PETITIO:
			skillratio += -100 + (1700 + pc_checkskill(sd,CD_MACE_BOOK_M) * 500) * skill_lv + 12 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		...
		case SHC_CROSS_SLASH:
			...
			break;
		case MT_AXE_STOMP:
			skillratio += -100 + 350 + 1170 * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case MT_MIGHTY_SMASH:
			skillratio += -100 + 100 + 238 * skill_lv + 5 * sstatus->pow;
			if (sc && sc->data[SC_AXE_STOMP])
				skillratio += 20  + 10 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case MT_RUSH_QUAKE:
		...
		case MT_SPARK_BLASTER:
			skillratio += -100 + 250 + 1435 * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case MT_TRIPLE_LASER:
			skillratio += -100 + 300 + 1220 * skill_lv + 12 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case MT_RUSH_STRIKE:
		...
		case ABC_ABYSS_DAGGER:
			skillratio += -100 + 100 + 1450 * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case ABC_UNLUCKY_RUSH:
			skillratio += 300 * skill_lv + 5 * sstatus->pow;
			if (sc && sc->data[SC_CHASING])
		...
		case ABC_CHAIN_REACTION_SHOT_ATK:
			skillratio += -100 + 600 + 2590 * skill_lv + 15 * sstatus->con;
			if (sc && sc->data[SC_CHASING])
				skillratio += 700 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case ABC_DEFT_STAB:
			skillratio += -100 + 350 + 585 * skill_lv + 7 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case ABC_FRENZY_SHOT:
			skillratio += -100 + 825 * skill_lv + 15 * sstatus->con;
			RE_LVL_DMOD(100);
			break;
		case WH_HAWKRUSH:
			skillratio += -100 + 625 * skill_lv + 5 * sstatus->con;
			RE_LVL_DMOD(100);
			break;
		case WH_HAWKBOOMERANG:
			skillratio += -100 + 750 * skill_lv + 10 * sstatus->con;
			if (tstatus->race == RC_BRUTE || tstatus->race == RC_FISH)
				skillratio += 250 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case WH_GALESTORM:
			skillratio += -100 + 1485 * skill_lv + 10 * sstatus->con;
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_CALAMITYGALE] && (tstatus->race == RC_BRUTE || tstatus->race == RC_FISH))
					skillratio += skillratio * 50 / 100;
			break;
		case WH_CRESCIVE_BOLT:
			skillratio += -100 + 550 + 1430 * skill_lv + 5 * sstatus->con;
			RE_LVL_DMOD(100);
			if(sc){
		...
		case BO_EXPLOSIVE_POWDER:
			skillratio += -100 + 400 + 670 * skill_lv + 5 * sstatus->pow;
			if (sc && sc->data[SC_RESEARCHREPORT])
				skillratio += 100 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		...
		case TR_RHYTHMSHOOTING: {
			int con = (sd ? pc_checkskill(sd, TR_STAGE_MANNER) : 3);
			if (tsc && tsc->data[SC_SOUNDBLEND]){
				con = 7;
			}
			skillratio += -100 + 450 + 820 * skill_lv + con * sstatus->con;
			if (tsc && tsc->data[SC_SOUNDBLEND]){
	...
	if (sc) {
		...
		if (sc->data[SC_EDP]) {
			switch(skill_id) {
				case AS_SPLASHER:
				case ASC_METEORASSAULT:
				if(sd && sd->bonus.verifica_oneh > 0){
		...
		if((wd->flag&(BF_LONG|BF_MAGIC)) == BF_LONG) {
			...
			if (sc->data[SC_SUPREMO_INFINITO]) { // Monster Transformation bonus
				ATK_ADDRATE(wd->damage, wd->damage2, sc->data[SC_SUPREMO_INFINITO]->val1);
				RE_ALLATK_ADDRATE(wd, sc->data[SC_SUPREMO_INFINITO]->val1);
			}
		...
		if (skill_id == 0 && sc->data[SC_EXEEDBREAK]) {
			struct map_data *mapdata = map_getmapdata(src->m);
			if(mapdata->flag[MF_GVG] || mapdata->flag[MF_PVP] || mapdata->flag[MF_BATTLEGROUND]){
				ATK_ADDRATE(wd->damage, wd->damage2, (sc->data[SC_EXEEDBREAK]->val2 * 15));
			}else {
...
#ifdef RENEWAL
		if(sd->bonus.cap_milagre)
			skillratio = min(skillratio,(450+sd->bonus.cap_milagre));
		else 
			skillratio = min(skillratio,450);
		
if (sd->timermilagre > time(NULL)) {
	  char msgnew[256];
	  sprintf(msgnew, "[Milagre] Valor do Cap: %d%%", skillratio);
	  clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
}
...
static void battle_calc_attack_plant(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (skill_id != SN_SHARPSHOOTING && skill_id != RA_ARROWSTORM && skill_id != NEW_SHARPSHOOT)
	...
	if( wd->damage + wd->damage2 ) { //There is a total damage value
		...
		if(!wd->damage2) {
			...
			if ((mapdata->flag[MF_EQUIPSGVG] || mapdata_flag_gvg2(mapdata)) && target->type != BL_MOB && src->type != BL_MOB)
				...
				wd->damage=battle_calc_pvp_damage(src,target,wd->damage,skill_id,wd->flag);
		}
		else if(!wd->damage) {
			...
			if ((mapdata->flag[MF_EQUIPSGVG] || mapdata_flag_gvg2(mapdata)) && target->type != BL_MOB && src->type != BL_MOB)
				...
				wd->damage2 = battle_calc_pvp_damage(src,target,wd->damage2,skill_id,wd->flag);
		}
		else {
		...
		if( src != target && //Don't reflect your own damage (Grand Cross)
			...
			(src->type == BL_SKILL && (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM || skill_id == SG_STAR_WARM))) ) {
				...
				if( rdamage > 0 ) { //Item reflect gets calculated before any mapflag reducing is applicated
					...
					if(skill_id == NPC_MAXPAIN_ATK) return;
...
static void battle_calc_weapon_final_atk_modifiers(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	struct status_data *sstatus = status_get_status_data(src);
	struct status_data *tstatus = status_get_status_data(target);
	int skill_damage = 0, damage = 0, olddamage = wd->damage;
	...
	{		
			...
			struct map_data *mapdata = map_getmapdata(target->m);
			if ((mapdata->flag[MF_EQUIPSGVG] || mapdata_flag_gvg2(mapdata)) && target->type != BL_MOB && src->type != BL_MOB)
				damage=battle_calc_gvg_damage(src,target,wd->damage,skill_id,wd->flag);
			else if( mapdata->flag[MF_BATTLEGROUND] && target->type != BL_MOB && src->type != BL_MOB )
				damage=battle_calc_bg_damage(src,target,wd->damage,skill_id,wd->flag);
			else if ((mapdata->flag[MF_PVP] || ((TBL_PC *)target)->sc.data[SC_EFEITO_RH_7]) && target->type != BL_MOB && src->type != BL_MOB)
				damage=battle_calc_pvp_damage(src,target,wd->damage,skill_id,wd->flag);

		
		wd->damage = damage;
		battle_absorb_damage(target, wd, skill_id, src, skill_lv);
		...
		if (sc->data[SC_FUSION]) {
			...
			if (sd && tsd) {
				...
				if (((int64)sstatus->hp * 100) <= ((int64)sstatus->max_hp * 20))
					hp = sstatus->hp;
			} else if(sd && sd->bonus.verifica_oneb > 0)
				hp = 1*hp/100; //1% hp loss per hit
			else
	...
	if (skill_id) {
		...
		switch(skill_id)
		{
			...
			case ABC_HIT_AND_SLIDING:
				...
				break;
			case HN_DOUBLEBOWLINGBASH:
				if(sd && sd->bonus.verifica_onec > 0){ wd.div_ += 4; break;}
				if (wd.miscflag > 1)
...
void battle_do_reflect(int attack_type, struct Damage *wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
		...
		(src->type == BL_SKILL && (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM || skill_id == SG_STAR_WARM ))))
	{
		...
		if(md && md->db->id == 28002) return;
		
		struct map_data *mapdata = map_getmapdata(src->m);

		if ((mapdata->flag[src->m, MF_SAMURAIMESTRE] || mapdata->flag[src->m, MF_FENDA] || mapdata->flag[src->m, MF_CHEFFTRANS] || mapdata->flag[src->m, MF_TUMBAVAZIO] || mapdata->flag[src->m, MF_ANIVERSARIO] || mapdata->flag[src->m, MF_TEMPLOULTIMATE] || mapdata->flag[src->m, MF_DOMINIOGERAL] || mapdata->flag[src->m, MF_ESCURIDAO] || mapdata->flag[src->m, MF_ESCURIDAOPVP] || mapdata->flag[src->m, MF_TEMPLOSAGRADO] || mapdata->flag[src->m, MF_GRAMPS] || mapdata->flag[src->m, MF_CARNAVAL])
			 && !tsc->data[SC_MAXPAIN]) return;
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
	...
	sd = BL_CAST(BL_PC, src);
//		char msgnew[256];
//	    sprintf(msgnew, "Parte 1: %d%%", wd.damage);
//	    clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
	tsd = BL_CAST(BL_PC, target);
...
//			char msgnew[256];
//			sprintf(msgnew, "Parte 2: %d%%", wd.damage);
//			clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
	...
	//clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
		...
		if (sd) { //monsters, homuns and pets have their damage computed directly
			if( skill_id == RK_DRAGONBREATH || skill_id == RK_DRAGONBREATH_WATER ){
				wd.damage = (int64)floor( (float)((wd.statusAtk + wd.weaponAtk + wd.equipAtk) * (100 + sstatus->patk) / 100 + wd.masteryAtk + bonus_damage));
				if (is_attack_left_handed(src, skill_id))
					wd.damage2 = (int64)floor( (float)((wd.statusAtk2 + wd.weaponAtk2 + wd.equipAtk2) * (100 + sstatus->patk) / 100 + wd.masteryAtk2 + bonus_damage));
			} else {
...
//				char msgnew[256];
//				sprintf(msgnew, "Parte 3: %lld%%", wd.damage);
//				clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
			}
			if (wd.flag & BF_SHORT)
		...
		if ((wd.damage + wd.damage2) && tstatus->res > 0) {
			...
			ignore_res = min(ignore_res, battle_config.max_res_mres_reduction);

			if (ignore_res > 0)
				res -= res * ignore_res / 100;
			
			if(sd && sd->bonus.ignore_res){
				res -= sd->bonus.ignore_res;
			}
...
//				char msgnew[256];
//				sprintf(msgnew, "Parte 4: %lld%%", wd.damage);
//				clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
		}
#else
		// final attack bonuses that aren't affected by cards
	...
	switch (skill_id) {
		...
		case NPC_MAXPAIN_ATK:
			if (sc && sc->data[SC_MAXPAIN]) {
				...
				// devon
				if(wd.damage >= 75000000) wd.damage = 75000000;
			}
			else 
				wd.damage = 0;
			break;
	...
	wd.s_ele = right_element;
	wd.l_ele = left_element;
//		memset(msgnew, 0, 255);
//		sprintf(msgnew, "Parte Final: %lld%%", wd.damage);
//		clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
...
struct Damage battle_calc_magic_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch(skill_id) {
		...
		case IG_IMPERIAL_PRESSURE:
			if (sc && sc->data[SC_GUARD_STANCE] != nullptr)
				s_ele = ELE_HOLY;
			break;
...
#define MATK_ADD(a) { ad.damage += a; }
			...
			default: {
				...
				switch(skill_id) {
					...
					case SS_TOKEDASU:
						...
						break;
					case SS_SEKIENHOU:
						skillratio += -100 + 720 + (1320 + 70 * pc_checkskill(sd, SS_ANTENPOU)) * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_FIRE_CHARM_POWER])
							skillratio += 8500;
					...
					case SS_REIKETSUHOU:
						...
						if (sc && sc->data[SC_WATER_CHARM_POWER])
							skillratio += 7000;
						skillratio = skillratio * 6;
						RE_LVL_DMOD(100);
						break;
					case SS_KINRYUUHOU:
						skillratio += -100 + 800 + (1500 + 15 * pc_checkskill(sd, SS_ANTENPOU)) * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_GROUND_CHARM_POWER])
							skillratio += 5500;
						RE_LVL_DMOD(100);
						break;
					case SS_ANKOKURYUUAKUMU:
					...
					case SS_RAIDENPOU:
						skillratio += -100 + 720 + (1320 + 70 * pc_checkskill(sd, SS_ANTENPOU)) * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_WIND_CHARM_POWER])
							skillratio += 8500;
						skillratio = skillratio * 2;
						RE_LVL_DMOD(100);
						break;
					case SS_ANTENPOU:
						skillratio += -100 + 540 + 1140 * skill_lv + 5 * sstatus->spl;
						RE_LVL_DMOD(100);
						if (mflag & SKILL_ALTDMG_FLAG)
							skillratio = skillratio * 9 / 10;
						break;
					case HN_NAPALM_VULCAN_STRIKE:
						skillratio += -100 + 403 + (748 + 4 * pc_checkskill(sd, HN_SELFSTUDY_SOCERY)) * skill_lv + 5 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					case HN_JUPITEL_THUNDER_STORM:
					...
					case HN_METEOR_STORM_BUSTER:
						...
						break;
					case SH_HYUN_ROKS_BREEZE:
						skillratio += -100 + 600 + 760 * skill_lv + 20 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY) + 5 * sstatus->spl;
						if ((sd && pc_checkskill(sd, SH_COMMUNE_WITH_HYUN_ROK)) || (sc && sc->data[SC_TEMPORARY_COMMUNION]))
							skillratio += 100 + 215 * skill_lv + 20 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
						RE_LVL_DMOD(100);
						break;
					case SH_HYUN_ROK_CANNON:
						skillratio += -100 + 700 + 2108 * skill_lv + 50 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY) + 5 * sstatus->spl;
						if ((sd && pc_checkskill(sd, SH_COMMUNE_WITH_HYUN_ROK)) || (sc && sc->data[SC_TEMPORARY_COMMUNION]))
							skillratio += 100 + 400 * skill_lv + 25 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
						RE_LVL_DMOD(100);
						break;
					...
					case SOA_TALISMAN_OF_BLUE_DRAGON:
						skillratio += -100 + 850 + (2250 + pc_checkskill(sd, SOA_TALISMAN_MASTERY) * 15) * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_T_FIFTH_GOD] )
							skillratio += 720 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					case SOA_TALISMAN_OF_WHITE_TIGER:
						skillratio += -100 + 200 + (1200 + pc_checkskill(sd, SOA_TALISMAN_MASTERY) * 15) * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_T_FIFTH_GOD])
							skillratio += 400 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					case SOA_TALISMAN_OF_RED_PHOENIX:
						skillratio += -100 + 1400 + (1450 + pc_checkskill(sd, SOA_TALISMAN_MASTERY) * 15) * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_T_FIFTH_GOD])
							skillratio += 440 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					...
					case WL_COMET:
					case NPC_COMET_BOSS:
					case DM_JOKENPO_TESOURA:
					case NPC_JOKENPO_PEDRA:
					case NPC_JOKENPO_PAPEL:
					...
					case SO_VARETYR_SPEAR:
						...
						RE_LVL_DMOD(100);
						if (sd && sd->status.ele_id && sd->ed->db->class_ == ELEMENTALID_VENTUS_M){
							skillratio += (skillratio / 100) * (sd->status.job_level * 5);
						}
						break;
					...
					case AG_ASTRAL_STRIKE_ATK:
						...
						break;
					case AG_ROCK_DOWN:
						skillratio += -100 + 1736 * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_CLIMAX])
							skillratio += 336 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					case AG_STORM_CANNON:
						skillratio += -100 + 1798 * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_CLIMAX])
							skillratio += 348 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					case AG_CRIMSON_ARROW:
					...
					case AG_FROZEN_SLASH:
						skillratio += -100 + 504 + 1064 * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_CLIMAX])
							skillratio += 168 + 392 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					...
					case IG_CROSS_RAIN:
						if (sc && sc->data[SC_HOLY_S])
							skillratio += -100 + (650 + 15 * pc_checkskill(sd, IG_SPEAR_SWORD_M)) * skill_lv + 7 * sstatus->spl;
						else
							skillratio += -100 + (320 + 10 * pc_checkskill(sd, IG_SPEAR_SWORD_M)) * skill_lv + 7 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					case IG_IMPERIAL_PRESSURE:
						...
						break;
					case CD_ARBITRIUM:
						skillratio += -100 + 1130 * skill_lv + 10 * sstatus->spl;
					...
					case ABC_ABYSS_STRIKE:
						...
						break;
					case ABC_ABYSS_SQUARE:
						skillratio += -100 + (750 + 20 * pc_checkskill(sd,ABC_MAGIC_SWORD_M)) * skill_lv + 5 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					...
					case TR_METALIC_FURY: {
						...
						skillratio += -100 + (4200 + 300 * count) * skill_lv + 5 * sstatus->spl;
						RE_LVL_DMOD(100);
						}
						break;
					case TR_SOUNDBLEND:
					...
					case EM_DIAMOND_STORM:
						skillratio += -100 + 400 + 2420 * skill_lv + 5 * sstatus->spl;
						if (sc && sc->data[SC_SUMMON_ELEMENTAL_DILUVIO])
							skillratio += 5000 + 660 * skill_lv + 5 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					case EM_LIGHTNING_LAND:
						skillratio += -100 + 500 + 1140 * skill_lv + 5 * sstatus->spl;
					...
					case ABC_FROM_THE_ABYSS_ATK:
						skillratio += -100 + 100 + 660 * skill_lv + 5 * sstatus->spl;
						RE_LVL_DMOD(100);
						break;
					...
					case EM_ELEMENTAL_BUSTER_POISON:
						skillratio += -100 + 500 + 2655 * skill_lv + 10 * sstatus->spl;
					...
					case NPC_RAINOFMETEOR:
						...
						break;
					case SH_HYUN_ROK_SPIRIT_POWER:
						skillratio += -100 + 350 + 243 * skill_lv;
...
#ifdef RENEWAL
		if(ad.damage > 0 && sd){
			if(sd->status.class_ == JOB_HYPER_NOVICE){
				ad.damage += (ad.damage / 100) * 50;
			} else if(sd->status.class_ == JOB_ELEMENTAL_MASTER){
				ad.damage += (ad.damage / 100) * 70;
			} else {
				ad.damage += ad.damage;
			}
		}
		...
		if (ad.damage && tstatus->mres > 0) {
			...
			ignore_mres = min(ignore_mres, battle_config.max_res_mres_reduction);

			if (ignore_mres > 0)
				mres -= mres * ignore_mres / 100;

			if(sd && sd->bonus.ignore_mres){
				mres -= sd->bonus.ignore_mres;
			}
	...
	if ((mapdata_flag_gvg2(mapdata) || mapdata->flag[MF_EQUIPSGVG]) && target->type != BL_MOB && src->type != BL_MOB)
		...
		ad.damage = battle_calc_pvp_damage(src,target,ad.damage,skill_id,ad.flag);
	...
	if(src->type != BL_PC){
		battle_absorb_damage(target, &ad, skill_id, src, skill_lv);
	}
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	if ((mapdata_flag_gvg2(mapdata) || mapdata->flag[MF_EQUIPSGVG]) && target->type != BL_MOB && src->type != BL_MOB)
		...
		md.damage = battle_calc_pvp_damage(src,target,md.damage,skill_id,md.flag);
	...
	if(src->type != BL_PC){
		battle_absorb_damage(target, &md, skill_id, src, skill_lv);
...
static void mineracao(struct map_session_data *sd, struct Damage *d, uint16 skill_id) {
	
	if (!sd)
		return;
	
	if(!skill_id && (sd->state.idarma == 66007 || sd->state.idarma == 66004)
	) {
		d->damage = 1;
		int chance = 50, qtd = 1;
		if(sd->state.idenchant == 50304) chance = 65;
		if(sd->state.idenchant == 50305) chance = 85;
		if(sd->state.idenchant == 50306) chance = 100;
		sd->state.mineracao += 1;
		if(sd->state.idarma == 66004 && rnd()%2 == 1) {
			sd->state.mineracao += 1;
			d->damage = 2;
		}
		if(sd->state.idarma == 66004) {
			qtd = 2;
		}
		if(sd->state.mineracao == 1)
			clif_cutin(sd,"mine_1",2);
		else if(sd->state.mineracao == 2)
			clif_cutin(sd,"mine_2",2);
		else if(sd->state.mineracao == 3)
			clif_cutin(sd,"mine_3",2);
		else if(sd->state.mineracao == 4)
			clif_cutin(sd,"mine_4",2);
		else if(sd->state.mineracao == 5)
			clif_cutin(sd,"mine_5",2);
		else if(sd->state.mineracao == 6)
			clif_cutin(sd,"mine_6",2);
		else if(sd->state.mineracao == 7)
			clif_cutin(sd,"mine_7",2);
		else if(sd->state.mineracao == 8)
			clif_cutin(sd,"mine_8",2);
		else if(sd->state.mineracao == 9)
			clif_cutin(sd,"mine_9",2);
		else {
			int pt = pc_readglobalreg(sd, add_str("#MINERACAO"));
			if(pt < 45){
				int feito;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `dominio_trava` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 'M' LIMIT 1;", sd->status.account_id) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
							Sql_GetData(mmysql_handle, 0, &data, NULL); feito = (uint16)atoi(data);
						}
						Sql_FreeResult(mmysql_handle);
					}
				}
				
				if(feito > pt) pt = feito;
			}
			
			clif_cutin(sd,"mine_10",2);
			if(pt < 45){
				sd->state.mineracao = 0;
				struct item tmp_item;
				memset(&tmp_item,0,sizeof(tmp_item));
				tmp_item.nameid = 50303;
				tmp_item.amount = qtd;
				tmp_item.identify = 1;
				char msgnew[256];
				if (rnd()%100 >= chance){
					sprintf(msgnew, "[Mineração] Parece que você não teve sorte dessa vez, tentativas restantes [%d/45].", pt + 1);
					clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
					clif_specialeffect(&sd->bl, EF_SUI_EXPLOSION, AREA);
				} else {
					sprintf(msgnew, "[Mineração] Sucesso !!! Tentativas restantes [%d/45].", pt + 1);
					clif_messagecolor(&sd->bl, 0xbe67ef, msgnew, true, SELF);
					clif_specialeffect(&sd->bl, EF_TEMP_OK, AREA);
					int mine = pc_additem(sd, &tmp_item, qtd, LOG_TYPE_SCRIPT);
					if(mine != 0){
						const char *sender, *title, *body, *name;
						struct mail_message msg;
						memset(&msg, 0, sizeof(struct mail_message));
						msg.dest_id = sd->status.char_id;
						sender = "History Reborn";
						...
						body = "Seu inventário estava lotado, por isso entregamos seu item pelo rodex !!";
						if( strlen(body) > MAIL_BODY_LENGTH ){
							ShowError( "buildin_mail: body can not be longer than %d characters.\n", MAIL_BODY_LENGTH );
							return;
						}
						safestrncpy(msg.body, body, MAIL_BODY_LENGTH);
						msg.item[0].nameid = 50303;
						msg.item[0].amount = qtd;
				...
				pt += 1;
				pc_setaccountreg(sd, add_str("#MINERACAO"), pt);
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `dominio_trava` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 'M';", pt, sd->status.account_id) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt == 45){
					int ptrepu = sd->reputation[9].points, f = 0, i;
					if(sd->state.idarma == 66004) f = 1;
					for(i = 0; i <= f; i++){
						int ptrepu = sd->reputation[9].points;
						if(ptrepu < 1000) 
							sd->reputation[9].points += 100;
						else if(ptrepu < 2000) {
							sd->reputation[9].points += 66;
							if(sd->reputation[9].points >= 1990) sd->reputation[9].points = 2000;
						}
						else if(ptrepu < 3000) {
							sd->reputation[9].points += 50;
						}
						else if(ptrepu < 4000) {
							sd->reputation[9].points += 40;
						}
						else if(ptrepu < 5000) {
							sd->reputation[9].points += 33;
							if(sd->reputation[9].points >= 4990) sd->reputation[9].points = 5000;
						}
						else if(ptrepu < 6000) {
							sd->reputation[9].points += 28;
							if(sd->reputation[9].points >= 5980) sd->reputation[9].points = 6000;
						}
					}
					clif_reputation(sd);
					ptrepu = sd->reputation[9].points;
					if(ptrepu == 1000 || ptrepu == 2000 || ptrepu == 3000 || ptrepu == 4000 || ptrepu == 5000 || ptrepu == 6000) status_calc_pc(sd, SCO_FORCE);
				}
			} else {
				clif_messagecolor(&sd->bl, 0xbe67ef, "[Mineração] Você já completou as [45/45] chances de hoje, tente após a meia-noite.", true, SELF);
			}
		}
	}
}

static void herbologia(struct map_session_data *sd, struct Damage *d, uint16 skill_id) {
	
	if (!sd)
		return;
	
	if(!skill_id && (sd->state.idarma == 66002 || sd->state.idarma == 66003)
	) {
		d->damage = 1;
		int chance = 50, qtd = 1;
		if(sd->state.idenchant == 50307) chance = 65;
		if(sd->state.idenchant == 50308) chance = 85;
		if(sd->state.idenchant == 50309) chance = 100;
		sd->state.herbologia += 1;
		if(sd->state.idarma == 66003 && rnd()%2 == 1) {
			sd->state.herbologia += 1;
			d->damage = 2;
		}
		if(sd->state.idarma == 66003) {
			qtd = 2;
		}
		if(sd->state.herbologia == 1)
			clif_cutin(sd,"herbo_1",2);
		else if(sd->state.herbologia == 2)
			clif_cutin(sd,"herbo_2",2);
		else if(sd->state.herbologia == 3)
			clif_cutin(sd,"herbo_3",2);
		else if(sd->state.herbologia == 4)
			clif_cutin(sd,"herbo_4",2);
		else if(sd->state.herbologia == 5)
			clif_cutin(sd,"herbo_5",2);
		else if(sd->state.herbologia == 6)
			clif_cutin(sd,"herbo_6",2);
		else if(sd->state.herbologia == 7)
			clif_cutin(sd,"herbo_7",2);
		else if(sd->state.herbologia == 8)
			clif_cutin(sd,"herbo_8",2);
		else if(sd->state.herbologia == 9)
			clif_cutin(sd,"herbo_9",2);
		else {
			int pt = pc_readglobalreg(sd, add_str("#HERBOLOGIA"));
			if(pt < 45){
				int feito;

				if (Sql_Query(mmysql_handle,
					"select `qtd` from `dominio_trava` where last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 'H' LIMIT 1;", sd->status.account_id) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
			...
			clif_cutin(sd,"herbo_10",2);
			if(pt < 45){
				sd->state.herbologia = 0;
				struct item tmp_item;
				memset(&tmp_item,0,sizeof(tmp_item));
				tmp_item.nameid = 50310;
				tmp_item.amount = qtd;
				tmp_item.identify = 1;
				char msgnew[256];
				if (rnd()%100 >= chance){
					sprintf(msgnew, "[Herbologia] Parece que você não teve sorte dessa vez, tentativas restantes [%d/45].", pt + 1);
					clif_messagecolor(&sd->bl, 0x67c240, msgnew, true, SELF);
					clif_specialeffect(&sd->bl, EF_SUI_EXPLOSION, AREA);
				} else {
					sprintf(msgnew, "[Herbologia] Sucesso !!! Tentativas restantes [%d/45].", pt + 1);
					clif_messagecolor(&sd->bl, 0x67c240, msgnew, true, SELF);
				...
				pt += 1;
				pc_setaccountreg(sd, add_str("#HERBOLOGIA"), pt);
				
				if (Sql_Query(mmysql_handle,
					"UPDATE `dominio_trava` SET `qtd`= %d WHERE last_unique_id = (SELECT `last_unique_id` FROM `login` WHERE `account_id` = %d) AND tipo = 'H';", pt, sd->status.account_id) == SQL_SUCCESS )
				{
					if(Sql_NumRows(mmysql_handle) > 0){
						while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
							char *data;
				...
				if(pt == 45){
					int ptrepu = sd->reputation[10].points, f = 0, i;
					if(sd->state.idarma == 66003) f = 1;
					for(i = 0; i <= f; i++){
						if(ptrepu < 1000) 
							sd->reputation[10].points += 100;
						else if(ptrepu < 2000) {
							sd->reputation[10].points += 66;
							if(sd->reputation[10].points >= 1990) sd->reputation[10].points = 2000;
						}
						else if(ptrepu < 3000) {
							sd->reputation[10].points += 50;
						}
						else if(ptrepu < 4000) {
							sd->reputation[10].points += 40;
						}
						else if(ptrepu < 5000) {
							sd->reputation[10].points += 33;
							if(sd->reputation[10].points >= 4990) sd->reputation[10].points = 5000;
						}
						else if(ptrepu < 6000) {
							sd->reputation[10].points += 28;
							if(sd->reputation[10].points >= 5980) sd->reputation[10].points = 6000;
						}
					}
					clif_reputation(sd);
					ptrepu = sd->reputation[10].points;
					if(ptrepu == 1000 || ptrepu == 2000 || ptrepu == 3000 || ptrepu == 4000 || ptrepu == 5000 || ptrepu == 6000) status_calc_pc(sd, SCO_FORCE);
				}
			} else {
				clif_messagecolor(&sd->bl, 0x67c240, "[Herbologia] Você já completou as [45/45] chances de hoje, tente após a meia-noite.", true, SELF);
			}
		}
	}
}

int64 battle_do_reflect_fenda(struct block_list* bl, struct block_list *target, struct Damage *wd, int64 dmg, struct map_session_data *sd){
	mob_data *md = BL_CAST(BL_MOB, target);
	t_tick tick = gettick(), rdelay = 0;
	int64 rdamage = 0, reduct = 0;
	int maxhp = (status_get_max_hp(bl) / 100000) * 5;
	
	if(dmg > 2147483647) dmg = 2147483647;
	
	rdamage = dmg * 0.00002708;
	if(rdamage <= 0) return 0;
	
	if(md && md->status.dmgtakenfloat) {
		if(md->status.dmgtakenfloat == 100) rdamage = rdamage * 1.25;
		if(md->status.dmgtakenfloat == 1) rdamage = rdamage * 5;
	}
	
	reduct = (rdamage / 100) * (maxhp + sd->bonus.reduct_reflect);
	rdamage = rdamage - reduct;
	
	if(sd->sc.data[SC_EFEITO_RH_14]){
		rdamage = rdamage - (rdamage / 100) * sd->sc.data[SC_EFEITO_RH_14]->val1;
	}
	
	struct status_data *sstatus = status_get_status_data(bl);
	
	rdelay = clif_damage(bl, bl, tick, wd->amotion, sstatus->dmotion, rdamage, 1, DMG_ENDURE, 0, false);
	battle_delay_damage(tick, wd->amotion, target, bl, 0, CR_REFLECTSHIELD, 0, rdamage, ATK_DEF, rdelay ,true, false);
	skill_additional_effect(target, bl, CR_REFLECTSHIELD, 1, BF_WEAPON|BF_SHORT|BF_NORMAL, ATK_DEF, tick);
	
	return 1;
}

int64 battle_do_reflect_torre(struct block_list* bl, struct block_list *target, struct Damage *wd, int64 dmg, struct map_session_data *sd){
	mob_data *md = BL_CAST(BL_MOB, target);
	t_tick tick = gettick(), rdelay = 0;
	int64 rdamage = 0, reduct = 0;
	int maxhp = (status_get_max_hp(bl) / 100000) * 5;
	...
	if(md && md->status.tiporeflect) {
		if(md->status.tiporeflect == 2) rdamage = rdamage * 1.25;
		if(md->status.tiporeflect == 3) rdamage = rdamage * 5;
	}
...
int battle_barra_boss(int partyid, struct block_list *target){
	
	struct status_data *status;
	status = status_get_status_data(target);
	int countbar = status->max_shield / 100;
	int baratual, i;
	
	baratual = (int)(status->shield / countbar);
	if(baratual < 0) baratual = 0;
	if(baratual > 100) baratual = 100;
	char msgbar[256];
	sprintf(msgbar, "hb_%d", baratual);
	
	struct party_data *p = party_search(partyid);
	if(p){
		for (i = 0; i < MAX_PARTY; i++) {
			if (p->party.member[i].online){
				...
				if(!psd)
					continue;
				if(status->shield > 0)
					clif_cutin(psd,msgbar,1);
				else
					clif_cutin(psd,msgbar,255);
			}
		}
	}
...
struct Damage battle_calc_attack(int attack_type,struct block_list *bl,struct block_list *target,uint16 skill_id,uint16 skill_lv,int flag)
{
	...
	struct map_session_data *sd = BL_CAST(BL_PC, bl);
	struct status_change *sc, *tscc;
	tscc = status_get_sc(target);
	sc = status_get_sc(bl);
	struct map_session_data *tsd; ///< Target session data if BL_PC
	tsd = BL_CAST(BL_PC, target);

	struct map_data *mapdata = map_getmapdata(bl->m);
	
	if (target->type == BL_MOB && mapdata->flag[MF_DOMINIOENT]) {
		mob_data *md = BL_CAST(BL_MOB, target);
		...
		if(sd){
			...
			d.damage2 = 0;
			if(md->db->id == 21853){
				if(!tscc || !tscc->data[SC_INVULNERA]){
					mineracao(sd,&d, skill_id);
				}
			}
			if(md->db->id == 22981){
				herbologia(sd,&d, skill_id);
			}
			if(d.damage > 0){
	...
	switch(attack_type) {
		...
		case BF_WEAPON:
			...
			if( sd && !nk[NK_IGNOREATKCARD] && (left&2) ) {
				...
				if (left&1) {
					if ( cardfix_ != 1000 )
						d.damage2 = d.damage2 * cardfix_ / 1000;
				} else {
					if ( cardfix != 1000 )
						d.damage = d.damage * cardfix / 1000;
				}
			}
			// Affected by target DEF bonuses
			else if( tsd && !nk[NK_IGNOREDEFCARD] && !(left&2) ) {
			...
			*/
		case BF_MAGIC:
			int64 original_damage = d.damage;
			int cardfix = 1000;
			int s_class, ///< Attacker class
				t_class; ///< Target class
			std::vector<e_race2> s_race2, /// Attacker Race2
			...
			t_class = status_get_class(target);
			s_class = status_get_class(bl);
			sstatus = status_get_status_data(bl);
			tstatus = status_get_status_data(target);
			s_race2 = status_get_race2(bl);
			t_race2 = status_get_race2(target);
			s_defele = (tsd) ? (enum e_element)status_get_element(bl) : ELE_NONE;
			
			int32 size_fix = 0;
			int rh_ele, lh_ele, left;
	...
	if(map_getcell( bl->m, bl->x, bl->y, CELL_CHKNODAMAGE )){
		d.damage = 0;
		d.damage2 = 0;
		return d;
	}
	...
	// devon
	if (mapdata->flag[MF_LUINA] && status_get_class_(target) == CLASS_BOSS){
		d.damage = (d.damage / 100) * 10;
		d.damage2 = (d.damage2 / 100) * 10;
	}

	// devon 
	// aumento de DANO MAGICO
	// --------------------------
	if(bl->type == BL_PC){
		if (mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP] || mapdata->flag[MF_TEMPLOULTIMATE]){
			if(d.damage < 1 && skill_id == WL_COMET){
				d.damage = 1;
				d.div_ = 1;
				return d;
			}
		}
		if(attack_type == BF_MAGIC && d.damage > 0 && target->type == BL_MOB){
			int aumento = 0;
			//if(skill_id == SO_VARETYR_SPEAR){
			//	d.damage = (d.damage / 100) * 70;
			//}
			if(battle_config.dano_magico > 0){
				aumento += battle_config.dano_magico;
			}

			if(sd->sc.data[SC_MTF_MATK2]){
				aumento += sd->sc.data[SC_MTF_MATK2]->val1;
			}
			if(sd->sc.data[SC_SUPREMO_INFINITO]){
				aumento += sd->sc.data[SC_SUPREMO_INFINITO]->val1;
			}
			if(aumento > 0)
				d.damage += (d.damage / 100) * aumento;
		}
		if(sd){
			if(attack_type == BF_WEAPON){
				if (sc->data[SC_VIGOR]) {
					...
					//if(mapdata->flag[MF_FENDA]) mod = mod / 3; 
					struct status_data *tstatus = status_get_status_data(target);
					if(d.damage > 0 && (d.flag&BF_SHORT))
						d.damage += d.damage * (mod) / 100;
					if(d.damage2 > 0 && (d.flag&BF_SHORT))
						d.damage2 += d.damage2 * (mod) / 100;
			...
			if(sd->state.despertar > 0 && !sc->data[SC_DELAYDESPERTAR]){
				int v = 5, des = sd->state.despertar;
				if(des == 8) { 
					v = 10;
				}
				else if(des == 9) { 
				...
				if (tstatus->def_ele == (sd->state.eledespertar - 1)){
					if(sd->state.despertarmenor) v = v / 4;
					d.damage += d.damage * v / 100;
				}
			}
			if(sd->state.despertartwo > 0 && !sc->data[SC_DELAYDESPERTAR]){
				int v = 5, des = sd->state.despertartwo;
				if(des == 8) { 
					v = 10;
				}
				else if(des == 9) { 
				...
				if (tstatus->def_ele == (sd->state.eledespertar - 1)){
					if(sd->state.despertarmenortwo) v = v / 4;
					d.damage += d.damage * v / 100;
				}
			}
			
			if(sd->sc.data[SC_MTF_MLEATKED]){
					if(d.damage > 0 && (d.flag&BF_SHORT))
						d.damage += (d.damage / 100) * sd->sc.data[SC_MTF_MLEATKED]->val1;
					if(d.damage2 > 0 && (d.flag&BF_SHORT))
						d.damage2 += (d.damage2 / 100) * sd->sc.data[SC_MTF_MLEATKED]->val1;
			}
			if(sd->sc.data[SC_SUPREMO_INFINITO]){
					if(d.damage > 0 && (d.flag&BF_SHORT))
						d.damage += (d.damage / 100) *  sd->sc.data[SC_SUPREMO_INFINITO]->val1;
					if(d.damage2 > 0 && (d.flag&BF_SHORT))
						d.damage2 += (d.damage2 / 100) *  sd->sc.data[SC_SUPREMO_INFINITO]->val1;
			}
			...
			if (skill_id) { 
				d.damage += (d.damage / 100) * pc_skillatk_bonus_devon(sd, skill_id);
				d.damage += (d.damage / 100) * pc_skillatk_bonus_final(sd, skill_id);
			}

			// 1x temporada
			if(sd->state.temporada == 1 && target->type == BL_MOB && sd->status.base_level >= 250){
				d.damage += (d.damage / 100) * 10;
				d.damage2 += (d.damage2 / 100) * 10;
			}
			...
			// devon
			// Elixir Selvagem
			if (sd->sc.data[SC_EFEITO_RH_12] && mapdata->flag[MF_BIOSAVAGE]) {
				d.damage += (d.damage / 100) * sd->sc.data[SC_EFEITO_RH_12]->val1;
			}
			if (sd->sc.data[SC_EFEITO_RH_37] && (mapdata->flag[MF_CHEFFEXTREME] || mapdata->flag[MF_BIOEXTREME] || 
						mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_FLORESTAPES])) {
				d.damage += (d.damage / 100) * sd->sc.data[SC_EFEITO_RH_37]->val1;
			}
			// Macaroons Sortidos
			if (sd->sc.data[SC_EFEITO_RH_18] && (attack_type == BF_WEAPON || attack_type == BF_MISC)) {
				d.damage += (d.damage / 100) * sd->sc.data[SC_EFEITO_RH_18]->val1;
			}
			// Taça da Felicidade
			if (sd->sc.data[SC_EFEITO_RH_19] && attack_type == BF_MAGIC) {
				d.damage += (d.damage / 100) * sd->sc.data[SC_EFEITO_RH_19]->val1;
			}

			int adicionalmari = 0;
			if(sd->sc.data[SC_MARIONETE_SAMU]) {
				int ms = sd->sc.data[SC_MARIONETE_SAMU]->val1;
				adicionalmari = 3+(ms>=2?3:0)+(ms>=3?3:0)+(ms>=4?3:0)+(ms>=5?3:0)+(ms>=6?3:0)+(ms>=7?6:0)+(ms>=8?6:0)+(ms>=9?15:0)+(ms>=10?15:0);
			}
			if (attack_type == BF_MAGIC) {
				d.damage += (d.damage / 100) * (sd->bonus.magic_dano_final + adicionalmari);
			}
			if (d.flag&BF_SHORT) {
				d.damage += (d.damage / 100) * (sd->bonus.short_dano_final + adicionalmari);
			}
			if (d.flag&BF_LONG) {
				d.damage += (d.damage / 100) * (sd->bonus.long_dano_final + adicionalmari);
			}
			if(sd && sd->state.grade_shadow_brinco >= 1){
				int gaumento = sd->state.grade_shadow_brinco;
				if(gaumento > 2) gaumento -= 1;
				d.damage += (d.damage / 100) * (gaumento * 5);
			}
			if(sd && sd->state.grade_shadow_colar >= 3){
				d.damage += (d.damage / 100) * 5;
			}
			...
			int guerreiro = pc_checkskill(sd,NT_GUERREIRO);
			if(guerreiro == 1)
				d.damage += (d.damage / 100) * 1;
			else if(guerreiro == 2)
				d.damage += (d.damage / 100) * 3;
			else if(guerreiro == 3)
				d.damage += (d.damage / 100) * 5;
			else if(guerreiro == 4)
				d.damage += (d.damage / 100) * 10;
			
			int maeaniver = pc_checkskill(sd,RB_MAESTRIA_ANIVERSARIO);
			if(maeaniver == 1)
				d.damage += (d.damage / 100) * 1;
			else if(maeaniver == 2)
				d.damage += (d.damage / 100) * 2;
			else if(maeaniver == 3)
				d.damage += (d.damage / 100) * 3;
			else if(maeaniver == 4)
				d.damage += (d.damage / 100) * 5;
		...
		if (mapdata->flag[MF_FLORESTAPES]){

			int countmob = unit_countfloresta(bl);
			//ShowWarning("monstros = %d \n", countmob);
			if(countmob >= 5){
				int contamob = 2;
				if(countmob >= 10) contamob = contamob * 2;
				if(countmob >= 14) contamob = contamob * 2;
				if(countmob >= 17) contamob = contamob * 2;
				if(countmob >= 19) contamob = contamob * 2;
				if(countmob >= 20) contamob = contamob * 2;

				d.damage = d.damage / contamob;
			}
		}
		
		if ((mapdata->flag[MF_MASMORRAINVERNAL] || mapdata->flag[MF_MECANICAS]) && sd->sc.data[SC_MASMORRA_DANO]){
			d.damage += (d.damage / 100) * sd->sc.data[SC_MASMORRA_DANO]->val1;
		}
		
		if(skill_id == GN_SPORE_EXPLOSION){
			d.damage += (d.damage / 100) * 30;
		}
		if(skill_id == GN_CARTCANNON){
			d.damage += (d.damage / 100) * 55;
		}
		if(skill_id == NC_ARMSCANNON){
			d.damage += (d.damage / 100) * 30;
		}
		if(skill_id == AB_ADORAMUS){
			d.damage += (d.damage / 100) * 15;
		}
		if(skill_id == NC_AXETORNADO){
			d.damage += (d.damage / 100) * 56;
		}
		if(skill_id == SU_PICKYPECK){
			d.damage += (d.damage / 100) * 180;
		}
		
		if (sd->sc.data[SC_VENEPROFUN_1]) {
				d.damage -= (d.damage / 100) * 10;
		}
		if (sd->sc.data[SC_VENEPROFUN_2]) {
				d.damage -= (d.damage / 100) * 20;
		}
		if (sd->sc.data[SC_VENEPROFUN_3]) {
				d.damage -= (d.damage / 100) * 30;
		}
		if (sd->sc.data[SC_VENEPROFUN_4]) {
				d.damage -= (d.damage / 100) * 99;
		}
		if (sd->sc.data[SC_DEBUFF_MINION]) {
				d.damage -= (d.damage / 100) * sd->sc.data[SC_DEBUFF_MINION]->val1;
		}
		
		if(
			mapdata->flag[MF_CHEFFTRANS] && skill_id && 
			(skill_id != WL_JACKFROST && skill_id != WL_CRIMSONROCK && skill_id != NJ_HUUJIN && skill_id != NJ_HYOUSENSOU && skill_id != NJ_KOUENKA && skill_id != SC_TRIANGLESHOT && skill_id != LG_RAYOFGENESIS && skill_id != PA_PRESSURE && skill_id != SP_SWHOO && skill_id != SP_SPA && skill_id != RK_HUNDREDSPEAR && skill_id != HN_DOUBLEBOWLINGBASH && skill_id != SR_RAMPAGEBLASTER && skill_id != RA_ARROWSTORM && skill_id != WM_SEVERE_RAINSTORM )
		){
			d.damage = 0;
		}
		
		if(mapdata->flag[MF_TRIALVALE]){
			if(sd->status.class_ == JOB_SHADOW_CROSS){
				if(d.damage) d.damage += (d.damage * 0.75);
		 		if(d.damage2) d.damage2 += (d.damage2 * 0.75);
			}
			if(sd->status.class_ == JOB_NIGHT_WATCH){
				if(d.damage) d.damage += (d.damage * 3.5);
				if(d.damage2) d.damage2 += (d.damage2 * 3.5);
				
				
			}
			if(sd->status.class_ == JOB_SOUL_ASCETIC){
				if(d.damage) d.damage += (d.damage * 6);
				if(d.damage2) d.damage2 += (d.damage2 * 6);
			}
			if(skill_id == NW_THE_VIGILANTE_AT_NIGHT || skill_id == NW_ONLY_ONE_BULLET || skill_id == NW_SPIRAL_SHOOTING || skill_id == NW_MAGAZINE_FOR_ONE || skill_id == NW_WILD_FIRE || skill_id == NW_HASTY_FIRE_IN_THE_HOLE || skill_id == NW_GRENADES_DROPPING || skill_id == NW_MISSION_BOMBARD || skill_id == SOA_TALISMAN_OF_FOUR_BEARING_GOD || skill_id == SOA_TALISMAN_OF_RED_PHOENIX || skill_id == SOA_TALISMAN_OF_WHITE_TIGER || skill_id == SOA_TALISMAN_OF_BLACK_TORTOISE || skill_id == SOA_TALISMAN_OF_BLUE_DRAGON){
				if(d.damage) d.damage = d.damage * 4;
				
			}
		}

	} else if (bl->type == BL_MOB){
		mob_data *mmd = BL_CAST(BL_MOB, bl);

		if(mapdata->flag[MF_MECANICAS]){
			if(skill_id == ABC_ABYSS_STRIKE || skill_id == SH_HYUN_ROKS_BREEZE || skill_id == AG_RAIN_OF_CRYSTAL || skill_id == AG_ASTRAL_STRIKE || skill_id == NPC_EARTHSTRAIN || skill_id == SS_ANKOKURYUUAKUMU)
				d.damage = 2000000000;
			
			if(skill_id == NPC_SELFDESTRUCTION){
				if(target->type == BL_MOB)
					d.damage = 0;
				else
					d.damage = 2000000000;
			...
			if(tsd && tsd->sc.data[SC_GRAVITYCONTROL] && (skill_id == NT_STORMGUST || !skill_id)){
				d.damage = 0;
				d.damage2 = 0;
			}
			
			if(skill_id == EM_VENOM_SWAMP){
				d.damage = (status_get_max_hp(target) / 100) * 15;
			}
		}
		
				
		if(tsd && tsd->sc.data[SC_EFEITO_RH_14]){
			d.damage = d.damage - (d.damage / 100) * tsd->sc.data[SC_EFEITO_RH_14]->val1;
		}
		
		if(
			mmd->db->id == 22934 || mmd->db->id == 22933 || // Hard
			mmd->db->id == 22946 || mmd->db->id == 22948 || // Unreal
			mmd->db->id == 22952 || mmd->db->id == 22951 || // Pesadelo
			mmd->db->id == 22958 || mmd->db->id == 22957 || mmd->db->id == 22962 || mmd->db->id == 22961 || // Ultimate
			mmd->db->id == 31831 || mmd->db->id == 31832 || mmd->db->id == 31833 || mmd->db->id == 31834 || mmd->db->id == 31835 || // history stage
			mmd->db->id == 31836 || mmd->db->id == 31837 || mmd->db->id == 31838 || // Evento Bonus
			mmd->db->id == 23308 || mmd->db->id == 23309 || mmd->db->id == 23310 || mmd->db->id == 23311 // halloween
		){
			if(
				skill_id == MG_THUNDERSTORM || skill_id == HN_GROUND_GRAVITATION || skill_id == WZ_STORMGUST || skill_id == EM_DIAMOND_STORM || 
				skill_id == WZ_METEOR || skill_id == NPC_EARTHSTRAIN || skill_id == EM_TERRA_DRIVE || skill_id == NW_GRENADES_DROPPING || 
				skill_id == AG_VIOLENT_QUAKE || skill_id == AG_MYSTERY_ILLUSION || skill_id == HN_HELLS_DRIVE || skill_id == AG_STORM_CANNON
			){
				if(skill_id == HN_HELLS_DRIVE)
					d.damage = (status_get_max_hp(target) / 100) * 50;
				else
					d.damage = 2000000000;
			}
			if(skill_id == NPC_SELFDESTRUCTION){
		...
		if(!skill_id && (mmd->db->id == 22935 || mmd->db->id == 22941 || mmd->db->id == 22947 || mmd->db->id == 22953 || mmd->db->id == 22959)){
			int danofixo = 0, danohp = 5;
			
			if(mmd->db->id == 22935) danohp = 10; // Hard: Boss instanciado
			if(mmd->db->id == 22947) danohp = 10; // Unreal: Boss instanciado
			if (mmd->db->id == 22953) {
				danofixo += danofixo; // Pesadelo: Boss Mapa / Boss instanciado
				danohp += danohp;
			}
			if (mmd->db->id == 22959) {
				danofixo = danofixo * 3; // Ultimate: boss instanciado
				danohp = danohp * 3;
			}

			d.damage += ((status_get_max_hp(target) / 100) * danohp) + danofixo;
		}
		
		if((sc && sc->data[SC_MAXPAIN]) ||  skill_id == NPC_MAXPAIN_ATK || skill_id == NPC_MAXPAIN){
			return d;
		}

		if(mapdata->flag[MF_BIOHARD] || mapdata->flag[MF_VARMEAL]){
			if(mapdata->flag[MF_DROPBABY]){
				d.damage += (d.damage / 100) * 70;
			} else {
				d.damage += d.damage;
		...
		if (mapdata->flag[MF_BIOEXTREME]){
			if(d.damage > 0) d.damage += (d.damage / 100) * battle_config.dano_extremebio;
			if(d.damage2 > 0) d.damage2 += (d.damage2 / 100) * battle_config.dano_extremebio;
		}
		if (mapdata->flag[MF_CHEFFEXTREME]){
			if(d.damage > 0) d.damage += (d.damage / 100) * battle_config.dano_extremecheff;
			if(d.damage2 > 0) d.damage2 += (d.damage2 / 100) * battle_config.dano_extremecheff;
		}	
		if (mapdata->flag[MF_BIOSAVAGE]){
			mob_data *md = BL_CAST(BL_MOB, bl);
			if(md->db->id != 28026 && md->db->id != 28027 && md->db->id != 28028){
				d.damage += d.damage * 3;
				d.damage2 += d.damage2 * 3;
			} else if(skill_id == NPC_DRAGONBREATH || skill_id == RK_DRAGONBREATH_WATER){
				d.damage += (d.damage / 2);
			}
		}
		if (mapdata->flag[MF_ILLUSIONGUILD] ){
			d.damage += d.damage * 6;
			d.damage2 += d.damage2 * 6;
		}
		if (mapdata->flag[MF_CONSTUNREAL]){
			d.damage += d.damage * 3;
			d.damage2 += d.damage2 * 3;
		}
		if (mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP] || mapdata->flag[MF_TEMPLOSAGRADO]){
			if(skill_id == ASC_BREAKER && !mapdata->flag[MF_TEMPLOSAGRADO]){
				d.damage += d.damage * 3;
			}
			if(skill_id == WL_EARTHSTRAIN || skill_id == WZ_STORMGUST || skill_id == WZ_METEOR || skill_id == WZ_VERMILION){
				d.damage = 2000000000;
		...
		if (mapdata->flag[MF_CHEFFTRANS]){
			d.damage += d.damage * 5;
			d.damage2 += d.damage2 * 5;
		}
		if (mapdata->flag[MF_VILAGRANDORC]){
			if(status_get_class_(bl) == CLASS_BOSS ){
				d.damage = d.damage * 3;
			}
		}

		if(mmd->db->id == 21533 && skill_id == 5227){
			d.damage = d.damage / 2;
		}
		if (mmd->db->id == 2235 && mapdata->flag[MF_CHEFFTRANS]) {
			d.damage -= (d.damage / 100) * 40;
		}
		if(mmd->db->id == 1112 && mmd->db->id == 2251){
			d.damage += d.damage;
		}
		
		if (mapdata->flag[MF_MASMORRAINVERNAL]){
			if(skill_id == NPC_SELFDESTRUCTION){
				if(target->type == BL_MOB)
					d.damage = 0;
				else
		...
		if (mapdata->flag[MF_COMETAPASSING]){
			if(skill_id == NPC_SELFDESTRUCTION){
				if(target->type == BL_MOB)
					d.damage = 0;
				else
			...
			mob_data *md = BL_CAST(BL_MOB, bl);
			if( skill_id == HT_CLAYMORETRAP || skill_id == EM_DIAMOND_STORM || skill_id == AG_RAIN_OF_CRYSTAL || skill_id == CD_PNEUMATICUS_PROCELLA || skill_id == NPC_COMET_BOSS || skill_id == NPC_EARTHSTRAIN || skill_id == AG_FLORAL_FLARE_ROAD || skill_id == WL_COMET || skill_id == WL_EARTHSTRAIN){
				d.damage = 2000000000;
			}
		...
		if(mapdata->flag[MF_TEMPLOULTIMATE]){
			if(skill_id == NPC_PSYCHIC_WAVE) d.damage = (status_get_max_hp(target) / 100) * 45;
			if(skill_id == MG_FIREBALL) d.damage += (status_get_max_hp(target) / 100) * 35;
			if(skill_id == WZ_HEAVENDRIVE) d.damage += (status_get_max_hp(target) / 100) * 25;
			if(skill_id == CR_SHIELDBOOMERANG) d.damage += (status_get_max_hp(target) / 100) * 25;
			if(skill_id == MG_LIGHTNINGBOLT) d.damage += (status_get_max_hp(target) / 100) * 20;
			if(skill_id == KN_SPEARSTAB) d.damage += (status_get_max_hp(target) / 100) * 30;
			if(skill_id == KN_BRANDISHSPEAR) d.damage += (status_get_max_hp(target) / 100) * 20;
			if(skill_id == KN_SPEARBOOMERANG) d.damage += (status_get_max_hp(target) / 100) * 20;
			if(skill_id == SU_CN_METEOR) d.damage += (status_get_max_hp(target) / 100) * 20;
		}

		// -------------------------------------------------------
		//   reducao de dano final por itens em hard e extreme
		// -------------------------------------------------------
		
		
		if(skill_id == NPC_PARRY && tsd){
			if(!tsd->sc.data[SC_PARRY]){
				d.damage = (status_get_max_hp(target) / 100) * 90;
				if(!tsd->sc.data[SC_DEBUFF_MINION]) sc_start(target, target, SC_DEBUFF_MINION, 100, 20, 30000);
			} else {
				sc_start(target, target, SC_MASMORRA_DANO, 100, 10, 30000);
				clif_skill_nodamage(&tsd->bl, &tsd->bl, AL_BLESSING, 1, 1);
			}
		}
		
		if(!skill_id && tsd && tsd->state.percent_dano > 0 && mapdata->flag[MF_MASMORRAINVERNAL] && mmd->db->id != 1904){
			d.damage += ((status_get_max_hp(target) / 100) * tsd->state.percent_dano) + 5000;
		}
		
		if(skill_id == NT_STORMGUST){
				d.damage = 2000000000;
		}
		

		
		if(tsd && mapdata->flag[MF_MASMORRAINVERNAL] && tsd->sc.data[SC_GRAVITYCONTROL] && (skill_id == NT_STORMGUST || !skill_id)){
			d.damage = 0;
			d.damage2 = 0;
		}
		
		if(mapdata->flag[MF_MASMORRAINVERNAL] && skill_id == NJ_KAENSIN){
			d.damage = (status_get_max_hp(target) / 100) * 15;
			sc_start4(bl,target,SC_SOMBRIO_03,30,skill_lv,1000,bl->id,0,15000);
		}
		


		if (mapdata->flag[MF_FLORESTAPES]){
			mob_data *fmd = BL_CAST(BL_MOB, bl);
			d.damage += d.damage * 1.5;
			if(!skill_id) d.damage += d.damage * 6;
			if(skill_id == MG_FIREBOLT || skill_id == MG_LIGHTNINGBOLT || skill_id == MG_COLDBOLT || skill_id == MG_FROSTDIVER) d.damage += d.damage * 4;

			if(fmd->db->id == 22922 && (skill_id == MG_FIREBOLT || skill_id == MG_COLDBOLT)) d.damage -= ((d.damage / 100) * 50);
			if(fmd->db->id == 22921 && skill_id == MG_FIREBOLT) d.damage -= ((d.damage / 100) * 50);

			if(skill_id == NPC_PSYCHIC_WAVE) d.damage = (status_get_max_hp(target) / 100) * 30;

			if(skill_id == AB_ADORAMUS) d.damage -= ((d.damage / 100) * 30);
			if(skill_id == AG_SOUL_VC_STRIKE) d.damage -= ((d.damage / 100) * 20);

			if(skill_id == NPC_GRANDDARKNESS) d.damage -= ((d.damage / 100) * 30);
			if(skill_id == ASC_METEORASSAULT) d.damage -= ((d.damage / 100) * 20);


			if(tsd && (tsd->class_&MAPID_UPPERMASK) == MAPID_ASSASSIN) d.damage -= ((d.damage / 100) * 60);

			if(skill_id == RK_DRAGONBREATH){
				d.damage = (status_get_max_hp(target) / 100) * 10;
			}

			if(!skill_id){
				int danofixo = 0, danohp = 5;
				if (mapdata->flag[MF_VULCAOPESADELO]){
					danofixo = 5000;
					danohp = 9;
				}
		...
		if (mapdata->flag[MF_MECANICAS] && !mapdata->flag[MF_SAMURAIMESTRE]){
			if(!skill_id){
				int danofixo = 5000, danohp = 9;
				// Normal
				if(mmd->db->id == 22941) danohp = 10; // Boss instanciado
				// HARD
				if(mmd->db->id == 22935) danohp = 15; // Boss instanciado 
				
				// Unreal
				if (mapdata->flag[MF_DOMINIOUNREAL]){
					if(mmd->db->id == 22947) {
						danofixo = 10000;
						danohp = 18; // Boss instanciado 
					}
				}
				// Pesadelo
				if (mapdata->flag[MF_DOMINIOPESADELO]){
					if(mmd->db->id == 22953) {
						danofixo = 15000;
						danohp = 27; // Boss instanciado 
					}
				}
				// Ultimate
				if (mapdata->flag[MF_DOMINIOULTIMATE]){
					if(mmd->db->id == 22959) {
						danofixo = 20000;
						danohp = 45; // Boss instanciado 
					}
				}
		...
		if (mapdata->flag[MF_MAPAS_275]){
			if(!skill_id){
				int danofixo = 1000, danohp = 5;
				// mVp
				if(mmd->db->id == 22404 || mmd->db->id == 22394) danohp = 20;

				d.damage += ((status_get_max_hp(target) / 100) * danohp) + danofixo;
			}
		}
		
		if (mapdata->flag[MF_DOMINIOGERAL]){
			if(!skill_id){
				int danofixo = 0, danohp = 5;
				// Normal
				if(mmd->db->id == 22940 || mmd->db->id == 22939) danohp = 7; // Boss Mapa / Boss Fate
				
				// Hard
				if(mmd->db->id == 22931 || mmd->db->id == 22932 || mmd->db->id == 22936) danohp = 7; // monstros
				if(mmd->db->id == 22934 || mmd->db->id == 22935 || mmd->db->id == 22933) danohp = 10; // Boss Mapa / Fate
				
				// Unreal
				if (mapdata->flag[MF_DOMINIOUNREAL]){
					danofixo = 5000;
					danohp = 9;
					if(mmd->db->id == 22946 || mmd->db->id == 22948) { // Boss Mapa / Fate
						danofixo = 7500;
						danohp = 13; 
					}
				}
				...
				if (mapdata->flag[MF_DOMINIOPESADELO]){
					danofixo = 7500;
					danohp = 16;
					if(mmd->db->id == 22952 || mmd->db->id == 22951) { // Boss Mapa / Fate
						danofixo = 10000; 
						danohp = 18;
					}
				}
				...
				if (mapdata->flag[MF_DOMINIOULTIMATE]){
					danofixo = 12500;
					danohp = 30;
					if(mmd->db->id == 22957 || mmd->db->id == 22958 || mmd->db->id == 22961) { // fate / boss mapa // Normal Horda
						danofixo = 15000; 
						danohp = 40;
					}
					if(mmd->db->id == 22962) { // fate Horda
						danofixo = 17500; 
						danohp = 40;
					}
				}
				...
				// Aumento dano Fate
				if(
					mmd->db->id == 22939 || mmd->db->id == 22933 ||
					mmd->db->id == 22948 || mmd->db->id == 22951 ||
					mmd->db->id == 22957 || mmd->db->id == 22962
				){
					d.damage += ((d.damage / 100) * 20);
				}
			}
			
			if(tsd && tsd->state.goblinferido == 1) {
				d.damage += d.damage / 2;
			}
			
			TBL_MER* mder = BL_CAST(BL_MER, target);
			if(mder && mder->master && mder->db->class_ >= 28056 && mder->db->class_ <= 28061){
				d.damage = 1;
			}
		}
		
		if (!skill_id && mmd) {
			if(mmd->status.receivehppercent){
				d.damage += ((status_get_max_hp(target) / 100) * mmd->status.receivehppercent);
			}
			if (mmd->status.receivehpfixed){
				d.damage += mmd->status.receivehpfixed;
			}
		}
		
		if (mapdata->flag[MF_SAMURAIMESTRE]){
			if(!skill_id){
				int danofixo = 0, danohp = 5;
				
				// Hard
				if(mmd->db->id == 31804) danohp = 10; // Boss Mapa / Fate
				
				// Unreal
				if(mmd->db->id == 31817) { // Boss Mapa / Fate
					danofixo = 7500;
					danohp = 13; 
				}
		...
		if (mapdata->flag[MF_GUERRAPVM]){
			if(!skill_id){
				int danofixo = 7500, danohp = 27;
				d.damage += ((status_get_max_hp(target) / 100) * danohp) + danofixo;
			}
			if(skill_id == EM_TERRA_DRIVE) d.damage = 2000000000;
			if(skill_id == NJ_HYOUSENSOU) d.damage = ((status_get_max_hp(target) / 100) * 20);
		}
		if (mapdata->flag[MF_EVENTOBONUS]){
			if(!skill_id){
				int danofixo = 15000, danohp = 27;
				d.damage += ((status_get_max_hp(target) / 100) * danohp) + danofixo;
			}
		}
		if (mapdata->flag[MF_HISTORYSTAGE]){
			if(!skill_id && (mmd->db->id == 31831 || mmd->db->id == 31832 || mmd->db->id == 31833 || mmd->db->id == 31834 || mmd->db->id == 31835)){
				int danofixo = 7500, danohp = 13;
		...
		if(mmd->db->id == 22976 && tsd && !tsd->sc.data[SC_DEBUFF_MINION]) sc_start(target, target, SC_DEBUFF_MINION, 100, 20, 10000);
		
		if (mapdata->flag[MF_STATUSAPRIMORADO] || mapdata->flag[MF_DOMINIOULTIMATE] || (mmd && mmd->status.chancestatusapri > 0)){
			int chanceapri = 1, maxchance = 100;
			if(mmd->status.chancestatusapri) 
				chanceapri = mmd->status.chancestatusapri;
			if(mapdata->flag[MF_DOMINIOULTIMATE]) chanceapri = 2;
			if(tsd && !tsd->sc.data[SC_EFEITO_RH_16] && rnd()% maxchance <= chanceapri
			){
				int random = rnd()%6;
				int chanceitem = rnd()%100;
				struct status_data *tstatusdom; ///< Target status data
				tstatusdom = status_get_status_data(target);
				
				if(random == 1 && tstatusdom->luk < 230) {
					if(chanceitem > tsd->bonus.resist_caos) { 
						sc_start(bl, target, SC_CAOS_V2, 100, 20, 10000); 
						clif_showscript(bl, "Caos Aprimorado", AREA); 
					}
				} else if(random == 2 && tstatusdom->agi < 230){
					if(chanceitem > tsd->bonus.resist_congela) { 
						sc_start(bl, target, SC_CONGELAR_V2, 100, 20, 10000); 
						clif_showscript(bl, "Congelar Aprimorado", AREA); 
					}
				} else if(random == 3 && tstatusdom->vit < 230){ 
					if(chanceitem > tsd->bonus.resist_envenenar) { 
						sc_start(bl, target, SC_ENVENENAR_V2, 100, 20, 10000); 
						clif_showscript(bl, "Envenenar Aprimorado", AREA); 
					}
				} else if(random == 4 && tstatusdom->dex < 230){ 
					if(chanceitem > tsd->bonus.resist_hipotermia){ 
						sc_start(bl, target, SC_HIPOTERMIA_V2, 100, 20, 10000); 
						clif_showscript(bl, "Hipotermia Aprimorada", AREA);
					}
				} else if(random == 5 && tstatusdom->luk < 230){
					if(chanceitem > tsd->bonus.resist_maldicao) {
						sc_start(bl, target, SC_MALDICAO_V2, 100, 20, 10000);
						clif_showscript(bl, "Maldição Aprimorada", AREA);
					}
				} else if(tstatusdom->int_ < 230){ 
					if(chanceitem > tsd->bonus.resist_silence){ 
						sc_start(bl, target, SC_SILENCE_V2, 100, 20, 10000); 
						clif_showscript(bl, "Silêncio Aprimorado", AREA);
					}
				}
			}
		}
		if (mapdata->flag[MF_DOMINIOULTIMATE] && skill_id == MG_THUNDERSTORM){
			d.damage = 2000000000;
		}
	}

	if(tscc && tscc->data[SC_ASH]){
		if( skill_get_ele(skill_id, skill_lv) == ELE_FIRE || status_get_attack_sc_element(bl,status_get_sc(bl)) == ELE_FIRE)
			d.damage += d.damage / 2;
	}
	...
	if((skill_id == EM_DIAMOND_STORM || skill_id == WZ_STORMGUST || skill_id == EM_TERRA_DRIVE || skill_id == WZ_METEOR || skill_id == SO_DIAMONDDUST || skill_id == WZ_VERMILION || skill_id == WL_EARTHSTRAIN) && map_getcell(target->m, target->x, target->y, CELL_CHKLANDPROTECTOR)){
		d.damage = 0;
	} 	
	// Shinkiro e Shiranui
	if(skill_id == SS_KUNAIKAITEN){
		d.damage += (d.damage / 100) * 54;
	}
	if(skill_id == SS_KUNAIWAIKYOKU){
		d.damage += (d.damage / 100) * 55;
	}
	if(skill_id == KO_JYUMONJIKIRI){
		d.damage += (d.damage / 100) * 53;
	}
	if(skill_id == RK_HUNDREDSPEAR){
		d.damage += (d.damage / 100) * 20;
	}
	...
	// 		Buff no Sa
	// ----------------------------------------------
	if(skill_id == HN_JUPITEL_THUNDER_STORM || skill_id == HN_NAPALM_VULCAN_STRIKE){
		if(sd && (sd->bonus.verifica_oned > 0 || sd->bonus.verifica_onee > 0))
			d.damage = d.damage;
		else
			d.damage += (d.damage / 100) * 20;
	}
	...
	// 		Buff no Renegado
	// ----------------------------------------------
	if(skill_id == SC_FATALMENACE){
		d.damage += (d.damage / 100) * 60;
	}
	...
	if (mapdata->flag[MF_TRIAL]) { 
		if(bl->type == BL_MOB){
			mob_data *md = BL_CAST(BL_MOB, bl);
			if(tsd && !tsd->status.shield) d.damage = (d.damage / 100) * 50;
			if(md && (md->db->id >= 21890 && md->db->id <= 21893)){
				d.damage = 2000000000;
			}
		}
		if(bl->type == BL_PC && skill_id == GC_CROSSIMPACT){
			d.damage += (d.damage / 100) * 50;
		}
	}

	if ((skill_id == WL_COMET || skill_id == EM_CONFLAGRATION) && mapdata->flag[MF_TEMPLOULTIMATE] && target->type == BL_MOB) { 
		mob_data *md = BL_CAST(BL_MOB, target);
		if(md && md->base_status && md->base_status->luk != 201)
			d.damage = 0;
	} 

	if (mapdata->flag[MF_FENDA]) { 
		if(skill_id == NPC_SELFDESTRUCTION){
			if(target->type == BL_MOB)
				d.damage = 0;
			else
		...
		if (sd && sd->sc.data[SC_EFEITO_RH_15]) {
			d.damage += (d.damage / 100) * sd->sc.data[SC_EFEITO_RH_15]->val1;
		}
		
		if (sd && skill_id == LG_CANNONSPEAR){
			d.damage += (d.damage / 100) * 75;
		}

		if (bl->type == BL_MOB && tsd && tsd->status.class_ == JOB_WINDHAWK){
				d.damage = (d.damage / 100) * 70;
		}
		...
		if(bl->type == BL_MOB){
			...
			if(md->db->id == 28042 || md->db->id == 28051 || (md->db->id >= 28101 && md->db->id <= 28110)){
				if(tsd && tsd->state.danoboss){
					d.damage = (d.damage / 100) * tsd->state.danoboss;
				}
			} else {
				if(tsd && tsd->state.danofenda){
					d.damage += (d.damage / 100) * tsd->state.danofenda;
				}
			}
		} else if(sd && sd->state.reducaofenda){
			d.damage = d.damage - ((d.damage / 100) * sd->state.reducaofenda);
			d.damage2 = d.damage2 - ((d.damage2 / 100) * sd->state.reducaofenda);
		}

		if(tsd && (tsd->class_&MAPID_UPPERMASK) == MAPID_ASSASSIN) d.damage -= ((d.damage / 100) * 30);
	}

	// -------------------------------------------------------
	//   Aumento de dano final por itens
	// -------------------------------------------------------
	if (skill_id == AB_ADORAMUS){
		d.damage += (d.damage / 100) * 25;
		if(d.damage2) d.damage2 += (d.damage2 / 100) * 25;
	}
	...
	if (skill_id == RK_DRAGONBREATH || skill_id == RK_DRAGONBREATH_WATER){
		d.damage += (d.damage / 100) * 30;
		if(d.damage2) d.damage2 += (d.damage2 / 100) * 30;
	}

	if (mapdata->flag[MF_TEMPLOULTIMATE]){
		if(skill_id == NJ_HYOUSENSOU && bl->type == BL_MOB){
			d.damage = 1000000000;
		}
		if (sd && sd->sc.data[SC_SOMBRIO_01]) d.damage = d.damage / 2;
		if(bl->type == BL_MOB && !skill_id) { d.damage = (status_get_max_hp(target) / 100) * (3 + (rnd() % 2)); }
		if((skill_id == RK_DRAGONBREATH_WATER || skill_id == RK_DRAGONBREATH) && bl->type == BL_MOB){
			if(tsd && checar_equip(EQI_SHADOW_ACC_L,tsd, 4, 0, 0) > 0){
				if(tsd->status.class_ == JOB_SPIRIT_HANDLER || tsd->status.class_ == JOB_CARDINAL || tsd->status.class_ == JOB_SOUL_ASCETIC)
					d.damage = (status_get_max_hp(target) / 100) * 30;
				else
					d.damage = (status_get_max_hp(target) / 100) * 40;
			} else
				d.damage = (status_get_max_hp(target) / 100) * 50;
		}
	}
	...
	// Coisas BL_PC
	if(sd){
		if(sd->reputation[13].points >= 1000 && mapdata->flag[MF_DOMINIOHARD]){
			d.damage += ((d.damage / 100) * (((sd->reputation[3].points / 1000) / 1))*2);
		}
		if(sd->reputation[14].points >= 1000 && mapdata->flag[MF_DOMINIOUNREAL]){
			d.damage += ((d.damage / 100) * (((sd->reputation[3].points / 1000) / 1))*2);
		}
		if(sd->reputation[15].points >= 1000 && mapdata->flag[MF_DOMINIOPESADELO]){
			d.damage += ((d.damage / 100) * (((sd->reputation[3].points / 1000) / 1))*2);
		}
		if(sd->reputation[11].points >= 1000 && mapdata->flag[MF_DOMINIOULTIMATE]){
			d.damage += ((d.damage / 100) * (((sd->reputation[3].points / 1000) / 1)));
		}
		if(sd->reputation[16].points >= 1000 && mapdata->flag[MF_DOMINIOULTIMATE]){
			d.damage += ((d.damage / 100) * (((sd->reputation[3].points / 1000) / 1)));
		}

		if (sd->bonus.dano_final) { 
			d.damage += (d.damage / 100) * (sd->bonus.dano_final * 1.1);
		}
		
		if ((d.flag&BF_MAGIC)){
			if (sd->bonus.corrupcao_magico) { 
				d.damage += (d.damage / 100) * sd->bonus.corrupcao_magico;
			}
		}
		
		if((d.flag&BF_SHORT)){
			if (sd->bonus.corrupcao_fisico) { 
				d.damage += (d.damage / 100) * sd->bonus.corrupcao_fisico;
			}
		}
		
		if(!(d.flag&BF_SHORT) && !(d.flag&BF_MAGIC)){
			if (sd->bonus.corrupcao_distancia) { 
				d.damage += (d.damage / 100) * sd->bonus.corrupcao_distancia;
			}
		}

		if (sd->bonus.penetracao) { 
			d.damage += (d.damage / 100) * (sd->bonus.penetracao * 1.1);
		}
		if(!mapdata->flag[MF_RESTIGUALITARIA]){
			if (sd->bonus.perfuracao) { 
				d.damage += (d.damage / 100) * (sd->bonus.perfuracao * 1.1);
			}
		}
		if (sd->bonus.determinacao) { 
			d.damage += (d.damage / 100) * sd->bonus.determinacao;
		}
		if(battle_config.desativarexcelencia){
			if (sd->reputation[12].points > 1000 && (mapdata->flag[MF_DOMINIOGERAL] || mapdata->flag[MF_FLORESTAPES] || mapdata->flag[MF_TEMPLOULTIMATE])) { // Excelência
				int ptsexc = sd->reputation[12].points / 1000;
				int danoadicional;
				if(ptsexc >= 1) danoadicional += 1;
				if(ptsexc >= 2) danoadicional += 1;
				if(ptsexc >= 3) danoadicional += 1;
				if(ptsexc >= 4) danoadicional += 2;
				if(ptsexc >= 5) danoadicional += 2;
				if(ptsexc >= 6) danoadicional += 2;
				if(ptsexc >= 7) danoadicional += 3;
				if(ptsexc >= 8) danoadicional += 3;
				if(ptsexc >= 9) danoadicional += 3;
				d.damage += (d.damage / 100) * danoadicional;
			}
		}

		
		if(mapdata->flag[MF_DOMINIOPESADELO]){
			if(!sd->sc.data[SC_DOM_POT_PESADELO])
				d.damage = 0;
			else
				d.damage = (d.damage / 10);
				d.damage2 = (d.damage2 / 10);
		}
		if(mapdata->flag[MF_DOMINIOHARD] && !sd->sc.data[SC_DOM_POT_HARD]){
				d.damage = 0;
		}
		if(mapdata->flag[MF_DOMINIOUNREAL] && !sd->sc.data[SC_DOM_POT_UNREAL]){
				d.damage = 0;
		}
		if(mapdata->flag[MF_DOMINIOULTIMATE]){
			if(mapdata->flag[MF_DOMINIOGERAL]){
				if(sd->state.exceldrop){
					d.damage = (d.damage / 1000);
					d.damage2 = (d.damage2 / 1000);
				} else {
					mob_data *md = BL_CAST(BL_MOB, target);
					if( md && (md->db->id == 22962 || md->db->id == 22961 || md->db->id == 22958 || md->db->id == 22959)){
						if(!sd->sc.data[SC_DOM_POT_ULTIMATE_2])
							d.damage = 0;
						else
							d.damage = (d.damage / 1000);
					} else {
						if(!sd->sc.data[SC_DOM_POT_ULTIMATE])
							d.damage = 0;
						else
							d.damage = (d.damage / 1000);
					}
		...
		if(mapdata->flag[MF_DOMINIOGERAL] && sd->sc.data[SC_DOM_POT_BUFF]){
			mob_data *md = BL_CAST(BL_MOB, target);
			if( md && (
				md->db->id == 22937 || md->db->id == 22940 ||
				md->db->id == 22936 || md->db->id == 22934 ||
				md->db->id == 22945 || md->db->id == 22946 ||
				md->db->id == 22950 || md->db->id == 22952 ||
				md->db->id == 22958 || md->db->id == 22961
				)
			){
				d.damage += (d.damage / 10);
				d.damage2 += (d.damage2 / 10);
		...
		if(mapdata->flag[MF_NINEREDUCT]){
			d.damage = (d.damage / 10);
			d.damage2 = (d.damage2 / 10);
		}
		if(mapdata->flag[MF_NINENINEREDUCT]){
			d.damage = (d.damage / 100);
			d.damage2 = (d.damage2 / 100);
		}
		if(mapdata->flag[MF_GUERRAPVM]){
			d.damage = (d.damage / 10000000);
			d.damage2 = (d.damage2 / 10000000);
		}
		
		if(mapdata->flag[MF_DOMINIOGERAL] && mapdata->flag[MF_DOMINIONORMAL] && !sd->sc.data[SC_FADIGA_FLORESTA]){
			d.damage = 0;
			d.damage2 = 0;
		}
	}

	if (tsd && tsd->sc.data[SC_PARRY]) d.damage = 0;
	
	if (tsd && d.damage != 2000000000) {
		int ninja = tsd->bonus.reducao_ninja;
		if(tscc && tscc->data[SC_BUFFSAGRADO] && mapdata->flag[MF_BUFFSAGRADO]){
			ninja = 55;
		}
		if(ninja > 70) ninja = 70;
		d.damage -= (d.damage / 100) * ninja;
	}
	
	
	if(battle_config.desativarexcelencia){
		if(tsd){
			if (tsd->reputation[12].points > 1000 && (mapdata->flag[MF_DOMINIOGERAL] || mapdata->flag[MF_FLORESTAPES] || mapdata->flag[MF_TEMPLOULTIMATE])) { // Excelência
				int ptsexc = tsd->reputation[12].points / 1000;
				int danoreduct;
				if(ptsexc >= 1) danoreduct += 1;
				if(ptsexc >= 2) danoreduct += 1;
				if(ptsexc >= 3) danoreduct += 1;
				if(ptsexc >= 4) danoreduct += 1;
				if(ptsexc >= 5) danoreduct += 1;
				if(ptsexc >= 6) danoreduct += 1;
				if(ptsexc >= 7) danoreduct += 1;
				if(ptsexc >= 8) danoreduct += 1;
				if(ptsexc >= 9) danoreduct += 1;
				d.damage -= (d.damage / 100) * danoreduct;
			}
		}
	}

	if (target->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, target);
		if(mapdata->flag[MF_TORREDECRISTAL] && md){
			if(md && md->status.dmgtakenfloatultra) {
				//ShowWarning("ultra = %" PRId64 " \n", md->status.dmgtakenfloatultra);
				//ShowWarning("dano = %" PRId64 " \n", d.damage);
				d.damage = i64max(d.damage * md->status.dmgtakenfloatultra / 10000000, 1);
				if(d.damage2) d.damage2 = i64max(d.damage2 * md->status.dmgtakenfloatultra / 10000000, 1);
				//ShowWarning("dano new = %" PRId64 " \n", d.damage);
				//d.damage = d.damage * 10;
				//if(d.damage2) d.damage2 = d.damage2 * 10;
			}

			if(md->status.capmonster > 0){
				int64 danocapmob = md->status.capmonster * 1000000;
				if(d.damage > danocapmob) d.damage = danocapmob;
				if(d.damage2 > danocapmob) d.damage2 = danocapmob;
			}
			
		} else if(md) {
			// Vulnerabilidade
			if(md->status.elevulneravel && sd){
				if(md->status.elevulneravel == 1 && sd->bonus.vulnera_fire_shadow > 0) d.damage += (d.damage / 100) * (sd->bonus.vulnera_fire_shadow + sd->bonus.vulnera_all);
				else if(md->status.elevulneravel == 2 && sd->bonus.vulnera_water_undead > 0) d.damage += (d.damage / 100) * (sd->bonus.vulnera_water_undead + sd->bonus.vulnera_all);
				else if(md->status.elevulneravel == 3 && sd->bonus.vulnera_wind_poison > 0) d.damage += (d.damage / 100) * (sd->bonus.vulnera_wind_poison + sd->bonus.vulnera_all);
				else if(md->status.elevulneravel == 4 && sd->bonus.vulnera_earth_neutral > 0) d.damage += (d.damage / 100) * (sd->bonus.vulnera_earth_neutral + sd->bonus.vulnera_all);
				else if(md->status.elevulneravel == 5 && sd->bonus.vulnera_holy_ghost > 0) d.damage += (d.damage / 100) * (sd->bonus.vulnera_holy_ghost + sd->bonus.vulnera_all);
				else if(sd->bonus.vulnera_all > 0) d.damage += (d.damage / 100) * sd->bonus.vulnera_all;
			}
			if(mapdata->flag[MF_ANIVERSARIO]){
				//if(md->db->id == 21910 || md->db->id == 21911 || md->db->id == 21912){
					if(skill_id) 
						d.damage = 0;
					else
						d.damage = 1;
						if(d.damage2) d.damage2 = 0;
						
				//}
			}
			if(mapdata->flag[MF_TUMBAVAZIO]){
				if(skill_id) 
					d.damage = 0;
				else
					d.damage = 1;
			...
			if(md->db->id == 2411 && strlen(md->name) == 21){
				d.damage = (int)(d.damage / 10);
			}
			if(md->db->id == 2411 && strlen(md->name) == 22){
				d.damage = (int)(d.damage / 100);
			}
			if(md->db->id == 2411 && strlen(md->name) == 23){
				d.damage = (int)(d.damage / 1000);
			}
			if(md->db->id == 2411 && strlen(md->name) == 24){
				d.damage = (int)(d.damage / 100000);
			}
			if(md->db->id == 2411 && strlen(md->name) == 25){
				d.damage = (int)(d.damage / 1000000);
			}
		
			if(md && md->status.dmgtakenfloat) {
				d.damage = i64max(d.damage * md->status.dmgtakenfloat / 100000, 1);
				if(d.damage2) d.damage2 = i64max(d.damage2 * md->status.dmgtakenfloat / 100000, 1);
			}
			
			// sala testes
			if(md->db->id >= 28135 && md->db->id <= 28139 && strlen(md->name) == 17){
				d.damage = (int)(d.damage / 10);
			}
			if(md->db->id >= 28135 && md->db->id <= 28139 && strlen(md->name) == 18){
				d.damage = (int)(d.damage / 100);
			}
			if(md->db->id >= 28135 && md->db->id <= 28139 && strlen(md->name) == 19){
				d.damage = (int)(d.damage / 1000);
			}
			if(md->db->id >= 28135 && md->db->id <= 28139 && strlen(md->name) == 20){
				d.damage = (int)(d.damage / 10000);
			}
			if(md->db->id >= 28135 && md->db->id <= 28139 && strlen(md->name) == 21){
				d.damage = (int)(d.damage / 100000);
			}

			
			if(sd){
				if(md->db->id == 22976){ //suporte
					d.damage = 1;
					return d;
				}
				if(md->db->id == 3456){ // Melancia veneno mortal
					d.damage = 1;
					return d;
				}
				if(md->db->id == 22977){ // Agua
					int elemento = d.s_ele;
					if(attack_type == BF_WEAPON) 
						elemento = battle_get_weapon_element(&d, bl, target, skill_id, skill_lv, EQI_HAND_R, false);		
					else if(flag&BF_LONG) 
						elemento = sd->bonus.arrow_ele;
					if(elemento != 4) {
						d.damage = 0;
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Runa Elemental] Apenas danos com o elemento VENTO podem me acertar.", false, SELF);
					} else {
						d.damage = 1 * d.div_;
						return d;
					}
				}
				if(md->db->id == 22978){ // Fogo
					int elemento = d.s_ele;
					if(attack_type == BF_WEAPON) 
						elemento = battle_get_weapon_element(&d, bl, target, skill_id, skill_lv, EQI_HAND_R, false);		
					else if(flag&BF_LONG) 
					...
					if(elemento != 1) {
						d.damage = 0;
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Runa Elemental] Apenas danos com o elemento ÁGUA podem me acertar.", false, SELF);
					} else {
						d.damage = 1 * d.div_;
						return d;
					}
				}
				if(md->db->id == 22979){ // terra
					int elemento = d.s_ele;
					if(attack_type == BF_WEAPON) 
						elemento = battle_get_weapon_element(&d, bl, target, skill_id, skill_lv, EQI_HAND_R, false);		
					else if(flag&BF_LONG) 
					...
					if(elemento != 3) {
						d.damage = 0;
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Runa Elemental] Apenas danos com o elemento FOGO podem me acertar.", false, SELF);
					} else {
						d.damage = 1 * d.div_;
						return d;
					}
				}
				if(md->db->id == 22980){ // Vento
					int elemento = d.s_ele;
					if(attack_type == BF_WEAPON) 
						elemento = battle_get_weapon_element(&d, bl, target, skill_id, skill_lv, EQI_HAND_R, false);		
					else if(flag&BF_LONG) 
					...
					if(elemento != 2) {
						d.damage = 0;
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Runa Elemental] Apenas danos com o elemento TERRA podem me acertar.", false, SELF);
					} else {
						d.damage = 1 * d.div_;
						return d;
					}
			...
			if(!damagetaken && mapdata->flag[MF_MAPAS_275] && !mapdata->flag[MF_REDUCAOEXP])
				damagetaken = 10;

			int capdano = 75000000;
			//if(sd && sd->history_state.bindconta >= 10) capdano = 95000000;

			if(
				(mapdata->flag[MF_BIOSAVAGE] || mapdata->flag[MF_ESCURIDAO] || mapdata->flag[MF_ESCURIDAOPVP]) && 
				(skill_id == GC_CROSSIMPACT || skill_id == SN_SHARPSHOOTING || skill_id == NEW_SHARPSHOOT)
			){
				capdano = 120000000;
			}

			if(sd && skill_id == WL_SOULEXPANSION && sd->status.weapon != W_STAFF){
				capdano = 110000000;
			}

			if(mapdata->flag[MF_FENDA]){
				if(sd)
					capdano = sd->state.capfenda ? sd->state.capfenda : 1000000000;
			}


			if(mapdata->flag[MF_VARMUNDTNOVO]){
				capdano = 50000000;
			}

			if (sd && sd->status.class_ == JOB_INQUISITOR){
					capdano = capdano * 2;
			}

			if (sd && sd->status.class_ == JOB_NIGHT_WATCH){
					capdano = capdano * 2;
			}

			if (sd && sd->status.class_ == JOB_WINDHAWK && mapdata->flag[MF_CHEFFEXTREME]	){
					capdano = 120000000;
			}

			if(md->db->id == 28125) capdano = 1000000;
			...
			if (sd && sd->bonus.cap_dano > 0){
				capdano += (capdano / 100) * sd->bonus.cap_dano;
			}
			...
			if(mapdata->flag[MF_LABIRINTOVALK] && d.damage > 250000){
				d.damage = 250000;
			}

			if(mapdata->flag[MF_DOMINIONORMAL]){
				capdano = 4000000;
			}
			if(mapdata->flag[MF_DOMINIOHARD]){
				capdano = 30000000;
			}
			if(mapdata->flag[MF_DOMINIOUNREAL]){
				capdano = 600000000;
			}
			if(md->db->id == 22941) capdano = 3000000 * 3;
			if(md->db->id == 22935) capdano = 30000000 * 3;
			if(md->db->id == 22947) capdano = 600000000 * 3;
			if(md->db->id == 22953) { 
				capdano = 0; 
				d.damage = (d.damage / 10);
			}
			...
			if(sc && sc->data[SC_BUFFSAGRADO] && mapdata->flag[MF_BUFFSAGRADO] && sd->status.class_ != JOB_INQUISITOR){
				d.damage = d.damage * 2;
				capdano = capdano * 2;
			}
			if(sc && sc->data[SC_BUFFDOMUNREAL] && (mapdata->flag[MF_DOMINIOUNREAL] || md->db->id == 22947)){
				capdano = capdano * 2;
			}
			...
			if (!mapdata->flag[MF_NOCAPDANOFINAL] && capdano > 0 && md && damagetaken <= 10 && d.damage > capdano && !mapdata->flag[MF_TEMPLOULTIMATE] && !mapdata->flag[MF_SAMURAIMESTRE] && !mapdata->flag[MF_ESCURIDAO] && !mapdata->flag[MF_ESCURIDAOPVP] && !mapdata->flag[MF_FLORESTAPES] && !mapdata->flag[MF_MASMORRAINVERNAL] && !mapdata->flag[MF_HISTORYSTAGE] && !mapdata->flag[MF_EVENTOBONUS] && !mapdata->flag[MF_GUERRAPVM] && !mapdata->flag[MF_MAPAS_275] &&
				md->db->id != 28130 && md->db->id != 28131 && md->db->id != 28132 && md->db->id != 28133 && md->db->id != 28134 && md->db->id != 21533 && md->db->id != 2409 && md->db->id != 2410 && md->db->id != 2411
				&& md->db->id != 28141 && md->db->id != 28142 && md->db->id != 28143 && md->db->id != 28144 && md->db->id != 28145 && md->db->id != 28135 && md->db->id != 28136 && md->db->id != 28137 && md->db->id != 28138 && md->db->id != 28139
			){
				d.damage = capdano;
			}
			...
			if (mapdata->flag[MF_TEMPLOSAGRADO]){
				capdano = 15000000;
				if(sd && sd->status.class_ == JOB_INQUISITOR){
					capdano = 30000000;
				}
				
				if(sc && sc->data[SC_BUFFSAGRADO] && mapdata->flag[MF_BUFFSAGRADO]){
					capdano = capdano * 2;
				}
				...
				if(d.damage > capdano){
					d.damage = capdano;
					if (sd && sd->bonus.cap_dano > 0){
						d.damage += (d.damage / 100) * sd->bonus.cap_dano;
					}
				}
			}
			if(mapdata->flag[MF_VILAGRANDORC] && d.damage > 1000000 && status_get_class_(bl) != CLASS_BOSS)
				d.damage = 1000000;
			
			if((skill_id == NC_AXETORNADO || skill_id == SU_PICKYPECK || skill_id == GN_SPORE_EXPLOSION || skill_id == AB_ADORAMUS) && sd && sd->bonus.cap_limit && d.damage > sd->bonus.cap_limit) d.damage = sd->bonus.cap_limit;
		}

		if(md && md->status.protecao){
			int tipoprotect = md->status.protecao;
			// [F][M]
			if (tipoprotect == 3 && !(d.flag&BF_SHORT) && !(d.flag&BF_MAGIC)){
				d.damage = 0;
				d.damage2 = 0;
			}
			// [M][D]
			if (tipoprotect == 2 && (d.flag&BF_SHORT)){ 
				d.damage = 0;
				d.damage2 = 0;
			}
			//[D][F]
			if (tipoprotect == 1 && (d.flag&BF_MAGIC)){
				d.damage = 0;
				d.damage2 = 0;
			}
			// [F]
			if (tipoprotect == 4 && (!(d.flag&BF_SHORT) || (d.flag&BF_MAGIC))){
				d.damage = 0;
				d.damage2 = 0;
			}
			// [M]
			if (tipoprotect == 5 && !(d.flag&BF_MAGIC)){ 
				d.damage = 0;
				d.damage2 = 0;
			}
			//[D]
			if (tipoprotect == 6 && ((d.flag&BF_SHORT) || (d.flag&BF_MAGIC))){
				d.damage = 0;
				d.damage2 = 0;
			}
		}
	...
	mob_data *mdd = BL_CAST(BL_MOB, bl);
	if(mdd && (mdd->special_state.ai == AI_ABR || mdd->special_state.ai == AI_BIONIC)){
		mob_data *mdv = BL_CAST(BL_MOB, target);
		if(mdv && (mapdata->flag[MF_FENDA] || mapdata->flag[MF_EQUIPSGVG] || mapdata->flag[MF_HISTORYSTAGE] || mapdata->flag[MF_DOMINIOENT] || mapdata->flag[MF_GUERRAPVM])){
			d.damage = 0;
		}
	}

	if(mdd && mdd->db->id == 28125 && skill_id == WZ_STORMGUST){
		d.damage = 2000000;
	}

	if(target->type == BL_ELEM){
		map_session_data *esd = BL_CAST(BL_PC, battle_get_master(target));
		if(esd && esd->bonus.dano_elemental) d.damage = 0;
	}

 	if((bl->type == BL_PC || (mdd && (mdd->special_state.ai == AI_ABR || mdd->special_state.ai == AI_BIONIC))) && !mapdata->flag[MF_EQUIPSGVG]){
		switch(attack_type) {
			case BF_WEAPON: 
				battle_absorb_damage(target, &d, skill_id, bl, skill_lv);

				battle_do_reflect(BF_WEAPON,&d, bl, target, skill_id, skill_lv); //WIP [lighta]
			break;
			case BF_MAGIC:
				battle_absorb_damage(target, &d, skill_id, bl, skill_lv);
			break;
			case BF_MISC:
				...
				battle_do_reflect(BF_MISC,&d, bl, target, skill_id, skill_lv); //WIP [lighta]
			break;
		}
	}

	if(bl->type == BL_MOB && skill_id == HT_CLAYMORETRAP && mapdata->flag[MF_VILAGRANDORC])
		d.damage = 1000000;

	if(mapdata->flag[MF_ANIVERSARIO]){
		if(bl->type == BL_MOB && skill_id == WZ_VERMILION) d.damage = 2000000;
		if(bl->type == BL_PC && skill_id) d.damage = 0;
	}

	if(skill_id == SKE_STAR_CANNON && target->type == BL_PC && battle_config.star_no_pvp) d.damage = 1;

	if(mapdata->flag[MF_TIPODANORANK] && sd){
		int tiporank = sd->history_state.tipodanorank;
		if (tiporank == 1 && (!(d.flag&BF_SHORT) || (d.flag&BF_MAGIC))){
			d.damage = 0;
			d.damage2 = 0;
		}
		if (tiporank == 2 && !(d.flag&BF_MAGIC)){
			d.damage = 0;
			d.damage2 = 0;
		}
		if (tiporank == 3 && ((d.flag&BF_SHORT) || (d.flag&BF_MAGIC))){
			d.damage = 0;
			d.damage2 = 0;
		}
	}
	...
	if(mapdata->flag[MF_COMETAPASSING]){
		if (tscc && (tscc->data[SC_AUTOCURA] || (tscc->data[SC_ONLYMAGIC] && !(d.flag&BF_MAGIC)) || (tscc->data[SC_ONLYSHORT] && (!(d.flag&BF_SHORT) || (d.flag&BF_MAGIC))) || (
			tscc->data[SC_ONLYLONG] && ((d.flag&BF_SHORT) || (d.flag&BF_MAGIC))))
			) {
			struct status_data *status;
			status = status_get_status_data(target);

			if(mapdata->flag[MF_GUERRAPVM]){
				clif_skill_nodamage(nullptr, target, AL_HEAL, d.damage, 1);
				if((d.damage / 1000000) > status->danorecebido){
					status->danorecebido = 0;
				} else 
					status->danorecebido -= (d.damage / 1000000);
				d.damage = 1;
				d.damage2 = 0;
			} else {
				int64 danoautocura = d.damage + d.damage2;
				if(danoautocura > 2147483647) danoautocura = 2147483647;
				
				clif_skill_nodamage(nullptr, target, AL_HEAL, danoautocura, 1);
				
				if(status->shield > 0){
					int contashield = (danoautocura / 1000000);

					status->shield += contashield;
					
					if(status->shield > status->max_shield) status->shield = status->max_shield;
					d.damage = 1;
					d.damage2 = 0;
				} else {
					status_heal(target, danoautocura, 0, 0);
					d.damage = 1;
					d.damage2 = 0;
				}
			}
	...
	// NOVAS MECANICAS
	if((skill_id == NPC_ONLY_SIT || skill_id == NPC_JOKENPO_PEDRA || skill_id == NPC_JOKENPO_PAPEL || skill_id == NPC_JOKENPO_TESOURA) && bl->type == BL_MOB){

		if(skill_id == NPC_ONLY_SIT){
			if(tsd && !pc_issit(tsd))
				d.damage = 2000000000;
			else 
				d.damage = 0;
		}
		if(skill_id == NPC_JOKENPO_PEDRA){
			if(tsd && !tsd->sc.data[SC_JOKENPO_PAPEL])
				d.damage = 2000000000;
			else 
				d.damage = 0;
		}
		if(skill_id == NPC_JOKENPO_PAPEL){
			if(tsd && !tsd->sc.data[SC_JOKENPO_TESOURA])
				d.damage = 2000000000;
			else 
				d.damage = 0;
		}
		if(skill_id == NPC_JOKENPO_TESOURA){
			if(tsd && !tsd->sc.data[SC_JOKENPO_PEDRA])
				d.damage = 2000000000;
			else 
				d.damage = 0;
		}
	...
	mob_data *mdt = BL_CAST(BL_MOB, target);
	if(mdt && mdt->db->id == 1904) d.damage = 0;

	if(target->type == BL_MOB && mapdata->flag[MF_NODAMAGE]){
		d.damage = 0;
		d.damage2 = 0;  
		mob_data *mdv = BL_CAST(BL_MOB, target);
		if(mdv && (mdv->db->id == 21866 || mdv->db->id == 21867 || mdv->db->id == 21868 || mdv->db->id == 21869)){
			d.damage = 1;  
		}
	}

	//if (sd && sd->sc.data[SC_CLIMAX] && sd->sc.data[SC_CLIMAX]->val1 == 4 && (skill_id == AG_ALL_BLOOM || skill_id == AG_ALL_BLOOM_ATK2 || skill_id == AG_ALL_BLOOM_ATK)){
	//	sc_start(bl, target, skill_get_sc(5222), 100, skill_lv, skill_get_time2(5222, skill_lv));
	//	d.damage = 0;
	//}
	

	
	if(sd && sd->sc.data[SC_DANO_DPS]){
		if((d.damage / 1000000) >= 1) {
			uint64 danodps = d.damage;
			if(danodps > 2147483647) danodps = 2147483647;
			int danomilhao = danodps / 1000000;
			int danorestante = danodps - (danomilhao * 1000000);
			
			sd->danomilhao += danomilhao;
			sd->danorestante += danorestante;
			if(sd->danorestante >= 1000000){
				int rest = sd->danorestante / 1000000;
				sd->danorestante = sd->danorestante - (rest * 1000000);
				sd->danomilhao += rest;
			}
		} else {
			sd->danorestante += d.damage;
			if(sd->danorestante >= 1000000){
				int rest = sd->danorestante / 1000000;
				sd->danorestante = sd->danorestante - (rest * 1000000);
				sd->danomilhao += rest;
	...
	if(mapdata->flag[MF_FENDADIMENSIONAL]){
		if(sd){
			battle_do_reflect_fenda(bl, target, &d, d.damage, sd);
			mob_data *md = BL_CAST(BL_MOB, target);
			if(md && (md->db->id == 22996 || md->db->id == 22989) && sd->status.party_id)
				battle_barra_boss(sd->status.party_id, target);
			
			// Barreira de gelo
			if(target->type == BL_SKILL){
				TBL_SKILL *su = (TBL_SKILL*)target;
				if(su->group->skill_id == WZ_ICEWALL){
					d.damage = 1; d.damage2 = 0;
				}
			}
		}
		if(target->type == BL_PC && (skill_id == WZ_FIREPILLAR || skill_id == MG_FIREWALL)){
			d.damage = 2000000000;
		}
	}

	if(mapdata->flag[MF_TORREDECRISTAL]){
		if(sd){
			mob_data *md = BL_CAST(BL_MOB, target);
			if(md && md->status.tiporeflect > 0){
				battle_do_reflect_torre(bl, target, &d, d.damage, sd);
			}
			if(md && (md->db->id == 20928 || md->db->id == 21588 || md->db->id == 1708 || md->db->id == 20996 || md->db->id == 28126 || md->db->id == 22370 || md->db->id == 24004) && sd->status.party_id)
				battle_barra_boss(sd->status.party_id, target);
		}
	}
...
int64 battle_calc_return_damage(struct block_list* bl, struct block_list *src, int64 *dmg, int flag, uint16 skill_id, bool status_reflect){
	...
	if (flag & BF_SHORT) {//Bounces back part of the damage.
		if ( (skill_get_inf2(skill_id, INF2_ISTRAP) || !status_reflect) && sd && sd->bonus.short_weapon_damage_return ) {
			...
			mob_data *md = BL_CAST(BL_MOB, bl);
			if(md && md->db->id == 28002) return 0;
			
			struct map_data *mapdata = map_getmapdata(src->m);

			if (mapdata->flag[src->m, MF_FENDA] || mapdata->flag[src->m, MF_CHEFFTRANS] || mapdata->flag[src->m, MF_TUMBAVAZIO] || mapdata->flag[src->m, MF_ANIVERSARIO] || mapdata->flag[src->m, MF_TEMPLOULTIMATE] || mapdata->flag[src->m, MF_DOMINIOGERAL] || mapdata->flag[src->m, MF_ESCURIDAO] || mapdata->flag[src->m, MF_ESCURIDAOPVP] || mapdata->flag[src->m, MF_TEMPLOSAGRADO] || mapdata->flag[src->m, MF_GRAMPS] || mapdata->flag[src->m, MF_SAMURAIMESTRE] || mapdata->flag[src->m, MF_CARNAVAL]) return 0;
			...
			if(skill_id) {
				...
				// --------------------------------------------------------------------------------------------------
				struct map_data *mapdata = map_getmapdata(bl->m);
				if ((mapdata_flag_gvg2(mapdata) || mapdata->flag[MF_EQUIPSGVG]) && bl->type != BL_MOB && src->type != BL_MOB)
					damage=battle_calc_gvg_damage(src,bl,damage,skill_id,flag);
				else if( mapdata->flag[MF_BATTLEGROUND] && bl->type != BL_MOB && src->type != BL_MOB )
					damage=battle_calc_bg_damage(src,bl,damage,skill_id,flag);
				else if ((mapdata->flag[MF_PVP] || ((TBL_PC *)bl)->sc.data[SC_EFEITO_RH_7]) && bl->type != BL_MOB && src->type != BL_MOB)
					damage=battle_calc_pvp_damage(src,bl,damage,skill_id,flag);
				// --------------------------------------------------------------------------------------------------
			}
			...
			if ( sc->data[SC_REFLECTSHIELD] && skill_id != WS_CARTTERMINATION && skill_id != NPC_MAXPAIN_ATK ) {
				// Don't reflect non-skill attack if has SC_REFLECTSHIELD from Devotion bonus inheritance
				if (!skill_id && battle_config.devotion_rdamage_skill_only && sc->data[SC_REFLECTSHIELD]->val4)
					rdamage = 0;
				else {
	...
	if (ssc) {
		...
		// devon golden x
		if (rdamage > 0 && (ssc->data[SC_REF_T_POTION] || ssc->data[SC_EFEITO_RH_34]) && !map_getmapflag(bl->m, MF_EQUIPSGVG))
...
void battle_drain(struct map_session_data *sd, struct block_list *tbl, int64 rdamage, int64 ldamage, int race, int class_)
{
	...
	// DEVON
	if(sd && mapdata->flag[sd->bl.m, MF_NOHEALHPITEM])
		thp = 0;

	if(sd && (mapdata->flag[sd->bl.m, MF_BIOEXTREME] || mapdata->flag[sd->bl.m, MF_CHEFFEXTREME] || 
		mapdata->flag[sd->bl.m, MF_TEMPLOULTIMATE] ||
		mapdata->flag[sd->bl.m, MF_ESCURIDAO] || mapdata->flag[sd->bl.m, MF_ESCURIDAOPVP] ||
		mapdata->flag[sd->bl.m, MF_TEMPLOSAGRADO] || mapdata->flag[sd->bl.m, MF_DOMINIOGERAL] || mapdata->flag[sd->bl.m, MF_SAMURAIMESTRE] || mapdata->flag[sd->bl.m, MF_GUERRAPVM]))
		thp = 0;

	if(sd && mapdata->flag[sd->bl.m, MF_CONSTUNREAL])
		thp = 0;

	if(sd && mapdata->flag[sd->bl.m, MF_FENDA])
		thp = 0;
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, t_tick tick, int flag) {
	...
	if (sd) {
		...
		if( sc ){
			if( sc->data[SC_SERVANTWEAPON] && sd->servantball > 0 && rnd() % 100 < (20 + sc->data[SC_SERVANTWEAPON]->val1 *5) ){
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	switch( src->type ) { //Checks on actual src type
		...
		case BL_MER:
			...
			if(mapdata->flag[MF_DOMINIOGERAL]) return 0;
			break;
	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			if( s_bl != t_bl )
			{
			if(mapdata->flag[MF_GUERRAPVM] && t_bl->type == BL_MOB) {
				mob_data *md = BL_CAST(BL_MOB, t_bl);
				if(md && (md->db->id < 4000 || md->db->id == 31841)) return 0;
			}
			if((mapdata->flag[MF_FENDADIMENSIONAL] || mapdata->flag[MF_PANDEMONIUM]) && t_bl->type == BL_MOB) {
				mob_data *md = BL_CAST(BL_MOB, t_bl);
				if(md && md->db->id == 31052) return 0;
			}
			if(mapdata->flag[MF_SAMBODROMO] && t_bl->type == BL_MOB) {
				mob_data *md = BL_CAST(BL_MOB, t_bl);
				if(md && (md->db->id == 23992 || md->db->id == 23995)) return 0;
			}

		if( !mapdata->flag[MF_TOWN] && !mapdata->flag[MF_HISTORYSTAGE] && t_bl->type == BL_PC )
		{ // Prevent novice engagement on pk_mode (feature by Valaris)
			TBL_PC *sd2 = (TBL_PC*)t_bl;
			if (
				(sd2->sc.data[SC_EFEITO_RH_7]) && (sd->sc.data[SC_EFEITO_RH_7])
		...
		case BL_MOB:
		{
			if( (mapdata->flag[MF_MASMORRAINVERNAL] || mapdata->flag[MF_MECANICAS]) && t_bl->type == BL_PC )
			{ // Prevent novice engagement on pk_mode (feature by Valaris)
				TBL_PC *sd2 = (TBL_PC*)t_bl;
				if (sd2 && sd2->sc.data[SC_GRAVITYCONTROL]){
						return -1;
				}	
			}
			if( mapdata->flag[MF_TRIALVALE] && t_bl->type == BL_PC )
			{ // Prevent novice engagement on pk_mode (feature by Valaris)
				TBL_PC *sd2 = (TBL_PC*)t_bl;
				if (sd2 && sd2->sc.data[SC_GRAVITYCONTROL]){
						return -1;
			...
			if( mapdata->flag[MF_CARNAVAL] && t_bl->type == BL_PC )
			{ // Prevent novice engagement on pk_mode (feature by Valaris)
				if (md->db->id == 1376){
						return -1;
				}	
...
static const struct _battle_data {
	...
	{ "exp_rate_adicional",                 &battle_config.exp_rate_adicional,              100,    0,      INT_MAX,        },
	{ "exp_rate_relogio",                 &battle_config.exp_rate_relogio,              100,    0,      INT_MAX,        },
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "tempoiniciar",                       &battle_config.tempoiniciar,                    240000,  0,  INT_MAX,        },
	...
	{ "pvp_flee_penalty",                   &battle_config.pvp_flee_penalty,                20,     0,      INT_MAX,        },
...
// rAthena
	...
	{ "max_res_mres_reduction",             &battle_config.max_res_mres_reduction,          50,    0,      100,       },
	...
	// devon
	{ "hpratesavage",                        &battle_config.hpratesavage,                   1,     0,      100,             },
	{ "hplimitsavage",                       &battle_config.hplimitsavage,                  1,     0,      1000000000,      },
	{ "hprateextremebio",                       &battle_config.hprateextremebio,                  1,     0,      100,             },
	{ "hplimitextremebio",                      &battle_config.hplimitextremebio,                 1,     0,      1000000000,      },
	{ "hpratehard",                          &battle_config.hpratehard,                     1,     0,      100,             },
	{ "hplimithard",                         &battle_config.hplimithard,                    1,     0,      1000000000,      },
	{ "dano_magico",                         &battle_config.dano_magico,                    1,     0,      100,             },
	{ "aumento_drop",                        &battle_config.aumento_drop,                   0,     0,      100,             },
	{ "dano_extremebio",                        &battle_config.dano_extremebio,                   0,     0,      1000,            },
	{ "hprateextremecheff",                       &battle_config.hprateextremecheff,                  1,     0,      100,             },
	{ "hplimitextremecheff",                      &battle_config.hplimitextremecheff,                 1,     0,      1000000000,      },
	{ "dano_extremecheff",                        &battle_config.dano_extremecheff,                   0,     0,      1000,            },
	{ "star_no_pvp",       &battle_config.star_no_pvp,           0,     0,      1,        },
	...
	{ "desativarkafra",                        &battle_config.desativarkafra,                   0,     0,      100,             },
	{ "desativardrop",                        &battle_config.desativardrop,                   0,     0,      100,             },
	{ "desativarhgc",                        &battle_config.desativarhgc,                   0,     0,      100,             },
	{ "desativarmail",                        &battle_config.desativarmail,                   0,     0,      100,             },
	{ "scriptlog",                        &battle_config.scriptlog,                   0,     0,      100,             },
	{ "desativarchecar",                        &battle_config.desativarchecar,                   0,     0,      100,             },
	{ "dropextreme",                        &battle_config.dropextreme,                   0,     0,      100,             },
	{ "desativarexcelencia",                        &battle_config.desativarexcelencia,                   0,     0,      100,             },
	{ "desativarchannel",                        &battle_config.desativarchannel,                   0,     0,      100,             },
	{ "scripttimer",                        &battle_config.scripttimer,                   0,     0,      100,             },
	{ "multiserver",                        &battle_config.multiserver,                   1,     0,      100,             },
	{ "blackfriday",                        &battle_config.blackfriday,                   0,     0,      100,             },
	{ "carnaval",                        &battle_config.carnaval,                   0,     0,      100,             },
	{ "aniversario",                        &battle_config.aniversario,                   0,     0,      100,             },
	{ "stoptimer",                        &battle_config.stoptimer,                   0,     0,      100,             },
	{ "stopdano",                        &battle_config.stopdano,                   0,     0,      100,             },
	{ "stopmobskill",                        &battle_config.stopmobskill,                   0,     0,      100,             },
	{ "capsula",                        &battle_config.capsula,                   0,     0,      INT_MAX,             },
	{ "ticket_avancado",                        &battle_config.ticket_avancado,                   0,     0,      INT_MAX,             },
	{ "flogo_glacial",                        &battle_config.flogo_glacial,                   0,     0,      INT_MAX,             },
	{ "energia_congelada",                        &battle_config.energia_congelada,                   0,     0,      INT_MAX,             },
	{ "essencia_primordial",                        &battle_config.essencia_primordial,                   0,     0,      INT_MAX,             },
	{ "cristal_bio_sav",                        &battle_config.cristal_bio_sav,                   0,     0,      INT_MAX,             },
	{ "moeda_cheff_sav",                        &battle_config.moeda_cheff_sav,                   0,     0,      INT_MAX,             },
	{ "nucleo_pes",                        &battle_config.nucleo_pes,                   0,     0,      INT_MAX,             },
	{ "runa_corrompida",                        &battle_config.runa_corrompida,                   0,     0,      INT_MAX,             },
	{ "moeda_evento",                        &battle_config.moeda_evento,                   0,     0,      INT_MAX,             },
	{ "cupom_visuais",                       &battle_config.cupom_visuais,                   0,     0,      INT_MAX,             },
	{ "fragmento_elemental",                        &battle_config.fragmento_elemental,                   0,     0,      INT_MAX,             },
	{ "refinadora_complexa",                        &battle_config.refinadora_complexa,                   0,     0,      INT_MAX,             },
	{ "espirito_nebula",                        &battle_config.espirito_nebula,                   0,     0,      INT_MAX,             },
	{ "grade_armadura",                        &battle_config.grade_armadura,                   0,     0,      INT_MAX,             },
	{ "hero",                        &battle_config.hero,                   0,     0,      INT_MAX,             },
	{ "release_of_magic",                        &battle_config.release_of_magic,                   0,     0,      INT_MAX,             },
	{ "fury",                        &battle_config.fury,                   0,     0,      INT_MAX,             },
	{ "epifania",                        &battle_config.epifania,                   0,     0,      INT_MAX,             },
	{ "dynast",                        &battle_config.dynast,                   0,     0,      INT_MAX,             },
	{ "arcana",                        &battle_config.arcana,                   0,     0,      INT_MAX,             },
	{ "palito_baunilha",                        &battle_config.palito_baunilha,                   0,     0,      INT_MAX,             },
	{ "palito_cassis",                        &battle_config.palito_cassis,                   0,     0,      INT_MAX,             },
	{ "palito_chocolate",                        &battle_config.palito_chocolate,                   0,     0,      INT_MAX,             },
	{ "palito_laranja",                        &battle_config.palito_laranja,                   0,     0,      INT_MAX,             },
	{ "palito_limao",                        &battle_config.palito_limao,                   0,     0,      INT_MAX,             },
	{ "palito_morango",                        &battle_config.palito_morango,                   0,     0,      INT_MAX,             },
	{ "super_mega_elunium",                        &battle_config.super_mega_elunium,                   0,     0,      INT_MAX,             },
	{ "minerio_ultimate",                        &battle_config.minerio_ultimate,                   0,     0,      INT_MAX,             },
	{ "mini_refinadora",                        &battle_config.mini_refinadora,                   0,     0,      INT_MAX,             },
	{ "grade_escudo",                        &battle_config.grade_escudo,                   0,     0,      INT_MAX,             },
	{ "runa_corrom_sup",                        &battle_config.runa_corrom_sup,                   0,     0,      INT_MAX,             },
	{ "runa_corrom_new",                        &battle_config.runa_corrom_new,                   0,     0,      INT_MAX,             },
	{ "runa_corrom_sup_new",                        &battle_config.runa_corrom_sup_new,                   0,     0,      INT_MAX,             },
	{ "rotacao_dom_one",                        &battle_config.rotacao_dom_one,                   0,     0,      INT_MAX,             },
	{ "rotacao_dom_two",                        &battle_config.rotacao_dom_two,                   0,     0,      INT_MAX,             },
	{ "fragmento_kafra",                        &battle_config.fragmento_kafra,                   0,     0,      INT_MAX,             },
