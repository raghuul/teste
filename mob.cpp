...
void mvptomb_destroy(struct mob_data *md) {
	...
	if ( (nd = map_id2nd(md->tomb_nid)) ) {
		...
		if(mapdata->flag[MF_NATAL]){ map_setmapflag_sub(nd->bl.m, MF_NOSKILL, false, &args);}
...
//	md->master_id = 0;
	...
	if(md->db->id == 28002){
		// devon
		if (mapdata->flag[MF_BIOSAVAGE] && md->bl.type == BL_MOB && md->base_status->max_hp > 1){
			newHp = md->base_status->max_hp * battle_config.hpratesavage;	
			if(newHp > battle_config.hplimitsavage)
				newHp = battle_config.hplimitsavage;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);
		}
		if (mapdata->flag[MF_BIOEXTREME] && md->bl.type == BL_MOB && md->base_status->max_hp > 1){
			newHp = md->base_status->max_hp * battle_config.hprateextremebio;	
			if(newHp > battle_config.hplimitextremebio)
				newHp = battle_config.hplimitextremebio;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);

		}

		if (mapdata->flag[MF_CHEFFEXTREME] && md->bl.type == BL_MOB && md->base_status->max_hp > 1){
			newHp = md->base_status->max_hp * battle_config.hprateextremecheff;	
			if(newHp > battle_config.hplimitextremecheff)
				newHp = battle_config.hplimitextremecheff;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);

		}

		if (mapdata->flag[MF_BIOHARD] && md->bl.type == BL_MOB && md->base_status->max_hp > 1){
			newHp = md->base_status->max_hp * battle_config.hpratehard;	
			if(newHp > battle_config.hplimithard)
				newHp = battle_config.hplimithard;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);
		}

		if (mapdata->flag[MF_ILLUSIONGUILD] && md->bl.type == BL_MOB && md->base_status->max_hp > 1){
			newHp = md->base_status->max_hp * 100;	
			if(newHp > 1999999999)
				newHp = 1999999999;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);
		}

		if (mapdata->flag[MF_CHEFFTRANS] && md->bl.type == BL_MOB && md->base_status->max_hp > 1){
			newHp = md->base_status->max_hp * 4;	
			if(newHp > 999999999)
				newHp = 999999999;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);
		}
		if (mapdata->flag[MF_TEMPLOSAGRADOTWO] && md->bl.type == BL_MOB && md->base_status->max_hp > 1){
			newHp = md->base_status->max_hp * 2;
			newHp -= (newHp / 100) * 15;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);
		}
	} else {
		// devon
		if (mapdata->flag[MF_BIOSAVAGE] && md->bl.type == BL_MOB && md->base_status->max_hp > 1 && status_get_class_(&md->bl) == CLASS_BOSS && md->status.shield < 1){
			newHp = ((md->base_status->max_hp * battle_config.hpratesavage) / 1000000);
			if(md->db->id == 28003) newHp = 2700;
			if(md->db->id == 3097) newHp = 1200;
			if(md->db->id == 20601) newHp = 500;
			if(md->db->id == 28004) newHp -= newHp / 2;
			//if(md->db->id == 3097) md->base_status->mode += MD_CANMOVE;
			md->status.shield = md->status.max_shield = newHp;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)1000000; 
			status_set_maxhp(&md->bl, (unsigned int)newHp, 0);
			clif_name_area(&md->bl);
		}
		if (mapdata->flag[MF_BIOSAVAGE] && md->bl.type == BL_MOB && (md->db->id == 28026 || md->db->id == 28027 || md->db->id == 28028)){
			newHp = 600;
			md->status.shield = md->status.max_shield = newHp;
			status_set_maxhp(&md->bl, (unsigned int)newHp, 0);
			clif_name_area(&md->bl);
		...
		if (mapdata->flag[MF_BIOEXTREME] && md->bl.type == BL_MOB && md->base_status->max_hp > 1 && status_get_class_(&md->bl) == CLASS_BOSS && md->status.shield < 1){
			newHp = ((md->base_status->max_hp * battle_config.hprateextremebio) / 1000000);
			if(md->db->id == 3097) newHp = 450;
			//if(md->db->id == 3097) md->base_status->mode += MD_CANMOVE;
			md->status.shield = md->status.max_shield = newHp;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)1000000; 
			status_set_maxhp(&md->bl, (unsigned int)newHp, 0);
		...
		if (mapdata->flag[MF_CHEFFEXTREME] && md->bl.type == BL_MOB && md->base_status->max_hp > 1 && status_get_class_(&md->bl) == CLASS_BOSS && md->status.shield < 1){
			newHp = ((md->base_status->max_hp * battle_config.hprateextremecheff) / 1000000);
			if(md->db->id == 20610 || md->db->id == 20611) newHp = 638; // valk
			if(md->db->id == 21317) newHp -= (newHp / 100) * 30; // freyja
			if(md->db->id == 20573 || md->db->id == 3097) newHp = 660; // amdarais e deus morroc
			md->status.shield = md->status.max_shield = newHp;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)1000000; 
			status_set_maxhp(&md->bl, (unsigned int)newHp, 0);
			clif_name_area(&md->bl);
		...
		if (mapdata->flag[MF_VULCAOPESADELO] && md->db->id == 3331){
			md->status.shield = md->status.shield / 10;
			md->status.max_shield = md->status.max_shield / 10;
			md->base_status->hp = md->base_status->max_hp = md->base_status->max_hp / 10;
		}
		if (mapdata->flag[MF_BIOHARD] && md->bl.type == BL_MOB && md->base_status->max_hp > 1 && status_get_class_(&md->bl) == CLASS_BOSS && md->status.shield < 1){
			newHp = ((md->base_status->max_hp * battle_config.hpratehard) / 1000000);
			md->status.shield = md->status.max_shield = newHp;
			//if(md->db->id == 3097) md->base_status->mode += MD_CANMOVE;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)1000000; 
			status_set_maxhp(&md->bl, (unsigned int)newHp, 0);
		...
		if (mapdata->flag[MF_ILLUSIONGUILD] && md->bl.type == BL_MOB && md->base_status->max_hp > 1 && status_get_class_(&md->bl) == CLASS_BOSS && md->status.shield < 1){
			newHp = ((md->base_status->max_hp * 100) / 1000000);
			md->status.shield = md->status.max_shield = newHp;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)1000000; 
			status_set_maxhp(&md->bl, (unsigned int)newHp, 0);
			clif_name_area(&md->bl);
		...
		if (mapdata->flag[MF_CHEFFTRANS] && md->bl.type == BL_MOB && md->base_status->max_hp > 1 && status_get_class_(&md->bl) == CLASS_BOSS && md->status.shield < 1){
			newHp = ((md->base_status->max_hp * 300) / 1000000);
			if(md->db->id == 2251 || md->db->id == 1112) newHp = 1500;
			md->status.shield = md->status.max_shield = newHp;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)1000000; 
			md->base_status->dex = md->base_status->dex * 5;
			md->base_status->hit = md->base_status->hit * 5;
			md->base_status->speed = md->base_status->speed * 1.5;
			status_set_maxhp(&md->bl, (unsigned int)newHp, 0);
			status_calc_misc(&md->bl, &md->status, md->level);
			status_calc_bl_(&md->bl, status_db.getSCB_BATTLE());
			clif_name_area(&md->bl);
		}
		if (mapdata->flag[MF_TEMPLOSAGRADOTWO] && md->bl.type == BL_MOB && md->base_status->max_hp > 1 && status_get_class_(&md->bl) == CLASS_BOSS){
			newHp = md->base_status->max_hp * 2;
			newHp -= (newHp / 100) * 15;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);
		}
		if (mapdata->flag[MF_CARNAVAL] && md->bl.type == BL_MOB){
			newHp = 50;
			md->base_status->hp = md->base_status->max_hp = (unsigned int)newHp; status_set_maxhp(&md->bl, (unsigned int)newHp, 0); clif_name_area(&md->bl);
		}
		if (mapdata->flag[MF_DOMINIOGERAL] || mapdata->flag[MF_DOMINIONORMAL] || mapdata->flag[MF_DOMINIOHARD] || mapdata->flag[MF_DOMINIOUNREAL] || mapdata->flag[MF_DOMINIOPESADELO] || mapdata->flag[MF_DOMINIOULTIMATE]){
			md->base_status->hit = md->base_status->hit * 5;
			int ele[10] = {ELE_NEUTRAL,ELE_WATER,ELE_EARTH,ELE_FIRE,ELE_WIND,ELE_POISON,ELE_HOLY,ELE_DARK,ELE_GHOST,ELE_UNDEAD};
			int race[10] = {RC_ANGEL, RC_BRUTE, RC_DEMIHUMAN, RC_DEMON, RC_DRAGON, RC_FISH, RC_FORMLESS, RC_INSECT, RC_PLANT, RC_UNDEAD};
			if(md->db->id == 22940) {	// Unreal
				int raceFlores = mapreg_readreg(add_str("$raceFlores"));
				int eleFlores = mapreg_readreg(add_str("$eleFlores"));
				if(raceFlores) { md->status.race = md->base_status->race = race[raceFlores-1]; }
				if(eleFlores) md->base_status->def_ele = ele[eleFlores-1];
			}
			if(md->db->id == 22934) {	// Hard
				int raceEspiri = mapreg_readreg(add_str("$raceEspiri"));
				int eleEspiri = mapreg_readreg(add_str("$eleEspiri"));
				if(raceEspiri) { md->status.race = md->base_status->race = race[raceEspiri-1]; }
				if(eleEspiri) md->base_status->def_ele = ele[eleEspiri-1];
			}
			if(md->db->id == 22946) {	// Unreal
				int raceCristal = mapreg_readreg(add_str("$raceCristal"));
				int eleCristal = mapreg_readreg(add_str("$eleCristal"));
				if(raceCristal) { md->status.race = md->base_status->race = race[raceCristal-1]; }
				if(eleCristal) md->base_status->def_ele = ele[eleCristal-1];
			}
			if(md->db->id == 22952){ 	// Pesadelo
				int raceDesola = mapreg_readreg(add_str("$raceDesola"));
				int eleDesola = mapreg_readreg(add_str("$eleDesola"));
				if(raceDesola) { md->status.race = md->base_status->race = race[raceDesola-1]; }
				if(eleDesola) md->base_status->def_ele = ele[eleDesola-1];
			}
		}
	}
...
int mob_randomwalk(struct mob_data *md,t_tick tick)
{
	...
	if(DIFF_TICK(md->next_walktime,tick)>0 ||
	   ...
	   !unit_can_move(&md->bl) ||
	   !status_has_mode(&md->status,MD_CANMOVE) && !md->status.nomove)
		return 0;
...
// devon drop final
// -----------------------------------------
int getdropfinal(struct map_session_data *sd,struct map_data *mapdata){
	int dropfinal = 0;
	
	if (sd->sc.data[SC_EFEITO_RH_23]) {
		dropfinal += sd->sc.data[SC_EFEITO_RH_23]->val1;
	}
	else if (sd->sc.data[SC_EFEITO_RH_22]) {
		dropfinal += sd->sc.data[SC_EFEITO_RH_22]->val1;
	}
	else if (sd->sc.data[SC_FENDAD_INFERNAL]) {
		dropfinal += sd->sc.data[SC_FENDAD_INFERNAL]->val1;
	}
	else if (sd->sc.data[SC_EFEITO_RH_21]) {
		dropfinal += sd->sc.data[SC_EFEITO_RH_21]->val1;
	}
	
	// TOP RANKINGS Desafio do Guardião
	...
	if(sd->sc.data[SC_BLACKFRIDAY])
		dropfinal += 6;
	if(sd->sc.data[SC_DOCE_AGI])
		dropfinal += 2;
	if(sd->sc.data[SC_PERGA_AMANTES])
		dropfinal += sd->sc.data[SC_PERGA_AMANTES]->val1;
	if(sd->sc.data[SC_PERGA_CAMPEOES])
		dropfinal += sd->sc.data[SC_PERGA_CAMPEOES]->val1;
	
	if(sd->pd && sd->pd->pet.equip >= 77730 && sd->pd->pet.equip <= 77732){
		dropfinal += 1 + sd->pd->pet.refine;
	}
	
	if(sd->history_state.resetaccount > 0){
		dropfinal += 1;
		if(sd->history_state.resetaccount >= 2) dropfinal += 1;
		if(sd->history_state.resetaccount >= 3) dropfinal += 1;
		if(sd->history_state.resetaccount >= 4) dropfinal += 2;
		if(sd->history_state.resetaccount >= 5) dropfinal += 3;
	}
	// Perga escolhidos  e Fenda Dimensional
	if(sd->sc.data[SC_ESCOLHIDOS] && mapdata->flag[MF_FENDADIMENSIONAL]){
		dropfinal += 5;
	}
	// Rank Trial
	...
	// relogio festa
	if(battle_config.exp_rate_relogio){
		dropfinal += 3;
	}
	if(sd->bonus.drop_final > 0) dropfinal += sd->bonus.drop_final;
...
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			if (sd->sc.data[SC_ITEMBOOST] && !sd->sc.data[SC_CALICE_I] && !sd->sc.data[SC_CALICE_II])
				drop_rate_bonus += sd->sc.data[SC_ITEMBOOST]->val1;

			// devon
			if(battle_config.aumento_drop > 0)
				drop_rate_bonus += battle_config.aumento_drop;
			if(battle_config.aniversario){
			...
			if (pc_isvip(sd)) { // Increase item drop rate for VIP.
				...
				if(sd->state.dias_assinatura){
					drop_rate_bonus += battle_config.vip_drop_increase * 2;
				} else {
			...
			if(!mapdata->flag[MF_CHEFFORIGINAL]){
				if(sd->state.rate_servidor >= 1 && sd->status.base_level >= 200){
					if(sd->state.temporada == 1){ 
						if(sd->status.base_level >= 250) drop_rate_bonus += 60; 
						if(sd->status.base_level >= 260) drop_rate_bonus += 5; 
					}
					else if(sd->state.rate_servidor == 1) drop_rate_bonus += 50;
					else if(sd->state.rate_servidor == 12) drop_rate_bonus += 37;
					else if(sd->state.rate_servidor == 25) drop_rate_bonus += 25;
					else if(sd->state.rate_servidor == 50) drop_rate_bonus += 12;
				}
			}
			...
			// devon
			if (sd->sc.data[SC_EFEITO_RH_5] && !sd->sc.data[SC_CALICE_I] && !sd->sc.data[SC_CALICE_II]) {
				drop_rate_bonus += sd->sc.data[SC_EFEITO_RH_5]->val1;
			}

			// devon
			if (sd->sc.data[SC_CALICE_I]) {
				drop_rate_bonus += sd->sc.data[SC_CALICE_I]->val1;
			}
			if (!sd->sc.data[SC_CALICE_I] && sd->sc.data[SC_CALICE_II]) {
				drop_rate_bonus += sd->sc.data[SC_CALICE_II]->val1;
			}
			// devon
			if (sd->sc.data[SC_EFEITO_RH_8] && !sd->sc.data[SC_CALICE_I] && !sd->sc.data[SC_CALICE_II]) {
				drop_rate_bonus += sd->sc.data[SC_EFEITO_RH_8]->val1;
			}

			// devon
			if (sd->sc.data[SC_EFEITO_RH_25]) {
				drop_rate_bonus += sd->sc.data[SC_EFEITO_RH_25]->val1;
			}

			// devon
			if (sd->sc.data[SC_EFEITO_RH_26]) {
				drop_rate_bonus += sd->sc.data[SC_EFEITO_RH_26]->val1;
			}

			// devon
			// reputation
			// cheff extreme
			if(sd->reputation[2].points >= 1000){
				drop_rate_bonus += ((sd->reputation[2].points / 1000) / 1) * 2;
			}
			// cheff Selada
			if(sd->reputation[7].points >= 1000){
				drop_rate_bonus += ((sd->reputation[7].points / 1000) / 1);
			}
			// Bio 5 Extreme
			if(sd->reputation[4].points >= 1000){
				drop_rate_bonus += ((sd->reputation[4].points / 1000) / 1) * 2;
			}
			// temporada
			if(sd->reputation[6].points >= 1000){
				drop_rate_bonus += ((sd->reputation[6].points / 1000) / 1) * 2;
			}
			...
			// devon
			if (mapdata->flag[MF_ESCURIDAOPVP]) {
				drop_rate_bonus += 25;
			}

			// devon
			if (mapdata->flag[MF_VARMUNDTDROP]) {
				drop_rate_bonus += 50;
			}
			
			// devon
			if(sd->status.grandcomp && mapdata->flag[MF_MAPASFARM] && battle_config.desativarhgc){
				drop_rate_bonus += 20;
			}			
			// devon
			if(sd->status.grandcomp && mapdata->flag[MF_LABIRINTOVALK] && battle_config.desativarhgc){
				drop_rate_bonus += 50;
			}
			
			// Excelencia
			if(battle_config.desativarexcelencia){
				if(mapdata->flag[MF_DOMINIOREFERENCIA]){
					int repudom = sd->reputation[12].points;
					if(repudom >= 1000) drop_rate_bonus += 1;
					if(repudom >= 2000) drop_rate_bonus += 2;
					if(repudom >= 3000) drop_rate_bonus += 3;
					if(repudom >= 4000) drop_rate_bonus += 4;
					if(repudom >= 5000) drop_rate_bonus += 5;
					if(repudom >= 6000) drop_rate_bonus += 6;
					if(repudom >= 7000) drop_rate_bonus += 7;
					//if(repudom >= 8000) drop_rate_bonus += 8;
					//if(repudom >= 9000) drop_rate_bonus += 9;
					//if(repudom >= 10000) drop_rate_bonus += 15;
				}
			}

			drop_rate = (int)( 0.5 + drop_rate * drop_rate_bonus / 100. );

			int dropfinal = getdropfinal(sd, mapdata);
				
			if(dropfinal > 0)
				drop_rate += (int)( 0.5 + drop_rate * dropfinal / 100. );
...
int getgarrafareput(map_session_data *sd, int item_id){
	...
	tmp_item.amount = 1;
	tmp_item.identify = 1;
	int flag = pc_additem(sd, &tmp_item, 1, LOG_TYPE_SCRIPT);
	...
	clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Garrafa de Reputação] Após matar 4000 monstros você recebeu uma garrafa de reputação.", false, SELF);
	
	return 1;
}

int drop_sub(map_session_data *sd, int item_id, int base_rate, int goma, int64 imenso, int map, int amount, const char* nomeitem, const char* local, const char* chance_escrita, const char* mapaname, int grupo){
	int drop_rate = base_rate;
	int drop_rate_bonus = 100;
	int64 cap = imenso;
	int qtd = 0, id = 0, pontos;
	
	struct map_data *mapdata = map_getmapdata(sd->bl.m);

	if(sd->history_state.first_gepard && session[sd->fd]->gepard_info.unique_id != sd->history_state.first_gepard && (base_rate != imenso)){
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Sistema He-Man] Você não é o primeiro jogador que acessou essa conta hoje e por isso o seu drop foi reduzido a 0.", false, SELF);
		return 0;
	}	

	if(mapdata->flag[MF_DOMINIOGERAL] && mapdata->flag[MF_DOMINIONORMAL] && !sd->sc.data[SC_FADIGA_FLORESTA]){
		if(sd->state.aviso != 1){
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Domínio Normal] Você atingiu o limite diário de 4 horas de drops nesse mapa, por isso você não está recebendo nenhum drop, volte após às 00h.", false, SELF);
			sd->state.aviso = 1;
		}
		return 0;
	}
	/*	
	if(mapdata->flag[MF_FLORESTAPES] && !mapdata->flag[MF_VULCAOPESADELO] && !sd->sc.data[SC_FADIGA_FLORESTA]){
		if(sd->state.aviso != 1){
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Floresta Infernal] Você atingiu o limite diário de 4 horas de drops nesse mapa, por isso você não está recebendo nenhum drop, volte após às 00h.", false, SELF);
			sd->state.aviso = 1;
		}
		return 0;
	}	
	*/
	if(mapdata->flag[MF_VULCAOPESADELO] && !sd->sc.data[SC_FADIGA_VULCAO]){
		if(sd->state.aviso != 1){
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Vulcão Pesadelo] Você atingiu o limite diário de 4 horas de drops nesse mapa, por isso você não está recebendo nenhum drop, volte após às 00h.", false, SELF);
			sd->state.aviso = 1;
		}
		return 0;
	}	
	if(mapdata->flag[MF_TEMPLOULTIMATE] && !sd->sc.data[SC_FADIGA_TEM_PESA]){
		if(sd->state.aviso != 1){
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Templo Pesadelo] Você atingiu o limite diário de 4 horas de drops nesse mapa, por isso você não está recebendo nenhum drop, volte após às 00h.", false, SELF);
			sd->state.aviso = 1;
		}
		return 0;
	}	
	if(mapdata->flag[MF_ESCURIDAO] && !mapdata->flag[MF_TEMPLOULTIMATE] && !sd->sc.data[SC_FADIGA_ESCU]){
		if(sd->state.aviso != 1){
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "[Templo da Escuridão] Você atingiu o limite diário de 4 horas de drops nesse mapa, por isso você não está recebendo nenhum drop, volte após às 00h.", false, SELF);
			sd->state.aviso = 1;
		}
		return 0;
	}
	if(sd->engarrafar && item_id != 38077 && item_id != 38073 && (mapdata->flag[MF_CHEFFSAVAGE] || mapdata->flag[MF_BIOSAVAGE])){
		return 0;
	}
	...
	if(base_rate < imenso){
		...
		if(item_id >= 38070 && item_id <= 38077 && sd->state.drop_extra){	
			drop_rate += sd->state.drop_extra;
		}
		
		if(item_id >= 38071 && item_id <= 38077 && item_id != 38074){	
			char msgrep[256];
			int rep;
			if(item_id == 38077){
				rep = sd->reputation[3].points;
				if(rep < 5000){
					sd->mobs_cheff_savage += 1;

					int mobs = sd->mobs_cheff_savage;
					
					if(rep < 1000) { if(mobs >= 5) { sd->reputation[3].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Savage] Você recebeu 1 ponto de Reputação após derrotar 5 MvPs."); } }
					else if(rep < 2000) { if(mobs >= 10) { sd->reputation[3].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Savage] Você recebeu 1 ponto de Reputação após derrotar 10 MvPs."); } }
					else if(rep < 3000) { if(mobs >= 20) { sd->reputation[3].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Savage] Você recebeu 1 ponto de Reputação após derrotar 20 MvPs."); } }
					else if(rep < 4000) { if(mobs >= 40) { sd->reputation[3].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Savage] Você recebeu 1 ponto de Reputação após derrotar 40 MvPs."); } }
					else if(rep < 5000) {  if(mobs >= 80) { sd->reputation[3].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Savage] Você recebeu 1 ponto de Reputação após derrotar 80 MvPs."); } }
					if(mobs == 0) {
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msgrep, false, SELF);
						sd->mobs_cheff_savage = 0;
					}
				} else {
					if(sd->engarrafar){
						sd->mobs_cheff_savage += 1;
						...
						int mobs = sd->mobs_cheff_savage;
						int qtdmobs = 4000;
						if(sd->group_id >= 10) qtdmobs = 10;
						
						if(mobs >= qtdmobs){
							sd->mobs_cheff_savage = 0;
							getgarrafareput(sd, 79996);
			...
			if(item_id == 38073){
				rep = sd->reputation[5].points;
				if(rep < 5000){
					sd->mobs_bio_savage += 1;

					int mobs = sd->mobs_bio_savage;

					if(rep < 1000) { if(mobs >= 5) { sd->reputation[5].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Savage] Você recebeu 1 ponto de Reputação após derrotar 5 MvPs."); } }
					else if(rep < 2000) { if(mobs >= 10) { sd->reputation[5].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Savage] Você recebeu 1 ponto de Reputação após derrotar 10 MvPs."); } }
					else if(rep < 3000) { if(mobs >= 20) { sd->reputation[5].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Savage] Você recebeu 1 ponto de Reputação após derrotar 20 MvPs."); } }
					else if(rep < 4000) { if(mobs >= 40) { sd->reputation[5].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Savage] Você recebeu 1 ponto de Reputação após derrotar 40 MvPs."); } }
					else if(rep < 5000) {  if(mobs >= 80) { sd->reputation[5].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Savage] Você recebeu 1 ponto de Reputação após derrotar 80 MvPs."); } }
					if(mobs == 0) {
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msgrep, false, SELF);
						sd->mobs_bio_savage = 0;
					}
				} else {
					if(sd->engarrafar){
						sd->mobs_bio_savage += 1;
			...
			if(item_id == 38076){
				if(sd->sc.data[SC_EFEITO_RH_28]) drop_rate = drop_rate / 3;
				else if(sd->sc.data[SC_EFEITO_RH_27]) drop_rate = drop_rate / 2;
			}
			if(item_id == 38075){
				if(sd->sc.data[SC_EFEITO_RH_30]) drop_rate = drop_rate / 3;
				else if(sd->sc.data[SC_EFEITO_RH_29]) drop_rate = drop_rate / 2;
			}
			if(item_id == 38077){
				if(sd->sc.data[SC_EFEITO_RH_32]) drop_rate = drop_rate / 3;
				else if(sd->sc.data[SC_EFEITO_RH_31]) drop_rate = drop_rate / 2;
			}
			
			if(item_id == 38072){
				if(sd->sc.data[SC_BIO_EXT_2]) drop_rate = drop_rate / 3;
				else if(sd->sc.data[SC_BIO_EXT_1]) drop_rate = drop_rate / 2;
			}
			if(item_id == 38071){
				if(sd->sc.data[SC_BIO_HARD_2]) drop_rate = drop_rate / 3;
				else if(sd->sc.data[SC_BIO_HARD_1]) drop_rate = drop_rate / 2;
			}
			if(item_id == 38073){
				if(sd->sc.data[SC_BIO_SAV_2]) drop_rate = drop_rate / 3;
				else if(sd->sc.data[SC_BIO_SAV_1]) drop_rate = drop_rate / 2;
			}
		}


		// remo  o do aumento de drop
		if(goma == 2) drop_rate_bonus = 100;
		else if(goma == 3 || goma == 4){	// apenas goma funfa
			drop_rate_bonus = 100;
			if (sd->sc.data[SC_ITEMBOOST] && !sd->sc.data[SC_CALICE_I] && !sd->sc.data[SC_CALICE_II])
				drop_rate_bonus += sd->sc.data[SC_ITEMBOOST]->val1;
			...
			// Pk Mode
			if(!mapdata->flag[MF_BIOEXTREME] && sd->sc.data[SC_EFEITO_RH_7]){
				drop_rate_bonus += 25;
			}
		...
		drop_rate_bonus += sd->indexed_bonus.dropaddrace[RC_ALL];

		// devon (pocao revitalizadora funfa)
		if (sd->sc.data[SC_EFEITO_RH_5] && !sd->sc.data[SC_CALICE_I] && !sd->sc.data[SC_CALICE_II]) {
			drop_rate_bonus += sd->sc.data[SC_EFEITO_RH_5]->val1;
		}
		...
		if(sd->sc.data[SC_MEM_HISTORY_GC] && mapdata->flag[MF_DROPHGC])
			drop_rate_bonus += 20;	
		...
		if(battle_config.desativarexcelencia){
			if(mapdata->flag[MF_DOMINIOREFERENCIA]){
				...
				//if(repudom >= 8000) drop_rate_bonus += 8;
				//if(repudom >= 9000) drop_rate_bonus += 9;
				//if(repudom >= 10000) drop_rate_bonus += 25;
			}
		}
		...
		if(sd->history_state.bindtype == 3 || !sd->history_state.bindtype){
			if(
				(
				mapdata->flag[MF_AURA95] || 
				mapdata->flag[MF_AURA99] || 
				mapdata->flag[MF_AURA999] || 
				mapdata->flag[MF_AURA999] || 
				mapdata->flag[MF_FENDADIMENSIONAL] || 
				mapdata->flag[MF_BINDMAP]
				) && sd->history_state.bindconta < 10){
				imenso = imenso * 1000;
				int reducao = 750 + (sd->history_state.bindconta * 25);
				drop_rate = drop_rate * reducao;
			}
		} else if(sd->history_state.bindtype == 2){
			if(
				mapdata->flag[MF_AURA999] || 
				mapdata->flag[MF_AURA99] ||  
				mapdata->flag[MF_AURA95] || 
				(mapdata->flag[MF_BINDMAP] && sd->state.nivelconteudo >= 4 && sd->state.nivelconteudo <= 8) || 
				(mapdata->flag[MF_FENDADIMENSIONAL] && sd->state.nivelconteudo <= 7)
			){
		...
		} else if(sd->history_state.bindtype == 1){
			if(
				mapdata->flag[MF_AURA95] || mapdata->flag[MF_AURA99] || 
				(mapdata->flag[MF_BINDMAP] && sd->state.nivelconteudo >= 4 && sd->state.nivelconteudo <= 6) || 
				(mapdata->flag[MF_FENDADIMENSIONAL] && sd->state.nivelconteudo <= 3)
			){
				if(sd->history_state.bindconta < 10){
					imenso = imenso * 1000;
					int reducao = 750 + (sd->history_state.bindconta * 25);
		...
		//// Cap it to 100%
		drop_rate = min( drop_rate, imenso );
		
		if(grupo == 1 && (!sd->state.biohq || !sd->state.cheffhq) && (item_id == 38071 || item_id == 38075)){
			sd->contagrupo = sd->contagrupo + 1;
			if(sd->contagrupo >= 50){
				// Bio Hard Quests
				if(item_id == 38071){
					/*
					...
					int j, f;
					if(p){
						for (f = 0; f < MAX_PARTY; f++) {
							if (p->party.member[f].online){
								struct map_session_data *psd = p->data[f].sd;
								if(!psd)
									continue;
								if(psd->bl.m == sd->bl.m)
									j++;
					...
					if(j >= 3){
						*/
						int compbiohq = pc_readglobalreg(sd, add_str("#COMPLETEBIOHQ"));
						if(compbiohq < 1000) pc_setglobalreg(sd, add_str("#COMPLETEBIOHQ"), compbiohq + sd->contagrupo);
					//}
				}
				// cheff Hard Quest
				if(item_id == 38075){
					...
					if(j >= 3){
						*/
						int compcheffhq = pc_readglobalreg(sd, add_str("#COMPLETECHEFFHQ"));
						if(compcheffhq < 1000) pc_setglobalreg(sd, add_str("#COMPLETECHEFFHQ"), compcheffhq + sd->contagrupo);
					//}
				}
				sd->contagrupo = 0;
			}
		...
		if(sd->group_id >= 10){
			char msgnew[256];
			sprintf(msgnew, "Drop do Item: %d -> Chance: %d / Imenso: %d / Goma: %d", item_id, drop_rate, imenso, goma);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msgnew, false, SELF);
		}

		if (rnd() % imenso >= drop_rate){
			return 0;
		}

		if(item_id == 38076){
			sd->addfadiga[6].qtd += 1;
		} else if(item_id == 38072){
			sd->addfadiga[2].qtd += 1;
		} else if(item_id == 38075){
			sd->addfadiga[5].qtd += 1;
		} else if(item_id == 38071){
			sd->addfadiga[1].qtd += 1;
		}else if(item_id == 38077){
			sd->addfadiga[7].qtd += 1;
		} else if(item_id == 38073){
			sd->addfadiga[3].qtd += 1;
		}
		
		if(item_id >= 38070 && item_id <= 38077){		
			if(sd->group_id >= 10){
				char msgnew[256];
				sprintf(msgnew, "chance = %d", drop_rate);
				clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msgnew, false, SELF);
			}
			...
			if(y < 0) y = 0;
			sd->pontos[y].qtd += 1;
			if(sd->pontos[y].qtd % 10 == 0) {
				sprintf(msgcb, "Moedas/Cristais armazenados: %d (Utilize o comando @pontos para receber)", sd->pontos[y].qtd);
				clif_messagecolor(&sd->bl, 0, msgcb, false, SELF);
			}
	...
	if(item_id >= 9000 && item_id <= 9999 ){
		pet_create_egg(sd, item_id);
	} else {
		struct item tmp_item;
		memset(&tmp_item,0,sizeof(tmp_item));
		tmp_item.nameid = item_id;
		...
		tmp_item.identify = 1;
		int flag = pc_additem(sd, &tmp_item, amount, LOG_TYPE_SCRIPT);
		if(flag != 0){
			if(base_rate == imenso || (imenso >= 100000 && base_rate < 100)){
				const char *sender, *title, *body, *name;
				struct mail_message msg;
				...
				clif_messagecolor(&sd->bl, color_table[COLOR_RED], "[Inventário Cheio] Libere espaço antes de tentar dropar outro item.", false, SELF);
			}
		}
	}
	...
	char mensagem[256];
	if(map == 1){
		sprintf(mensagem, "[%s] O jogador %s dropou o item %s [%s].", local,sd->status.name,nomeitem,chance_escrita);
		map_foreachinmap(buildin_announce_sub, sd->bl.m, BL_PC,mensagem, strlen(mensagem)+1, 8&BC_COLOR_MASK, NULL, FW_NORMAL, 12, 0, 0);
	} else if(map == 2){
		sprintf (mensagem, "[%s] O jogador %s dropou o item %s [%s].", local,sd->status.name,nomeitem,chance_escrita);

		intif_broadcast(mensagem, (int)strlen(mensagem)+1, 8&BC_COLOR_MASK);
	} else if(map == 3){
		sprintf (mensagem, "[%s] O jogador %s dropou o item %s [%s].", local,sd->status.name,nomeitem,chance_escrita);
		map_foreachinmap(buildin_announce_sub, sd->bl.m, BL_PC,mensagem, strlen(mensagem)+1, 8&BC_COLOR_MASK, NULL, FW_NORMAL, 12, 0, 0);
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if(sd){
		if(sd->killcount_timer){
			int now = (int)time(NULL);
			if(sd->killcount_timer >= now){
				sd->killcount++;
			} else {
				sd->killcount_timer = 0;
				char msg[300];
				sprintf(msg, "[@killcount] Durante os 15 minutos que o comando ficou ativo você matou %d monstros.", sd->killcount); 
			}
		}
		
		if(sd->status.party_id && !mapdata->flag[MF_MAPASOLO] && (
				(mapdata->flag[MF_TEMPLOULTIMATE] && md->npc_event[0]) || 
				(mapdata->flag[MF_MOBITEMDROP] && md->npc_event[0] && mapdata->dropgrupo)
			)
		){
			int x1 = sd->bl.x - 15;
			int x2 = sd->bl.x + 15;
			...
			int maxplayer = 0, countplayer = 0;
			
			struct party_data *p = party_search(sd->status.party_id);
			if(p){
				if(mapdata->flag[MF_CHEFFSAVAGE] || mapdata->flag[MF_BIOSAVAGE]) maxplayer = 8;
				if(mapdata->flag[MF_BIOHARD]) maxplayer = 4;
				if(mapdata->flag[MF_CHEFFBOOSTER]) maxplayer = 8;
				for (i = 0; i < MAX_PARTY; i++) {
					if (p->party.member[i].online){
						struct map_session_data *psd = p->data[i].sd;
						if(!psd)
							continue;
						if(psd->bl.m == sd->bl.m && (psd->bl.x >= x1 && psd->bl.x <= x2) && (psd->bl.y >= y1 && psd->bl.y <= y2)){
							countplayer++;
							if(maxplayer > 0 && countplayer > maxplayer) {
								continue;
							}
							if(mapdata->flag[MF_TEMPLOULTIMATE] && md->npc_event[0]){
								for (const auto &it : mapdata->mobitemdrop_list) {
									for (const auto &subit : it.mobitemdrop_sub_list) {
										if(!subit.nameid || !subit.chance || !subit.boost || !subit.imenso || !subit.amount || !subit.nomeitem || !subit.local || !subit.cescrita)
											continue;
										drop_sub(psd, subit.nameid, subit.chance, subit.boost, subit.imenso, subit.map, subit.amount, subit.nomeitem, subit.local, subit.cescrita, "", 1);
									}
								}
							} else if(mapdata->flag[MF_MOBITEMDROP] && md->npc_event[0]){
									for (const auto &it : mapdata->mobitemdrop_list) {
										if(strlen(md->npc_event) == (strlen(it.eventname) - 2 )){ 
											for (const auto &subit : it.mobitemdrop_sub_list) {
												if(!subit.nameid || !subit.chance || !subit.boost || !subit.imenso || !subit.amount || !subit.nomeitem || !subit.local || !subit.cescrita)
													continue;
												//ShowWarning("%d \n",subit.nameid);
												if(subit.grupo || psd->status.account_id == sd->status.account_id)
													drop_sub(psd, subit.nameid, subit.chance, subit.boost, subit.imenso, subit.map, subit.amount, subit.nomeitem, subit.local, subit.cescrita, "", subit.grupo);
											}
										}
									}
							}
			...
			if((mapdata->flag[MF_FLORESTAPES] || mapdata->flag[MF_TEMPLOULTIMATE]) && md->npc_event[0]){
				for (const auto &it : mapdata->mobitemdrop_list) {
					for (const auto &subit : it.mobitemdrop_sub_list) {
						if(!subit.nameid || !subit.chance || !subit.boost || !subit.imenso || !subit.amount || !subit.nomeitem || !subit.local || !subit.cescrita)
							continue;
						drop_sub(sd, subit.nameid, subit.chance, subit.boost, subit.imenso, subit.map, subit.amount, subit.nomeitem, subit.local, subit.cescrita, "", 0);
					}
				}
			} else if(mapdata->flag[MF_MOBITEMDROP] && md->npc_event[0]){
				for (const auto &it : mapdata->mobitemdrop_list) {
		...
		if(mapdata->flag[MF_DROPSPECIFICMOB]){
			if(sd->state.exceldrop < 1 || !mapdata->flag[MF_DOMINIOGERAL]) {
				if(mapdata->flag[MF_DOMINIONORMAL] || mapdata->flag[MF_DOMINIOHARD] || mapdata->flag[MF_DOMINIOUNREAL] || mapdata->flag[MF_DOMINIOPESADELO]){
					const char *mapa = "dominio_f";
					if(mapdata->flag[MF_DOMINIOHARD]) mapa = "dominio_es";
					else if(mapdata->flag[MF_DOMINIOUNREAL]) mapa = "dominio_c";
					else if(mapdata->flag[MF_DOMINIOPESADELO]) mapa = "dominio_d";
					
					int16 mapm = map_mapname2mapid(mapa);
					struct map_data *mapdatadom = map_getmapdata(mapm);
					
					for (const auto &it : mapdatadom->dropspecificmob_list) {
						if(it.mob_id == md->mob_id){
							for (const auto &subit : it.dropspecificmob_sub_list) {
								//ShowWarning("item = %d",subit.nameid );
								if(!subit.nameid || !subit.chance || !subit.boost || !subit.imenso || !subit.amount || !subit.nomeitem || !subit.local || !subit.cescrita)
									continue;
								drop_sub(sd, subit.nameid, subit.chance, subit.boost, subit.imenso, subit.map, subit.amount, subit.nomeitem, subit.local, subit.cescrita, "", 0);
							}
					...
					for (const auto &it : mapdata->dropspecificmob_list) {
						if(it.mob_id == md->mob_id){
							for (const auto &subit : it.dropspecificmob_sub_list) {
								//ShowWarning("item = %d",subit.nameid );
								if(!subit.nameid || !subit.chance || !subit.boost || !subit.imenso || !subit.amount || !subit.nomeitem || !subit.local || !subit.cescrita)
									continue;
								// monstro horda ultimate - Rotação
								if(md->mob_id == 22961 && (subit.nameid == 470180 || subit.nameid == 420236 || subit.nameid == 490337 || subit.nameid == 29724)){
									if(subit.nameid != battle_config.rotacao_dom_one && subit.nameid != battle_config.rotacao_dom_two) 
										continue;
								}
								drop_sub(sd, subit.nameid, subit.chance, subit.boost, subit.imenso, subit.map, subit.amount, subit.nomeitem, subit.local, subit.cescrita, "", 0);
							}
	...
	if(md->db->id == 3456 && sd && (sd->sc.data[SC_VENEPROFUN_1] || sd->sc.data[SC_VENEPROFUN_2] || sd->sc.data[SC_VENEPROFUN_3] || sd->sc.data[SC_VENEPROFUN_4])){
		if(sd->sc.data[SC_VENEPROFUN_1]) status_change_end(src, SC_VENEPROFUN_1, INVALID_TIMER);
		else {
			int idven;
			
			if(sd->sc.data[SC_VENEPROFUN_2]) { idven = SC_VENEPROFUN_2; status_change_end(src, SC_VENEPROFUN_2, INVALID_TIMER); }
			if(sd->sc.data[SC_VENEPROFUN_3]) { idven = SC_VENEPROFUN_3; status_change_end(src, SC_VENEPROFUN_3, INVALID_TIMER); }
			if(sd->sc.data[SC_VENEPROFUN_4]) { idven = SC_VENEPROFUN_4; status_change_end(src, SC_VENEPROFUN_4, INVALID_TIMER); }


			//struct TimerData* timer = (struct TimerData*)get_timer(sd->sc.data[idven]->timer); (timer->tick - gettick())
			
			//if( timer )
			//{// return the amount of time remaining
				if(idven == SC_VENEPROFUN_2) sc_start(src, src, SC_VENEPROFUN_1, 100, 1, 30000);
				if(idven == SC_VENEPROFUN_3) sc_start(src, src, SC_VENEPROFUN_2, 100, 1, 30000);
				if(idven == SC_VENEPROFUN_4) sc_start(src, src, SC_VENEPROFUN_3, 100, 1, 30000);
			//}
			sc_start(&sd->bl, &sd->bl, SC_VELO_ESPECIAL, 100, 50, 5000);
			clif_skill_nodamage(src, src, AL_INCAGI, 1, 1);
		}
	}
	
	if((md->db->id == 22977 || md->db->id == 22978 || md->db->id == 22979 || md->db->id == 22980) && sd){
		//status_change_end(src, SC_DEBUFF_MINION, INVALID_TIMER);
		map_setmapflag(sd->bl.m, MF_NODAMAGE, false);
		sc_start(&sd->bl, &sd->bl, SC_MASMORRA_DANO, 100, 10, 5000);
		clif_skill_nodamage(src, src, AL_BLESSING, 1, 1);
	}
	
	int travaretornoscript = 0;
	
	if(mapdata->flag[MF_FENDA]){
		if(md && (
		md->db->id == 28040 || md->db->id == 28041 || md->db->id == 28043 || md->db->id == 28044 || md->db->id == 28045 || md->db->id == 28062 || 
		md->db->id == 28046 || md->db->id == 28047 || md->db->id == 28048 || md->db->id == 28049 || md->db->id == 28050 || md->db->id == 28052
		)){
			travaretornoscript =  1;
			if(sd){
				struct party_data *p = party_search(sd->status.party_id);
				if(p){
					if(rnd()%100 == 55){ npc_event_do_id("StatusFenda::OnStatus", sd->status.account_id); }
					int pontuacao = 1500, baratual = 0;
					if(p->nivel >= 16) pontuacao = 2000;
					if(sd->group_id >= 10) pontuacao = 100;

					if(md->db->id == 28041 || md->db->id == 28052){
						p->pontos += 75;
						p->aviso += 75;
						
						mob_once_spawn(sd, m, sd->bl.x, sd->bl.y,"Bomba", 1904, 1, "", SZ_SMALL, AI_NONE);
						
						int countbar = pontuacao / 100;
						baratual = (int)(p->pontos / countbar);
						if(baratual < 0) baratual = 0;
						if(baratual > 100) baratual = 100;
						char msgbar[256];
						sprintf(msgbar, "%d", baratual);
						clif_cutin(sd,msgbar,2);
					} else {
						p->pontos += 1;
						p->aviso += 1;
						p->bar += 1;
					...
					int pgoblin = pontuacao / 2;
					if(!p->goblin && p->pontos >= pgoblin){
						p->goblin = 1;
						int pct = rnd()%1000;
						if(pct >= 10 && pct < 30){
							struct npc_data *nd;
							nd = npc_name2id("Portal Fenda");
							if(nd){
								static char npcname[NAME_LENGTH];
								snprintf(npcname, sizeof(npcname), "dup_%d_%d", p->instance_id, nd->bl.id);
								char mapamsg[256];
								sprintf(mapamsg, "%s::OnCallGoblin", npcname);
								npc_event_do_id(mapamsg, sd->status.account_id);
							}
						} else if(pct >= 33 && pct <= 34){
							struct npc_data *nd;
							nd = npc_name2id("Portal Fenda");
							if(nd){
								static char npcname[NAME_LENGTH];
								snprintf(npcname, sizeof(npcname), "dup_%d_%d", p->instance_id, nd->bl.id);
								char mapamsg[256];
								sprintf(mapamsg, "%s::OnCallGoblinArco", npcname);
								npc_event_do_id(mapamsg, sd->status.account_id);
							}
						}
					}
					if(p->pontos >= pontuacao){
						travaretornoscript = 0;
						for (i = 0; i < MAX_PARTY; i++) {
							if (p->party.member[i].online){
								struct map_session_data *psd = p->data[i].sd;
								if(!psd)
					...
					} else if((p->aviso / 100) > 0){
						p->aviso = p->aviso - 100;
						
						char mensagem[256];
						sprintf(mensagem, "[Fenda] Pontuação atual: %d", p->pontos);
						map_foreachinmap(buildin_announce_sub, sd->bl.m, BL_PC,mensagem, strlen(mensagem)+1, 8&BC_COLOR_MASK, NULL, FW_NORMAL, 12, 0, 0);
					}
				}
			}
	...
	if(mapdata->flag[MF_FENDADIMENSIONAL]){
		if(md && (md->db->id >= 22983 && md->db->id) && md->db->id != 22989 && md->db->id < 28056){
			travaretornoscript =  1;
			if(sd){
				struct party_data *p = party_search(sd->status.party_id);
				if(p){
					if(rnd()%100 == 55){ npc_event_do_id("StatusFendaInf::OnStatus", sd->status.account_id); }
					int pontuacao = 2000, baratual = 0;
					
					//if(p->nivel >= 4) pontuacao = 2000;
					if(p->nivel >= 7) pontuacao = 1500;
					if(sd->group_id >= 10) pontuacao = 200;
					if(md->db->id == 22995 || md->db->id == 22988){
						p->pontos += 75;
						p->aviso += 75;
					...
					if(!p->goblin && p->pontos >= pgoblin){
						p->goblin = 1;
						int pct = rnd()%1000;
						//if(sd->group_id >= 10) pct = 20;  //Testar
						if(pct >= 10 && pct < 20){
							struct npc_data *nd;
							nd = npc_name2id("Portal Fenda D");
							if(nd){
								static char npcname[NAME_LENGTH];
								snprintf(npcname, sizeof(npcname), "dup_%d_%d", p->instance_id, nd->bl.id);
								char mapamsg[256];
					...
					if(p->pontos >= pontuacao){
						...
						char mensagem[256];
						sprintf(mensagem, "[Fenda Dimensional] Pontuação atual: %d", p->pontos);
						map_foreachinmap(buildin_announce_sub, sd->bl.m, BL_PC,mensagem, strlen(mensagem)+1, 8&BC_COLOR_MASK, NULL, FW_NORMAL, 12, 0, 0);
					}
				}
			}
	...
	if(sd && md && mapdata->flag[MF_CHEFFEXTREME]){
		travaretornoscript =  1;
		int blockdrop = 0;
		struct party_data *p = party_search(sd->status.party_id);
		if(p){
			if(strlen(md->npc_event) == strlen("#cheffbio::OnRootDead")){
				char msgrep[256];
				int rep = sd->reputation[2].points;
				mob_once_spawn(sd, m, 0, 0,"--ja--", md->db->id, 1, md->npc_event, SZ_SMALL, AI_NONE);

				if(rep < 5000){
					sd->state.mobs_cheff_ext += 1;

					int mobs = sd->state.mobs_cheff_ext;

					if(rep < 1000) { if(mobs >= 5) { sd->reputation[2].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Extreme] Você recebeu 1 ponto de Reputação após derrotar 5 MvPs."); } }
					else if(rep < 2000) { if(mobs >= 10) { sd->reputation[2].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Extreme] Você recebeu 1 ponto de Reputação após derrotar 10 MvPs."); } }
					else if(rep < 3000) { if(mobs >= 20) { sd->reputation[2].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Extreme] Você recebeu 1 ponto de Reputação após derrotar 20 MvPs."); } }
					else if(rep < 4000) { if(mobs >= 40) { sd->reputation[2].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Extreme] Você recebeu 1 ponto de Reputação após derrotar 40 MvPs."); } }
					else if(rep < 5000) {  if(mobs >= 80) { sd->reputation[2].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Cheffênia Extreme] Você recebeu 1 ponto de Reputação após derrotar 80 MvPs."); } }
					if(mobs == 0) {
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msgrep, false, SELF);
						sd->state.mobs_cheff_ext = 0;
					}
				} else {
					if(sd->engarrafar){
						sd->state.mobs_cheff_ext += 1;
	...
	if(sd && md && mapdata->flag[MF_BIOEXTREME]){
		travaretornoscript = 1;
		int blockdrop = 0;
		struct party_data *p = party_search(sd->status.party_id);
		if(p){
			if(strlen(md->npc_event) == strlen("#extremebio::OnRootDead")){
				char msgrep[256];
				int rep = sd->reputation[4].points;
				mob_once_spawn(sd, m, 0, 0,"--ja--", md->db->id, 1, md->npc_event, SZ_SMALL, AI_NONE);

				if(rep < 5000){
					sd->state.mobs_bio_ext += 1;

					int mobs = sd->state.mobs_bio_ext;

					if(rep < 1000) { if(mobs >= 5) { sd->reputation[4].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Extreme] Você recebeu 1 ponto de Reputação após derrotar 5 MvPs."); } }
					else if(rep < 2000) { if(mobs >= 10) { sd->reputation[4].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Extreme] Você recebeu 1 ponto de Reputação após derrotar 10 MvPs."); } }
					else if(rep < 3000) { if(mobs >= 20) { sd->reputation[4].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Extreme] Você recebeu 1 ponto de Reputação após derrotar 20 MvPs."); } }
					else if(rep < 4000) { if(mobs >= 40) { sd->reputation[4].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Extreme] Você recebeu 1 ponto de Reputação após derrotar 40 MvPs."); } }
					else if(rep < 5000) {  if(mobs >= 80) { sd->reputation[4].points = rep +1; mobs = 0; clif_reputation(sd); sprintf(msgrep, "[Reputação: Bio 5 Extreme] Você recebeu 1 ponto de Reputação após derrotar 80 MvPs."); } }
					if(mobs == 0) {
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], msgrep, false, SELF);
						sd->state.mobs_bio_ext = 0;
					}
				} else {
					if(sd->engarrafar){
						sd->state.mobs_bio_ext += 1;
	...
	if(!(type&2) && //No exp
		(!mapdata->flag[MF_PVP] || battle_config.pvp_exp) && //Pvp no exp rule [MouseJstr]
		(!md->master_id || !md->special_state.ai) && //Only player-summoned mobs do not give exp. [Skotlex]
		(!mapdata->flag[MF_NOBASEEXP] || !mapdata->flag[MF_NOJOBEXP]) //Gives Exp
		...
		for(i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if (mapdata->flag[MF_NOBASEEXP] || !md->db->base_exp)
				base_exp = 0;
			else {
				double exp = apply_rate2(md->db->base_exp, per, 1);
				exp = apply_rate(exp, bonus);
				exp = apply_rate(exp, mapdata->flag[MF_BEXP]);
				base_exp = (t_exp)cap_value(exp, 1, MAX_EXP);
			}

			if (mapdata->flag[MF_NOJOBEXP] || !md->db->job_exp
#ifndef RENEWAL
				...
				exp = apply_rate(exp, bonus);
				exp = apply_rate(exp, mapdata->flag[MF_JEXP]);
			...
			if (base_exp && tmpsd[i] && tmpsd[i]->hd)
				hom_gainexp(tmpsd[i]->hd, (base_exp * battle_config.homunculus_exp_gain / 100) * tmpsd[i]->state.rate_servidor); // Homunculus only receive 10% of EXP
#else
	...
	if(sd) {
		...
		for (const auto &it : sd->add_drop) {
				...
				(it.class_ == CLASS_ALL || it.class_ == status->class_)) //Matched class
			{
				...
				// caça tesouro trava
				if(it.nameid == 11248){
					if(sd->carnaval_state.cacatesouro >= 10)
						continue;
					sd->carnaval_state.cacatesouro = sd->carnaval_state.cacatesouro + 1;
					
					struct item tmp_item;
					memset(&tmp_item,0,sizeof(tmp_item));
					tmp_item.nameid = 11248;
					tmp_item.amount = 1;
					tmp_item.identify = 1;
					int flag = pc_additem(sd, &tmp_item, 1, LOG_TYPE_SCRIPT);
					if(flag != 0){
	...
	if( !(type&1) && !mapdata->flag[MF_NOMOBLOOT] && !md->state.rebirth && (
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			int mrate = 10000; int m = 1;
			if( mvp_sd && it->type == IT_CARD && md->get_bosstype() == BOSSTYPE_MVP ) {
			...
			// devon
			if( mvp_sd && it->type == IT_CARD && md->get_bosstype() == BOSSTYPE_MVP ) {
				if(mapdata->flag[MF_NODROPCARDMVP])
					continue;
				char message[128];
				sprintf (message, msg_txt(NULL,541), mvp_sd->status.name, md->name, it->ename.c_str(), (float)(drop_rate/100)/m);
				//MSG: "'%s' won %s's %s (chance: %0.02f%%)"

				if(!mapdata->flag[MF_CHEFFORIGINAL]){
					intif_broadcast(message,strlen(message)+1,BC_DEFAULT);
				} else {
					map_foreachinmap(buildin_announce_sub, mvp_sd->bl.m, BL_PC,
						message, strlen(message)+1, BC_COLOR_MASK, NULL, FW_NORMAL, 12, 0, 0);
				}
				// devon historico MVP card
				if( Sql_Query( mmysql_handle, "INSERT INTO `log_drop_card` (`char_id`, `card_id`, `nome_personagem`) VALUES( %d, %d, '%s' );",
					mvp_sd->status.char_id, it->nameid, mvp_sd->status.name) != SQL_SUCCESS ){
	...
	if( mvp_sd && md->get_bosstype() == BOSSTYPE_MVP ){
		...
		if( md->db->mexp > 0 && !( mapdata->flag[MF_NOBASEEXP] || type&2 ) ){
		...
		if( !(mapdata->flag[MF_NOMVPLOOT] || type&1) ) {
	...
	if( !rebirth ) { // Only trigger event on final kill
		...
		if(!mapdata->flag[MF_STOPDROPESPECIAL] && travaretornoscript == 0){
			if( md->npc_event[0] && !md->state.npc_killmonster ) {
				if( sd && battle_config.mob_npc_event_type && !mapdata->flag[MF_MVPONKILLER] ) {
					pc_setparam(sd, SP_KILLEDGID, md->bl.id);
					pc_setparam(sd, SP_KILLEDRID, md->mob_id);
					pc_setparam(sd, SP_KILLERRID, sd->bl.id);
					pc_setparam(sd, SP_KILLEDMOBX, md->bl.x);
					pc_setparam(sd, SP_KILLEDMOBY, md->bl.y);
					...
					pc_setparam(mvp_sd, SP_KILLEDMOBX, md->bl.x);
					pc_setparam(mvp_sd, SP_KILLEDMOBY, md->bl.y);
	...
	/*
	if(sd && md->status.class_ == CLASS_NORMAL){ 
		struct map_data *mapdata = map_getmapdata(sd->bl.m);
		if(!sd->state.autotrade && !mapdata->flag[MF_TOWN] && 
		!mapdata->flag[MF_PVP] && !mapdata->flag[MF_GVG] && !mapdata->flag[MF_BATTLEGROUND]
		&& !mapdata->flag[MF_NOTELEPORT]){
		int conta = pc_readglobalreg(sd, add_str("contamacro")) + 1;
		if(conta >= 2000){
			npc_event(sd, "macrochecker::OnCheckBot", 0);
			pc_setglobalreg(sd, add_str("contamacro"), 0);
		} else {
			pc_setglobalreg(sd, add_str("contamacro"), conta);
		}
	}
	}
	*/
...
int mob_getfriendhprate_sub(struct block_list *bl,va_list ap)
{
	...
	if(mmd && mmd->status.shield) 
		rate = get_percentage(mmd->status.shield, mmd->status.max_shield);
	else
...
int mobskill_use(struct mob_data *md, t_tick tick, int event)
{
	...
	std::vector<std::shared_ptr<s_mob_skill>> &mss = md->db->skill;

	md->special_state.skill = mss;

	if(map_getmapflag(md->bl.m, MF_TEMPLOULTIMATE) && md->db->id != 28002 && md->special_state.ai != AI_ABR && md->special_state.ai != AI_BIONIC){
		mid = 29022;

		if(md->base_status && md->base_status->max_hp == 125000001) mid = 29021;
		else if(md->base_status && md->base_status->max_hp == 125000002) mid = 29020;

		std::shared_ptr<s_mob_db> mob = mob_db.find(mid);
		if (mob != nullptr)
			md->special_state.skill = mob->skill;
	}
	
	// devon
	if(md->status.mobidskill){
		std::shared_ptr<s_mob_db> mob = mob_db.find(md->status.mobidskill);
		if (mob != nullptr)
			md->special_state.skill = mob->skill;
	}

	if (!battle_config.mob_skill_rate || md->ud.skilltimer != INVALID_TIMER || md->special_state.skill.empty() || status_has_mode(&md->status,MD_NOCAST))
		return 0;

	if (event == -1 && DIFF_TICK(md->ud.canact_tick, tick) > 0)
		return 0; //Skill act delay only affects non-event skills.
	
	if(md->status.noskill == 1)
		return 0;


	//Pick a starting position and loop from that.
	i = battle_config.mob_ai&0x100?rnd()%md->special_state.skill.size():0;
	for (n = 0; n < md->special_state.skill.size(); i++, n++) {
		int c2, flag = 0;

		if (i == md->special_state.skill.size())
			i = 0;

		if (DIFF_TICK(tick, md->skilldelay[i]) < md->special_state.skill[i]->delay)
			continue;

		c2 = md->special_state.skill[i]->cond2;

		if (md->special_state.skill[i]->state != md->state.skillstate) {
			if (md->state.skillstate != MSS_DEAD && (md->special_state.skill[i]->state == MSS_ANY ||
				(md->special_state.skill[i]->state == MSS_ANYTARGET && md->target_id && md->state.skillstate != MSS_LOOT)
			)) //ANYTARGET works with any state as long as there's a target. [Skotlex]
				;
			else
				continue;
		}
		if (rnd() % 10000 > md->special_state.skill[i]->permillage) //Lupus (max value = 10000)
			continue;

		if (md->special_state.skill[i]->cond1 == event)
			flag = 1; //Trigger skill.
		else if (md->special_state.skill[i]->cond1 == MSC_SKILLUSED)
			...
			switch (md->special_state.skill[i]->cond1)
			{
				case MSC_ALWAYS:
					flag = 1; break;
				case MSC_MYHPLTMAXRATE:		// HP< maxhp%
					if(md->status.shield) 
						flag = get_percentage(md->status.shield, md->status.max_shield);
					else
				...
				case MSC_MYHPINRATE:
					if(md->status.shield) 
						...
						flag = get_percentage(md->status.hp, md->status.max_hp);
					flag = (flag >= c2 && flag <= md->special_state.skill[i]->val[0]);
					break;
				case MSC_MYSTATUSON:		// status[num] on
				case MSC_MYSTATUSOFF:		// status[num] off
					if (!md->sc.count) {
						flag = 0;
					} else if (md->special_state.skill[i]->cond2 == -1) {
						for (j = SC_COMMON_MIN; j <= SC_COMMON_MAX; j++)
							if ((flag = (md->sc.data[j]!=NULL)) != 0)
								break;
					} else {
						flag = (md->sc.data[md->special_state.skill[i]->cond2]!=NULL);
					}
					flag ^= (md->special_state.skill[i]->cond1 == MSC_MYSTATUSOFF); break;
				case MSC_FRIENDHPLTMAXRATE:	// friend HP < maxhp%
					flag = ((fbl = mob_getfriendhprate(md, 0, md->special_state.skill[i]->cond2)) != NULL); break;
				case MSC_FRIENDHPINRATE	:
					flag = ((fbl = mob_getfriendhprate(md, md->special_state.skill[i]->cond2, md->special_state.skill[i]->val[0])) != NULL); break;
				case MSC_FRIENDSTATUSON:	// friend status[num] on
				case MSC_FRIENDSTATUSOFF:	// friend status[num] off
					flag = ((fmd = mob_getfriendstatus(md, md->special_state.skill[i]->cond1, md->special_state.skill[i]->cond2)) != NULL); break;
				...
				case MSC_MASTERHPLTMAXRATE:
					flag = ((fbl = mob_getmasterhpltmaxrate(md, md->special_state.skill[i]->cond2)) != NULL); break;
		...
		//Execute skill
		skill_target = status_has_mode(&md->db->status,MD_RANDOMTARGET) ? MST_RANDOM : md->special_state.skill[i]->target;
		if (skill_get_casttype(md->special_state.skill[i]->skill_id) == CAST_GROUND)
		{	//Ground skill.
			short x, y;
			switch (skill_target) {
				case MST_RANDOM: //Pick a random enemy within skill range.
					bl = battle_getenemy(&md->bl, DEFAULT_ENEMY_TYPE(md),
						skill_get_range2(&md->bl, md->special_state.skill[i]->skill_id, md->special_state.skill[i]->skill_lv, true));
					break;
				case MST_TARGET:
				case MST_AROUND5:
				case MST_AROUND6:
			...
			md->skill_idx = i;
			map_freeblock_lock();
			if (!battle_check_range(&md->bl, bl, skill_get_range2(&md->bl, md->special_state.skill[i]->skill_id, md->special_state.skill[i]->skill_lv, true)) ||
				!unit_skilluse_pos2(&md->bl, x, y, md->special_state.skill[i]->skill_id, md->special_state.skill[i]->skill_lv, md->special_state.skill[i]->casttime, md->special_state.skill[i]->cancel))
			{
				map_freeblock_unlock();
				continue;
			}
			...
			map_freeblock_lock();
			if (!battle_check_range(&md->bl, bl, skill_get_range2(&md->bl, md->special_state.skill[i]->skill_id, md->special_state.skill[i]->skill_lv, true)) ||
				!unit_skilluse_id2(&md->bl, bl->id, md->special_state.skill[i]->skill_id, md->special_state.skill[i]->skill_lv, md->special_state.skill[i]->casttime, md->special_state.skill[i]->cancel))
			{
				map_freeblock_unlock();
				continue;
			}
		...
		if ( md->special_state.skill[i]->msg_id ){ //Display color message [SnakeDrak]
			mob_chat_display_message(*md, md->special_state.skill[i]->msg_id);
		}
		if(!(battle_config.mob_ai&0x200)) { //pass on delay to same skill.
			for (j = 0; j < md->special_state.skill.size(); j++){
				// Devon
				if(md->special_state.skill[i]->skill_id == NPC_JOKENPO_PEDRA || md->special_state.skill[i]->skill_id == NPC_JOKENPO_PAPEL || md->special_state.skill[i]->skill_id == NPC_JOKENPO_TESOURA){
					if(md->special_state.skill[j]->skill_id == NPC_JOKENPO_PEDRA || md->special_state.skill[j]->skill_id == NPC_JOKENPO_PAPEL || md->special_state.skill[j]->skill_id == NPC_JOKENPO_TESOURA)
						md->skilldelay[j]=tick;
				} else {
					if (md->special_state.skill[j]->skill_id == md->special_state.skill[i]->skill_id)
						md->skilldelay[j]=tick;
				}
			}
		} else
...
uint64 MobDatabase::parseBodyNode(const YAML::Node &node) {
	...
	if (this->nodeExists(node, "Shield")) {
		...
		if (!this->asUInt32(node, "Shield", shield))
			return 0;

		mob->status.shield = shield;
	} else {
		if (!exists)
			mob->status.shield = 0;
	}

	if (this->nodeExists(node, "MaxShield")) {
		uint32 maxshield;

		if (!this->asUInt32(node, "MaxShield", maxshield))
			return 0;

		mob->status.max_shield = maxshield;
	} else {
		if (!exists)
			mob->status.max_shield = 0;
	}

	if (this->nodeExists(node, "Feito")) {
		uint32 feito;

		if (!this->asUInt32(node, "Feito", feito))
			return 0;

		mob->status.feito = feito;
	} else {
		if (!exists)
			mob->status.feito = 0;
	}
...
static bool mob_read_sqldb_sub(std::vector<std::string> str) {
	...
	if (!str[++index].empty())
		modes["DispelImunne"] = std::stoi(str[index]) ? "true" : "false";
	if (!str[++index].empty())
